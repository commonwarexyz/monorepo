...
const FIGURES = {
  ...
  // The same inbound legs and the same axis as the simplex figure, so tx A is
  // pixel-for-pixel the arrival that figure drew and tx B is the same journey
  // that misses its proposal.
  'multimmit-fig-cadence': {
    ...
    axis: { min: -0.4, max: 7.6, origin: null },
    variants: [{
      tag: 'blocks every 2δ: 0-2δ of wait, δ on average',
      end: 6.6,
      phases: [],
      events: [
        ...[0, 4, 6].map(t => ({ k: 'emit', row: 'lead', t, color: RED })),
        { k: 'emit', row: 'lead', t: 2, color: RED, label: 'block' },
        { k: 'msg', from: 'user', to: 'api', t: 0, d: 1, style: 'tx', label: 'tx A' },
        { k: 'msg', from: 'api', to: 'lead', t: 1, d: 1, style: 'tx' },
        { k: 'note', t: 2, tx: 2, y: 240, lines: ['no wait'], size: 15 },
        { k: 'msg', from: 'user', to: 'api', t: 2.3, d: 1, style: 'tx', label: 'tx B' },
        { k: 'msg', from: 'api', to: 'lead', t: 3.3, d: 1, style: 'tx' },
        { k: 'msg', from: 'lead', to: 'lead', t: 4.3, d: 1.7, style: 'bad' },
        { k: 'note', t: 4.3, tx: 5.15, y: 240, lines: ['waits almost 2δ'], size: 15, fill: RED },
      ],
      token: [],
    }],
  },
