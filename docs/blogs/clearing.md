...
*Design preview using QMDB Current Ordered with MMB for account state. End-to-end performance has not been measured.*
...
::: {.image-caption}
Figure 2: The close binds the three resulting roots. Each payer's payment tree is nested in its activity record, which remains present even when its payments leave the balance unchanged.
:::
...
::: {.image-caption}
Figure 3: Each validator checks the same update against its complete prior state. With 100 validators and at most 33 faulty members, a 67-signature certificate includes at least 34 honest validators that checked and retain the entire close.
:::

An honest signer makes the close and its proof-serving state durable before publishing its vote. It keeps predecessor and successor proofs available while the close is pending and through its challenge deadline $\Delta_e$, and retains the last finalized state for recovery. A new validator joins from an authenticated database snapshot, then applies the following updates in order.
...
An exact withdrawal releases its amount if enough funds remain at the epoch boundary. An account close sweeps that epoch's final balance. Once the carrying close finalizes, the user claims the certified payout with an opening in that close's withdrawal-output BMT. Each output can be claimed only once.
...
::: {.image-caption}
Figure 4: After predecessor admission and successor registration, the admitted epoch-$e$ balance is $80+\rho_a=85$, while the live epoch-$e+1$ head becomes $80-20+\rho_a-15=50$. Both rails account for the same predecessor credit, $\rho_a=5$. Importing that credit adds to the live head and preserves every successor debit.
:::
...
The size model below keeps the signed-payment field widths from [#4664](https://github.com/commonwarexyz/monorepo/pull/4664), with full account keys replacing ranks in the live state. Every account pays its next neighbor once, and each of 100 validators receives the same update.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td style="text-align:right;">106 KB</td>
      <td style="text-align:right;">1.03 MB</td>
      <td style="text-align:right;">10.4 MB</td>
      <td style="text-align:right;">104 MB</td>
    </tr>
    <tr>
      <td>Total operator egress</td>
      <td style="text-align:right;">10.6 MB</td>
      <td style="text-align:right;">103 MB</td>
      <td style="text-align:right;">1.04 GB</td>
...
::: {.image-caption}
Figure 5: Modeled keyed updates, with one signed unit payment per account and no boundary flows. Egress is 100 copies of the dealing, excluding transport and other messages. Account records count 32-byte keys and 8-byte balances; QMDB indexes, history, and retained evidence are additional. Admission supplies roots and totals alongside the commitment and certificate.
:::

With 1,024 senders each paying a different recipient outside that group, only 2,048 of a million accounts change and the modeled update is 141 KB. Validators check those senders and payment entries, then update the affected accounts in QMDB.
...
The receipt and per-close claim trees remain BMTs. A debit mismatch for a changed payer, an entry mismatch, and an acknowledgment fork therefore keep their existing proof formats.

```{=html}
<div class="clearing-benchmark-table">
<table>
  <thead>
    <tr><th rowspan="2" style="text-align:left; vertical-align:bottom;">Challenge</th><th colspan="4" style="text-align:center;">Changed accounts, all sending</th></tr>
  ...
  <tbody>
    <tr><td>Debit mismatch</td><td style="text-align:right;">620 B</td><td style="text-align:right;">748 B</td><td style="text-align:right;">844 B</td><td style="text-align:right;">940 B</td></tr>
    <tr><td>Entry mismatch</td><td style="text-align:right;">671 B</td><td style="text-align:right;">799 B</td><td style="text-align:right;">895 B</td><td style="text-align:right;">991 B</td></tr>
...
::: {.image-caption}
Figure 7: Reference BMT challenge payloads for payers included in the close, with Ed25519 receipts and one-entry vectors. They exclude the separately supplied close and chain transaction framing.
:::
...
<table>
  <thead>
    <tr><th rowspan="2" style="text-align:left; vertical-align:bottom;">QMDB proof payload</th><th colspan="5" style="text-align:center;">Live accounts</th></tr>
    <tr><th style="text-align:right;">1</th><th style="text-align:right;">1,024</th><th style="text-align:right;">10,000</th><th style="text-align:right;">100,000</th><th style="text-align:right;">1,000,000</th></tr>
  </thead>
  <tbody>
    <tr><td>Account present</td><td style="text-align:right;">207 B</td><td style="text-align:right;">497 B</td><td style="text-align:right;">593 B</td><td style="text-align:right;">691 B</td><td style="text-align:right;">819 B</td></tr>
    <tr><td>Account absent</td><td style="text-align:right;">240 B</td><td style="text-align:right;">530 B</td><td style="text-align:right;">626 B</td><td style="text-align:right;">724 B</td><td style="text-align:right;">852 B</td></tr>
    <tr><td>Recovery balance opening</td><td style="text-align:right;">238 B</td><td style="text-align:right;">528 B</td><td style="text-align:right;">624 B</td><td style="text-align:right;">722 B</td><td style="text-align:right;">850 B</td></tr>
  </tbody>
</table>
</div>
```
...
::: {.image-caption}
Figure 9: Measured Current Ordered proofs after one insertion batch, using the middle key and adjacent absence. SHA-256/MMB, 32-byte bitmap chunks, and 8-byte balances. Lookup rows omit the known account key; recovery includes it. All omit the trusted root and chain framing. Sizes vary with history and proof position.
:::
...
::: {.image-caption}
Figure 10: Modeled keyed update per validator, with total operator egress in parentheses. Dotted: all live account records (40 bytes each), before database overhead and retained evidence. Both axes are logarithmic; certificates, transport, and other messages are excluded.
...
::: {.image-caption}
Figure 11: The four-account network carrying 100 million payments of \$0.000001, one atomic unit each. Every sender uses its own opening funds. The arrows group independent payments by sender and recipient, with both directions between $b$ and $c$ retained in the close.
:::
