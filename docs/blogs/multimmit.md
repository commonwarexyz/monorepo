...
For a user, a blockchain's speed is the time from submitting a transaction to seeing it final. The protocols underneath, however, are usually measured against something narrower, the time from a leader proposing a block to the network accepting it. The difference is the first leg of the trip, where your transaction crosses the network to whichever node is leading before it can appear in a block. The return leg caps the network's throughput, with every byte flowing back out of that one node while every other validator's bandwidth idles.
...
The escape is to reference blocks before they certify, and the known ways to do it each break something. Autobahn sketches uncertified references as an optimization: a voter missing a referenced batch holds its vote and fetches the batch from the leader. Faulty producers that disclose their latest batches to the leader alone can then force $\Omega(n^2)$ batch transmissions through the leader's uplink before the view can progress, exactly the traffic pattern producer chains exist to avoid.
