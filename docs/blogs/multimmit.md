...
A transaction block sent at time $t$ finalizes by $t+2\delta$ at best and $t+3\delta$ in expectation, on a clock that starts at the producer's broadcast rather than a leader's proposal. The figures assume $n \geq 5f+1$ and hold for an honest producer's blocks even while up to $f$ producers misbehave. No fault-tolerant consensus can finalize in fewer than [two message delays](https://arxiv.org/abs/2102.07240), even in its best case, and that floor has belonged to leader blocks alone. Multimmit extends it to every producer's blocks at once, without handing back the throughput: the best of both worlds.
