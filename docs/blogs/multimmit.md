...
Referencing blocks before they certify would skip the wait, but the known ways to do it each break something. Autobahn sketches uncertified references as an optimization: a voter missing a referenced batch holds its vote and fetches the batch from the leader. Faulty producers that disclose their latest batches to the leader alone can then force $\Omega(n^2)$ batch transmissions through the leader's uplink before the view can progress, exactly the traffic pattern producer chains exist to avoid.
