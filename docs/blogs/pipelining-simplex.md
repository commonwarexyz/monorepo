...
The term boundary is also a leader handoff. By default, optimistic work stops there, and the first view of a new term must start from certified ancestry. A *pipelined handoff* lets the incoming leader prepare its proposal before the parent certifies. The application may also permit early publication, which trusts the outgoing leader not to equivocate. Rotating leaders benefit most, since every view is a handoff. Together, these rules let Simplex views pipeline without changing the evidence required for finalization.
