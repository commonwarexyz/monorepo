...
\begin{abstract}
UNO is an append-oriented storage protocol that atomically publishes pending epochs of several
...
invisible prepared root; witnesses form a canonical successor ring. A complete, valid ring is the
commit decision. All participant durability operations can therefore occupy one serial barrier
layer, even though each file retains its own durability boundary.
...
submission order. It states the assumptions, invariants, and conditional safety proof; gives a
separate proof for rings broken after deletion finalization; derives bounded-recovery and
acknowledged-durability properties; and identifies the cases outside the theorem. In particular,
...
authority, unconsumed rejected witnesses, and filesystem-backend cancellation during admitted work;
\S\ref{sec:implementation-status} states them explicitly. The result is a reviewable refinement
argument, not a claim of machine verification.
\end{abstract}
...
\noindent\fcolorbox{Accent}{AccentSoft}{%
  ...
  proposed R14 repair. The safety theorem is for
  the transition system specified here. Current implementation deviations---including stale-ring
  authority, rejected-witness reuse, cancellation-safe namespace ownership, one integrity-boundary error, and
  unenforced direct-file/no-alias namespace preconditions---are listed
  in \S\ref{sec:implementation-status}; no conformance claim is made until they are repaired.}}
...
operation: on success its addressed bytes and required retrieval metadata enter the durable base,
without proving unrelated earlier writes. UNO uses that weaker obligation only where the candidate
does not depend on those unrelated bytes, such as a deletion candidate naming the prior committed
payload.
A write beyond the current end also issues the length extension needed to address its bytes. Before
a barrier, crash selection may retain that length update independently; bytes beyond the selected
end are inaccessible and cannot satisfy candidate validation. A selected shrinking truncate
preserves the prefix and discards the old tail. If the file is later extended, UNO assigns fresh
write stamps and never treats discarded tail contents as durability credit.
...
not to authenticate data. The safety theorems are therefore probabilistic and conditional on this
validity event and on the medium honoring completed barriers.
\end{assumption}
...
\code{pwritev2(RWF\_DSYNC)} has durability semantics for the addressed range; it is not
\code{RWF\_ATOMIC} and supplies no torn-write protection.

The initial conformance profile is a local, non-DAX regular file on ext4 in \code{data=ordered}
(the default) or \code{data=journal} mode, with normal JBD2 replay enabled and a storage stack that
honors cache flushes and write barriers.\footnote{Linux ext4 journal and data-mode documentation:
\url{https://www.kernel.org/doc/html/latest/filesystems/ext4/journal.html} and
\url{https://www.kernel.org/doc/html/latest/admin-guide/ext4.html}.} UNO does not rely on ext4's
helpful pre-sync ordering: before a successful barrier, Assumption~\ref{a:fault} permits any
addressed-byte subset. Delayed allocation, block allocation, and inode-size updates are considered
unproved until the file barrier succeeds. A sync error is indeterminate and invokes
...
outside the theorem. Independent byte survival is conservative relative to ordinary block-sized
...
\begin{description}
  ...
  candidate slot. No prepared root is an ordinary visible root.
...
There is no later coordinator write. If a crash happens before acknowledgement, recovery may find
an incomplete ring and choose old, or may find that every unsynchronized prepare happened to
survive and choose new. Both are legal indeterminate outcomes. If acknowledgement occurred, all
prepare records and covered payload are durable.
...
outside the crash-atomicity theorem; UNO does not reconstruct its payload or promise failover
semantics.
\end{limitation}
...
system rather than additional assumptions.
...
If recovery validates a complete prepared ring with no group-bound final certificate, every participant's
candidate state is reconstructible in full.
\end{lemma}
\begin{proof}
...
certificate to $G$; an unbound $R$ is excluded by Definition~\ref{def:certificate}.
\end{proof}
...
\begin{theorem}[UNO group crash atomicity]\label{thm:atomic}
A conforming implementation satisfies Definition~\ref{def:atomic}: during $G$'s protected
logical-key interval, every protocol-reachable crash image has a recovery observation exposing
either $G$'s complete old vector or its complete new vector, never a mixed vector.
\end{theorem}
\begin{proof}
Lemma~\ref{lem:preservation} supplies the protocol invariants. Partition the trace at the crash into
...
Theorem~\ref{thm:atomic} is a theorem of the state machine plus its assumptions, not
a machine-checked proof of the Rust implementation. Current code has counterexamples to newer-$T$
authority, group-bound certificate selection, and rejected-witness reuse, plus filesystem namespace-ownership gaps and an
ineligible integrity candidate; it also relies on, but does not mechanically enforce, the
direct-regular-file and no-alias namespace preconditions. Executable correspondence and each repair obligation are recorded
in \S\ref{sec:implementation-status}.
\end{remark}
...
durable complete ring and $\mathsf{decide}(G)$ even if no root has yet been materialized. Apply
Theorem~\ref{thm:atomic}; later groups are serialized on overlapping participants and preserve or
transfer the earlier proof before slot reuse. Lemma~\ref{lem:composition} covers later namespace
changes that physically break the earlier ring.
\end{proof}

\begin{corollary}[Indeterminate admitted window]
If physical admission occurred but \code{start\_apply} did not return \code{Ok}, recovery may return
old or new, but cannot return partially new. A validation failure or cancellation proved to precede
admission returns only old selected states.
\end{corollary}
\begin{proof}
Before all prepare barriers, an arbitrary subset may survive. If it is incomplete,
Lemma~\ref{lem:incomplete} applies. If every prepare and payload byte happened to survive,
Lemma~\ref{lem:complete} applies. Theorem~\ref{thm:atomic} excludes a mixed vector.
\end{proof}
...
has been acknowledged, a crash may recover either group, both, or neither, subject to each group's
own atomicity. An acknowledged group still satisfies Corollary~\ref{cor:ack} independently.
\end{proposition}
\begin{proof}
...
\begin{description}
  ...
  \item[Error or cancellation after admission.] Outcome may be old or new. Reopen; do not reuse a
  poisoned generation. Cancellation proved to precede admission is old-only.
\end{description}
...
preflight. The theorem therefore applies only when the owned namespace was created through
...
\subsection{Current blocker: cancellation-safe namespace ownership}

The filesystem backends do not yet fully refine Assumption~\ref{a:namespace} under caller
cancellation. In Tokio,
\code{open\_versioned\_inner} owns the namespace guard in its async frame while awaiting a blocking
\code{recover\_named\_embedded} job and, after a removal verdict, an asynchronous pathname unlink.
\code{scan} has the same shape around \code{recover\_partition\_embedded}. \code{Storage::remove}
similarly awaits witness recovery and pathname or partition removal before generation invalidation.
The inline \code{V2Publisher::publish} path used by synchronous atomic writes has the same ownership
gap while awaiting blocking materialization of a carried decision in both Tokio and io\_uring.
Dropping any caller future releases the guard, but already admitted blocking filesystem work may
continue.

A violating execution is therefore possible:
\begin{enumerate}
  \item recovery validates a lingering tombstone and admits cleanup;
  \item the caller is canceled, releasing the namespace guard while cleanup remains queued;
  \item another operation acquires the guard and publishes a fresh incarnation at the same key;
  and
  \item stale cleanup resumes and unlinks that path by name.
\end{enumerate}
This violates Invariant~\ref{i:cleanup}; it is not merely delayed reclamation. The io\_uring
open/remove recovery paths perform their synchronous cleanup calls while the guard is held and do
not have this pathname-cleanup await boundary; its inline publisher nevertheless has the carried-
materialization boundary described next.

Inline publication has a second stale-write shape: cancel a synchronous write while its blocking
job is materializing an older carried multi-participant group, let a later publication advance and
eventually reuse the affected slot, then allow the old job to overwrite that slot with the older
$M$ root. This can invalidate a later acknowledged decision even though no pathname is removed.

Ordinary Tokio open has a third detached-I/O shape. Root recovery may schedule a blocking
\code{set\_len} to discard physical surplus. If the open future is canceled after admission but
before that job executes, the namespace guard can be released without installing shared state.
A second open may then append and acknowledge a publication on the same inode; the delayed first
truncate can subsequently remove the acknowledged payload. Cancellation-safe ownership must cover
ordinary recovery resize as well as linked recovery and namespace cleanup.

A conforming repair must retain owned namespace authority until every admitted cleanup or
materialization operation has stopped. One direct implementation is to move the guard into a single blocking job that
performs recovery, exact tombstone/inode validation, and unlink, returning the guard only after the
job completes. Partition recovery and exact/whole-partition remove need the same treatment through
requested unlink, directory durability, generation invalidation, and sidecar cleanup. Tests must
pause each path after admission, cancel its caller, attempt same-name recreation, and prove that the
new incarnation/inode survives. A separate synchronous-publication regression must pause carried
group materialization, cancel the write, advance through slot reuse, and prove the stale job cannot
overwrite the newer authority on both Tokio and io\_uring. Making admitted publication self-driving
is another valid repair. Tokio also needs a regression that pauses ordinary recovery's scheduled
truncate, cancels open, reopens and acknowledges an append, then resumes the stale truncate and
checks the next crash recovery.
...
CRC bytes, generation-1 same-slot source/generation-2 predecessor/generation-3 candidate lineage, a
...
\code{ee66bfc4...a33465f4}; full digest in the companion note) typechecks. Apalache found
...
Those require codec tests, filesystem refinement tests, and Rust/cancellation correspondence work.
...
choice gives append-oriented structures write-once payload, one durability layer across an
arbitrary participant set, direct per-file reads, and recovery work independent of historical blob
size. The central proof obligation is correspondingly narrow but nonstandard: complete-ring
validation must survive arbitrary subset tears, distinct logical participants must not alias one
inode, and deletion must transfer authority from the ring
to independently meaningful final roots before destroying any link. Alternating slots, prepared
invisibility, stamp-specific payload coverage, group-bound $M/T$ certificates, group-wide proof
transfer, generation-colored state provenance, canonical full-slot prepares, disk-derived orphan
normalization, code-last generation-consuming abort roots, final-before-unlink materialization, tombstone
dominance, and incarnation-safe cleanup
jointly establish that obligation under the stated medium and namespace assumptions. Descending
directory-synced deletion accelerates reclamation but is not needed to keep the selected logical
vector atomic.

The refinement audit is deliberately stricter than the theorem. Revision \code{26a532dd6} still
...
candidates. It also does not enforce the theorem's direct-regular-file and
no-alias preconditions for a pre-populated directory. Until those paths are repaired and regression-tested, the
theorem describes the target protocol rather than the Rust backend. Keeping that distinction
explicit is part of the specification's correctness result.

The specification also makes the trade explicit. UNO provides per-group atomicity, not a global
timeline or a general transactional filesystem. That limitation is what lets the protocol remain a
wrapper over ordinary blobs instead of becoming a WAL, allocator, or volume.
...
for repair. A direct non-batch correctness-first publisher may use
