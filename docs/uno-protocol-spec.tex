...
cannot expose a mixed vector. The checked-in implementation now uses the R14 root grammar,
two ordered prepare layers, exact-$B$ decision recovery, and code-last abort normalization.
Section~\ref{sec:implementation-status} maps those mechanisms to source and keeps the remaining
filesystem-refinement and model-checking obligations explicit. The former one-phase R14-1P proposal
and generation-uncolored R13 design remain historical nonconforming controls; neither is a
compatibility format. R14-1P can splice an older witness topology with newly issued candidate
...
refinement theorem remains absent, so these production anchors are implementation evidence rather
than an end-to-end proof.
\end{abstract}
...
\noindent\fcolorbox{Accent}{AccentSoft}{%
  ...
  The root format, canonical slot image, two-phase prepare, final transition, and code-last abort
  below have production anchors. Conjecture~\ref{thm:atomic} is stated
  for that transition system; its current bounded Kani obligations are pending, and the abstraction
  layers are not joined by one mechanized refinement theorem. R14-1P and R13 are retained only as
  negative controls. Remaining implementation deviations---including unenforced direct-file and
  no-alias namespace preconditions and the pending broad Kani invariant---are listed in
...
Let $D_{\mathrm{root}}$ be the ASCII checksum-domain string in Table~\ref{tab:root}. The current
...
executable model and the checked-in R14 root codec.
...
resistance is insufficient. That alternative is not the reference protocol. The discarded R13
design remains a generation-uncolored negative control, not a supported on-disk format.
\end{limitation}
...
$g$-\emph{quiescent} only when it is all zero with $g\le 1$, or is a canonical image of exactly
generation $g-1$.
...
transfer to the current source. The production Rust now implements the R14 root encoding,
two-phase prepare, exact-$B$/final certificate selection, newer-$T$ suppression, and code-last
rejected-generation consumption. The reviewed cancellation repair separately makes admitted
namespace and publication work self-driving. Current code still relies on, but does not
mechanically enforce, the direct-regular-file and no-alias namespace preconditions, and the known
integrity-geometry cases below remain independent refinement blockers. Executable correspondence
and each remaining obligation are recorded in
\S\ref{sec:implementation-status}.
\end{remark}
...
backend. It schedules at most $\min(m,32)$ chunks, each with at most
$\lceil m/\min(m,32)\rceil$ locked operations. Within a phase-1 chunk, each prepared participant
writes the two non-guard slot ranges and then completes its file barrier; chunks run concurrently.
The scheduler joins every phase-1 chunk before constructing any phase-2 job. Phase 2 then writes
only byte 7 with a durability completion and joins every chunk before acknowledging the decision.
Deletion follows the same two joins. Clean locked operations need not become ring participants, so
the scheduling bound is intentionally stated in $m$, not only in ring size $n$. The io\_uring
backend fans phase-1 participant futures out directly, joins them all, then independently fans out
and joins the one-byte guard operations.
...
Table~\ref{tab:code} is an audit and refinement map, not a proof of conformance. It distinguishes
production R14 mechanisms from specification-only Kani models. Function names are used instead of
line numbers so ordinary edits do not stale the map.
...
\midrule
R14 root, color, canonical slot, and final transition & \path{runtime/src/storage/atomic.rs} &
\path{RootState}, \path{root_guard}, \path{encode_root}, \path{canonical_slot},
\path{candidate_root_status} \\
R14-2P phase-1 slot body and payload/extent evidence & \path{runtime/src/storage/atomic.rs},
\path{runtime/src/storage/uno.rs} & \path{PreparedCommit::batch_body},
\path{Core::stage_batch_commit}, \path{Core::stage_batch_delete},
\path{PayloadChecksumTracker} \\
R14-2P phase-1 durability and group join & \path{runtime/src/storage/tokio/mod.rs},
\path{runtime/src/storage/iouring.rs} & \path{stage_batch_chunk}, first
\path{join_all} in \path{start_apply_guarded} \\
R14-2P one-byte $B$ guard and phase-2 join & same; \path{runtime/src/storage/uno.rs} &
\path{PreparedCommit::batch_guard}, \path{Core::stage_batch_guard},
\path{stage_batch_guard_chunk}, second \path{join_all} in \path{start_apply_guarded} \\
code-last $A$ normalization and raw-slot quiescence & \path{runtime/src/storage/atomic.rs},
\path{runtime/src/storage/uno.rs} & \path{rejection_plan},
\path{normalize_rejected_generation}, \path{Core::recover} \\
payload preflush and suffix proof & \path{runtime/src/storage/atomic.rs} &
\path{PayloadChecksumTracker}, \path{validate_payload_checksum} \\
ordinary-blob wrapper & \path{runtime/src/storage/uno.rs} & \path{Core}, \path{AtomicBlob},
\path{activate_batch_commit} \\
canonical operation set & \path{runtime/src/storage/batch.rs} & \path{canonicalize_operations} \\
R14/L14 embedded preflight and ring format & \path{runtime/src/storage/batch/coordinator.rs} & \path{preflight},
...
exact-$B$/final decision recovery & same; \path{runtime/src/storage/atomic.rs} & \path{recover_embedded},
\path{recover_linked_decision}, \path{candidate_status},
...
Tokio two-phase batch scheduling & \path{runtime/src/storage/tokio/mod.rs}, \path{runtime/src/storage/uno.rs} &
\path{start_apply_guarded}, \path{stage_batch_chunk}, \path{stage_batch_guard_chunk} \\
io\_uring two-phase batch scheduling & \path{runtime/src/storage/iouring.rs} & \path{start_apply_guarded} \\
...
reviewed cancellation ownership repair & \path{runtime/src/storage/tokio/mod.rs},
...
\bottomrule
\end{tabularx}
\caption{Reference-to-source audit. Production anchors implement the listed R14 mechanisms; the
table is not an end-to-end refinement proof.}
\label{tab:code}
\end{table*}

\section{Implementation refinement status}\label{sec:implementation-status}

\subsection{Implemented R14 mechanisms}

The production root codec uses the seven-byte \code{CWUNO14} prefix and the generation-colored
$P/B/R/A/M/T$ guard at byte 7. Batch preparation constructs one canonical zero-padded 2 KiB slot.
Both filesystem schedulers write the two non-guard ranges and complete every participant barrier,
join that complete phase, then issue and durably join only the one-byte $B$ guards. The durable
acknowledgement is sent after the second join. Single-participant filesystem publication uses the
same ordering.

Recovery requires exact $B$ or an operation-specific $B$-to-$M/T$ final transition; an ordinary
$R$ is not group evidence. A newer exact $R/A/M/T$ suppresses an older witness, including the
tombstone-over-older-ring case. Incomplete rings and wrapper-independent non-quiescent slots are
normalized through a predecessor-equivalent $A$: 111 non-guard root bytes, a barrier, then the
one-byte $A$ guard with its own durability completion. Generic ordinary-blob recovery applies the
same plan, so a rejected direct prepare also consumes its generation before a mutable state is
returned. Finalization still makes every participant independently $M/T$ before the first unlink.

Native regressions pin the colored guard, canonical slot padding, phase ordering, all eight
three-participant phase-2 guard-survival masks, direct and batch generation consumption, arbitrary
torn final roots, newer-tombstone dominance, and broken deletion frontiers. These tests establish
the named implementation properties; they do not replace the missing end-to-end refinement proof
or discharge the namespace-shape assumptions below.
...
out external namespace mutation during operation, but the current code does not detect a
...
stronger computational assumption; that is not the selected reference contract. The checked-in
Rust schedulers implement both R14-2P phases as mapped in Table~\ref{tab:code}.

\subsection{Closed gap: newer tombstone versus an older ring}

An earlier recovery predicate suppressed stale witnesses for newer exact $R/M$ roots but omitted
$T$, allowing a broken newer deletion ring to lose to an older retained ring. Current candidate
selection scans both slots and treats exact $R/A/M/T$ as newer authority before accepting a
witness. The regression \path{newer_tombstone_suppresses_an_older_complete_ring} constructs
$H=\{A,C\}$ followed by deletion $D=\{A,B\}$, removes $B$, and verifies that opening $A$ retains
the newer tombstone rather than reinstalling $H$.

\subsection{Historical R13 negative control: exact unissued authority synthesis}

The discarded R13 spellings used one generation-independent magic per state. Under arbitrary
...
in Invariant~\ref{i:prepareorder} is separately required. Production now implements both changes.
The colored-guard, guard-subset, abort-publication, and torn-final regressions exercise their native
byte encodings; the specification-only model retains R13 and R14-1P solely as negative controls.

\subsection{Closed gap: ordinary roots are not group certificates}

An earlier recovery path treated \code{candidate\_is\_committed} ($R$) as evidence that group
...
The protocol therefore uses only the generation-colored, group-bound $M/T$ pair in
...
its $R$ as group evidence. Production removed \code{candidate\_is\_committed} from decision
recovery: \code{candidate\_status} accepts only exact $B$ or the operation-specific final
transition, while code-last normalization consumes rejected generations before direct reuse.

\subsection{Closed gap: rejected witnesses survive payload reuse}

An earlier \code{recover\_embedded} path could find a structurally complete ring whose speculative
suffix failed validation and continue to an older authority without durably consuming the rejected
generation.
...
The implemented reject-and-consume transition prevents this by first making any linked predecessor
...
zeroing a wrapper is not sufficient under repeated arbitrary-subset slot rewrites. Production uses
the two code-last $A$ writes in both filesystem namespace recovery and runtime-agnostic ordinary
recovery; the direct and partial-ring regressions verify that the next mutation uses the opposite
slot rather than reusing the rejected generation.

The abort root alone is insufficient unless recovery can recognize a crash \emph{during} its
write. Production therefore pads every candidate slot canonically, classifies the complete raw
opposite slot independently of wrapper validity, and publishes $A$ body-first and guard-last. The
native abort test verifies that an abort body without its guard has no authority and that the exact
guard advances the internal generation without changing the logical projection. Exhaustive
repeated-crash coverage remains a validation obligation rather than an unimplemented format step.

\subsection{Reviewed cancellation repair: self-driving namespace ownership}

The earlier implementation kept the Tokio namespace guard in the cancelable observer for
...
precondition at that boundary. The subsequent R14 work adds the colored guard, code-last prepare,
no-recovery-$B$ rule, and code-last abort normalization, but neither change by itself is a
refinement proof from filesystem behavior to Conjecture~\ref{thm:atomic}.
...
The checked-in R14 Rust tests exercise the production encoding, ordering, recovery mechanisms, and
regression partitions:
\begin{itemize}
  \item all six state guards follow the generation-color formula, reject a checksum-correct wrong
  color, and same-slot reuse has a distinct color;
  \item phase 1 writes exactly the two non-guard ranges, its barrier completes before the one-byte
  phase-2 guard, and all eight three-participant guard-survival masks yield old except the complete
  mask, which yields new;
  \item code-last abort body without its guard has no authority, while exact $A$ consumes the
  generation without changing the logical projection; delayed participants normalize independently;
  ...
  exact-name open;
  \item a newer tombstone suppresses an older complete retained ring;
...
These tests are finite evidence for their exact R14 paths. They do not reproduce every historical
R14-1P/R13 negative control or prove refinement to Conjecture~\ref{thm:atomic}. Repeated crashes at
every individual abort-body byte, every backend cancellation boundary, and every multi-generation
overlap remain broader obligations than the focused native matrix.
...
does not call production R14 code, and supplies no ext4/JBD2 or public-API refinement. Its three
layers intentionally separate physical encoding, decoded ring observations, and lifecycle
induction.

The physical layer in \path{src/codec.rs} fixes three participants and an independently encoded R14 fixture.
...
generation wraparound, arbitrary payload geometry, production Rust transitions, cancellation,
...
The production implementation now follows that physical ordering and recovery discipline: colored
R14 states, canonical full-slot bodies, two group-wide durability joins, exact-$B$/final decision
evidence, newer-authority suppression, and code-last $A$ consumption. The independent
direct-regular-file/no-alias namespace preconditions are still not enforced for arbitrary
pre-populated directories, the integrity-geometry issues above remain open, and no filesystem or
end-to-end mechanized refinement is claimed.
...
coherent manual per-group conjecture with production mechanism anchors, not a claimed end-to-end
machine proof, ext4 refinement, or complete implementation-refinement result.
