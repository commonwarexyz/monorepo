...
\subtitle{Participant-linked UNO / V2 with caller-delimited payload integrity}
...
\begin{frame}[t]{V2: one root page, payload at final offsets}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node[box,minimum width=6.75cm,minimum height=1.0cm,right=.08cm of r1] (payload) {append-only encoded payload log\\\scriptsize caller data + V2-owned integrity footers};
    ...
    \node[panel,below=.82cm of hdr.south west,anchor=north west,text width=6.65cm,minimum height=1.3cm] (rootfields) {\textbf{R13: 112-byte root header}\\\tiny magic 8 \quad generation 8 \quad encoded length 8\\open unit start 8 \quad open CRC32C 4 \quad integrity scheme 4 \quad unit width 4\\caller application tag 64 \quad root CRC32C 4};
    \node[panel,right=.32cm of rootfields,text width=5.25cm,minimum height=1.3cm] (witness) {\textbf{One 2 KiB slot}\\\scriptsize 112 B root + 16 B witness envelope\\+ $\leq$1,920 B local link / unused space};
  \end{tikzpicture}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.32\textwidth}
      ...
      \small Write data and V2-owned footers once at final encoded offsets.
    \end{column}
    \begin{column}{0.32\textwidth}
  ...
  \vfill
  \takeaway{No extent map, hole punching, or compaction: a root selects a reachable encoded prefix.}

  \note{UNO is a development nickname, not a new public acronym: it reflects the one payload write and one hot-path barrier target. Both alternating 2 KiB slots share one 4 KiB mutable page after the immutable header. They are disjoint addressed ranges: generation parity writes one while preserving the other as fallback under the old/new/subset-of-addressed-write model. Collateral corruption of the containing physical block is a stronger media-failure model and is not made atomic by this layout. R13 keeps the complete 64-byte tag caller-owned. The start, rolling CRC32C, scheme, and unit-width encoding of the only unfinished integrity unit live in dedicated root fields. Root lengths and integrity offsets are relative to the encoded payload, including completed footers; the OS file offset is 8,192 plus that encoded offset. AtomicWriter and contiguous expose footer-free logical positions by mapping through their persisted page geometry.}
\end{frame}
...
\begin{frame}[t]{Dependent writes can be built, then published together}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.53\textwidth}
      ...
      \vspace{0.08cm}
      \begin{tikzpicture}
        \node[panel,text width=5.95cm,align=left] {\texttt{start = offsets[i]}\\\texttt{end = offsets[i + 1]}\\\texttt{decode data[start..end]}\\[-0.02cm]\scriptsize Footer-free checked-page coordinates; page geometry\\maps the root's encoded length to this logical size.};
      \end{tikzpicture}
    \end{column}
...
\begin{frame}[t]{Recovery is correct for arbitrary surviving write subsets}
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.6cm} >{\raggedright\arraybackslash}p{4.55cm} X}
    ...
    No M/T proof; any link, incarnation, candidate, or publication suffix is missing/mismatched & The linked group decision is incomplete & Old roots for the whole group \\
...
\begin{frame}[t]{Contiguous V2 removes its \texttt{Metadata} checkpoint}
  ...
  \begin{itemize}\setlength\itemsep{0.08cm}
    ...
    \item One fixed section or variable pair holds the start; names + encoded lengths map through page geometry to recover logical bounds.
  ...
  \note{This is the direct contiguous payoff. We do not encode more journal frames just to recover the start. We use the application-owned root tag. Contiguous's current marker encoding requires the remaining 56 bytes to be zero, leaving room for a deliberately versioned extension without making arbitrary trailing bytes authoritative. Integrity state no longer consumes any caller tag bytes; it lives in dedicated R13 root fields. Consuming mutation methods prevent reuse after failed or cancelled storage operations, while snapshots remain available for concurrent readers.}
\end{frame}

\begin{frame}[t]{Integrity units: fixed pages or caller-delimited values}
  \vspace{-0.03cm}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \eyebrow{Chunked scheme: page cache}

      \vspace{0.02cm}
      \begin{tikzpicture}[node distance=0cm]
        \node[box,text width=3.65cm,minimum height=1.36cm] (page) {page payload\\\scriptsize 4,092 bytes};
        \node[root,text width=1.05cm,minimum height=1.36cm,right=of page] (footer) {CRC\\\scriptsize 4 B};
        \node[warn,text width=4.82cm,minimum height=.62cm,below=.32cm of page.south west,anchor=north west] (open) {small A + \texttt{sync} + small B + \texttt{sync}\\\scriptsize one open unit continues; no footer yet};
        \draw[flow,draw=Amber] (footer.south |- open.north) -- node[midway,right=.04cm,font=\tiny,text=Amber]{seal at 4,092 B} (footer.south);
      \end{tikzpicture}

      \vspace{0.04cm}
      \scriptsize \texttt{Chunked(4092)} binds the fixed payload width. Write calls and syncs do not close a short unit. One footer completes a 4,096-byte physical page, which is then immutable and cacheable.
    \end{column}
    \begin{column}{0.49\textwidth}
      \eyebrow{Variable scheme: GLOB / long value}

      \vspace{0.02cm}
      \begin{tikzpicture}[node distance=0cm]
        \node[box,text width=4.15cm,minimum height=1.36cm] (value) {one value from many appends\\\scriptsize part 1 \quad part 2 \quad $\cdots$ \quad last bytes};
        \node[root,text width=1.05cm,minimum height=1.36cm,right=of value] (vfooter) {CRC\\\scriptsize 4 B};
        \node[panel,text width=5.32cm,minimum height=.62cm,below=.24cm of value.south west,anchor=north west] (complete) {\texttt{Continue} while streaming; \texttt{Complete} once\\\scriptsize the caller chooses the semantic boundary};
        \draw[flow] (vfooter.south |- complete.north) -- node[midway,right=.04cm,font=\tiny,text=Accent]{one footer} (vfooter.south);
      \end{tikzpicture}

      \vspace{0.04cm}
      \scriptsize Many appends may form one integrity unit. \texttt{Complete} emits exactly one footer for the open unit. There is no fixed unit width; V2 owns checksum calculation and encoding.
    \end{column}
  \end{columns}

  \vspace{-0.04cm}
  \eyebrow{R13 selected root: exact 112-byte layout}

  \vspace{0cm}
  \begin{center}
    \begin{tikzpicture}
      \node[panel,text width=12.95cm,align=center,inner ysep=.04cm] {\scriptsize
        magic 8 $\mid$ generation 8 $\mid$ encoded length 8\\[-.02cm]
        open unit start 8 $\mid$ open CRC32C 4 $\mid$ integrity scheme 4 $\mid$ unit width 4\\[-.02cm]
        caller application tag 64 $\mid$ root CRC32C 4};
    \end{tikzpicture}
  \end{center}

  \vfill
  \takeaway{An unfinished integrity unit lives in dedicated root fields---never in the caller's 64-byte application tag.}

  \note{Chunked mode is the page-cache layout. With a 4,092-byte payload width, V2 appends a four-byte footer at each fixed boundary, producing aligned 4,096-byte physical units. Small writes extend one open unit across any number of syncs; each selected root publishes its start and rolling checksum until it fills. Variable mode is for semantic units such as a GLOB or long value: Continue permits many appends, and Complete appends one footer for the whole nonempty unit. The scheme name describes checksum boundaries, not the journal kind: both fixed and variable contiguous journals currently use Chunked checked pages. In either mode, callers choose boundaries but never calculate or encode CRC32C. Encoded length includes completed footers. The dedicated open-unit fields let the complete 64-byte application tag remain caller-owned.}
\end{frame}

\begin{frame}[t]{CRC API: caller boundaries, V2 checksums}
  \vspace{0cm}
  \eyebrow{One coherent mutation chain; stale tokens fail before payload I/O}

  \vspace{0.04cm}
  \begin{center}
    \begin{tikzpicture}[node distance=.32cm]
      \node[box,text width=3.7cm,minimum height=.82cm] (snapshot) {\texttt{integrity\_snapshot()}\\\scriptsize encoded length + scheme + tag\\tail bytes + token};
      \node[root,text width=4.65cm,minimum height=.82cm,right=of snapshot] (append) {\texttt{append\_integrity(token, bytes, boundary, tag?)}\\\scriptsize V2 calculates CRCs and inserts any footer};
      \node[panel,text width=2.75cm,minimum height=.82cm,right=of append] (next) {encoded offset\\\scriptsize + next token};
      \draw[flow] (snapshot) -- (append);
      \draw[flow] (append) -- (next);
    \end{tikzpicture}
  \end{center}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.32\textwidth}
      \begin{tikzpicture}
        \node[panel,text width=3.75cm,minimum height=.72cm,align=left] {\textbf{\texttt{Continue}}\\\scriptsize Add no caller boundary; an existing scheme still applies.};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{tikzpicture}
        \node[root,text width=3.75cm,minimum height=.72cm,align=left] {\textbf{\texttt{Complete}}\\\scriptsize Append one footer at a nonempty caller boundary.};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{tikzpicture}
        \node[root,text width=3.75cm,minimum height=.72cm,align=left] {\textbf{\texttt{Chunked(n)}}\\\scriptsize Append one footer every $n$ data bytes.};
      \end{tikzpicture}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      ...
      \vspace{0.03cm}
      \scriptsize \texttt{read\_integrity(unit)} validates data + footer; snapshot / \texttt{read\_integrity\_tail()} validates the open unit. Raw blob reads expose unvalidated encoded bytes.
    \end{column}
    \begin{column}{0.49\textwidth}
      \eyebrow{Rewind}

      \vspace{0.03cm}
      \scriptsize \texttt{rewind\_integrity(token, end, unit?, tag?)} validates at most the containing unit, then rebuilds the retained prefix's open-unit CRC.
    \end{column}
  \end{columns}

  \vspace{0.08cm}
  {\centering\scriptsize
  \textbf{CRC scopes:} footer / open-unit field: reads \quad $\mid$ \quad witness suffix: crash publication \quad $\mid$ \quad root / link wrapper: metadata\par}

  \vfill
  \takeaway{Callers own boundaries and indexes; V2 owns CRCs and stale-writer fencing.}

  \note{The API exposes encoded offsets because completed-unit footers occupy the payload stream. Higher-level wrappers such as AtomicWriter translate between their logical offsets and this encoded layout. Continue does not itself choose a durable scheme; Complete selects Variable, while Chunked selects and persists its fixed width. Once bound, the scheme cannot change. The compare token versions the immediately visible integrity state, so two cached writers cannot both append or retag from the same assumed tail. A sync or linked batch publishes encoded length, open-unit state, and the complete caller tag together. Integrity-unit CRCs remain useful for later reads; the separate witness checksum exists only to decide whether unsynchronized publication bytes survived a crash. CRC32C detects accidental local-disk corruption, not malicious tampering.}
\end{frame}
...
\begin{frame}[t]{V2 caching: immutable payload pages, mutable RAM}
  ...
  \vspace{0cm}
  \eyebrow{Proposed cache invariants}
  ...
  \vfill
  \takeaway{A future cache may replace RAM entries; V2 never overwrites completed payload pages.}
...
\begin{frame}[t]{Rollover and root recovery stay bounded}
  \vspace{0cm}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \begin{tikzpicture}[node distance=.19cm]
        ...
        \node[root,minimum width=4.35cm,below=of roots] (bounds) {raw \texttt{AtomicBlob}: bounds only\\\scriptsize no payload read};
  ...
  \vfill
  \takeaway{No historical-page scan: raw blob open reads roots; contiguous init reads only retained partial tails.}

  \note{The bounded final group is the important optimization for contiguous. Rollover can stage sealed sections ahead of publication. The one movable authority marker makes all names between the retained start and active tail reachable at once. The variable journal needs four roots because both data and offsets for the prior authority and current tail must move together; this journal name is unrelated to the IntegrityScheme::Variable checksum mode. Raw AtomicBlob recovery reads only the two fixed roots and any declared P ring, never payload. Current contiguous init goes one step further by constructing AtomicWriter instances: fixed may validate one bounded partial Chunked unit for every retained section, while sealed variable pairs are page-complete and only the active data and offsets writers may each validate a tail. Completed pages remain lazy and validate when read, so reopening still never walks complete blobs.}
\end{frame}

\begin{frame}[t]{Raw UNO median: APFS 96.2\%; ext4 98.0\%}
  \vspace{0.02cm}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.54\textwidth}
      \eyebrow{Linked-witness raw append vs ordinary blobs}
      ...
      \begin{tikzpicture}[node distance=.16cm]
        \node[root,text width=2.8cm,minimum height=.7cm] (apfsresult) {\Large\bfseries 3.8\%\\[-.04cm]\tiny APFS median overhead};
        \node[warn,text width=2.8cm,minimum height=.7cm,right=of apfsresult] {\Large\bfseries 2.0\%\\[-.04cm]\tiny ext4 median overhead};
      \end{tikzpicture}
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{itemize}\setlength\itemsep{0.02cm}
        ...
        \item raw \texttt{append}: no integrity-unit footers
        \item 250 groups per side; 5 paired seeds
        \item Tokio, 2 workers; inflight 1; sides interleaved
      \end{itemize}
  ...
  \vfill
  \takeaway{This isolates group publication; \texttt{Variable} and \texttt{Chunked(4092)} still need paired measurements.}

  \note{The headline and overhead boxes report the median of the five displayed paired ratios. These retained measurements exercise raw AtomicBlob append with integrity-mode none rather than append_integrity; they establish the linked-group publication baseline but do not measure the new integrity-unit API. Each trial alternates ordinary and UNO groups in one process and times full ordinary sync completion versus full UNO batch-handle completion. APFS is the native development filesystem. ext4 is a Docker-volume filesystem inside a Linux VM; its absolute latency and range reflect virtualization, so only the paired direction is claimed. The benchmark harness now accepts integrity-mode variable or chunked and records the chunk width. Those modes must be rerun before claiming CRC API overhead. Do not generalize these numbers to deletes, migration, incompatible participant changes, or every journal configuration.}
\end{frame}
