...
\begin{frame}[plain]
  ...
  \vspace{0.18cm}
  {\small\color{Amber}\textbf{Status: opt-in ALPHA API; ordinary V0/V1 formats and legacy Blob calls remain compatible.}\par}
...
\begin{frame}[t]{Filesystem hot path: one blob fits in one barrier}
  \vspace{0.14cm}
  ...
  \vfill
  \takeaway{Filesystem: the self-link decides in one barrier. Memory uses a simpler two-barrier publisher.}
...
\begin{frame}[t]{The protocol imposes no order between independent groups}
  \vspace{0.03cm}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      \begin{tikzpicture}
        \node[warn,text width=5.15cm] {\textbf{No durable protocol-wide order}\\\scriptsize no root WAL, coordinator, LSN, or commit sequence\\[.1cm]\textbf{Current implementation}\\\scriptsize one process-local namespace lock serializes filesystem groups};
      \end{tikzpicture}
    \end{column}
  \end{columns}

  \vspace{0.12cm}
  \eyebrow{Protocol-permitted crash states---not a current concurrency guarantee}
  ...
  \vfill
  \takeaway{UNO defines no cross-group order; current filesystem publishers still serialize \texttt{start\_apply}.}
