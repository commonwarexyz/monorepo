    ...
    V: Variant,
{
    ...
    /// persisted material has inconsistent epochs.
    ...
    pub(crate) async fn resolve<E: Context>(
        ...
        if !recovered_epoch.is_some_and(|epoch| epoch > candidate.info.epoch) {
...
mod tests {
    ...
    fn epoch_mismatch_panics_at_init() {
        deterministic::Runner::default().start(|mut context| async move {
