...
{
    inner: UnmerkleizedBatch<mmr::Family, H, U>,
    ...
    Operation<mmr::Family, unordered::Update<K, V>>: Codec,
{
    ...
    pub const fn inner(&self) -> &UnmerkleizedBatch<mmr::Family, H, unordered::Update<K, V>> {
        &self.inner
    }
    ...
    Operation<mmr::Family, unordered::Update<K, V>>: Codec,
{
    ...
    async fn merkleize(self) -> Result<Self::Merkleized, Error<mmr::Family>> {
        let db = self.db.read().await;
        let merkleized = self.inner.merkleize(&*db, self.metadata).await?;
        Ok(AnyMerkleized {
            inner: merkleized,
            db: self.db.clone(),
        })
...
/// committed state.
///
/// `finalize` applies the merkleized batch and durably commits it to disk.
