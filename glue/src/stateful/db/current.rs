...
mod open {
    ...
    {
        match max_size {
            Some(size) => Db::init_at_most(context, config, size).await,
            None => Db::init(context, config).await,
        }
    }
    ...
    {
        match max_size {
            Some(size) => OrderedVariableDb::init_at_most(context, config, size).await,
            None => OrderedVariableDb::init(context, config).await,
        }
    }
}
