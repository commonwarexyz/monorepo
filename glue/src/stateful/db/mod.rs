...
pub trait ManagedDb<E>: Send + Sync + Sized {
    ...
    /// Open a database, recovering and validating the complete expected target when supplied.
    /// Return an error unless the recovered root and range exactly match that target.
    fn init(
        context: E,
        config: Self::Config,
...
/// # Mutation Safety
///
/// Calls to [`Self::apply`], [`Self::finalize`], and [`Self::prune`] must not overlap. Implementations panic if an
...
mod tests {
    ...
    struct CountingInitializationDb {
        current_target: u64,
        init_count: usize,
    }
    ...
    impl<E: Send> ManagedDb<E> for CountingInitializationDb {
        type Unmerkleized = TestUnmerkleized;
        type Merkleized = TestMerkleized;
        type Error = Infallible;
        type Config = u64;
    ...
    impl<E: Send> ManagedDb<E> for RecoverableStartupDb {
        ...
        async fn init(
            ...
            if selected > retained {
                return Err("database is behind");
            }
            state.store(selected, Ordering::Relaxed);
            if fail {
                return Err("initialization interrupted after durable repair");
            }
            Ok(Self(selected))
        }
        fn new_batch(_database: BatchContext<'_, Self>) -> Self::Unmerkleized {
    ...
    fn tuple_initialization_validates_ahead_and_aligned_databases() {
        deterministic::Runner::default().start(|context| async move {
            type DbSet = (
                Shared<CountingInitializationDb>,
                Shared<CountingInitializationDb>,
            );
            let (left, right) =
