...
pub trait ManagedDb<E>: Send + Sync + Sized {
    ...
    /// Apply a merkleized batch to the underlying database.
    ///
    /// In QMDB, this encapsulates calling `db.apply_batch(merkleized)` and
    /// `db.commit()`.
    fn finalize(
        &mut self,
...
pub trait DatabaseSet<E>: Clone + Send + Sync + 'static {
    ...
    /// Apply each merkleized batch to its underlying database.
    ///
