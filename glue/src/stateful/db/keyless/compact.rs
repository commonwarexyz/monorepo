    ...
    Operation<F, FixedEncoding<V>>: EncodeShared + CodecRead<Cfg = ()>,
{
    ...
    async fn init(
        ...
        let db = <Self>::init(context, config, Some(target.size)).await?;
...
mod tests {
    ...
    fn managed_db_initializes_fixed_keyless_unjournaled_multiple_commit_ranges() {
        deterministic::Runner::default().start(|context| async move {
            ...
            // Commit two more ranges so the bounded initialization below spans multiple commits.
            for i in [2u64, 3] {
                let floor = db.inactivity_floor_loc();
                let batch = db
                    .new_batch()
    ...
    fn managed_db_prune_bounds_fixed_keyless_unjournaled_bounded_initialization_history() {
        deterministic::Runner::default().start(|context| async move {
            ...
            // Prune to the second target: the first is no longer an initialization target, but the
            // second still is.
