    ...
    async fn read(&self) -> ReadLease<'_, T> {
        ...
        match AsyncRwLockReadGuard::try_map(guard, |state| match state {
            ...
            Err(guard) => {
                ...
                // this task is dropped with the rest of the actor. Say so first,
                // because a parked lease is silent otherwise: if this is ever
                // reached while the actor keeps running, the symptom is a read
                // that never returns and nothing else to go on.
                drop(guard);
