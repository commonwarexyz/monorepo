...
use commonware_cryptography::{
    ...
    certificate::{ConstantProvider, Provider},
    ed25519,
...
/// Initialize the finalization and block archives a marshal needs, under `prefix`.
pub(crate) async fn marshal_archives(
    ...
    >,
    immutable::Archive<deterministic::Context, Sha256Digest, TestBlock>,
) {
    ...
    let finalized_blocks = immutable::Archive::init(
        ...
        archive_config(page_cache, &format!("{prefix}-blocks")),
    )
    .await
...
/// The marshal configuration shared by every test harness: genesis start, no epoch
/// rotation, and buffers sized for deterministic runs.
pub(crate) fn marshal_config<P: Provider<Scope = Epoch>>(
    provider: P,
    page_cache: CacheRef,
    prefix: &str,
) -> marshal::Config<P, FixedEpocher, Sequential, TestBlock, TestBlock> {
    marshal::Config {
        provider,
...
pub(crate) async fn start_marshal(
    ...
    let (mut finalizations_by_height, finalized_blocks) =
