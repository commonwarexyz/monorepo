...
enum FinalizedHandoff<B> {
    Covered(B, Exact),
    Reflected(B, Exact),
    /// An above-artifact report whose receipt waits for handoff durability, including duplicates.
    Apply(B, Exact),
}
...
mod tests {
    ...
    #[test]
    fn live_floor_reports_handoff_once_after_durability() {
        deterministic::Runner::timed(Duration::from_secs(10)).start(|context| async move {
            ...
            acknowledgement.acknowledge();
            while marshal.mailbox.get_processed_height().await != Some(Height::new(1)) {
                context.sleep(Duration::from_millis(10)).await;
            }
            ...
            while marshal.mailbox.get_processed_height().await != Some(Height::new(3)) {
                context.sleep(Duration::from_millis(10)).await;
            }
            assert_eq!(control.applied.load(Ordering::Relaxed), 2);
