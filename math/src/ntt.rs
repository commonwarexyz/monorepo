...
impl VanishingPoints {
    ...
    /// Iterate over all the bits of this set, in normal order.
    #[cfg(test)]
...
impl<F: FieldNTT> PolynomialColumn<F> {
    ...
    /// Evaluate this polynomial at a single point.
    #[cfg(test)]
    ...
    /// Get the effective degree of this polynomial.
    #[cfg(test)]
...
mod test {
    ...
    fn any_polynomial_vector(
        max_log_rows: usize,
        max_cols: usize,
    ) -> impl Strategy<Value = PolynomialVector<F>> {
    ...
    fn any_bit_vec_not_all_0(max_log_rows: usize) -> impl Strategy<Value = VanishingPoints> {
        (0..=max_log_rows).prop_flat_map(move |lg_rows| {
            ...
            (0..rows).prop_flat_map(move |set_row| {
                proptest::collection::vec(any::<bool>(), 1 << lg_rows).prop_map(move |mut bools| {
                    bools[set_row] = true;
                    let mut out = VanishingPoints::new(lg_rows as u32);
    ...
    proptest! {
        ...
        fn test_ntt_eq_naive(p in any_polynomial_vector(6, 4)) {
            let ntt: EvaluationVector<F> = p.clone().evaluate();
            let ntt_naive: EvaluationVector<F> = p.evaluate_naive();
            assert_eq!(ntt, ntt_naive);
        }

        #[test]
        fn test_evaluation_then_inverse(p in any_polynomial_vector(6, 4)) {
            let cloned: PolynomialVector<F> = p.clone();
            assert_eq!(cloned, p.evaluate().interpolate());
        }
