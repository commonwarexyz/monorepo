...
/// Key Derivation Function (KDF) to derive directional ChaCha20Poly1305 ciphers
/// using HKDF-SHA256.
///
/// This function derives two ChaCha20Poly1305 ciphers based on:
/// - The input key material (IKM), usually the shared secret from the
///   Diffie-Hellman key exchange
/// - An ordered list of byte slices (salts), where the order is critical for
///   consistent derivation between the dialer and the listener. The list
///   usually contains:
...
///
/// The dialer and listener must use the ciphers in the same order to ensure
...
pub fn derive(ikm: &[u8], salts: &[&[u8]]) -> Result<(ChaCha20Poly1305, ChaCha20Poly1305), Error> {
    ...
    // The salt is generated from a commonware-specific prefix and the list of salts
    // provided.
...
mod tests {
    ...
    fn test_derive_with_empty_salt_element_in_slice() {
        ...
        // Further check: ensure it's different from derivation with non-empty salt or
        // fully empty salts
