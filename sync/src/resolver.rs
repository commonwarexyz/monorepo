    ...
    T: Translator,
{
    async fn get_proof(
        ...
        let current_op_count = self.db.op_count();

        // Validate that we have the requested range
        if start_index >= current_op_count {
            return Err(Error::InvalidResolver(format!(
                "Requested start_index {} is beyond available operations (count: {})",
                start_index, current_op_count
            )));
        }

        // Calculate how many operations we can actually provide
        let available_ops = NonZeroU64::new(current_op_count - start_index).unwrap();
        let ops_to_provide = std::cmp::min(max_ops, available_ops);

        // Get proof and operations from the database
        let (proof, operations) = self
            .db
            .proof(start_index, ops_to_provide)
            .await
