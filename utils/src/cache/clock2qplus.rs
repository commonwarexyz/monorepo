...
//! after eviction, so the key bypasses Small and enters Main.
//!
...
//! While unused capacity remains, cold entries fill Small first and then Main.
//!
//! # Correlation Filtering
//!
//! The newest half of Small, rounded up, forms a correlation window. Hits in that
...
//! rounded up. A capacity of one uses Main only.
//!
//! At capacities 2 through 19, Small holds one slot that never leaves the
//! correlation window, so its hits cannot earn promotion. Cold entries can
//! still fill Main during warm-up. When the cache is full, new admissions to
//! Main require a Ghost hit.
//!
//! Resident keys and values remain in stable [super::Cache] slots. Resident
//! topology and Ghost history are policy-owned, while each slot's correlation
//! marker and reference bit are stored inline as [Policy::SlotState].
//!
//! # References
//!
...
struct GhostQueue<K> {
    ...
    /// Hashes and rehash callbacks come from the owning cache's current
    /// [`HashContext`], so this table retains no independent hash state.
...
impl<K: Hash + Eq> GhostQueue<K> {
    ...
    #[inline]
    fn push(&mut self, key: K, hash: u64, context: &HashContext<'_, K>) {
        if self.capacity == 0 {
            return;
        }
        ...
        let slot = if let Some(slot) = self.free.pop() {
            ...
            let historical_hash = context.hash_one(
                self.slots[slot]
                    .key
                    .as_ref()
    ...
    #[inline]
    fn discard(&mut self, key: &HashContext<'_, K>) -> bool {
        let slot = {
            let slots = &self.slots;
            let Ok(entry) = self.index.find_entry(key.hash(), |slot| {
                slots[*slot]
                    .key
                    .as_ref()
                    .expect("indexed Ghost entry must have a key")
...
/// keys normally enter Small, while keys found in bounded Ghost history enter Main.
...
/// exact bounded Ghost history. Resident keys and values remain in
/// [super::Cache]. The target proportions are 10% Small, 90% Main, and 50%
/// Ghost, with a correlation window covering half of Small. Small and Ghost use
/// floor division, Small receives at least one slot when total capacity exceeds
/// one, and the window rounds up. A capacity of one uses Main as a one-entry CLOCK.
///
        ...
        C: FnOnce(Option<Slot>) -> Claimed<'a, K>,
    {
        ...
        let slot = match (plan, claim(victim)) {
            ...
            (
                ...
                Claimed::Evicted { key: evicted, hash },
            ) => {
...
impl<K> core::fmt::Debug for Clock2QPlus<K> {
...
mod tests {
    ...
    impl<K: Hash + Eq + Clone + core::fmt::Debug, V> Cache<K, V, Clock2QPlus<K>> {
        ...
        pub(crate) fn check_policy_invariants(&self) {
            ...
            let young_len = policy
                ...
                .unwrap_or(0);
            assert!(policy.small.admissions <= ADMISSION_GENERATION_MASK);
            ...
            for (rank, &slot) in small.iter().enumerate() {
                ...
                if young {
                    ...
                    let age = policy
                        .small
                        .admissions
                        .wrapping_sub(policy.slots[slot].admitted_at())
                        & ADMISSION_GENERATION_MASK;
                    assert!(age < policy.small.correlation_window);
                }
            ...
            for (rank, &slot) in main.iter().enumerate() {
                ...
                assert_eq!(policy.slots[slot].admitted_at(), 0);
    ...
    fn test_correlation_age_survives_admission_counter_wrap() {
        ...
        let mut wrapped = TestCache::new(NZUsize!(40));
        wrapped.policy.small.admissions = ADMISSION_GENERATION_MASK - 1;
        let mut plain = TestCache::new(NZUsize!(40));
        for key in 1..=40u64 {
            wrapped.put(key, key);
            plain.put(key, key);
        }
        ...
        assert_eq!(wrapped.policy.small.young_tail, None);
        ...
        for cycle in 0..8u64 {
            ...
            assert_eq!(wrapped.policy.small.young_tail, expected_young_tail);
        ...
        wrapped.clear();
        assert_eq!(wrapped.policy.small.admissions, 0);
        wrapped.check_invariants();
    }
    ...
    fn exercise_colliding_ghost(capacity: usize, reserve: bool) {
        let owner = Rc::new(());
        let hasher = super::super::Hasher::with_seeds(1, 2, 3, 4);
        ...
        for step in 0..2048u64 {
            let value = (step * 17 + step / 5) % 127;
            {
                ...
                let context = HashContext::prehashed(&requested, requested_hash, &hasher);
                assert_eq!(ghost.discard(&context), position.is_some());
                ...
                if step % 5 != 0 {
                    ...
                    ghost.push(historical, historical_hash, &context);
                    if capacity != 0 {
    ...
    #[test]
    fn test_cache_context_drives_resident_and_ghost_rehashes() {
        let mut cache = TestCache::new(NZUsize!(20));

        // Removing both reservations forces both indexes to call their rehash
        // callbacks while the public Cache API drives all policy transitions.
        cache.index = HashTable::new();
        cache.policy.ghost.index = HashTable::new();
        cache.hasher = super::super::Hasher::with_seeds(1, 2, 3, 4);

        let mut resident_grew = false;
        let mut ghost_grew = false;
        for key in 0..256 {
            let resident_capacity = cache.index.capacity();
            let ghost_capacity = cache.policy.ghost.index.capacity();
            cache.put(CollidingKey(key), key);
            resident_grew |= cache.index.capacity() > resident_capacity;
            ghost_grew |= cache.policy.ghost.index.capacity() > ghost_capacity;
            cache.check_invariants();
        }
        ...
        let ghost = cache.ghost_keys();
        assert_eq!(ghost.len(), cache.policy.ghost.capacity);
