...
fn validate_participant_count(count: usize) -> Result<(), Error> {
    u32::try_from(count)
        .map(|_| ())
        .map_err(|_| Error::TooManyParticipants)
}
...
impl<P: Ord> TryFromIterator<(P, u64)> for Committee<P> {
    ...
    fn try_from_iter<I: IntoIterator<Item = (P, u64)>>(iter: I) -> Result<Self, Self::Error> {
        ...
        validate_participant_count(members.len())?;
...
mod test {
    ...
    #[test]
    fn test_committee_rejects_too_many_participants() {
        ...
        fn test_committee_quorum_matches_brute_force(weights in prop::collection::vec(1u64..20, 1..9)) {
            ...
            for mask in 0..subset_count {
                ...
                if sum >= quorum {
                    ...
                    for superset in 0..subset_count {
                        if superset & mask == mask {
                            let superset_participants = (0..weights.len())
                                .filter(|index| superset & (1u16 << index) != 0)
                                .map(|index| Participant::new(u32::try_from(index).unwrap()));
                            prop_assert!(committee.sum_ordered_weights(superset_participants).unwrap() >= quorum);
                        }
                    }
                }
            }
