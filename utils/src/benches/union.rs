...
mod method_2 {
    ...
    pub fn union_unique(namespace: &[u8], msg: &[u8]) -> Vec<u8> {
        ...
        unsafe {
            ...
            std::ptr::copy_nonoverlapping(namespace.as_ptr(), result.as_mut_ptr().add(offset), namespace_len);
...
fn bench_concat(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 10000, 100000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("one", size), &(a.as_slice(), b.as_slice()), |bench, (a, b)| {
            bench.iter(|| method_1_extend::union(black_box(a), black_box(b)));
        });
        
        group.bench_with_input(BenchmarkId::new("two", size), &(a.as_slice(), b.as_slice()), |bench, (a, b)| {
...
fn bench_union_unique(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("one", size), &(namespace.as_slice(), msg.as_slice()), |bench, (namespace, msg)| {
            bench.iter(|| method_1_extend::union_unique(black_box(namespace), black_box(msg)));
        });
        
        group.bench_with_input(BenchmarkId::new("two", size), &(namespace.as_slice(), msg.as_slice()), |bench, (namespace, msg)| {
