...
struct DeliveryTrackerState {
    ...
    pending_sequences: Arc<Mutex<HashMap<u64, (bool, Option<u64>)>>>,
}

impl Drop for DeliveryGuardInner {
    fn drop(&mut self) {
        let mut pending = self.tracker.pending_sequences.lock().unwrap();
        if let Some((delivered, batch_id)) = pending.get_mut(&self.sequence) {
            *delivered = true;
        ...
        while let Some((delivered, _)) = pending.get(&(current_watermark + 1)) {
            if *delivered {
                pending.remove(&(current_watermark + 1));
                current_watermark += 1;
                self.tracker.watermark.store(current_watermark, Ordering::Release);
            } else {
                break;
            }
        }
...
impl DeliveryTracker {
    ...
    fn create_guard(&self, batch_id: Option<u64>) -> DeliveryGuard {
        ...
        // Track this sequence
        self.state.pending_sequences.lock().unwrap().insert(sequence, (false, batch_id));
...
impl<T> TrackedSender<T> {
    ...
    pub async fn send(&mut self, msg: T, batch_id: Option<u64>) -> Result<DeliveryGuard, SendError> {
    ...
    pub fn try_send(&mut self, msg: T, batch_id: Option<u64>) -> Result<DeliveryGuard, TrySendError<TrackedMessage<T>>> {
        let guard = self.tracker.create_guard(batch_id);
        let tracked_msg = TrackedMessage {
            data: msg,
            guard: guard.clone(),
    ...
    pub fn batch_pending_count(&self, batch_id: u64) -> usize {
        self.tracker.state.batch_counts.lock().unwrap().get(&batch_id).copied().unwrap_or(0)
    }
    
    /// Returns all batch IDs that have pending messages.
    pub fn pending_batches(&self) -> Vec<u64> {
        self.tracker.state.batch_counts.lock().unwrap().keys().copied().collect()
    }
    
    /// Returns the total number of pending messages across all batches.
    pub fn total_pending(&self) -> usize {
        self.tracker.state.pending_sequences.lock().unwrap()
            .values()
            .filter(|(delivered, _)| !delivered)
            .count()
    }
}
...
mod tests {
    ...
    fn test_pending_batches() {
        block_on(async move {
            let (mut sender, mut _receiver) = tracked_channel::<i32>(10);
            
            let _g1 = sender.send(1, Some(100)).await.unwrap();
            let _g2 = sender.send(2, Some(200)).await.unwrap();
            let _g3 = sender.send(3, Some(100)).await.unwrap();
            
            let mut batches = sender.pending_batches();
            batches.sort();
            assert_eq!(batches, vec![100, 200]);
        });
    }
