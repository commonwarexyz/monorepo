...
pub trait Policy<K> {
    ...
    /// replaces that live slot and returns its previous key and resident hash.
    ///
...
mod tests {
    ...
    impl<K> Policy<K> for TestPolicy {
        ...
        fn remove(&mut self, slot: Option<Slot>, _key: HashContext<'_, K>) {
