...
cfg_if::cfg_if! {
    if #[cfg(all(target_os = "linux", not(feature = "loom"), not(miri)))] {
        ...
        /// Number of LLC domains, for wake-routing (1 disables the preference).
        pub(crate) fn domain_count() -> usize {
            topology().domains().max(1)
        }

        /// The calling thread's current domain index, for wake-routing.
        pub(crate) fn current_domain_index() -> usize {
        ...
        /// Always 1: a single domain (routing preference disabled).
        pub(crate) fn domain_count() -> usize {
            1
        }
