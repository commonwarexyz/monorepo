...
//!
//!     let (sender1, receiver1) = oracle.control(peers[0].clone()).register::<Bytes>(0).await.unwrap();
//!     let (sender2, receiver2) = oracle.control(peers[1].clone()).register::<Bytes>(0).await.unwrap();
//!
...
mod tests {
    ...
    fn simulate_messages(seed: u64, size: usize) -> (String, Vec<usize>) {
        ...
        executor.start(|context| async move {
            ...
            for i in 0..size {
                ...
                let (sender, mut receiver) = oracle.control(pk.clone()).register::<Bytes>(0).await.unwrap();
    ...
    fn test_message_too_big() {
        ...
        executor.start(|mut context| async move {
            ...
            for i in 0..10 {
                let pk = PrivateKey::from_seed(i as u64).public_key();
                let (sender, _) = oracle.control(pk.clone()).register::<Bytes>(0).await.unwrap();
                agents.insert(pk, sender);
            }
    ...
    fn test_linking_self() {
        ...
        executor.start(|context| async move {
            ...
            oracle.control(pk.clone()).register::<Bytes>(0).await.unwrap();
    ...
    fn test_duplicate_channel() {
        ...
        executor.start(|context| async move {
            ...
            let pk = PrivateKey::from_seed(0).public_key();
            oracle.control(pk.clone()).register::<Bytes>(0).await.unwrap();
            let result = oracle.control(pk.clone()).register::<Bytes>(0).await;
    ...
    fn test_simple_message_delivery() {
        ...
        executor.start(|context| async move {
            ...
            let (mut sender1, mut receiver1) =
                oracle.control(pk1.clone()).register::<Bytes>(0).await.unwrap();
            let (mut sender2, mut receiver2) =
                oracle.control(pk2.clone()).register::<Bytes>(0).await.unwrap();

            // Register unused channels
            let _ = oracle.control(pk1.clone()).register::<Bytes>(1).await.unwrap();
            let _ = oracle.control(pk2.clone()).register::<Bytes>(2).await.unwrap();
    ...
    fn test_send_wrong_channel() {
        ...
        executor.start(|context| async move {
            ...
            let pk1 = PrivateKey::from_seed(0).public_key();
            let pk2 = PrivateKey::from_seed(1).public_key();
            let (mut sender1, _) = oracle.control(pk1.clone()).register::<Bytes>(0).await.unwrap();
            let (_, mut receiver2) = oracle.control(pk2.clone()).register::<Bytes>(1).await.unwrap();
    ...
    async fn test_bandwidth_between_peers(
        ...
        let (mut sender, _) = oracle.control(pk1.clone()).register::<Bytes>(0).await.unwrap();
        let (_, mut receiver) = oracle.control(pk2.clone()).register::<Bytes>(0).await.unwrap();
    ...
    fn test_bandwidth_contention() {
        ...
        executor.start(|context| async move {
            ...
            for i in 0..=NUM_PEERS {
                let pk = PrivateKey::from_seed(i as u64).public_key();
                let (sender, receiver) = oracle.control(pk.clone()).register::<Bytes>(0).await.unwrap();
                peers.push(pk);
                senders.push(sender);
    ...
    fn test_many_to_one_bandwidth_sharing() {
        ...
        executor.start(|context| async move {
            ...
            for i in 0..10 {
                ...
                let (tx, _) = oracle.control(sender.clone()).register::<Bytes>(0).await.unwrap();
                sender_txs.push(tx);
            ...
            let (_, mut receiver_rx) = oracle.control(receiver.clone()).register::<Bytes>(0).await.unwrap();
    ...
    fn test_one_to_many_fast_sender() {
        ...
        executor.start(|context| async move {
            ...
            let (sender_tx, _) = oracle.control(sender.clone()).register::<Bytes>(0).await.unwrap();
            ...
            for i in 0..10 {
                ...
                let (_, rx) = oracle.control(receiver.clone()).register::<Bytes>(0).await.unwrap();
    ...
    fn test_dynamic_bandwidth_affects_new_transfers() {
        ...
        executor.start(|context| async move {
            ...
            let (mut sender_tx, _) = oracle.control(pk_sender.clone()).register::<Bytes>(0).await.unwrap();
    ...
    fn test_peer_set_window_management() {
        ...
        executor.start(|context| async move {
            ...
            let (mut sender1, _receiver1) = oracle.control(pk1.clone()).register::<Bytes>(0).await.unwrap();
            let (mut sender2, _receiver2) = oracle.control(pk2.clone()).register::<Bytes>(0).await.unwrap();
            let (mut sender3, _receiver3) = oracle.control(pk3.clone()).register::<Bytes>(0).await.unwrap();
            let (_mut_sender4, _receiver4) = oracle.control(pk4.clone()).register::<Bytes>(0).await.unwrap();
