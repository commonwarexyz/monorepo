...
/// Reservation for a peer in the network. This is used to ensure that the peer
/// is reserved only once, and that the reservation is released when the peer
...
pub struct Reservation<E: Spawner + Metrics, P: PublicKey> {
    ...
    /// Used to automatically notify the completion of the reservation when it
    /// is dropped.
    ///
    /// Stored as an `Option` to avoid unnecessary cloning by `take`ing the
    /// value.
...
impl<E: Spawner + Metrics, P: PublicKey> Drop for Reservation<E, P> {
    fn drop(&mut self) {
        ...
        // If the mailbox is not full, release the reservation immediately without
        // spawning a task.
        ...
        // If the mailbox is full, we avoid blocking by spawning a task to handle the
        // release. While it may not be immediately obvious how a deadlock could
        // occur, we take the conservative approach of avoiding it.
