...
mod tests {
    ...
    fn test_connect_authorized_peer_receives_tracker_info() {
        ...
        executor.start(|context| async move {
            ...
            let _res = mailbox.listen(auth_pk.clone()).await.ok().flatten().unwrap();
    ...
    fn test_handle_peers_learns_unknown_peer() {
        ...
        executor.start(|context| async move {
            ...
            let _r1 = connect_to_peer(&mailbox, &pk1).await;
            let _r2 = connect_to_peer(&mailbox, &pk2).await;
    ...
    fn test_dial_message() {
        ...
        executor.start(|context| async move {
            ...
            let reservation = mailbox.dial(boot_pk.clone()).await.ok().flatten();
            assert!(reservation.is_some());
