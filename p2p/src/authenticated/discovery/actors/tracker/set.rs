...
mod tests {
    ...
    fn test_set_initialization_and_sorting() {
        ...
        let set = Set::new(OrderedSet::try_from(peers).unwrap());
    ...
    fn test_update_knowledge_single_peer() {
        let peers = create_test_peers();
        let mut set = Set::new(OrderedSet::try_from(peers).unwrap());
    ...
    fn test_len() {
        ...
        let single_set = Set::new(OrderedSet::try_from(single_peer).unwrap());
        assert_eq!(single_set.len(), 1);

        let empty_peers: Vec<ed25519::PublicKey> = vec![];
        let empty_set = Set::new(OrderedSet::try_from(empty_peers).unwrap());
    ...
    fn test_into_iterator() {
        ...
        let set = Set::new(OrderedSet::try_from(peers_data).unwrap());
