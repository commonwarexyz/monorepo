...
pub struct PeerSet<P: PublicKey> {
...
impl<P: PublicKey> PeerSet<P> {
    /// Creates a new [PeerSet] for the given index.
    pub fn new(ordered: Set<P>) -> Self {
...
impl<'a, P: PublicKey> IntoIterator for &'a PeerSet<P> {
    type Item = &'a P;
...
impl<P: PublicKey> std::ops::Index<usize> for PeerSet<P> {
    type Output = P;
...
impl<P: PublicKey> Deref for PeerSet<P> {
    type Target = Set<P>;
...
mod tests {
    ...
    fn test_set_initialization_and_sorting() {
        ...
        let set = PeerSet::new(peers.into());
    ...
    fn test_update_knowledge_single_peer() {
        let peers = create_test_peers();
        let mut set = PeerSet::new(peers.into());
    ...
    fn test_len() {
        ...
        let single_set = PeerSet::new(single_peer.into());
        assert_eq!(single_set.len(), 1);

        let empty_peers: Vec<ed25519::PublicKey> = vec![];
        let empty_set = PeerSet::new(empty_peers.into());
    ...
    fn test_into_iterator() {
        ...
        let set = PeerSet::new(peers_data.into());
