...
impl<C: PublicKey> Mailbox<C> {
    pub(super) fn new(tx: mpsc::Sender<Message<C>>) -> Self {
        Self { tx }
    }
    ...
    pub async fn bit_vec(&mut self, bit_vec: types::BitVec) {
        let _ = self.tx.send(Message::BitVec(bit_vec)).await;
    }

    pub async fn peers(&mut self, peers: Vec<types::PeerInfo<C>>) {
        let _ = self.tx.send(Message::Peers(peers)).await;
    }

    pub async fn kill(&mut self) {
        let _ = self.tx.send(Message::Kill).await;
    }
}
