...
mod tests {
    ...
    async fn link_bidirectional(
        ...
        let peers = manager.peer_set(0).await.unwrap_or_default().unwrap_or_default();
        manager.track(
            0,
    ...
    fn test_message_dropped_for_closed_subchannel() {
        ...
        executor.start(|context| async move {
            ...
            let (mut tx2, _) = handle1.register(2).await.unwrap();
            let (_, rx1) = handle2.register(1).await.unwrap();
            let (_, mut rx2) = handle2.register(2).await.unwrap();

            // Explicitly close the first subchannel.
            drop(rx1);
