...
/// Switches to the next availability zone for a region.
/// Deletes the old subnet and creates a new one in the next AZ.
async fn switch_to_next_az(
    ...
    delete_subnet(ec2_client, &resources.subnet_id).await?;
    info!(
        subnet = resources.subnet_id.as_str(),
        az = resources.current_az(),
        region = region,
        "deleted subnet in failed AZ"
    );
    ...
    info!(
        az = new_az,
        region = region,
        "switching to next availability zone"
    );
    ...
    resources.subnet_id = new_subnet_id;
    Ok(())
}
...
pub async fn create(config: &PathBuf, concurrency: usize) -> Result<(), Error> {
    ...
    // Process regions sequentially to allow AZ switching, but instances within a region in parallel
    ...
    for (region, instances_in_region) in binary_configs_by_region {
        ...
        loop {
            // Try to launch all instances in this region
            let launch_futures: Vec<_> = instances_in_region
                .iter()
                .map(|(instance, ami_id)| {
                    let instance_type =
                    ...
                    async move {
                        ...
                        Ok::<(String, InstanceConfig), Error>((instance_id, (*instance).clone()))
                    }
                })
                .collect();

            let results = try_join_all(launch_futures).await;
            match results {
                Ok(launched) => {
                    for (instance_id, instance_config) in launched {
                        binary_launches.push((instance_id, region.clone(), instance_config));
                    }
                    break;
                }
                Err(e) => {
                    if is_insufficient_capacity_error(&e) && resources.has_more_azs() {
                        info!(
                            region = region.as_str(),
                            az = resources.current_az(),
                            error = %e,
                        ...
                        switch_to_next_az(ec2_client, resources, tag, &region).await?;
                        continue;
                    }
                    return Err(e);
                }
