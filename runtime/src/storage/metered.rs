...
mod tests {
    ...
    async fn test_metered_blob_metrics() {
        ...
        let mut read = [0u8; 11];
        blob.read_at(&mut read[..], 0).await.unwrap();
    ...
    async fn test_cloned_blobs_share_metrics() {
        ...
        clone1.write_at(&b"world"[..], 5).await.unwrap();
        let mut buf1 = [0u8; 10];
        let mut buf2 = [0u8; 10];
        clone1.read_at(&mut buf1[..], 0).await.unwrap();
        clone2.read_at(&mut buf2[..], 0).await.unwrap();
