...
fn is_canonical(targets: &[RemoveTarget]) -> Result<bool, Error> {
        ...
        fn ensure_generation(&mut self) -> io::Result<&Path> {
            if self.generation.is_none() {
                ...
                loop {
                    ...
                    let generation =
                        graveyard.join(format!("{:08x}-{nonce:016x}", std::process::id()));
...
pub(crate) use filesystem::interrupt_committed_for_test;
#[cfg(windows)]
pub(crate) use filesystem::remove_windows;
#[cfg(not(target_arch = "wasm32"))]
pub(crate) use filesystem::{recover, remove_batch};
