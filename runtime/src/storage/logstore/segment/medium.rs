// The Sim crash model and its helpers are exercised only under cfg(test) and
// the fuzz feature, so non-test lib builds see them as dead.
#![allow(dead_code)]
//! The I/O seam under the segment backend: files, directory entries, and barriers.
//!
//! The segment backend's correctness argument is about per-file barriers and directory
//! durability, neither of which [crate::Blob] exposes, so the backend is generic over
//! this trait instead of composing over another [crate::Storage]. The protocol is
//! written once against [Medium]; per-platform differences live in implementations
//! below the seam.
//!
//! # Crash model
//!
//! A completed [File::sync] is a barrier for that file only: every write completed
//! through any handle to the file before the sync began is durable. Between barriers,
...
//! Ranges extended by [File::set_len] or never written read as zeros.
//!
//! # Implementations
//!
...
//!   uncovered (the reachability rule).
//! - `tokio` (sibling module): the production medium over a real filesystem.
...
pub trait File: Clone + Send + Sync + 'static {
    ...
    /// A barrier: all writes completed through any handle to the file before this
    /// call began are durable when it returns Ok (fdatasync semantics).
...
pub trait Medium: Clone + Send + Sync + 'static {
    ...
    /// Free bytes remaining on the medium, or None when unknown (the default:
    /// treat as unbounded). [Sim] reports its capacity fuse; the tokio medium
    /// (`super::tokio::Fs`) reports the root filesystem's statvfs.
    fn free_bytes(&self) -> impl Future<Output = Result<Option<u64>, Error>> + Send {
        async { Ok(None) }
    }
...
/// A durability claim one durable structure makes about another (the reachability
/// rule): the claimed state must have been covered by a completed barrier before the
/// record or root that asserts it is written.
#[derive(Debug)]
pub enum Claim<'a> {
...
struct SimState {
    ...
    syncs: u64,
    /// Post-crash handle invalidation: a session is one process lifetime of the
    /// medium, and handles remember the session they were opened in.
    session: u64,
    /// Barriers left before every barrier fails (fault injection); None = never fail.
    sync_fuse: Option<u64>,
    /// Writes left before every write fails (fault injection); None = never fail.
    write_fuse: Option<u64>,
    /// Removes left before every remove fails (fault injection); None = never fail.
    remove_fuse: Option<u64>,
    /// Free bytes remaining (capacity fuse); None = unbounded. File extension
    /// charges it, truncation and removal refund it, and an extension that
    /// does not fit fails like ENOSPC, changing nothing. An approximation by
    /// design: a crash does not re-derive the counter from the settled
    /// files, and an open handle's bytes are refunded at unlink, not at last
    /// close.
    capacity: Option<u64>,
    next_file: u64,
...
impl SimState {
    ...
    /// Burns one write off the fuse; errors once it is spent. A failed write
    /// changes nothing.
    fn burn_write_fuse(&mut self) -> Result<(), Error> {
        match &mut self.write_fuse {
            None => Ok(()),
            Some(0) => Err(io_error(
                std::io::ErrorKind::Other,
    ...
    /// Burns one remove off the fuse; errors once it is spent. A failed remove
    /// changes nothing.
    fn burn_remove_fuse(&mut self) -> Result<(), Error> {
        match &mut self.remove_fuse {
            None => Ok(()),
            Some(0) => Err(io_error(
                std::io::ErrorKind::Other,
    ...
    /// Charges `bytes` of new allocation against the capacity fuse; an
    /// extension that does not fit fails whole, changing nothing.
    fn charge_capacity(&mut self, bytes: u64) -> Result<(), Error> {
    ...
    /// Refunds `bytes` of freed allocation to the capacity fuse.
    const fn refund_capacity(&mut self, bytes: u64) {
        if let Some(free) = &mut self.capacity {
            *free += bytes;
        }
    }
...
impl Sim {
    ...
    /// After `n` more successful writes, every write fails, changing nothing:
    /// fault injection for commit-failure tests.
    pub fn fail_writes_after(&self, n: u64) {
        self.state.lock().write_fuse = Some(n);
    }

    /// After `n` more successful removes, every remove fails, changing nothing:
    /// fault injection for deletion-failure tests.
    pub fn fail_removes_after(&self, n: u64) {
        self.state.lock().remove_fuse = Some(n);
    }

    /// Sets the capacity fuse: `bytes` of free space remain (None removes the
    /// bound). File extension charges it, truncation and removal refund it,
    /// and an extension that does not fit fails like ENOSPC, changing
    /// nothing. A crash leaves the counter as it stands.
    ...
    pub fn crash(&self) {
        ...
        state.write_fuse = None;
        state.remove_fuse = None;
...
impl SimFileHandle {
    ...
    fn check_session(&self, state: &SimState) -> Result<(), Error> {
...
impl File for SimFileHandle {
    ...
    async fn write_at(&self, offset: u64, data: impl Into<IoBufs> + Send) -> Result<(), Error> {
        ...
        let len = state
            ...
            .live
            .len() as u64;
        state.charge_capacity(end.saturating_sub(len))?;
        let file = state.files.get_mut(&self.id).expect("checked above");
    ...
    async fn set_len(&self, len: u64) -> Result<(), Error> {
        ...
        let old = state
            ...
            .len() as u64;
        if len > old {
            state.charge_capacity(len - old)?;
        } else {
            state.refund_capacity(old - len);
        }
        let file = state.files.get_mut(&self.id).expect("checked above");
...
impl Medium for Sim {
    ...
    async fn rename(&self, dir: &str, from: &str, to: &str) -> Result<(), Error> {
        ...
        let replaced = entry.live.insert(to.to_string(), id);
        // A replaced destination is unlinked: refund its allocation.
        if let Some(old) = replaced {
            let bytes = state.files.get(&old).map_or(0, |f| f.live.len() as u64);
            state.refund_capacity(bytes);
        }
        Ok(())
    }
    ...
    async fn remove(&self, dir: &str, name: &str) -> Result<(), Error> {
        let mut state = self.state.lock();
        state.burn_remove_fuse()?;
        let entry = state
            .dirs
            .get_mut(dir)
            .ok_or_else(|| missing("directory"))?;
        let id = entry.live.remove(name).ok_or_else(|| missing("file"))?;
        let bytes = state.files.get(&id).map_or(0, |f| f.live.len() as u64);
        state.refund_capacity(bytes);
        Ok(())
    }
    ...
    async fn free_bytes(&self) -> Result<Option<u64>, Error> {
        Ok(self.state.lock().capacity)
    }
}

/// A [Medium] wrapper that tracks what every completed barrier covered, making the
/// reachability rule machine-checkable: [Medium::covered] answers whether a claim's
/// state was durable, at the content generation the claimant last wrote, when the last
/// barrier completed.
///
...
impl<M: Medium> Checked<M> {
    /// Wraps `inner` with reachability-rule coverage tracking.
...
mod tests {
    ...
    #[test]
    fn sim_capacity_charges_extension_and_refunds_release() {
        block_on(async {
            let sim = Sim::new(4);
            sim.set_capacity(Some(10));
            let file = sim.create("dir", "a").await.unwrap();
            file.write_at(0, vec![1u8; 6]).await.unwrap();
            assert_eq!(sim.free_bytes().await.unwrap(), Some(4));
            // Overwrites within the length are free.
            file.write_at(0, vec![2u8; 6]).await.unwrap();
            assert_eq!(sim.free_bytes().await.unwrap(), Some(4));
            // An extension that does not fit fails whole and changes nothing.
            assert!(file.write_at(6, vec![3u8; 5]).await.is_err());
            assert_eq!(file.size().await.unwrap(), 6);
            assert_eq!(sim.free_bytes().await.unwrap(), Some(4));
            // Truncation refunds; growth charges; removal refunds it all.
            file.set_len(2).await.unwrap();
            assert_eq!(sim.free_bytes().await.unwrap(), Some(8));
            assert!(file.set_len(11).await.is_err());
            file.write_at(2, vec![4u8; 8]).await.unwrap();
            assert_eq!(sim.free_bytes().await.unwrap(), Some(0));
            sim.remove("dir", "a").await.unwrap();
            assert_eq!(sim.free_bytes().await.unwrap(), Some(10));
        });
    }

    #[test]
    fn sim_rename_replacement_refunds_destination() {
        block_on(async {
            let sim = Sim::new(4);
            sim.set_capacity(Some(10));
            let a = sim.create("dir", "a").await.unwrap();
            a.write_at(0, vec![1u8; 4]).await.unwrap();
            let b = sim.create("dir", "b").await.unwrap();
            b.write_at(0, vec![2u8; 6]).await.unwrap();
            assert_eq!(sim.free_bytes().await.unwrap(), Some(0));
            sim.rename("dir", "a", "b").await.unwrap();
            assert_eq!(sim.free_bytes().await.unwrap(), Some(6));
        });
    }

    #[test]
    fn sim_failed_write_changes_nothing() {
        block_on(async {
            let sim = Sim::new(4);
            let file = sim.create("dir", "a").await.unwrap();
            file.write_at(0, vec![1u8; 4]).await.unwrap();
            sim.fail_writes_after(1);
            file.write_at(4, vec![2u8; 4]).await.unwrap();
            assert!(file.write_at(8, vec![3u8; 4]).await.is_err());
            assert_eq!(file.size().await.unwrap(), 8);
        });
    }

    #[test]
    fn sim_failed_remove_changes_nothing() {
        block_on(async {
            let sim = Sim::new(4);
            sim.create("dir", "a").await.unwrap();
            sim.create("dir", "b").await.unwrap();
            sim.fail_removes_after(1);
            sim.remove("dir", "a").await.unwrap();
            assert!(sim.remove("dir", "b").await.is_err());
            assert!(sim.open("dir", "b").await.unwrap().is_some());
        });
    }
