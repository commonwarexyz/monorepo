use super::{
    Affinity, Alarm, Batch, Builder, Deadline, DriverFailure, DriverSignal, ENTRY_FAILED,
    ENTRY_FIRED, ENTRY_QUIESCENT, ENTRY_WAITING, EXPIRY_YIELD_BUDGET, Entry, NOT_IN_HEAP,
    RegisteredSleep, Shard, ShardLifecycle, THREAD_ASSIGNMENTS, ThreadAssignments, Timer,
    WAKE_BATCH, register_relative, run_driver, run_driver_loop,
    test_helpers::{allocator_claims, assignment},
};
use crate::{
...
fn register_after<A: Alarm>(shard: &Arc<Shard<A>>, duration: Duration) -> RegisteredSleep<A> {
...
fn registration_is_eager_and_cancellation_is_immediate() {
    ...
    let registered = register_after(&shard, Duration::from_nanos(100));
...
async fn injected_readiness_fires_a_registered_sleep() {
    ...
    let registered = register_after(&shard, Duration::from_nanos(20));
    let entry = Arc::clone(&registered.entry);
...
fn stop_quiesces_queued_sleeps_without_retaining_wakers_or_alarm() {
    ...
    let (shard, control) = fake_shard();
    let first = register_after(&shard, Duration::from_nanos(10));
    let first_entry = Arc::clone(&first.entry);
    let second = register_after(&shard, Duration::from_nanos(20));
...
fn clean_runtime_shutdown_does_not_report_pending_sleeps() {
    ...
    const TEST_NAME: &str =
        "tokio::timer::scheduler::tests::clean_runtime_shutdown_does_not_report_pending_sleeps";
...
async fn wait_error_and_panic_fail_pending_sleep_with_precise_diagnostics() {
    ...
    for (inject, expected_diagnostics) in cases {
        ...
        let (shard, control, panicked) = observed_fake_shard();
        let registered = register_after(&shard, Duration::from_nanos(10));
        let entry = Arc::clone(&registered.entry);
        consume_signal(&shard);
        inject(&control);
...
#[test]
fn affinity_caches_fallback_assignment() {
    // A non-worker selects one fallback shard on first use.
    let affinity = affinity(2);
    let fallback = affinity.select();

    // Repeated use retains that shard without advancing either allocator.
    assert_eq!(fallback, affinity.select());
    assert_eq!(affinity.next_worker.load(AtomicOrdering::Relaxed), 0);
...
fn timer_build_waits_for_every_initial_worker_claim() {
    ...
    runtime_builder
        ...
        .enable_all();
    let timer_builder = Builder::install(&mut runtime_builder, WORKERS);
    ...
    let claims_at_return = allocator_claims(&timer);
...
fn thread_assignment_cache_prunes_dropped_runtimes_on_cold_paths() {
    ...
    assignments.install(&first, 3);
    let second = affinity(1);
    assignments.install(&second, 4);
    assert_eq!(assignment(&assignments, &first), Some(3));
    ...
    assert_eq!(assignment(&assignments, &second), Some(4));

    // Replacing a dead current assignment does not preserve it as inactive.
    drop(second);
    assignments.install(&third, 5);
    assert!(assignments.inactive.is_empty());
    assert_eq!(assignment(&assignments, &third), Some(5));
}
...
fn cross_runtime_clock_use_preserves_worker_assignment() {
    ...
    // Runtime two must also retain its fallback assignment across another switch.
    assert_eq!(second.select(), 0);
    assert_eq!(second.next_fallback.load(AtomicOrdering::Relaxed), 1);
}
    ...
    runtime.block_on(async {
        for _ in 0..REPETITIONS {
            ...
            let probe = tokio::spawn(async move {
                ...
                let cached = THREAD_ASSIGNMENTS.with(|assignments| {
                    assignment(&assignments.borrow(), &probe_affinity)
            ...
            let (index, cached) = progress.expect("replacement worker did not run the probe task");
