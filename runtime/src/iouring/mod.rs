...
impl Handle {
    ...
    pub(crate) async fn write_at(
        ...
        self.write_at_inner(file, offset, bufs, options, cache, None)
            .await
    }

    /// Submit a write while retaining `resource` until the logical request's final CQE.
    #[cfg(feature = "iouring-storage")]
    pub(crate) async fn write_at_retained<T: Send + Sync + 'static>(
        &self,
        file: Arc<File>,
        offset: u64,
        bufs: IoBufs,
        ...
        self.write_at_inner(file, offset, bufs, options, cache, Some(resource))
            .await
    }
