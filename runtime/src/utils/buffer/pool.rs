...
pub struct ImmutableBlob<B: Blob> {
    ...
    /// Internal page buffer & other cached info.
    buffer: Arc<RwLock<Buffer>>,
}

impl<B: Blob> ImmutableBlob<B> {
...
impl<B: Blob> Blob for ImmutableBlob<B> {
    async fn read_at(
        &self,
        ...
        while !buf_slice.is_empty() {
            println!(
                "reading from buffer pool. blob={}, buf_len={}, offset={offset}",
                buf_slice.len(),
                self.id,
            );
            ...
            // Get a write lock on our reusable page buffer.
            let _buffer = self.buffer.write().await;
            let bytes_to_read = if page_offset + PAGE_SIZE_U64 > blob_len {
                blob_len - page_offset
            } else {
                PAGE_SIZE_U64
            };
            let mut page_buf = vec![0u8; bytes_to_read as usize];
            page_buf = self
                .blob
                .read_at(page_buf, page_offset)
                .await?
                .as_ref()
                .to_vec();
            ...
            // Put the page in the cache, which reads the appropriate bytes from it.
            {
                ...
                count = buffer_pool.cache_page(page_buf.as_mut(), self.id, buf_slice, offset);
            }
            offset += count as u64;
            buf_slice = &mut buf_slice[count..];
        }
    ...
    async fn write_at(&self, _buf: impl Into<StableBuf> + Send, _offset: u64) -> Result<(), Error> {
        panic!("ImmutableBlob does not support writes");
    }
    ...
    async fn truncate(&self, _len: u64) -> Result<(), Error> {
        panic!("ImmutableBlob does not support truncation");
    }
...
impl BufferPool {
    ...
    fn read_at(&self, blob_id: u32, buf: &mut [u8], offset: u64) -> usize {
        ...
        let Some(page) = page else {
            println!("Page fault: {page_num} {offset}");
            return 0;
        };
    ...
    fn cache_page(&mut self, page: &mut [u8], blob_id: u32, buf: &mut [u8], offset: u64) -> usize {
        let (page_num, offset) = Self::offset_to_page(offset);
        println!("Caching page {page_num} {}", page.len());

        let page_array = if page.len() < PAGE_SIZE {
            debug!("Caching short page {page_num} len={}", page.len());
