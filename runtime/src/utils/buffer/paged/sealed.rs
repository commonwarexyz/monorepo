...
//! # Sealing
//!
//! [`super::Writer::seal`] consumes the write handle and returns a [`Sealed`] handle together with
//! a sync completion handle.
//!
//! # Cheap sharing
//!
...
mod tests {
    ...
    fn test_seal_starts_sync() {
        ...
        executor.start(|context: deterministic::Context| async move {
            ...
            let sealed = append.seal().await.unwrap().into_synced().await.unwrap();
    ...
    fn test_seal_replay_without_sync() {
        ...
        executor.start(|context: deterministic::Context| async move {
            ...
            let data: Vec<u8> = (0u8..=255).cycle().take(total).collect();
            append.append(&data).await.unwrap();
            let (sealed, sync) = append.seal().await.unwrap().into_parts();
    ...
    fn test_seal_recovered_synced_partial_page_no_write() {
        ...
        executor.start(|context: deterministic::Context| async move {
            ...
            let sealed = recovered.seal().await.unwrap().into_synced().await.unwrap();
