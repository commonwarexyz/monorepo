...
mod tests {
    ...
    fn test_writes_use_uncached_hint() {
        ...
        executor.start(|context: deterministic::Context| async move {
            let blob = PartialWriteBlob::new(SyncTrackingBlob::new(), usize::MAX, 0);
    ...
    struct PartialWriteBlob<B: Blob> {
        ...
        uncached_writes: Arc<AtomicUsize>,
        uncached_sync_writes: Arc<AtomicUsize>,
    ...
    impl<B: Blob> PartialWriteBlob<B> {
        fn new(inner: B, fail_on: usize, partial_len: usize) -> Self {
            Self {
                ...
                uncached_writes: Arc::new(AtomicUsize::new(0)),
                uncached_sync_writes: Arc::new(AtomicUsize::new(0)),
        ...
        fn uncached_snapshot(&self) -> (usize, usize) {
            (
                self.uncached_writes.load(Ordering::SeqCst),
                self.uncached_sync_writes.load(Ordering::SeqCst),
            )
        }
    }
    ...
    impl<B: Blob> crate::Blob for PartialWriteBlob<B> {
        ...
        async fn write_at_with_options(
            ...
            if options.contains(WriteOptions::DONT_CACHE) {
                let counter = if options.contains(WriteOptions::SYNC) {
                    &self.uncached_sync_writes
                } else {
                    &self.uncached_writes
                };
            ...
            if write == self.fail_on {
                ...
                self.inner
                    .write_at_with_options(offset, bytes.slice(..partial_len), options)
                    .await?;
    ...
    impl<B: Blob> crate::Blob for TornExtensionBlob<B> {
        ...
        async fn write_at_with_options(
            ...
            self.inner
                ...
                .await?;
            self.inner
                .write_at_with_options(footer_offset, bytes.slice(footer_start..), write_options)
                .await?;
            self.inner.sync().await?;
