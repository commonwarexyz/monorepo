...
pub struct Writer<B: Blob> {
    ...
    /// The partial-page checksum covered by the last completed durability barrier. Its slot must
    /// be preserved until another barrier completes.
    synced_partial_page_state: Option<ActiveChecksum>,
...
impl<B: Blob> Writer<B> {
    ...
    const fn mark_partial_page_synced(&mut self) {
        self.synced_partial_page_state = self.partial_page_state;
...
mod tests {
    ...
    fn test_unsynced_snapshot_and_replay_preserve_completed_start_sync() {
        ...
        executor.start(|context: deterministic::Context| async move {
            ...
            assert_eq!(latest_crc.get_slot(latest_crc.authoritative()).0, 4);
    ...
    fn test_reopen_syncs_visible_partial_page_before_rewrite() {
        ...
        executor.start(|context: deterministic::Context| async move {
            ...
            assert_eq!(latest_crc.get_slot(latest_crc.authoritative()).0, 3);
    ...
    fn test_resize_same_page_shrink_cannot_reactivate_retired_checksum() {
        ...
        executor.start(|context| async move {
            ...
            let page = blob
                ...
                .coalesce();
            let record = read_crc_record_from_page(page.as_ref());
            assert_eq!(record.get_slot(record.authoritative()).0, 50);
