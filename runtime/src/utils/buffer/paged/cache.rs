...
impl CacheRef {
    ...
    /// page or the gathering cap, and a success over that prefix is returned as authoritative.
    ...
    /// Like [Self::decode_cached_prefix] but requires the encoding to occupy exactly `len`
...
impl Cache {
    ...
    /// logical pages ([Self::insert] asserts `page.len() == page_size`, `fetch_cacheable_page`
    /// rejects partial pages, and [Self::invalidate_from] drops pages at and beyond a truncation
    /// point on resize). Every byte reachable through a gather is therefore a committed logical
    /// byte of the blob, and the page overlapping the tip is never resident, so gathering
    /// naturally stops at the flushed/tip boundary. A successful decode within the gathered
    /// prefix is thus valid even if gathering was truncated, PROVIDED two caller obligations
    /// hold: the decoder must be driven solely by the bytes it consumes (one whose result
    /// depends on `remaining()` would observe a residency-dependent window rather than the
    /// requested range; documented on the public prefix API), and resizes must be serialized
