...
mod tests {
    use super::*;
    use crate::{
        buffer::tests::SyncTrackingBlob, deterministic, Buf, BufferPoolConfig, BufferPooler as _,
        IoBufsMut, Runner as _, Storage as _, Supervisor as _,
    };
    ...
    fn test_read_many_into_straddle_prefix_miss() {
        ...
        executor.start(|context: deterministic::Context| async move {
            ...
            let cache_ref = CacheRef::new(context.child("page_cache"), PAGE_SIZE, NZUsize!(1));
            let append = Append::new(blob, blob_size, BUFFER_SIZE, cache_ref)
                .await
                .unwrap();
    ...
    fn test_sync_releases_tip_pool_slot_after_full_drain() {
        let cfg = deterministic::Config::new().with_storage_buffer_pool_config(
    ...
    fn test_reopen_partial_tail_append_and_resize() {
        ...
        executor.start(|context| async move {
            ...
            let cache_ref = CacheRef::new(context.child("page_cache"), PAGE_SIZE, NZUsize!(4));
