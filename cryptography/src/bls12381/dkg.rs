...
pub enum Error {
    ...
    DkgFailed,
    #[error("polynomial evaluated to zero")]
    ZeroShare,
}
...
impl DealerPrivMsg {
    ...
    pub const fn new(share: Private) -> Self {
        Self { share }
    }
}
...
impl<V: Variant, S: Signer> Dealer<V, S> {
    ...
    pub fn start<M: Faults>(
        ...
        let my_poly = Poly::new_with_constant(&mut rng, info.degree::<M>(), share.expose_unwrap());
        let priv_msgs = info
            .players
            .iter()
            .map(|pk| {
                let eval = my_poly.eval_msm(
                    &info.player_scalar(pk).expect("player should exist"),
                    &Sequential,
                );
                let private = Private::new(eval).ok_or(Error::ZeroShare)?;
                Ok((pk.clone(), DealerPrivMsg::new(private)))
            })
...
impl<V: Variant, S: Signer> Player<V, S> {
    ...
    pub fn finalize<M: Faults>(
        ...
        let private = Private::new(private).ok_or(Error::ZeroShare)?;
...
pub fn deal<V: Variant, P: Clone + Ord, M: Faults>(
    ...
    let shares: Vec<_> = players
        ...
        .map(|(i, p)| {
            ...
            let private = Private::new(eval).ok_or(Error::ZeroShare)?;
            let share = Share::new(participant, private);
            Ok((p.clone(), share))
        })
        .collect::<Result<_, _>>()?;
    let shares: Map<_, _> = shares
        .into_iter()
        .try_collect()
        .expect("players are unique");
    let output = Output {
            ...
            for (i_round, round) in self.rounds.into_iter().enumerate() {
                ...
                for &i_dealer in &round.dealers {
                        ...
                        round.shift_degrees.get(&i_dealer)
                    {
                        ...
                        let my_poly =
                            Poly::new_with_constant(&mut rng, degree, share.expose_unwrap());
                        let priv_msgs = info
                            .players
                            .iter()
                            .map(|pk| {
                                (
                                    pk.clone(),
                                    DealerPrivMsg::new(
                                        Private::new(my_poly.eval_msm(
                                            &info.player_scalar(pk).expect("player should exist"),
                                            &Sequential,
                                        ))
                                        .expect("polynomial evaluation is non-zero"),
                                    ),
                                )
                            })
                            .collect::<Vec<_>>();
                    ...
                    for (player, priv_msg) in &mut priv_msgs {
                        ...
                        if round.bad_shares.contains(&(i_dealer, player_key_idx)) {
                            *priv_msg = DealerPrivMsg::new(Private::random(&mut rng));
                        }
                    }
                        ...
                        DealerResult::Ok(results) => {
                            ...
                            for &i_player in &round.players {
                                ...
                                *results
                                    ...
                                    .ok_or_else(|| anyhow!("unknown player: {:?}", &player_pk))? =
                                    AckOrReveal::Reveal(DealerPrivMsg::new(Private::random(
                                        &mut rng,
                                    )));
                            }
                        }
...
mod test {
    ...
    fn test_dealer_priv_msg_redacted() {
        ...
        let msg = DealerPrivMsg::new(Private::random(&mut rng));
        let debug = format!("{:?}", msg);
        assert!(debug.contains("REDACTED"));
    }

    #[test]
    fn test_dealer_priv_msg_rejects_zero() {
        use commonware_codec::{DecodeExt, Error::Invalid};
        let zero_bytes = [0u8; 32];
        let result = DealerPrivMsg::decode(&mut zero_bytes.as_slice());
