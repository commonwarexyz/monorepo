...
impl<P: PublicKey, V: Variant, N: Namespace> Generic<P, V, N> {
        ...
        D: Digest,
    {
        ...
        if certificate.signers.count() < self.participants.quorum::<Bft3f1>() as usize {
            return false;
        }
...
mod tests {
    ...
    fn test_verify_certificate_rejects_sub_quorum<V: Variant>() {
        ...
        let mut certificate = schemes[0].assemble::<_, Bft3f1>(attestations, &Sequential).unwrap();
    ...
    fn test_assemble_certificate_rejects_duplicate_signers<V: Variant>() {
        ...
        schemes[0].assemble::<_, Bft3f1>(attestations, &Sequential);
    }
