...
use commonware_codec::{EncodeFixed, EncodeSize, Error as CodecError, FixedSize, Read, ReadExt, Write};
...
impl Write for PrivateKey {
    fn write(&self, buf: &mut impl BufMut) {
        self.inner.expose(|x| buf.put_slice(&x.encode_fixed::<SCALAR_LENGTH>()));
    }
}
