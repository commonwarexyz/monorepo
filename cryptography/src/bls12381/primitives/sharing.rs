...
impl<V: Variant> Sharing<V> {
    ...
    pub fn precompute_partial_publics(&self) {
        ...
        self.evals
            ...
            .for_each(|(e, s)| {
                e.get_or_init(|| self.poly.eval_msm(&s, &ParallelNone));
            })
    }
        ...
        cfg_if! {
            if #[cfg(feature = "std")] {
                self.evals
                    .get(i as usize)
                    .map(|e| {
                        *e.get_or_init(|| self.poly.eval_msm(&self.scalar(i).expect("i < total"), &ParallelNone))
                    })
                    .ok_or(Error::InvalidIndex)
            } else {
                Ok(self.poly.eval_msm(&self.scalar(i).ok_or(Error::InvalidIndex)?, &ParallelNone))
            }
        }
    }
