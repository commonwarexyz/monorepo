...
mod tests {
    ...
    use commonware_codec::{Copying, Encode, FixedSize};
    ...
    fn off_subgroup_point(rng: &mut impl CryptoRng) -> G1 {
        loop {
            ...
            if let Ok(point) = G1::read_unchecked(&mut Copying(&bytes))
                && !point.in_subgroup()
            {
                return point;
            }
    ...
    fn plan_matches_exhaustive_search() {
        ...
        for combinations in [1usize, 2, 5, 37, 81, 128, 200] {
            for n in [
                0usize, 1, 7, 130, 200, 999, 1000, 6000, 20_000, 100_000, 400_000,
            ] {
                let mut widest = 0u32;
                while widest < MAX_WIDTH
                    && folded(widest + 1) <= 4 * n.max(1)
    ...
    fn round_isolates_each_combination() {
        ...
        let mut ids: Vec<u32> = (0..points.len()).map(|i| (i % folded(M)) as u32).collect();
        let mut round = Round::new(M);
        ...
        let mixed: Vec<G1> = points
            ...
            .map(|(index, point)| {
                ...
                if index % 3 == 0 && point != &G1::zero() {
                    G1::read_unchecked(&mut point.encode()).expect("decodes")
                } else {
                    *point
                }
            })
    ...
    fn round_fuzz_against_direct() {
        ...
        for seed in 0..400u64 {
            ...
            let ids: Vec<u32> = match seed % 4 {
                ...
                1 => (0..n)
                    .map(|i| ((seed as u32) % buckets) | if i % 2 == 0 { ID_NEGATE } else { 0 })
                    .collect(),
            ...
            for j in 0..m {
                ...
                if !affine_is_inf(&got_affine) {
                    assert_eq!(
                        got_affine.x.l, expected_affine.x.l,
                        "seed={seed} m={m} j={j}"
                    );
                    assert_eq!(
                        got_affine.y.l, expected_affine.y.l,
                        "seed={seed} m={m} j={j}"
                    );
                }
            }
    ...
    fn measure_strategies() {
        ...
        fn forced(
            ...
            let affine = G1::batch_to_affine(points);
            let mut trits = Trits::new(rng);
            let id_sets: Vec<Vec<u32>> = (0..rounds).map(|_| trits.fill(m, affine.len())).collect();
            strategy
                .map_init_collect_vec_with_multiplier(
                    id_sets.iter(),
                    affine.len(),
                ...
                .map_init_collect_vec_with_multiplier(
                    ...
                    |round, ids| {
                        ...
                        (0..3usize.pow(m)).all(|bucket| {
                            ...
                            !round.live[bucket]
                                || unsafe { blst_p1_affine_in_g1(&round.sums[bucket]) }
                        })
                    },
                )
                .into_iter()
