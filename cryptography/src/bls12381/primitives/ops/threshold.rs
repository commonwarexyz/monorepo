...
pub fn aggregate_partial_signatures<'a, V: Variant>(
    ...
    let signature = aggregate::combine_signatures::<V, _>(
        non_empty![@entries.iter().map(|entry| &entry.2.value)],
    );
...
mod tests {
    ...
    use commonware_utils::{Faults, N3f1, N5f1, NZU32, NZUsize, non_empty, test_rng, union_unique};
    ...
    fn aggregate_shape<V: Variant>(messages: &[(&[u8], &[u8])]) {
        let mut rng = test_rng();
        let (sharing, shares) =
            dkg::deal_anonymous::<V, N5f1>(&mut rng, Mode::NonZeroCounter, NZU32!(6));
        let partials: Vec<_> = shares
            .iter()
            .zip(messages)
