...
pub enum Error {
    ...
    InvalidFormattedBytesLength,
}

/// Types that can be fallibly read from a fixed-size bytes.
///
/// If a byte sequencer is not properly formatted, `TryFrom` must return an error.
pub trait FormattedBytes:
    Clone
    + Send
    + Sync
    + 'static
    ...
    + SizedSerialize
{
    ...
    fn read_from<B: Buf>(buf: &mut B) -> Result<Self, Error> {
        ...
        if buf.remaining() < len {
            return Err(Error::InvalidFormattedBytesLength);
        }
