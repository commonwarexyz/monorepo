...
commonware_macros::stability_scope!(BETA {
    ...
    pub trait Hasher: Default + Clone + Send + Sync + 'static {
        ...
        /// Pending computation returned by [`Hasher::update`].
        type Pending<'a>: PendingHasher<Digest = Self::Digest>
        where
            Self: 'a;

        /// Start hashing a message.
        fn update(&mut self, message: &[u8]) -> Self::Pending<'_>;

        /// Reset the hasher without generating a hash.
        ///
        /// This function does not need to be called after [`PendingHasher::finalize`].
        ...
        /// Hash an encoded value.
        ///
        /// The value is encoded with [`Encode`] and those exact bytes are hashed.
        ///
        /// Implementations are encouraged to specialize this for short, fixed-size encodable values
...
mod tests {
    ...
    fn test_hasher_multiple_runs<H: Hasher>() {
        ...
        let pending = hasher.update(b"hello mars");
        drop(pending);
        hasher.reset();
    ...
    fn test_hasher_empty_input<H: Hasher>() {
        let mut hasher = H::new();
        let digest = hasher.update(b"").finalize();
        assert!(H::Digest::decode(digest.as_ref()).is_ok());
    }
    ...
    fn test_hasher_dropped_pending_resets<H: Hasher>() {
        let mut hasher = H::new();
        let pending = hasher.update(b"discarded");
        drop(pending);

        let clean = hasher.update(b"hello world").finalize();
        assert_eq!(clean, H::hash(b"hello world"));
    }
    ...
    #[test]
    fn test_sha256_hasher_dropped_pending_resets() {
        test_hasher_dropped_pending_resets::<Sha256>();
    }
}
