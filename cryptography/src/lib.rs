...
pub enum Error {
    ...
    InvalidFormattedArrayLength,
}

/// Types that can be fallibly read from fixed-size bytes.
///
/// `FormattedArray` is typically used to parse things like `PublicKeys` and `Signatures`
/// from a byte sequence. Once parsed, these types are assumed to be well-formed (which
/// prevents duplicate validation).
///
/// If a byte sequencer is not properly formatted, `TryFrom` must return an error.
pub trait FormattedArray:
    Clone
    + Send
    + Sync
    + 'static
    ...
    + SizedSerialize
{
    ...
    fn read_from<B: Buf>(buf: &mut B) -> Result<Self, Error> {
        ...
        if buf.remaining() < len {
            return Err(Error::InvalidFormattedArrayLength);
        }
