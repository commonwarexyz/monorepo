...
commonware_macros::stability_scope!(BETA {
    ...
    /// # `Clone` resets
    ///
    /// This trait requires [`Clone`] because hashers are frequently embedded in
    /// structs that are themselves cloned. Implementations **must** make `Clone`
    /// return a freshly-reset hasher rather than a copy of any in-progress state.
    /// In other words, `hasher.clone()` is equivalent to `Self::default()`.
    /// Callers must not rely on cloning to duplicate partially-updated state.
    pub trait Hasher: Default + Clone + Send + Sync + 'static {
        ...
        /// small, fixed-shape inputs to avoid the overhead of the streaming machinery.
...
mod tests {
    ...
    fn test_hasher_multiple_updates<H: Hasher>() {
        ...
        // Generate hash in multi-part one-shot
        let digest_parts = H::hash([b"hello".as_slice(), b" world".as_slice()]);
        assert!(H::Digest::decode(digest_parts.as_ref()).is_ok());
