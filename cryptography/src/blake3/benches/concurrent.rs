                ...
                |b| {
                    b.iter_custom(|iters| {
                        measure(threads, iters, |thread| {
                            ...
                            black_box(Blake3::hash_pair(black_box(left), black_box(right)));
                        })
                    })
                },
            );
    ...
    for len in [256, 4096, 65_536, 1_048_576, 16_777_216] {
        ...
        for threads in THREADS {
            group.bench_function(format!("op=hash len={len} threads={threads}"), |b| {
                b.iter_custom(|iters| {
                    measure(threads, iters, |thread| {
                        black_box(Blake3::hash(&[black_box(&messages[thread][0])]));
                    })
                })
            });
        }
        ...
        for threads in THREADS {
            group.bench_function(
                ...
                |b| {
                    b.iter_custom(|iters| {
                        measure(threads, iters, |thread| {
                            black_box(Blake3::hash_many(black_box(&batches[thread])));
                        })
                    })
                },
            );
