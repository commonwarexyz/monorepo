...
mod tests {
    ...
    #[test]
    fn test_weighted_exact_quorum_acceptance_and_rejection() {
        let mut rng = test_rng();
        let (schemes, verifier) = setup_weighted_signers(&mut rng, &[1, 1, 1, 6]);
        let schemes = by_weight(&schemes);
        let expected = verifier.participants().quorum_weight::<N3f1>();
        ...
        assert_eq!(
            ...
            Err(AssemblyError::InsufficientAttestations(expected, 6))
        );
        ...
        let below_quorum = Certificate {
            ...
            signatures: vec![heavy.signature],
        };
        assert!(!verifier.verify_certificate::<_, Sha256Digest>(
            &mut rng,
            TestSubject {
    ...
    #[test]
    fn test_weighted_count_quorum_can_have_insufficient_weight() {
        let mut rng = test_rng();
        let (schemes, _) = setup_weighted_signers(&mut rng, &[1, 1, 1, 6]);
        let schemes = by_weight(&schemes);
    ...
    #[test]
    fn test_weighted_quorum_can_use_fewer_signers() {
        let mut rng = test_rng();
        let (schemes, verifier) = setup_weighted_signers(&mut rng, &[1, 1, 1, 6]);
        let schemes = by_weight(&schemes);
        let attestations: Vec<_> = [schemes[0], schemes[3]]
            .map(|scheme| {
                scheme
                    .sign::<Sha256Digest>(TestSubject {
                        message: Bytes::from_static(MESSAGE),
    ...
    #[test]
    fn test_weighted_single_participant_quorum() {
        let mut rng = test_rng();
        let (schemes, verifier) = setup_weighted_signers(&mut rng, &[1, 1, 1, 7]);
        let schemes = by_weight(&schemes);
        let attestation = schemes[3]
            .sign::<Sha256Digest>(TestSubject {
                message: Bytes::from_static(MESSAGE),
            })
            .unwrap();
        let certificate = schemes[3]
            .assemble(non_empty![attestation], &Sequential)
            .unwrap();
        ...
        let remaining: Vec<_> = schemes[..3]
            ...
            .collect();
        assert_eq!(
            schemes[0].assemble(non_empty![@remaining], &Sequential),
            Err(AssemblyError::InsufficientAttestations(7, 3))
        );
    }
    ...
    fn test_certificate_rejects_invalid_signer() {
        ...
        let unknown = Participant::new(999);
        attestations[0].signer = unknown;

        assert_eq!(
            schemes[0].assemble(non_empty![@attestations], &Sequential),
            Err(AssemblyError::UnknownSigner(unknown))
        );
    }
    ...
    fn test_assemble_certificate_rejects_duplicate_signers() {
        ...
        let duplicate_signer = duplicate.signer;
        attestations.push(duplicate);

        assert_eq!(
            schemes[0].assemble(non_empty![@attestations], &Sequential),
            Err(AssemblyError::DuplicateSigner(duplicate_signer))
        );
    }
