...
use crate::bls12381::{
    extension::{Fp2, Fp12, bounded::Fp2Ring, fp12},
    group::{G1, G2},
};
...
fn prepare<B: Backend>(
    ...
    for (p, q) in pairs {
        ...
        let [qz] =
            fp2.batch_reduce_expand(&[fp2.ready::<800>(
...
mod tests {
    ...
    fn check_affine_batch<B: Backend>(ring: &Ring<Bls12381, B>) {
        ...
        for (i, pair) in pairs.iter_mut().enumerate() {
            ...
            *pair = (
                {
                    let (x, y, _) = (p.x, p.y, p.z);
                    G1 {
                        x: x.mul(pz.square()),
                        y: y.mul(pz.square().mul(pz)),
                        z: pz,
                    }
                },
                {
                    let (x, y, _) = (q.x, q.y, q.z);
                    G2 {
                        x: x.mul(qz.square()),
                        y: y.mul(qz.square().mul(qz)),
                        z: qz,
                    }
                },
            );
        ...
        for identities in [false, true] {
            ...
            for len in 0..=pairs.len() {
                ...
                for (p, q) in &pairs[..len] {
                    let (Some((px, py)), Some((qx, qy))) = (p.to_affine(), q.to_affine()) else {
                        continue;
                    };
