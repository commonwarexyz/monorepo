...
abstract contract LibQMDBCurrentTest is UnorderedOracle {
    ...
    /// @dev Measure verification with a literal 32-byte bitmap chunk size.
    ...
    function checked(QMDBCase calldata c) external view returns (bool) {
        return _checked(c, VerificationKind.Membership, hex"", LibQMDBCurrent.ExclusionEncoding(0, 0));
    }
    ...
    function checkedExclusion(QMDBCase calldata c, bytes32 key) external view returns (bool) {
        return
            _checked(c, VerificationKind.FixedExclusion, abi.encodePacked(key), LibQMDBCurrent.ExclusionEncoding(0, 0));
    }
    ...
    function _checked(
        ...
        for (uint256 repeat; repeat < 2; ++repeat) {
            ...
            bool result;
            if (kind == VerificationKind.VariableExclusion) {
            ...
            } else if (kind == VerificationKind.FixedExclusion) {
                result = _family() == LibMerkle.Family.MMB
    ...
    function build(uint256 n, uint256 location, bytes memory operation, bool active, uint256 chunkBytes)
        external
        pure
    ...
    function test_IndependentBoundaryTrees() public view {
        ...
        for (uint256 i; i < sizes.length; ++i) {
            ...
            for (uint256 j; j < locations.length; ++j) {
                QMDBCase memory c = this.build(n, locations[j], hex"00112233445566778899aabbcc", true, 32);
                if (_family() == LibMerkle.Family.MMR) {
    ...
    function testFuzz_ArbitraryOperation(bytes memory operation, uint16 seed) public view {
        ...
        QMDBCase memory c = this.build(n, uint256(seed) % n, operation, true, 32);
        assertTrue(this.checked(c));
    ...
    function test_OperationLengths() public view {
        ...
        for (uint256 i; i < lengths.length; ++i) {
            ...
            assertTrue(this.checked(this.build(383, 127, operation, true, 32)));
        }
    }
    ...
    function test_BitmapBitOrder() public view {
        ...
        for (uint256 i; i < locations.length; ++i) {
            QMDBCase memory c = this.build(383, locations[i], hex"00", true, 32);
            assertTrue(this.verify(c));
            c = this.build(383, locations[i], hex"00", false, 32);
    ...
    function test_VariableChunkBoundaries() public view {
        ...
        for (uint256 i; i < sizes.length; ++i) {
            ...
            for (uint256 j; j < leaves.length; ++j) {
                uint256 n = leaves[j];
                assertTrue(this.checked(this.build(n, 0, hex"00", true, chunkBytes)));
                assertTrue(this.checked(this.build(n, n - 1, hex"01", true, chunkBytes)));
            }
        }
    }
    ...
    function test_MalformedChunkConfigurationAndLength() public view {
        QMDBCase memory c = this.build(33, 32, hex"0102", true, 2);
        assertTrue(this.checked(c));
    ...
    function rejectMutations(QMDBCase memory c, VerificationKind kind, bytes32 key) internal view {
        assertTrue(kind != VerificationKind.VariableExclusion, "membership or fixed exclusion verification required");
        assertTrue(kind == VerificationKind.FixedExclusion ? this.checkedExclusion(c, key) : this.checked(c));
        c.root ^= bytes32(uint256(1));
        assertFalse(kind == VerificationKind.FixedExclusion ? this.checkedExclusion(c, key) : this.checked(c));
        c.root ^= bytes32(uint256(1));
        ...
        assertFalse(
            kind == VerificationKind.FixedExclusion ? this.checkedExclusion(c, key) : this.checked(c),
            "trailing witness accepted"
        );
    ...
    function test_TamperedProofsAndBounds() public view {
        ...
        for (uint256 i; i < sizes.length; ++i) {
            rejectMutations(
                this.build(sizes[i], sizes[i] - 1, hex"010203", true, 32), VerificationKind.Membership, bytes32(0)
            );
    ...
    function test_AbsentDigestsAndZeroRoot() public view {
        QMDBCase memory c = this.build(512, 0, "", true, 32);
        assertEq(c.proof.partialDigest, 0);
        c.proof.partialDigest = bytes32(uint256(1));
        assertFalse(this.checked(c));
        c = this.build(383, 0, "", true, 32);
        assertEq(c.proof.pending, 0);
        c.proof.pending = bytes32(uint256(1));
        assertFalse(this.checked(c));
        c = this.build(1, 0, "", true, 32);
        c.root = 0;
        assertFalse(this.checked(c));
    ...
    function test_CrossFamilyRejection() public view {
        ...
        for (uint256 i; i < sizes.length; ++i) {
            QMDBCase memory c = this.build(sizes[i], sizes[i] - 1, hex"010203", true, 32);
            assertTrue(this.checked(c));
    ...
    function test_ExclusionIntervalsAndValueLengths() public view {
        ...
        for (uint256 i; i < lengths.length; ++i) {
            ...
            for (uint256 mode; mode < 3; ++mode) {
                ...
                QMDBCase memory c = this.build(383, 127, abi.encodePacked(bytes1(0xd2), left, value, right), true, 32);
    ...
    function test_ExclusionEmptyCommit() public view {
        ...
        for (uint256 i; i < lengths.length; ++i) {
            ...
            assertTrue(this.checkedExclusion(this.build(257, 256, operation, true, 32), 0));
        ...
        assertTrue(this.checkedExclusion(this.build(1, 0, absent, true, 32), bytes32(uint256(7))));
        absent = abi.encodePacked(hex"d300", uint64(382), new bytes(55));
        assertTrue(this.checkedExclusion(this.build(383, 382, absent, true, 32), 0));
        absent = abi.encodePacked(hex"d300", uint64(381), new bytes(55));
        assertFalse(this.checkedExclusion(this.build(383, 382, absent, true, 32), 0));
    }
    ...
    function test_ExclusionMalformedOperations() public view {
        assertFalse(this.checkedExclusion(this.build(1, 0, "", true, 32), 0));
        for (uint256 length = 1; length < 65; ++length) {
            bytes memory operation = new bytes(length);
            operation[0] = 0xd2;
            assertFalse(this.checkedExclusion(this.build(1, 0, operation, true, 32), bytes32(uint256(1))));
            operation[0] = 0xd3;
            assertFalse(this.checkedExclusion(this.build(1, 0, operation, true, 32), bytes32(uint256(1))));
        }
        ...
        for (uint256 tag; tag < 256; ++tag) {
            ...
            assertFalse(this.checkedExclusion(this.build(1, 0, commit, true, 32), bytes32(uint256(1))));
        }
        commit[0] = 0xd3;
        for (uint256 flag = 2; flag < 256; ++flag) {
            commit[1] = bytes1(uint8(flag));
            assertFalse(this.checkedExclusion(this.build(1, 0, commit, true, 32), 0));
        }
        commit[1] = 0;
        for (uint256 i = 10; i < commit.length; ++i) {
            commit[i] = 0x01;
            assertFalse(this.checkedExclusion(this.build(1, 0, commit, true, 32), 0), "nonzero commit padding");
            commit[i] = 0;
        }
        commit = abi.encodePacked(hex"d300", new bytes(33), uint64(0), new bytes(55));
        for (uint256 i = 2; i < 35; ++i) {
            commit[i] = 0x01;
            assertFalse(this.checkedExclusion(this.build(1, 0, commit, true, 32), 0), "nonzero absent metadata");
            commit[i] = 0;
        }
        commit = abi.encodePacked(hex"d301", bytes32(uint256(42)), uint64(0), new bytes(55));
        commit[commit.length - 1] = 0x01;
        assertFalse(this.checkedExclusion(this.build(1, 0, commit, true, 32), 0), "metadata commit padding");
    }
    ...
    function test_ExclusionProofAuthentication() public view {
        ...
        for (uint256 i; i < sizes.length; ++i) {
            ...
            for (uint256 mode; mode < 2; ++mode) {
                ...
                rejectMutations(c, VerificationKind.FixedExclusion, bytes32(uint256(15)));
    ...
    /// @dev Decode a production Current proof and activity verdict for an unordered log with 32-byte activity chunks.
    function generateUnordered32(
        uint256 leaves,
        uint256 location,
        uint256 floor,
        ...
        (
            ...
            abi.decode(
                unorderedFixture(
                    leaves, location, floor, encoding, operation, history, RootKind.Current, valueLength, 32
                ),
        ...
        for (uint256 encoding; encoding < 2; ++encoding) {
            for (uint256 i; i < operations.length; ++i) {
                for (uint256 j; j < (encoding == 0 ? 1 : lengths.length); ++j) {
                    (QMDBCase memory c, bool expected) = generateUnordered32(
                        1023,
                        1022,
                        i == 0 ? 768 : 0,
                    ...
                    if (expected) rejectMutations(c, VerificationKind.Membership, bytes32(0));
                }
            }
        }
    }
    ...
    function test_DifferentialInactivePeaks() public {
        ...
        for (uint256 i; i < floors.length; ++i) {
            ...
            rejectMutations(c, VerificationKind.Membership, bytes32(0));
            c = generate(n, n - 1, floors[i], 32);
    ...
    /// @dev Decode an exclusion fixture with 32-byte activity chunks and the Rust verifier's verdict for the queried key.
    function generateExclusion32(uint256 leaves, uint256 location, bytes32 key, string memory mode, bool metadata)
        internal
        returns (QMDBCase memory c, bool expected)
    {
        string[] memory args = new string[](metadata ? 18 : 17);
        args[0] = string.concat(vm.projectRoot(), "/../target/release/commonware-sol-fuzz");
        args[1] = "qmdb";
        args[2] = "exclude";
        args[3] = "--leaves";
        ...
        for (uint256 i; i < sizes.length; ++i) {
            ...
            for (uint256 j; j < locations.length; ++j) {
                ...
                for (uint256 k; k < queries.length; ++k) {
                    (QMDBCase memory c, bool expected) = generateExclusion32(n, location, queries[k], "interval", false);
                    bool member = n == 1
                        ? queries[k] != left
                        : location == n - 1
            ...
            for (uint256 j; j < 2; ++j) {
                ...
                (QMDBCase memory c, bool expected) = generateExclusion32(n, n - 1, query, "single", false);
                assertEq(expected, j != 0, "oracle single fixture");
                assertEq(this.checkedExclusion(c, query), expected, "Rust single disagreement");
                (c, expected) = generateExclusion32(n, n - 1, query, "empty", j != 0);
    ...
    function test_Gas() public {
        ...
        string[3] memory names =
            [string("grafted"), _family() == LibMerkle.Family.MMB ? "pending" : "grafted-second", "partial"];
        for (uint256 i; i < locations.length; ++i) {
            QMDBCase memory c = this.build(638, locations[i], new bytes(97), true, 32);
            assertTrue(this.verify32(c));
            emit log_named_uint(
                string.concat(_group(_family() == LibMerkle.Family.MMB ? "QMDBMMB" : "QMDBMMR"), "/", names[i]),
    ...
    function checkedVariableExclusion(
        QMDBCase calldata c,
        bytes calldata key,
        LibQMDBCurrent.ExclusionEncoding calldata encoding
    ) external view returns (bool) {
        return _checked(c, VerificationKind.VariableExclusion, key, encoding);
    }
    ...
    function test_VariableExclusionMixedFields() public view {
        ...
        for (uint256 keySize; keySize < 2; ++keySize) {
            for (uint256 valueSize; valueSize < 2; ++valueSize) {
                LibQMDBCurrent.ExclusionEncoding memory encoding = LibQMDBCurrent.ExclusionEncoding(
                    keySize == 0 ? variableSize : 1, valueSize == 0 ? variableSize : 1
                );
        ...
        LibQMDBCurrent.ExclusionEncoding memory emptyValue = LibQMDBCurrent.ExclusionEncoding(variableSize, 0);
    ...
    function checkVariableExclusion(
        ...
        string[] memory args = new string[](19 + (encoding.valueSize == variableSize ? 2 : 0) + options.length);
        args[0] = string.concat(vm.projectRoot(), "/../target/release/commonware-sol-fuzz");
        args[1] = "qmdb";
        ...
        args[14] = vm.toString(chunkBytes);
        args[15] = "--key-size";
        args[16] = encoding.keySize == variableSize ? "variable" : vm.toString(encoding.keySize);
        args[17] = "--value-size";
        args[18] = encoding.valueSize == variableSize ? "variable" : vm.toString(encoding.valueSize);
        uint256 cursor = 19;
        if (encoding.valueSize == variableSize) {
    ...
    function test_DifferentialVariableExclusion() public {
        ...
        assertTrue(checkVariableExclusion(3, 0, hex"01", encoding, 16, 0, options));
        encoding.keySize = variableSize;
        encoding.valueSize = 0;
        assertTrue(checkVariableExclusion(3, 0, hex"01", encoding, 2, 0, options));
        ...
        encoding.keySize = 0;
        encoding.valueSize = 0;
        bool absent = checkVariableExclusion(1, 0, hex"", encoding, 32, 0, options);
