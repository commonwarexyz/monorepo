...
pub fn test_collect_traces(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #vis #outer_sig {
            ...
            // Set up a _global_ subscriber for the test duration, allowing it to be used across
            // multiple threads (useful if using the tokio runtime in tests.)
            Registry::default()
                .with(collecting_layer)
                .with(fmt_layer)
                .init();
