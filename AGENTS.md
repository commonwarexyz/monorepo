...
- Implement core mechanisms and algorithms inside the workspace rather than adding a dependency for them. On hot paths, prefer `Bytes`, cheap `Arc` clones, and static dispatch over allocations and dynamic dispatch. Decode from owned buffers by value and preserve `commonware_codec::Buf` through decoding helpers. Use `Copying` only for intentionally borrowed input; retained byte fields decoded through it allocate and copy.
