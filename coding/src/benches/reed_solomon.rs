...
fn bench_encode(c: &mut Criterion) {
    bench_encode_generic::<ReedSolomon<Blake3>>("reed_solomon::encode", c);
}

fn bench_decode(c: &mut Criterion) {
    bench_decode_generic::<ReedSolomon<Blake3>>("reed_solomon::decode", c);
}

fn bench_encode_gf8(c: &mut Criterion) {
    bench_encode_generic::<ReedSolomon8<Blake3>>("reed_solomon_gf8::encode", c);
}

fn bench_decode_gf8(c: &mut Criterion) {
    bench_decode_generic::<ReedSolomon8<Blake3>>("reed_solomon_gf8::decode", c);
}
