...
fn bench_encode_gf16(c: &mut Criterion) {
    bench_encode_generic::<ReedSolomon16<Sha256>>("reed_solomon::encode", c);
}

fn bench_decode_gf16(c: &mut Criterion) {
    bench_decode_generic::<ReedSolomon16<Sha256>>("reed_solomon::decode", c);
}

fn bench_encode_gf8(c: &mut Criterion) {
    bench_encode_generic::<ReedSolomon8<Sha256>>("reed_solomon8::encode", c);
}

fn bench_decode_gf8(c: &mut Criterion) {
    bench_decode_generic::<ReedSolomon8<Sha256>>("reed_solomon8::decode", c);
}

criterion_group! {
    name = benches;
    config = Criterion::default().sample_size(10);
    targets = bench_encode_gf16, bench_decode_gf16, bench_encode_gf8, bench_decode_gf8
}
