...
fn fuzz(input: FuzzInput) {
    ...
    runner.start(|context| async move {
        let mut bitmap: BitMap<<Sha256 as Hasher>::Digest, CHUNK_SIZE> = BitMap::new();
            ...
            match op {
                ...
                BitmapOperation::GetChunk { bit_offset } => {
                    if bit_count > 0 {
                        ...
                        let chunk_aligned = (safe_offset
                            / BitMap::<<Sha256 as Hasher>::Digest, CHUNK_SIZE>::CHUNK_SIZE_BITS)
                            * BitMap::<<Sha256 as Hasher>::Digest, CHUNK_SIZE>::CHUNK_SIZE_BITS;
                ...
                BitmapOperation::LastChunk => {
                    if bit_count > pruned_bits {
                        ...
                        assert!(bits <= BitMap::<<Sha256 as Hasher>::Digest, CHUNK_SIZE>::CHUNK_SIZE_BITS);
                ...
                BitmapOperation::DirtyChunks => {
                    ...
                    let bits_per_chunk = BitMap::<<Sha256 as Hasher>::Digest, CHUNK_SIZE>::CHUNK_SIZE_BITS;
                ...
                BitmapOperation::Proof { bit_offset } => {
                    if bit_count > pruned_bits && !bitmap.is_dirty() {
                        ...
                        if let Ok((proof, chunk)) = bitmap.proof(&mut hasher, bit_offset).await {
                            ...
                            assert!(
                                BitMap::<<Sha256 as Hasher>::Digest, CHUNK_SIZE>::verify_bit_inclusion(
                                    &mut hasher,
                                    &proof,
                                    &chunk,
                                    bit_offset,
                ...
                BitmapOperation::RestorePruned => {
                    BitMap::<<Sha256 as Hasher>::Digest, CHUNK_SIZE>::restore_pruned(
                        context.clone(),
