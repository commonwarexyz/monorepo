            ...
            mmr = match op {
                ...
                MmrJournaledOperation::Reinit => {
                    ...
                    let new_mmr = Mmr::init(
                        context.with_label(format!("mmr_init_{metrics_label_counter}").as_str()),
                        &mut hasher,
                        test_config("fuzz_test_mmr_journaled"),
                    )
                    .await
                    .unwrap();
                    metrics_label_counter += 1;
                ...
                MmrJournaledOperation::InitSync {
                    ...
                    if let Ok(sync_mmr) = CleanMmr::init_sync(
                        context.with_label(format!("init_sync_{metrics_label_counter}").as_str()),
                        sync_config,
                        &mut hasher,
                    )
                    .await
