...
impl<B: Blob, E: Storage<B> + Metrics, A: Array> Journal<B, E, A> {
    const CHUNK_SIZE: usize = u32::LEN_CODEC + A::LEN_CODEC;
    ...
    fn verify_integrity(buf: &[u8]) -> Result<A, Error> {
        let stored_checksum = u32::from_be_bytes(buf[A::LEN_CODEC..].try_into().unwrap());
        let checksum = crc32fast::hash(&buf[..A::LEN_CODEC]);
        if checksum != stored_checksum {
            return Err(Error::ChecksumMismatch(stored_checksum, checksum));
        }
        Ok(buf[..A::LEN_CODEC].try_into().unwrap())
    }
...
mod tests {
    ...
    fn test_fixed_journal_replay() {
        ...
        executor.start(async move {
            ...
            let checksum_offset = Digest::LEN_CODEC as u64
                + (ITEMS_PER_BLOB / 2) * (Digest::LEN_CODEC + u32::LEN_CODEC) as u64;
            let blob = context
