...
use commonware_runtime::{ReadOptions, Storage, WriteOptions};
...
fn decode_frame<V: CodecShared>(
    ...
    if compression.is_some() {
        ...
        V::decode_cfg(decompressed, codec_config).map_err(Error::Codec)
    } else {
        V::decode_cfg(buf.slice(..data_len), codec_config).map_err(Error::Codec)
    }
}
...
impl<B: Blob, V: CodecShared> Reader<B, V> {
    ...
    pub(crate) async fn get(&self, offset: u64, size: u32) -> Result<V, Error> {
        ...
        decode_frame(buf.into(), self.compression, self.codec_config.as_ref())
    }
}
