...
impl<E: Storage + Metrics, V: CodecShared> Journal<E, V> {
    ...
    pub fn try_get_sync(&self, section: u64, offset: u64) -> Option<V> {
        ...
        // Compressed items keep the two-phase copy path so decompression runs over an owned
        // buffer, away from the tip lock.
        if self.compression.is_some() {
            return self.try_get_sync_compressed(blob, offset, remaining);
        }

        // Fuse the varint header parse and the item decode into a single cache access,
        // decoding in place from cached page bytes. Gathering is capped by page residency
        // and a fixed page budget, so the large max_len keeps the gathered range bounded;
...
mod tests {
    ...
    fn test_try_get_sync_uncompressed() {
        ...
        executor.start(|context| async move {
            ...
            // An item fully inside a resident page (item 8 is within page 2).
            assert_eq!(
                journal.try_get_sync(section, offsets[8]),
                Some(items[8].clone())
            );

            // An item spanning two resident pages (item 9 crosses the page 2/3 boundary
            // at byte 309).
            assert_eq!(
            ...
            // An item whose header is resident (page 3) but whose payload extends beyond
            // the flushed pages falls back to the async path.
    ...
    fn test_try_get_sync_compressed() {
        let executor = deterministic::Runner::default();
        executor.start(|context| async move {
            // With compression enabled, the sync path must still serve items via the
