...
use crate::journal::{
    Error,
    frame::{
        CompressedFrame, FrameInfo, UncompressedFrame, decode_item, decode_length_prefix,
        find_frame, read_frame_at,
    },
};
use bytes::Bytes;
use commonware_codec::{
    Codec, CodecShared, Copying, Encode as _, EncodeSize, Write, varint::MAX_U32_VARINT_SIZE,
};
use commonware_runtime::{
    Blob, Buf, BufferPool, Error as RError, Handle, Metrics, ReadOptions, Storage,
...
impl<E: Storage + Metrics, V: CodecShared> Inner<E, V> {
    ...
    /// Returns an append writer, requiring reopened nonempty sections to finish replay first.
    ...
    async fn append(&mut self, section: u64, item: &V) -> Result<(u64, u32), Error> {
        match self.compression {
            Some(level) => {
                let frame = CompressedFrame::new(&self.pool, level, item)?;
                let offset = self.append_frame(section, &frame).await?;
                Ok((offset, frame.item_len()))
            }
            None => {
    ...
    /// Append a validated frame, encoding it directly into the write buffer when it fits.
    async fn append_frame(
        &mut self,
        section: u64,
        frame: &(impl EncodeSize + Write + Sync),
    ) -> Result<u64, Error> {
...
mod tests {
    ...
    use commonware_runtime::{
        Blob, BufMut, IoBuf, Runner, Storage, Supervisor as _, WriteOptions,
    ...
    impl<E: Storage + Metrics, V: CodecShared> Inner<E, V> {
        /// Append raw bytes, which may be an invalid frame, to the given section.
            ...
            for source_section in [0, 1] {
                for cap in [0, 9] {
                    ...
                    for pages in [16, 4096] {
                        ...
                        // Appending after the cap must not reconnect any discarded frames.
                        (journal, _, _) = journal.append(0, &999).await.unwrap();
    ...
    #[test_traced]
    fn test_bounded_initialization_recovers_torn_page() {
        deterministic::Runner::default().start(|context| async move {
    ...
    #[test_traced]
    fn test_journal_replay_rejects_mid_frame_offset() {
        deterministic::Runner::default().start(|context| async move {
            let cfg = Config {
                partition: "mid-frame".into(),
                compression: None,
                codec_config: (),
                page_cache: CacheRef::from_pooler(&context, PAGE_SIZE, PAGE_CACHE_SIZE),
                write_buffer: NZUsize!(1024),
            };
            let journal = Journal::<_, [u8; 16]>::init(context.child("storage"), cfg.clone())
                .await
                .unwrap();
            ...
            assert!(matches!(
                replay.next().await,
                Some(Err(Error::ItemOutOfRange(1)))
            ));
            drop(replay);

            let journal = Journal::<_, [u8; 16]>::init(context.child("reopen"), cfg)
                .await
                .unwrap();
            let mut replay = journal
                ...
                .await
                .unwrap();
            assert_eq!(replay.next().await.unwrap().unwrap().3, [127; 16]);
            assert!(replay.next().await.is_none());
    ...
    #[test_traced]
    fn test_journal_uncompressed_frames_cross_buffer_boundary() {
        deterministic::Runner::default().start(|context| async move {
            let cfg = Config {
                partition: "test-partition".into(),
                compression: None,
            ...
            // Sixteen 128-byte frames fill the write buffer. The next 130-byte frame must use the
            // capacity fallback.
            for i in 0..16 {
                let offset;
                let item_len;
