    ...
    Oversized<E, I, V>
{
    ...
    /// batch commits both together. Values are still staged before the
    /// index, so a sequentially replayed batch (the test-only mock
    /// fallback) syncs them in the same glob-before-index order as
    /// [Self::sync].
