...
pub(crate) struct RecoveryPreflight<E: Storage + Metrics, A: CodecFixed> {
    ...
    /// Highest section the recovery owner may open. Later sections stay closed until the
    /// authorized truncation removes them.
    ceiling: u64,
}
...
impl<E: Storage + Metrics, A: CodecFixedShared> Inner<E, A> {
    ...
    /// Recovery opens sections up to `ceiling`, which every floor must lie within.
    ...
    /// See [Journal::init]. Sections above `ceiling` are left unopened for the truncation that
    /// removes them.
    async fn init(
        context: E,
        cfg: Config,
...
impl<E: Storage + Metrics, A: CodecFixedShared> Journal<E, A> {
    ...
    /// and must be removed with [Self::truncate_pending_tail] before publication.
    ...
    /// Prove validation floors without opening a writer or mutating their blobs. Recovery then
    /// opens sections up to `ceiling` only.
...
mod tests {
    ...
    impl<E: Storage + Metrics, A: CodecFixedShared> Journal<E, A> {
        fn test_reopen_configuration(&self) -> (E, Config) {
        ...
        pub(in super::super) fn test_configuration(&self) -> (E, String, AppendFactory) {
