use crate::{append_fixed_random_data, get_variable_journal, ITEM_SIZE};
    ...
    for items in [1_000, 10_000, 100_000, 500_000] {
                ...
                |b| {
                    ...
                    if !initialized {
                        Runner::new(cfg.clone()).start(|ctx| async move {
                            let mut j = get_variable_journal(ctx, PARTITION).await;
        ...
        if initialized {
            Runner::new(cfg).start(|context| async move {
                let j = get_variable_journal::<ITEM_SIZE>(context, PARTITION).await;
