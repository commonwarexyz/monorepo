...
/// Collect items from a chain of stacked auth batches for [`FinalizedBatch`].
///
/// Each [`MerkleizedBatch`] holds its items in an `Arc<Vec<Item>>`.
/// [`MerkleizedBatch::finalize`] walks the chain via this trait, collecting
/// one Arc clone (refcount bump) per level. The base MMR implements the
/// trivial base case (no items).
///
/// This trait is public because it appears in trait bounds on
/// [`MerkleizedBatch::finalize`], but callers never need to implement or
/// name it directly.
pub trait ItemChain<Item> {
    /// Collect item Arcs from this node and all ancestors in append order.
    fn collect_item_arcs(&self, into: &mut Vec<Arc<Vec<Item>>>);
}

impl<E: Storage + Clock + Metrics, D: Digest, Item> ItemChain<Item> for Mmr<E, D> {
...
impl<'a, H: Hasher, P: MmrRead<H::Digest>, Item: Encode> Batch<'a, H, P, Item> {
    /// Add an item. Encodes it, adds its digest to the MMR batch, and stores the item.
    ...
    /// Merkleize the batch, computing the speculative root.
...
impl<'a, H: Hasher, P: MmrRead<H::Digest>, Item> MerkleizedBatch<'a, H, P, Item> {
    /// Return the root of the merkleized batch.
    pub fn root(&self) -> H::Digest {
        self.inner.root()
    }
}

impl<'a, H: Hasher, P: MmrRead<H::Digest>, Item: Send + Sync> MmrRead<H::Digest>
...
impl<'a, H: Hasher, P: MmrRead<H::Digest> + ChainInfo<H::Digest>, Item: Send + Sync>
    ChainInfo<H::Digest> for MerkleizedBatch<'a, H, P, Item>
{
    fn base_size(&self) -> Position {
        self.inner.base_size()
    }
    fn base_visible(&self) -> Position {
        self.inner.base_visible()
    }
    fn collect_chain_overwrites(&self, into: &mut BTreeMap<Position, H::Digest>) {
        self.inner.collect_chain_overwrites(into);
    }
}

impl<'a, H: Hasher, P: MmrRead<H::Digest> + ItemChain<Item>, Item: Send + Sync> ItemChain<Item>
    for MerkleizedBatch<'a, H, P, Item>
{
    fn collect_item_arcs(&self, into: &mut Vec<Arc<Vec<Item>>>) {
        self.inner.parent().collect_item_arcs(into); // recurse to parent first
        into.push(self.items.clone()); // Arc refcount bump, not a data clone
    }
}

impl<'a, H: Hasher, P: MmrRead<H::Digest>, Item: Send + Sync + Encode>
    ...
    P: MmrRead<H::Digest> + ChainInfo<H::Digest> + ItemChain<Item>,
{
    ...
    pub fn finalize(self) -> FinalizedBatch<H::Digest, Item> {
        let mut item_chain = Vec::new();
        self.collect_item_arcs(&mut item_chain);
        let changeset = self.inner.into_changeset();
        FinalizedBatch {
            changeset,
            item_chain,
        }
    ...
    H: Hasher,
{
    ...
    pub fn new_batch(&self) -> Batch<'_, H, Mmr<E, H::Digest>, C::Item> {
        Batch {
    ...
    H: Hasher,
{
    ...
    async fn align(
        ...
        if mmr_size > journal_size {
            ...
            mmr.pop(*pop_count as usize, hasher).await?;
    ...
    pub async fn append(&mut self, item: &C::Item) -> Result<Location, Error> {
        ...
        let changeset = {
            ...
            batch.finalize(&mut self.hasher)
        };
    ...
    pub async fn apply_batch(
        &mut self,
        batch: FinalizedBatch<H::Digest, C::Item>,
    ) -> Result<(), Error> {
        for items in &batch.item_chain {
    ...
    H: Hasher,
{
    ...
    async fn rewind(&mut self, size: u64) -> Result<(), JournalError> {
        ...
        if leaves > size {
            self.mmr
                .pop((leaves - size) as usize, &mut self.hasher)
                .await
