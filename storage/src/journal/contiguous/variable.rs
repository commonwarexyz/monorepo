...
impl<E: Context, V: CodecShared> Journal<E, V> {
    ...
    pub async fn init(context: E, cfg: Config<V::Cfg>) -> Result<Self, Error> {
        ...
        // `align` synced any repaired or adopted data before `offsets.sync()`. `Writable::recover`
        // only seals already-recovered writers; sealing an unchanged recovered partial page emits no
        // new write.
    ...
    /// Begin persisting data blobs. Once returned, the journal stays usable while the commit is in
    /// flight. Does not advance the recovery watermark (reopen may replay) or sync the offsets
    /// journal; use [`Self::sync`] for full durability.
    pub async fn start_commit(&mut self) -> Handle<()> {
        self.metrics.record_commit();
        self.blobs.start_sync().await
    }

    /// Durably persist data blobs (blocking) so committed data survives a crash. Does not advance the
    /// recovery watermark or sync the offsets journal (see [`Self::sync`]).
    ...
    /// Durably persist data blobs and all metadata for both the data and offsets journals, blocking
    /// until complete.
...
mod tests {
    ...
    use crate::journal::contiguous::tests::{
        delayed_start_sync, partition_sync_fault, run_contiguous_tests,
    };
    ...
    fn test_variable_recovery_rejects_overlong_data_blob() {
        ...
        executor.start(|context| async move {
            ...
            journal.blobs.start_sync().await.await.unwrap();
            drop(journal);
    ...
    fn test_variable_recovery_preserves_rolled_predecessors_without_commit() {
        ...
        executor.start(|context| async move {
            ...
            assert_eq!(journal.append(&42).await.unwrap(), 3);
    ...
    fn test_variable_recovery_preserves_first_rollover_without_commit() {
        ...
        executor.start(|context| async move {
            ...
            assert_eq!(journal.append(&42).await.unwrap(), 2);
    ...
    /// blob-contiguity check, so the post-gap blob is truncated and recovery returns only the synced
    /// prefix.
    #[test_traced]
    ...
    ///
    /// Dirty data blobs sync before offsets. This reproduces a crash after blobs 0 and 1 were
    /// synced but before blob 2 and the offsets journal were, then asserts recovery keeps exactly
    /// the contiguous prefix 0..20.
    #[test_traced]
