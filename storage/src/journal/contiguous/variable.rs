...
impl<E: Context, V: CodecShared> Journal<E, V> {
    #[inline]
    fn mark_dirty_from(inner: &mut Inner<E, V>, section: u64) {
    ...
    pub async fn prune(&self, min_position: u64) -> Result<bool, Error> {
        ...
        // Remove old data and offset sections from storage under the read lock so concurrent readers
        // are not blocked while the (potentially slow) storage removal happens. The op_lock
        // serializes us against other mutators, so the state observed here remains valid until we
        // take the write lock.
    ...
    fn test_variable_recovery_from_mid_section_durable_anchor() {
        ...
        executor.start(|context| async move {
            ...
            let journal = Journal::<_, u64>::init(context.child("second"), cfg.clone())
                .await
                .unwrap();
            assert_eq!(journal.bounds().await, 7..13);
            for i in 0..6u64 {
