...
impl<E: Context, V: CodecShared> Journal<E, V> {
    ...
    pub(crate) async fn init_sync(
        ...
        authenticated::init_sync(context, cfg, range).await
    }
...
mod tests {
    use super::*;
    use crate::{
        journal::{authenticated::Recovery as _, contiguous::tests::run_contiguous_tests},
        utils::codec::View,
    };
    ...
    /// This is an impossible/corrupted state.
    #[test_traced]
    fn test_variable_recovery_offsets_end_behind_data_oldest_is_corruption_live_api() {
        let executor = deterministic::Runner::default();
        executor.start(|context| async move {
            let cfg = Config {
                partition: "offsets-end-behind-data-oldest".into(),
    ...
    /// remaining data blobs.
    #[test_traced]
    fn test_variable_recovery_offsets_behind_data_multi_blob_live_api() {
        let executor = deterministic::Runner::default();
        executor.start(|context| async move {
            // === Setup: Create Variable wrapper with data across multiple blobs ===
            let cfg = Config {
    ...
    #[test_traced]
    fn test_variable_recovery_rejects_watermark_beyond_retained_data_live_api() {
        let executor = deterministic::Runner::default();
        executor.start(|context| async move {
            let cfg = Config {
                partition: "recovery-anchor-too-far".into(),
    ...
    #[test_traced]
    fn test_variable_recovery_rejects_missing_data_below_watermark_live_api() {
        let executor = deterministic::Runner::default();
        executor.start(|context| async move {
            let cfg = Config {
                partition: "recovery-short-middle-retry".into(),
