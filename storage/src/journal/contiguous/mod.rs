...
pub trait Mutable: Contiguous + Sized {
    ...
    /// Return the durable prefix of the retained positions.
    ///
    /// The returned range must be `self.bounds().start..end`, where
    /// `self.bounds().start <= end <= self.bounds().end`, and every position it contains must
    /// survive a crash. The end may trail readable or recovered state.
