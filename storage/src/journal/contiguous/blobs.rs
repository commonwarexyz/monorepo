...
impl<E: Context> Writable<E> {
    ...
    pub(super) async fn seal_tail(&mut self) -> Result<(), Error> {
        ...
        assert!(self.tail_predecessor_sync.completion.is_none());
        assert!(self.tail_sync.completion.is_none());
        self.tail_predecessor_sync.completion = Some(handle.boxed().shared());
        Ok(())
    }
    ...
    pub(super) async fn start_sync(&mut self) -> Handle<()> {
        ...
        if let Some(prior) = self.tail_sync.completion.clone()
        ...
        self.metrics.synced.inc();
        self.tail_sync.completion = Some(tail.clone());
        let predecessor = self.tail_predecessor_sync.completion.clone();
...
mod tests {
    ...
    impl<E: Context> Writable<E> {
        pub(in crate::journal::contiguous) const fn has_tail_predecessor_sync(&self) -> bool {
            self.tail_predecessor_sync.completion.is_some()
        }
