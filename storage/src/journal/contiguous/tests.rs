    ...
    J: Mutable<Item = u64>,
{
    ...
    let indexed_factory = |name: String, cap: Option<u64>| {
    ...
    test_truncate_to_middle(&indexed_factory).await;
    test_truncate_to_zero(&indexed_factory).await;
    test_truncate_current_size(&indexed_factory).await;
    test_truncate_invalid_forward(&indexed_factory).await;
    test_truncate_invalid_pruned(&indexed_factory).await;
    test_truncate_then_append(&indexed_factory).await;
    test_truncate_zero_then_append(&indexed_factory).await;
    test_truncate_after_prune(&indexed_factory).await;
...
where
    ...
    J: Mutable<Item = u64>,
{
    let journal = factory("empty".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("bounds-with-items".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("bounds-after-prune".into(), None).await.unwrap();
    ...
    let journal = factory("bounds-after-prune".into(), None).await.unwrap();
    let bounds = journal.bounds();
    assert!(bounds.is_empty());
    journal.destroy().await.unwrap();
}
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("append-and-size".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("sequential-appends".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("replay-from-start".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("replay-from-middle".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("replay-from-unsealed-tail".into(), None)
        .await
        .unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("replay-with-small-buffer".into(), None)
        .await
        .unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("prune-retains-size".into(), None).await.unwrap();
    ...
    let journal = factory("prune-retains-size".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let journal = factory("through-trait".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("replay-after-prune".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("prune-then-append".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("position-stability".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("sync-behavior".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let journal = factory("replay-on-empty".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("replay-at-exact-size".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("multiple-prunes".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("prune-beyond-size".into(), None).await.unwrap();
    ...
    J: Mutable<Item = u64>,
{
    ...
    {
        let mut journal = factory(test_name.clone(), None).await.unwrap();
    ...
    {
        let journal = factory(test_name.clone(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("read-by-position".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("read-many".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("read-out-of-range".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("read-after-prune".into(), None).await.unwrap();
...
/// Test truncating to the middle of the journal
async fn test_truncate_to_middle<F, J>(factory: &F)
where
    F: Fn(String, Option<u64>) -> BoxFuture<'static, Result<J, Error>>,
    J: Mutable<Item = u64>,
{
    let mut journal = factory("truncate-to-middle".into(), None).await.unwrap();
    ...
    // Truncate to 12 items
    drop(journal.sync().await.unwrap());
    journal = factory("truncate-to-middle".into(), Some(12))
        .await
        .unwrap();
...
/// Test truncating to empty journal
async fn test_truncate_to_zero<F, J>(factory: &F)
where
    F: Fn(String, Option<u64>) -> BoxFuture<'static, Result<J, Error>>,
    J: Mutable<Item = u64>,
{
    let mut journal = factory("truncate-to-zero".into(), None).await.unwrap();
    ...
    drop(journal.sync().await.unwrap());
    journal = factory("truncate-to-zero".into(), Some(0)).await.unwrap();
...
/// Test truncate to current size is no-op
async fn test_truncate_current_size<F, J>(factory: &F)
where
    F: Fn(String, Option<u64>) -> BoxFuture<'static, Result<J, Error>>,
    J: Mutable<Item = u64>,
{
    let mut journal = factory("truncate-current-size".into(), None).await.unwrap();

    for i in 0..10u64 {
        (journal, _) = journal.append(&i).await.unwrap();
    }

    // Truncate to current size should be no-op
    drop(journal.sync().await.unwrap());
    let journal = factory("truncate-current-size".into(), Some(10))
        .await
        .unwrap();
    assert_eq!(journal.bounds().end, 10);
...
/// Test truncate with invalid forward size
async fn test_truncate_invalid_forward<F, J>(factory: &F)
where
    F: Fn(String, Option<u64>) -> BoxFuture<'static, Result<J, Error>>,
    J: Mutable<Item = u64>,
{
    let mut journal = factory("truncate-invalid-forward".into(), None)
        .await
        .unwrap();
    ...
    // Try to truncate forward (invalid)
    drop(journal.sync().await.unwrap());
    let result = factory("truncate-invalid-forward".into(), Some(20)).await;
    let journal = result.unwrap();
    assert_eq!(journal.bounds().end, 10);
    drop(journal);

    let journal = factory("truncate-invalid-forward".into(), None)
        .await
        .unwrap();
    journal.destroy().await.unwrap();
}

/// Test truncate to pruned position
async fn test_truncate_invalid_pruned<F, J>(factory: &F)
where
    F: Fn(String, Option<u64>) -> BoxFuture<'static, Result<J, Error>>,
    J: Mutable<Item = u64>,
{
    let mut journal = factory("truncate-invalid-pruned".into(), None)
        .await
        .unwrap();
    ...
    // Try to truncate to pruned position (invalid)
    drop(journal.sync().await.unwrap());
    let result = factory("truncate-invalid-pruned".into(), Some(5)).await;
    assert!(matches!(result, Err(Error::ItemPruned(5))));

    let journal = factory("truncate-invalid-pruned".into(), None)
        .await
        .unwrap();
    journal.destroy().await.unwrap();
}

/// Test truncate then append maintains position continuity.
/// Assumes items_per_blob = 10.
async fn test_truncate_then_append<F, J>(factory: &F)
where
    F: Fn(String, Option<u64>) -> BoxFuture<'static, Result<J, Error>>,
    J: Mutable<Item = u64>,
{
    let mut journal = factory("truncate-then-append".into(), None).await.unwrap();
    ...
    // Truncate to position 8 (within first section, not at boundary)
    drop(journal.sync().await.unwrap());
    journal = factory("truncate-then-append".into(), Some(8))
        .await
        .unwrap();
...
/// Test that truncating to zero and then appending works
async fn test_truncate_zero_then_append<F, J>(factory: &F)
where
    F: Fn(String, Option<u64>) -> BoxFuture<'static, Result<J, Error>>,
    J: Mutable<Item = u64>,
{
    let mut journal = factory("truncate-zero-then-append".into(), None)
        .await
        .unwrap();
    ...
    // Truncate to 0 (empty journal)
    drop(journal.sync().await.unwrap());
    journal = factory("truncate-zero-then-append".into(), Some(0))
        .await
        .unwrap();
...
/// Test truncating after pruning to verify correct interaction between operations.
/// Assumes items_per_blob = 10.
async fn test_truncate_after_prune<F, J>(factory: &F)
where
    F: Fn(String, Option<u64>) -> BoxFuture<'static, Result<J, Error>>,
    J: Mutable<Item = u64>,
{
    let mut journal = factory("truncate-after-prune".into(), None).await.unwrap();
    ...
    // Truncate to position 20 (still in retained range)
    drop(journal.sync().await.unwrap());
    journal = factory("truncate-after-prune".into(), Some(20))
        .await
        .unwrap();
    let bounds = journal.bounds();
    ...
    // Attempt to truncate to a pruned position should fail
    drop(journal.sync().await.unwrap());
    let result = factory("truncate-after-prune".into(), Some(5)).await;
    assert!(matches!(result, Err(Error::ItemPruned(5))));

    let journal = factory("truncate-after-prune".into(), None).await.unwrap();
    journal.destroy().await.unwrap();
}
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("append-many-empty".into(), None).await.unwrap();
    ...
    let journal = factory("append-many-empty".into(), None).await.unwrap();
    journal.destroy().await.unwrap();
}
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("append-many-basic".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("append-many-sections".into(), None).await.unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("append-many-then-single".into(), None)
        .await
        .unwrap();
...
where
    ...
    J: Mutable<Item = u64>,
{
    let mut journal = factory("append-many-single".into(), None).await.unwrap();
...
/// A sync handle must not block journal use while backend sync is pending.
#[boxed]
    ...
    J: Mutable<Item = u64>,
{
    ...
    // Fail the parked rollover sync; the drain in truncate must surface it. A failed mutable
...
fn test_fixed_prune_surfaces_failed_sync() {
    ...
    executor.start(|context| async move {
        let pending = PendingSyncs::default();
        let cfg = fixed::Config {
            partition: "fixed-truncate-fail".into(),
            items_per_blob: NZU64!(3),
            page_cache: CacheRef::from_pooler(&context, NZU16!(44), NZUsize!(8)),
            write_buffer: NZUsize!(2048),
            replay_buffer: NZUsize!(2048),
...
fn test_variable_prune_surfaces_failed_sync() {
    ...
    executor.start(|context| async move {
        let pending = PendingSyncs::default();
        let cfg = variable::Config {
            partition: "variable-truncate-fail".into(),
            items_per_section: NZU64!(3),
            compression: None,
            codec_config: (),
            page_cache: CacheRef::from_pooler(&context, NZU16!(44), NZUsize!(8)),
