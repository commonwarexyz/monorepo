...
pub mod contiguous;
mod frame;
commonware_macros::stability_mod!(ALPHA, pub mod logstore);
pub mod segmented;
...
pub enum Error {
    ...
    #[error(
        "offset/data layout mismatch in section {section} at offset {offset}: offsets journal expected {expected_len}, data varint reports {actual_len}"
    )]
    OffsetDataMismatch {
        section: u64,
        offset: u64,
