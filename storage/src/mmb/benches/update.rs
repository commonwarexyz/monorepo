...
use commonware_storage::mmb::{mem::DirtyMmb, Location, StandardHasher};
...
/// Benchmark the performance of randomly updating leaves in an MMB.
                            ...
                            match strategy {
                                Strategy::NoBatching => {
                                    for (loc, element) in &leaf_map {
                                        mmb.update_leaf(&mut h, *loc, element).unwrap();
                                    }
                                }
                                _ => {
                                    let updates: Vec<(
                                    ...
                                    mmb.update_leaf_batched(&mut h, pool.clone(), &updates)
                                        .unwrap();
                                    mmb.merkleize(&mut h, pool);
                                }
                            }
