...
use super::utils::{append_random_immutable, get_immutable_archive, ImmutableArchiveType, Key};
...
async fn read_serial_keys(a: &mut ImmutableArchiveType, reads: &[Key]) {
    for k in reads {
        black_box(a.get(Identifier::Key(k)).await.unwrap().unwrap());
    }
}

async fn read_serial_indices(a: &mut ImmutableArchiveType, indices: &[u64]) {
    for idx in indices {
        black_box(a.get(Identifier::Index(*idx)).await.unwrap().unwrap());
    }
}

async fn read_concurrent_keys(a: &mut ImmutableArchiveType, reads: Vec<Key>) {
    let futures = reads.iter().map(|k| a.get(Identifier::Key(k)));
    black_box(try_join_all(futures).await.unwrap());
}

async fn read_concurrent_indices(a: &mut ImmutableArchiveType, indices: &[u64]) {
