...
mod tests {
    ...
    fn test_multi_put_restart() {
        ...
        executor.start(|context| async move {
            ...
            let archive = Archive::<_, _, _, i32>::init(context.with_label("init2"), cfg)
                .await
                .expect("reinit");
    ...
    #[test_traced]
    fn test_multi_put_mixed_indices() {
        let executor = deterministic::Runner::default();
        executor.start(|context| async move {
            let cfg = Config {
                translator: FourCap,
            ...
            let mut archive = Archive::init(context.clone(), cfg).await.expect("init");
