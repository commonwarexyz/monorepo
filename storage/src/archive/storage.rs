...
impl<T: Translator, K: Array, B: Blob, E: Storage<B> + Metrics> Archive<T, K, B, E> {
    const PREFIX_LEN: u32 = (u64::LEN_CODEC + K::LEN_CODEC + u32::LEN_CODEC) as u32;
    ...
    fn parse_prefix(mut data: Bytes) -> Result<(u64, Bytes), Error> {
        ...
        let found = crc32fast::hash(&data[..K::LEN_CODEC + u64::LEN_CODEC]);
        let index = data.get_u64();
        let key = data.copy_to_bytes(K::LEN_CODEC);
    ...
    pub async fn put(&mut self, index: u64, key: K, data: Bytes) -> Result<(), Error> {
        ...
        let buf_len = u64::LEN_CODEC
            .checked_add(K::LEN_CODEC)
            .and_then(|len| len.checked_add(u32::LEN_CODEC))
