    ...
    T: Translator,
{
    ...
    async fn resize_journal(
        ...
        let size = journal.size().await.map_err(Error::from)?;
        ...
        if size <= range.start {
            ...
            if size > range.end {
                return Err(Error::UnexpectedData(Location::new_unchecked(size)));
            }
...
mod tests {
    ...
    fn create_sync_config(suffix: &str) -> immutable::Config<TwoCap, ()> {
