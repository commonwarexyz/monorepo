...
impl<
        ...
        H: CHasher,
        T: Translator,
    > KeyValueStore<E, K, V, T> for Any<E, K, V, H, T>
{
    ...
    async fn get(&self, key: &K) -> Result<Option<V>, crate::log_db::Error> {
        self.get(key).await.map_err(Into::into)
    }

    async fn update(&mut self, key: K, value: V) -> Result<(), crate::log_db::Error> {
        self.update(key, value).await.map_err(Into::into)
    }

    async fn delete(&mut self, key: K) -> Result<(), crate::log_db::Error> {
        self.delete(key).await.map_err(Into::into)
    }

    async fn commit(&mut self) -> Result<(), crate::log_db::Error> {
        self.commit().await.map_err(Into::into)
    }

    async fn sync(&mut self) -> Result<(), crate::log_db::Error> {
        self.sync().await.map_err(Into::into)
    }

    async fn prune(&mut self, target_prune_loc: Location) -> Result<(), crate::log_db::Error> {
        self.prune(target_prune_loc).await.map_err(Into::into)
    }

    async fn close(self) -> Result<(), crate::log_db::Error> {
        self.close().await.map_err(Into::into)
    }

    async fn destroy(self) -> Result<(), crate::log_db::Error> {
