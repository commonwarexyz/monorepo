...
mod tests {
    ...
    fn test_operation_value() {
        ...
        let commit_floor_op = Operation::<U64, U64>::CommitFloor(Some(value.clone()), Location::new_unchecked(42));
        assert_eq!(&value, commit_floor_op.value().unwrap());

        let commit_floor_op_none = Operation::<U64, U64>::CommitFloor(None, Location::new_unchecked(42));
    ...
    fn test_operation_encode_size() {
        ...
        // Test Commit operation with value
        let commit_op = Operation::<U64, U64>::Commit(Some(value.clone()));
        assert_eq!(commit_op.encode_size(), 1 + Some(value.clone()).encode_size());
        assert_eq!(commit_op.encode().len(), commit_op.encode_size());

        // Test Commit operation without value
        let commit_op = Operation::<U64, U64>::Commit(None);
        assert_eq!(commit_op.encode_size(), 1 + Option::<U64>::None.encode_size());
        assert_eq!(commit_op.encode().len(), commit_op.encode_size());
        ...
        assert_eq!(commit_floor_op.encode().len(), commit_floor_op.encode_size());
    }
    ...
    fn test_operation_display() {
        ...
        assert_eq!(
            ...
            format!("[commit {} with inactivity floor: {}]", hex(&value.encode()), Location::new_unchecked(42))
        );
        ...
        assert_eq!(
            format!("{commit_floor_op}"),
            format!("[commit with inactivity floor: {}]", Location::new_unchecked(42))
        );
    }
    ...
    impl Read for VariableSizeValue {
        ...
        fn read_cfg(buf: &mut impl Buf, _cfg: &Self::Cfg) -> Result<Self, CodecError> {
            ...
            let len = usize::try_from(len).map_err(|_| {
                CodecError::Invalid("VariableSizeValue", "length overflow")
            })?;
