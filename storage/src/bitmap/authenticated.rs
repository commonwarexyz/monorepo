...
impl<E: Clock + RStorage + Metrics, H: MmrHasher, const N: usize> MerkleizedBitMap<E, H, N> {
    ...
    pub async fn proof(&self, bit: u64) -> Result<(Proof<H::Digest>, [u8; N]), Error> {
        if bit >= self.len() {
        ...
        let last_chunk_digest = self.mmr.hasher().clone().digest(last_chunk);
    ...
    pub fn into_dirty(self) -> UnmerkleizedBitMap<E, H, N> {
        UnmerkleizedBitMap {
            bitmap: self.bitmap,
...
impl<E: Clock + RStorage + Metrics, H: MmrHasher, const N: usize> UnmerkleizedBitMap<E, H, N> {
    ...
    pub fn merkleize(mut self) -> Result<MerkleizedBitMap<E, H, N>, Error> {
        // Clone the hasher from the MMR since the batch borrows &self.mmr and batch methods
        // need &mut hasher simultaneously.
        let mut hasher = self.mmr.hasher().clone();
        ...
        let cached_root = if self.bitmap.is_chunk_aligned() {
            ...
            partial_chunk_root::<_, N>(&mut hasher, &mmr_root, next_bit, &last_chunk_digest)
        };
