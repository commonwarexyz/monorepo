...
use crate::{
    ...
    mmr::{grafting::Storage as GraftingStorage, verification, Location, Proof, StandardHasher},
...
///
/// Note: The generic par<ameter N is not really generic, and must be manually set to double the size
    ...
    > Current<E, K, V, H, T, N>
{
    ...
    pub async fn init(context: E, config: Config<T>) -> Result<Self, Error> {
        ...
        merkleize_grafted_bitmap::<H, N>(&mut hasher, &mut status, &any.log.mmr, height).await?;
