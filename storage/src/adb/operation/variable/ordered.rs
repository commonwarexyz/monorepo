...
pub enum Operation<K: Array + Ord, V: Codec> {
...
impl<K: Array + Ord, V: Codec> Ordered for Operation<K, V> {
...
impl<K: Array + Ord, V: Codec> Write for Operation<K, V> {
...
impl<K: Array + Ord, V: Codec> Keyed for Operation<K, V> {
    type Key = K;
    type Value = V;
...
impl<K: Array + Ord, V: Codec> Committable for Operation<K, V> {
    fn is_commit(&self) -> bool {
        matches!(self, Self::CommitFloor(_, _))
    }
}

impl<K: Array + Ord, V: Codec> Read for Operation<K, V> {
        ...
        match u8::read(buf)? {
            ...
            operation::COMMIT_FLOOR_CONTEXT => {
                ...
                let floor_loc = Location::new(floor_loc.into()).ok_or_else(|| {
                    CodecError::Invalid(
                        "storage::adb::operation::variable::ordered::Operation",
