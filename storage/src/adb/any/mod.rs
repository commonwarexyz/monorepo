...
use crate::{
    adb::{
        build_snapshot_from_log, find_update_op,
    ...
    > OperationLog<E, C, O, I, H, T>
{
    ...
    /// Closes the loc, ensuring all modifications are durably persisted to disk.
    ...
    /// Reeds and returns the operation at location `loc` within the log.
    ...
    pub(crate) async fn update_key_with_op(&mut self, op: O) -> Result<Option<Location>, Error> {
        ...
        let res = self
            .update_loc(
                op.key().expect("update operations should have a key"),
                new_loc,
            )
            .await?;
    ...
    pub(super) async fn update_loc(
        ...
        update_loc(&mut self.snapshot, &self.log, key, new_loc).await
    }
