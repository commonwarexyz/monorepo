...
> {
    ...
    pub(crate) log: AuthenticatedLog<E, C, O, H>,
...
impl<
        ...
        T: Translator,
        H: Hasher,
    > AnyDb<O, H> for IndexedLog<E, C, O, I, H, T>
{
    /// Returns the root of the authenticated log.
    fn root(&self, hasher: &mut StandardHasher<H>) -> H::Digest {
    ...
    ///     - the last operation performed, or    ///     - the operation `max_ops` from the start.
...
pub(super) mod test {
    ...
    use crate::{
        ...
        mmr::{mem::Mmr as MemMmr, Proof, StandardHasher as Standard},
        translator::TwoCap,
    };
