...
pub type Any<E, K, V, H, T> =
    IndexedLog<E, Journal<E, Operation<K, V>>, Operation<K, V>, Index<T, Location>, H, T>;
    ...
    Any<E, K, V, H, T>
{
    ...
    pub(crate) async fn update_return_loc(
        ...
        self.update_key_with_op(Operation::Update(key, value)).await
    }
    ...
    pub(crate) async fn delete_return_loc(&mut self, key: K) -> Result<Option<Location>, Error> {
        self.delete_key(Operation::Delete(key)).await
    }
}
...
pub(super) mod test {
    use super::*;
    use crate::{
        adb::{any::AnyDb, operation::fixed::unordered::Operation, store::Db as _, verify_proof},
