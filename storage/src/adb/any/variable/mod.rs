...
impl<E: Storage + Clock + Metrics, K: Array, V: Codec, H: CHasher, T: Translator, S: State>
    Any<E, K, V, H, T, S>
{
    ...
    fn as_shared(&mut self) -> SharedState<'_, E, K, V, H, T, S> {
        SharedState {
...
impl<E: Storage + Clock + Metrics, K: Array, V: Codec, H: CHasher, T: Translator>
    Any<E, K, V, H, T, Clean<<H as CHasher>::Digest>>
{
    ...
    pub async fn init(
        ...
        let (mmr, inactivity_floor_loc) =
            align_mmr_and_floored_log(mmr.into_dirty(), &mut log, &mut hasher).await?;
...
pub(super) mod test {
    ...
    type AnyTest<S> = Any<deterministic::Context, Digest, Vec<u8>, Sha256, TwoCap, S>;
    ...
    pub fn test_any_variable_db_commit_on_empty_db() {
        ...
        executor.start(|context| async move {
            let _hasher = StandardHasher::<Sha256>::new();
