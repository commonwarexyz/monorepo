...
//! independent.
//!
//! One example use case is the [crate::log_db::adb::current] authenticated database, where a MMR is
//! built over a log of operations, and a merkle tree over a bitmap indicating the activity state of
//! each operation. If we were to treat the two trees as independent, then an inclusion proof for an
    ...
    fn test_hasher_grafting() {
        ...
        executor.start(|_| async move {
            ...
            build_test_mmr(&mut standard, &mut base_mmr);
            {
                ...
                assert_eq!(
                    hasher.destination_pos(Position::try_from(Location::new_unchecked(1)).unwrap()),
                    Position::new(5)
                );
                assert_eq!(
                    hasher.destination_pos(Position::try_from(Location::new_unchecked(2)).unwrap()),
                    Position::new(9)
                );
                assert_eq!(
                    hasher.destination_pos(Position::try_from(Location::new_unchecked(3)).unwrap()),
                    Position::new(12)
                );
                assert_eq!(
                    hasher.destination_pos(Position::try_from(Location::new_unchecked(4)).unwrap()),
                    Position::new(17)
                );
