...
mod tests {
    ...
    use commonware_cryptography::{sha256::Digest, Hasher as CHasher, Sha256, Validator};
    ...
    fn test_verify_element() {
        ...
        assert!(
            !proof.verify_element_inclusion(
                &Digest::from([0u8; size_of::<Digest>()]),
                POS,
                &root_hash,
                &mut hasher
            ),
        ...
        let root_hash2 = Digest::from([0u8; size_of::<Digest>()]);
        assert!(
        ...
        proof2.hashes[0] = Digest::from([0u8; size_of::<Digest>()]);
        assert!(
        ...
        proof2 = proof.clone();
        proof2.hashes.push(Digest::from([0u8; size_of::<Digest>()]));
        assert!(
            !proof2.verify_element_inclusion(&element, POS, &root_hash, &mut hasher),
    ...
    fn test_verify_range() {
        ...
        for i in 0..49 {
            elements.push(Digest::from([i as u8; size_of::<Digest>()]));
        ...
        let mut invalid_root_hash = vec![0; size_of::<Digest>()];
        ...
        assert!(
            !range_proof.verify_range_inclusion(
                ...
                start_pos,
                end_pos,
                &Digest::validate(&invalid_root_hash).unwrap(),
                &mut hasher,
            ),
        ...
        let mut invalid_proof = range_proof.clone();
        invalid_proof.hashes[1] = Digest::from([0u8; size_of::<Digest>()]);
        assert!(
        ...
        for i in 0..range_proof.hashes.len() {
            ...
            invalid_proof
                .hashes
                .insert(i, Digest::from([0u8; size_of::<Digest>()]));
            assert!(
                !invalid_proof.verify_range_inclusion(
                    valid_elements,
                    start_pos,
