//! A lightweight, borrow-based fork of a merkleized MMR.
//!
//! A diff stores only what changed (appended nodes, overwrites, pruning
//! advances, truncation) and reads everything else through a shared reference
//! to its parent. Multiple diffs can coexist on the same parent. Diffs can be
//! stacked to arbitrary depth.
...
impl<'a, D: Digest, P: MmrRead<D>> DirtyDiff<'a, D, P> {
    ...
    pub fn merkleize(mut self, hasher: &mut impl Hasher<Digest = D>) -> CleanDiff<'a, D, P> {
...
// --- CleanDiff: MmrRead impl ---

impl<'a, D: Digest, P: MmrRead<D>> MmrRead<D> for CleanDiff<'a, D, P> {
...
// --- CleanDiff: ChainInfo impl ---

impl<'a, D: Digest, P: MmrRead<D> + ChainInfo<D>> ChainInfo<D> for CleanDiff<'a, D, P> {
...
impl<'a, D: Digest, P: MmrRead<D> + ChainInfo<D>> CleanDiff<'a, D, P> {
...
mod tests {
    ...
    /// Fork from base, add leaves, merkleize, read root + proofs from CleanDiff, discard diff,
    ...
    /// Single-element and range proofs from CleanDiff verify against root.
    #[test]
    ...
    /// CleanDiff -> into_dirty -> more mutations -> merkleize -> verify.
    #[test]
