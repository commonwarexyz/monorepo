...
impl<H: CHasher> Mmr<H> {
    ...
    /// Change the digest of an existing leaf. NOTE that this will change the root hash and
    /// invalidate any previous inclusion proofs! This is useful if you want to use the MMR
    /// implementation as an updatable binary Merkle tree, and otherwise should be avoided. Returns
    /// ElementPruned if some element required to update the tree has been pruned.
    ///
