...
mod tests {
    ...
    fn test_mem_mmr_empty() {
        ...
        executor.start(|_| async move {
            ...
            assert!(matches!(dirty_mmr.pop(), Err(Empty)));
            let mut mmr = dirty_mmr.merkleize(&mut hasher, None);
            mmr.prune_all();
    ...
    fn test_mem_mmr_pop() {
        ...
        executor.start(|_| async move {
            ...
            let mut dirty_mmr = mmr.into_dirty();
            assert!(
                matches!(dirty_mmr.pop().unwrap_err(), Empty),
            ...
            assert!(dirty_mmr.bounds().is_empty());
        });
    }
