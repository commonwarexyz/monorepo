...
impl<E: RStorage + Clock + Metrics, H: CHasher> Mmr<E, H, Clean<<H as CHasher>::Digest>> {
    ...
    pub async fn init_from_pinned_nodes(
        ...
        Ok(Self {
            mem_mmr: mem_mmr.merkleize(&mut StandardHasher::new()),
            journal,
    ...
    pub async fn init(context: E, hasher: &mut impl Hasher<H>, cfg: Config) -> Result<Self, Error> {
        ...
        if journal_size == 0 {
            ...
            return Ok(Self {
                mem_mmr: mem_mmr.merkleize(hasher),
                journal,
                journal_size,
                metadata,
                pruned_to_pos: Position::new(0),
...
impl<E: RStorage + Clock + Metrics, H: CHasher> Mmr<E, H, Dirty> {
    ...
    pub async fn pop(&mut self, mut leaves_to_pop: usize) -> Result<(), Error> {
        ...
        for pos in nodes_to_pin(new_size) {
            let digest = Mmr::<E, H, Clean<<H as CHasher>::Digest>>::get_from_metadata_or_journal(
                &self.metadata,
                &self.journal,
                pos,
            )
...
mod tests {
    ...
    fn test_journaled_mmr_pop() {
        ...
        executor.start(|context| async move {
            ...
            let mmr_clean = mmr.merkleize(&mut hasher);
            mmr_clean
                .proof(Location::try_from(leaf_pos).unwrap())
                .await
                .unwrap();
            // prune all remaining leaves 1 at a time.
            let mut mmr = mmr_clean.into_dirty();
    ...
    fn test_journaled_mmr_pruning() {
        ...
        executor.start(|context| async move {
            ...
            let pruned_mmr = Mmr::init(context.clone(), &mut hasher, cfg_pruned.clone())
                .await
                .unwrap();
            ...
            let mmr = Mmr::init(context.clone(), &mut hasher, cfg_unpruned)
                .await
                .unwrap();
            let mut leaves = Vec::with_capacity(LEAF_COUNT);
            let mut positions = Vec::with_capacity(LEAF_COUNT);
            ...
            for i in 0usize..300 {
                ...
                leaves.push(digest);
                let last_leaf = leaves.last().unwrap();
                let mut pruned_mmr_dirty = pruned_mmr.into_dirty();
                let pos = pruned_mmr_dirty.add(&mut hasher, last_leaf).await.unwrap();
                positions.push(pos);
                let mut mmr_dirty = mmr.into_dirty();
                mmr_dirty.add(&mut hasher, last_leaf).await.unwrap();
                pruned_mmr = pruned_mmr_dirty.merkleize(&mut hasher);
                mmr = mmr_dirty.merkleize(&mut hasher);
            ...
            pruned_mmr
                ...
                .await
                .unwrap();
            let pruned_mmr = pruned_mmr.merkleize(&mut hasher);
        ...
        executor.start(|context| async move {
            ...
            for i in 0usize..200 {
                ...
                for j in 0..10 {
                    ...
                    let mmr_clean = mmr.merkleize(&mut hasher);
                    assert_eq!(mmr_clean.root(), mmr_clean.root());
                    mmr = mmr_clean.into_dirty();
                    let digest = test_digest(LEAF_COUNT + i);
                    leaves.push(digest);
                    let last_leaf = leaves.last().unwrap();
    ...
    fn test_journaled_mmr_historical_range_proof_with_pruning() {
        ...
        executor.start(|context| async move {
            ...
            // mmr is already Clean after prune_to_pos
            let historical_proof = mmr
                .historical_range_proof(
                    historical_size,
    ...
    fn test_journaled_mmr_init_from_pinned_nodes() {
        ...
        executor.start(|context| async move {
            ...
            let mut original_mmr = original_mmr.into_dirty();
            ...
            let mut original_mmr = original_mmr.merkleize(&mut hasher);
            original_mmr.sync().await.unwrap();
            let original_size = original_mmr.size();
            original_mmr.prune_to_pos(original_size).await.unwrap();

            // Get the journal digest
            let mut hasher = Standard::<Sha256>::new();
            let original_journal_digest = original_mmr.root();
            ...
            let new_mmr = Mmr::<_, Sha256>::init_from_pinned_nodes(
                context.clone(),
                pinned_nodes,
                original_size,
            ...
            let new_journal_digest = new_mmr.root();
            ...
            let original_mmr = original_mmr.merkleize(&mut hasher);

            let mut new_mmr = new_mmr.into_dirty();
            ...
            let new_mmr = new_mmr.merkleize(&mut hasher);

            // Verify the roots still match
            let original_mmr_root = original_mmr.root();
    ...
    fn test_journaled_mmr_init_from_pinned_nodes_edge_cases() {
        ...
        executor.start(|context| async move {
            ...
            let mut empty_mmr = empty_mmr.into_dirty();
            ...
            assert_eq!(single_journaled_mmr.root(), single_root);
