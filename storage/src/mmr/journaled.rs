...
impl<B: Blob, E: RStorage<B> + Clock + Metrics, H: Hasher> Mmr<B, E, H> {
    ...
    async fn get_from_metadata_or_journal(
        ...
        if bytes.is_none() {
            ...
            warn!("metadata did not have requested node {}", pos);
    ...
    /// Compute and add required nodes for the given pruning point to the metadata, and sync it the
    /// updated metadata to disk. Return the computed set of required nodes.
    ...
    pub async fn prune_to_pos(&mut self, pos: u64) -> Result<(), Error> {
        ...
        let outcomes = self.journal.prune_pos_boundaries(pos);
