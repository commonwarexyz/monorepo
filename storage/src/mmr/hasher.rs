...
use super::iterator::leaf_pos_to_num;
...
/// corresponding branches of both trees.
///
/// TODO: This needs to return None instead of panicking on bad input since it's used in proof
/// verification.
...
impl<H: CHasher, S: Storage<H::Digest>> Hasher<H> for Grafting<'_, H, S> {
    async fn leaf_digest(&mut self, pos: u64, element: &[u8]) -> Result<H::Digest, Error> {
        ...
        // No need to include position in the digest material here since the same position was
        // already included in the material used to derive base_node_digest.
    ...
    fn root_digest<'a>(
        ...
        self.hasher
            .root_digest(self.destination_pos(pos), peak_digests)
    }
...
pub struct GraftingVerifier<'a, H: CHasher> {
    ...
    /// The leaf elements from the peak tree that we are verifying.
    elements: Vec<&'a [u8]>,
...
impl<H: CHasher> Hasher<H> for GraftingVerifier<'_, H> {
    ...
    fn node_digest(
        ...
        // Check if we are at the grafting boundary.
        if pos_to_height(pos) == self.height {
            // This digest gets fed into a leaf from the peak tree that is grafted to this one
            // instead of getting used as-is in the parent hash. We perform that transformation
            // below.
            let source_pos = source_pos(pos, self.height);
            let index = leaf_pos_to_num(source_pos);
            let Some(mut index) = index else {
                // malformed proof input
                debug!(index, "grafting index is not a leaf");
                return digest;
            };
            if index < self.num {
                // malformed proof input
...
mod tests {
    ...
    fn test_hasher_source_dest_pos_conversion() {
        for grafting_height in 1..10 {
            for pos in 0..10000 {
                ...
                let source_pos = source_pos(pos, grafting_height);
    ...
    /// Test building a small grafted MMR, then generating & verifying proofs over it.
    #[test_traced]
        ...
        executor.start(|_| async move {
...

            {
                ...
                // Confirm element inclusion proof verification fails for various manipulations of the input.
                {
                    ...
                    // TODO: pos+1==5 verifies, I guess since node 5 is the parent of the "correct" position.  Should we make this fail too?
                    // Perhaps verification should just check that the position is indeed a leaf.
                    assert!(!proof
                ...
                // test range proving
                {
                    ...
                    let proof = Proof::<Sha256>::range_proof(&grafted_mmr, 0, 4)
                        .await
                        .unwrap();
