...
//! it concatenates consecutive bit elements into chunks of DIGEST_SIZE bytes in order to reduce
//! overhead.
...
/// An MMR over bit values. Instead of treating each bit as a distinct element, it concatenates
/// consecutive bit elements into chunks of DIGEST_SIZE bytes in order to reduce overhead.
pub struct Mmr<H: CHasher> {
    /// The underlying MMR with each leaf representing DIGEST_SIZE number of bits.
    mmr: MemMmr<H>,

    /// The next "pending" element of bits to be added to the MMR once it is a full DIGEST_SIZE*8
    /// bits.
    next_element: Vec<u8>,

    /// The position within next_element of the next bit to be appended.
    ///
...
impl<H: CHasher> Mmr<H> {
    ...
    /// The number of bits in the bitmap MMR.
    ...
    pub fn root(&mut self, hasher: &mut H) -> H::Digest {
        ...
        // &mut self.  (see https://github.com/commonwarexyz/monorepo/pull/569)
        self.mmr.add(
            hasher,
...
mod tests {
    ...
    fn test_bitmap_mmr_build() {
        // Build a bitmap MMR with DIGEST_SIZE*8 elements where bits come from the following digest:
