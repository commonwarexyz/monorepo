...
//! verification of the inclusion of an element in a list. Like the [MMR](crate::merkle::mmr), it stores
...
use crate::{merkle, merkle::Family as _};
...
impl merkle::Family for Family {
    ...
    fn pos_to_height(pos: Position) -> u32 {
        ...
        let birth = Self::position_to_location(Position::new(pos.as_u64() - 1))
...
impl merkle::Graftable for Family {
    fn leftmost_leaf(pos: Position, height: u32) -> Location {
        ...
        // Recover birth leaf from position, then compute leftmost via the
        // closed-form: leftmost = birth - (3*2^(h-1) - 2).
        let birth = Self::position_to_location(Position::new(pos.as_u64() - 1))
            .expect("position is neither leaf nor parent");
        Location::new(*birth - (3 * (1u64 << (height - 1)) - 2))
    }
    ...
    fn subtree_root_position(leaf_start: Location, height: u32) -> Position {
        ...
        // birth_leaf = last_leaf + 2^(h-1) - 1, where last_leaf = leaf_start + 2^h - 1
        let last_leaf = *leaf_start + (1u64 << height) - 1;
        let birth_leaf = last_leaf + (1u64 << (height - 1)) - 1;
        Position::new(Self::location_to_position(Location::new(birth_leaf)).as_u64() + 1)
    }
    ...
    fn chunk_peaks(
        ...
        let n = Location::try_from(size)
            .expect("chunk_peaks: invalid size")
            .as_u64();
        assert!(
            chunk_end <= n,
        ...
        //
        // An MMB with N leaves has p = ilog2(N+1) peaks. Let M = N+1. Peak k
        // (0 = oldest) has height h_k = (p-1-k) + bit(M, p-1-k) and covers
        // 2^{h_k} leaves. The cumulative leaf count after k peaks is:
        //
        ...
        //   S(k) = M - 2^x - (M mod 2^x)
        //
        // We want the first peak containing chunk_start, i.e. the largest k
        // where S(k) <= chunk_start. Rearranging:
        //
        //   M - chunk_start <= 2^x + (M mod 2^x)
        //
        // Since the RHS grows with x, we find the smallest qualifying x via
        // ilog2 of the LHS, with at most one correction step.
        let m = n + 1;
        let p = m.ilog2(); // number of peaks

        let diff = m - chunk_start;
        let maybe_x = diff.ilog2();
        let x = if diff <= (1u64 << maybe_x) + (m & ((1u64 << maybe_x) - 1)) {
            maybe_x
        } else {
            maybe_x + 1
        };
        ...
        //
        // Starting from peak lo, walk peaks rightward until we pass the
        // chunk. Each peak either fits within the chunk (height <= gh) or
        // entirely contains it (height > gh). Partial overlaps are impossible
        // because both peak starts and chunk boundaries are multiples of
        // their respective sizes (powers of two in non-increasing order).
        //
        // Node positions are computed using the birth-leaf formula: any MMB
        // node at height h covering leaves [s, s + 2^h) was born at leaf
        // (s + 2^h - 1) + (2^(h-1) - 1), giving:
        //   position = location_to_position(birth_leaf) + 1
        // For h = 0, the node is a bare leaf at location_to_position(s).
        ...
        (lo..p).map_while(move |k| {
            ...
            let (pos, h) = if height <= grafting_height as u64 {
                ...
                // Peak entirely contains the chunk. Compute the height-gh
                // sub-node in O(1) via the birth-leaf formula.
...
#[cfg(test)]
mod tests {
    use super::{mem::Mmb, Family, Location, Position, StandardHasher};
    use crate::merkle::{Family as _, Graftable as _};
    ...
    fn test_pos_to_height() {
        // Verify pos_to_height for every node by tracking positions as they are appended.
        // Each step appends a leaf (height 0) and optionally a parent (height from
        // parent_heights).
    ...
    fn test_subtree_root_position_virtual_roundtrip() {
        // Verify the round-trip for subtree positions that may not correspond to
        // any physical node in the MMB. For example, in the 8-leaf MMB with
        // grafting height 2, chunk 1 (leaves [4,8)) has no single height-2 node,
        // but subtree_root_position still produces a deterministic position that
        // round-trips through leftmost_leaf.
