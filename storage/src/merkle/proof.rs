...
impl<F: Family, D: Digest> Proof<F, D> {
    ...
    pub fn verify_multi_inclusion<H, E>(
        &self,
        hasher: &H,
        elements: impl IntoIterator<Item: Borrow<(E, Location<F>)>>,
        root: &D,
    ) -> bool
    where
        H: Hasher<F, Digest = D>,
        E: AsRef<[u8]>,
    {
        ...
        // Retain leaf digests and blueprints so streamed element buffers can be reused.
        let mut node_positions = BTreeSet::new();
        let mut blueprints = BTreeMap::new();

        for element in elements {
            let (element, loc) = element.borrow();
            ...
            if let Some((_, previous)) = blueprints.insert(*loc, (bp, digest))
                && previous != digest
            {
                return false;
            }
        ...
        for (loc, (bp, digest)) in blueprints {
            let proof = bp
                .build_proof(
                    hasher,
                    self.inactive_peaks,
            ...
            match proof.reconstruct_root_with(
                hasher,
                core::iter::once(digest),
                loc,
                None,
                |_, digest| digest,
            ) {
        ...
        E: AsRef<[u8]>,
    {
        ...
        let end_loc = start_loc.checked_add(element_count as u64)?;
    ...
    fn reconstruct_root_with<H, E>(
        &self,
        hasher: &H,
        mut elements: impl ExactSizeIterator<Item = E>,
        start_loc: Location<F>,
        collected: Option<&mut Vec<(Position<F>, D)>>,
...
impl<F: Family> Subtree<F> {
    ...
    fn reconstruct_digest<D, H>(
        &self,
        hasher: &H,
        range: &Range<Location<F>>,
        leaf: &mut impl FnMut(Position<F>) -> Result<D, ReconstructionError>,
        siblings: &[D],
        cursor: &mut usize,
...
mod tests {
    ...
    /// Exercise actual element consumption even when an iterator misreports its length.
    struct ClaimedSize<I> {
        inner: I,
        remaining: usize,
    }

    impl<I: Iterator> Iterator for ClaimedSize<I> {
        type Item = I::Item;
    ...
    impl<I: Iterator> ExactSizeIterator for ClaimedSize<I> {}
    ...
    fn encoded_verification<F: Family>() {
        ...
        for (bagging, inactive_peaks) in supported_root_shapes::<F>(mem.leaves()) {
            ...
            for range in [0..1, 0..31, 1..23, 7..16, 20..31, 30..31] {
                ...
                #[cfg(feature = "std")]
                {
                    let encoded_store = crate::merkle::verification::ProofStore::new(
                        &hasher,
                        &proof,
                        ops.iter().map(Encode::encode),
                        start,
                ...
                assert!(!proof.verify_range_inclusion(
                    &hasher,
                    extra_ops.iter().map(Encode::encode),
                    start,
                    &root
                ));
                ...
                assert!(!proof.verify_range_inclusion(
                    &hasher,
                    wrong_ops.iter().map(Encode::encode),
                    start,
                    &root
                ));
                assert!(matches!(
                ...
                assert!(matches!(
                    malformed.verify_range_inclusion_and_extract_digests(
                        &hasher,
                        ops.iter().map(Encode::encode),
                        start,
                        &root
                ...
                for i in 0..proof.digests.len() {
                    ...
                    assert!(!missing.verify_range_inclusion(
                        &hasher,
                        ops.iter().map(Encode::encode),
                        start,
                        &root
                ...
                assert!(!invalid_boundary.verify_range_inclusion(
                    &hasher,
                    ops.iter().map(Encode::encode),
                    start,
                    &root
                ...
                assert!(!invalid_leaves.verify_range_inclusion(
                    &hasher,
                    ops.iter().map(Encode::encode),
                    start,
                    &root
        ...
        assert!(empty.verify_range_inclusion(
            &hasher,
            operations[..0].iter().map(Encode::encode),
            Location::new(0),
            &empty_root
        ));
        assert!(empty.verify_proof_and_pinned_nodes(
            &hasher,
            operations[..0].iter().map(Encode::encode),
            Location::new(0),
            &[],
        ...
        assert!(!empty.verify_range_inclusion(
            &hasher,
            operations[..1].iter().map(Encode::encode),
            Location::new(0),
            &empty_root
        ));
        assert!(matches!(
            empty.reconstruct_root(
                &hasher,
                ClaimedSize {
                    inner: elements[..1].iter(),
                    remaining: 0
                },
                Location::new(0),
            ),
        ...
        for (actual, missing) in [(1, true), (3, false)] {
            let result = full.reconstruct_root(
                &hasher,
                ClaimedSize {
                    inner: elements[..actual].iter(),
                    remaining: 2,
                },
                Location::new(0),
            );
            if missing {
                ...
                assert!(matches!(result, Err(ReconstructionError::ExtraDigests)));
            }
        }
        let mut extra_digest = empty;
        extra_digest.digests.push(D::EMPTY);
        assert!(!extra_digest.verify_range_inclusion(
            &hasher,
            operations[..0].iter().map(Encode::encode),
            Location::new(0),
            &empty_root
    ...
    fn multi_proofs_verify_for_supported_root_shapes<F: Family>() {
        ...
        for (bagging, inactive_peaks) in supported_root_shapes::<F>(leaves) {
            ...
            let operations: Vec<_> = locations.iter().map(|loc| **loc).collect();
            let mut encoded: Vec<_> = operations
                .iter()
                .map(Encode::encode)
                .zip(locations)
                .collect();
            assert!(proof.verify_multi_inclusion(&hasher, &encoded, &root));
            encoded.push(encoded[0].clone());
            assert!(proof.verify_multi_inclusion(&hasher, &encoded, &root));
            let mut conflicting = encoded.clone();
            conflicting.last_mut().unwrap().0 = 999u64.encode();
            assert!(!proof.verify_multi_inclusion(&hasher, &conflicting, &root));
            encoded.reverse();
            assert!(proof.verify_multi_inclusion(&hasher, &encoded, &root));
            let mut extra_digest = proof.clone();
            extra_digest.digests.push(D::EMPTY);
            assert!(!extra_digest.verify_multi_inclusion(&hasher, &encoded, &root));
            ...
            assert!(!tampered_boundary.verify_multi_inclusion(&hasher, &encoded, &root));

            if !proof.digests.is_empty() {
                let mut tampered_digest = proof.clone();
                tampered_digest.digests[0].0[0] ^= 1;
                assert!(!tampered_digest.verify_multi_inclusion(&hasher, &encoded, &root));
                assert!(
