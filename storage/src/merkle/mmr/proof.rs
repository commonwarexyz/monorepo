...
impl<D: Digest> Proof<Family, D> {
    ...
    /// Core implementation for reconstructing the MMR root from a proof.
    ///
    ...
    fn reconstruct_root_impl<H, E>(
        ...
        elements: &[E],
        start_loc: Location,
        collected_digests: Option<&mut Vec<(Position, D)>>,
    ) -> Result<D, ReconstructionError>
    where
        H: Hasher<Family, Digest = D>,
        E: AsRef<[u8]>,
    {
        reconstruct_root_standalone(
            hasher,
            self.leaves,
            &self.digests,
            elements,
...
/// Standalone MMR root reconstruction from proof data.
///
/// Uses the fold-based proof layout: `[folded_prefix? | after_peaks... | siblings_dfs...]`.
/// If `collected_digests` is Some, all node digests encountered during reconstruction are
/// appended to the wrapped vector.
    ...
    E: AsRef<[u8]>,
{
    ...
    for (i, &after_peak_pos) in after_peaks.iter().enumerate() {
        let digest = proof_digests[after_start + i];
        if let Some(ref mut cd) = collected_digests {
...
mod tests {
    ...
    fn test_proving_verify_element() {
        ...
        for leaf in 0u64..11 {
            ...
            let proof: Proof<Family, Digest> = mmr.proof(&mut hasher, leaf).unwrap();
            assert!(
    ...
    fn test_proving_proof_serialization() {
        ...
        for i in 0..elements.len() {
            for j in i + 1..elements.len() {
                ...
                // max_digests is the number of digests in the proof
                let max_digests = proof.digests.len();
                let deserialized_proof =
                    Proof::<Family, Digest>::decode_cfg(serialized_proof, &max_digests).unwrap();
                assert_eq!(
                ...
                assert!(
                    Proof::<Family, Digest>::decode_cfg(serialized_proof, &max_digests).is_err(),
                ...
                if actual_digests > 0 {
                    ...
                    let serialized_proof = proof.encode();
                    assert!(
                        Proof::<Family, Digest>::decode_cfg(serialized_proof, &too_small).is_err(),
    ...
    fn test_proving_multi_proof_generation_and_verify() {
        ...
        wrong_size_proof.leaves = Location::new(*<Family as merkle::Family>::MAX_LOCATION + 2);
    ...
    fn test_max_location_is_provable() {
        ...
        let max_loc = <Family as merkle::Family>::MAX_LOCATION;
    ...
    fn test_max_proof_digests_per_element_is_maximum() {
        ...
        assert!(
            size_for_63_peaks > *<Family as merkle::Family>::MAX_POSITION as u128,
