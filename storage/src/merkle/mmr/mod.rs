...
impl merkle::Graftable for Family {
    fn chunk_peaks(
        ...
        // In an MMR, every aligned chunk of 2^h leaves has exactly one subtree root
        // at height h.
    ...
    fn subtree_root_position(leaf_start: Location, height: u32) -> Position {
        ...
        Position::new(*leaf_pos + (1u64 << (height + 1)) - 2)
    }

    fn leftmost_leaf(pos: Position, height: u32) -> Location {
        let leftmost_pos = Position::new(*pos + 2 - (1u64 << (height + 1)));
...
mod tests {
    use super::*;
    use crate::merkle::Graftable as _;
    ...
    fn test_pos_to_height() {
        ...
        // Each step appends a leaf (height 0) then parents with heights from parent_heights.
    ...
    fn test_subtree_root_position() {
        // Verify subtree_root_position produces actual node positions by checking
        // every node in a growing MMR.
        let mut next_pos = 0u64;
        for leaf_idx in 0u64..500 {
            ...
            // For each parent created at this step, verify subtree_root_position
            // matches the actual position. In an MMR, a height-h parent covers
            // 2^h leaves ending at leaf_idx, so its leftmost leaf = leaf_idx + 1 - 2^h.
