...
pub trait Family: Copy + Clone + Debug + Send + Sync + 'static {
    ...
    /// # Panics
    ///
    /// Implementations may panic if `pos` does not correspond to a node that exists in
    /// some valid instance of this family (i.e., it is not a position that would appear
    /// in a structure of any size).
    fn pos_to_height(pos: Position<Self>) -> u32;
}

/// Extension of [`Family`] with methods needed for grafting bitmap chunks onto a Merkle
/// structure. Grafting combines an activity bitmap with an ops Merkle structure by hashing
/// bitmap chunks together with ops subtree roots. These methods provide the coordinate
/// conversions and chunk-to-peak mappings required by that process.
pub trait Graftable: Family {
    /// Return the nodes that collectively cover the leaf range of a bitmap chunk in a
    /// structure of the given `size`.
    ///
    /// A chunk at index `chunk_idx` with grafting height `grafting_height` covers leaves
    /// `[chunk_idx << grafting_height, (chunk_idx + 1) << grafting_height)`. The returned
    /// nodes partition that range: each node's leaf range is entirely within the chunk,
    ...
    /// # Panics
    ///
    /// Panics if `size` is not a valid size or if the chunk's leaf range exceeds
    /// the structure's leaf count.
    fn chunk_peaks(
        size: Position<Self>,
        chunk_idx: u64,
        grafting_height: u32,
    ...
    /// Return the deterministic position of the node at `height` whose leftmost leaf
    /// is at `leaf_start`.
    ///
    /// For some families, this position corresponds to a node that physically exists in
    /// any structure containing those leaves. For others (e.g. MMB with delayed merging),
    /// it may be a "virtual" position that no actual node occupies, but is still
    /// deterministic and unique for the given leaf range and height.
    ///
    /// Used by grafting to map grafted-structure positions to ops-structure positions
    /// for domain separation in hash pre-images.
    fn subtree_root_position(leaf_start: Location<Self>, height: u32) -> Position<Self>;

    /// Return the location of the leftmost leaf covered by the node at `pos` with `height`.
    /// For a leaf (height 0), returns its own location.
