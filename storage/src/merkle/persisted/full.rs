...
impl<F: Family, E: Context, D: Digest, S: Strategy> Merkle<F, E, D, S> {
    ...
    pub(crate) async fn prepare(
        ...
        for pos in F::nodes_to_pin(leaves) {
            pinned_nodes.push(Self::get_from_recovery(&metadata, Some(&journal), pos).await?);
        }
        ...
        for pos in F::nodes_to_pin(prune_loc) {
            extra_pinned.insert(
                pos,
                Self::get_from_recovery(&metadata, Some(&journal), pos).await?,
            );
        }
    ...
    /// Read a pinned node from metadata or initialization-owned journal storage. Without a
    /// journal, every node it would hold is pruned.
    async fn get_from_recovery(
        ...
        match journal.read(*pos).await {
            Ok(node) => Ok(node),
            Err(JError::ItemPruned(_)) => Err(Error::MissingNode(pos)),
    ...
    pub async fn init_sync(context: E, cfg: SyncConfig<F, D, S>) -> Result<Self, Error<F>> {
        ...
        // Missing boundary pins must come from the journal, and their iteration order need
        // not follow their positions. With no missing pins, coverage decides range reuse.
        ...
        let journal =
            if Journal::<E, D>::covers(&journal_context, &journal_cfg, *journal_probe).await? {
        ...
        let reset = opened.is_some()
            && (bounds.start > *prune_pos || (recovered_size <= prune_pos && *prune_pos != 0));
        let journal_size = if reset {
            prune_pos
        } else {
            recovered_size.min(end_pos)
        };
        ...
        // Metadata is synced before the journal is mutated, so an interrupted sync to another
        // range can leave that range's pins beside a journal it never touched. Retain only this
        // boundary's pins so supplied values can replace them, and so no later open prefers a
        // stale pin over a journal node at or above the selected end.
        metadata
        ...
        if prune_pos < journal_size {
            ...
            for pos in F::nodes_to_pin(prune_loc) {
                extra.insert(pos, Self::get_from_recovery(&metadata, opened, pos).await?);
            }
        ...
        // Sync targets refer to the same append-only history, so an opened journal that must
        // reset may still supply pins. Persist them before clearing it. An unopened journal
        // supplies none and is cleared here as well. Either mutation happens only after all
        // required pins have been validated and persisted.
...
mod tests {
    ...
    fn init_sync_after_interrupted_bounded_publication_inner<F: Family>() {
        let ((root_at_boundary, root_at_end), checkpoint) = deterministic::Runner::default()
            .start_and_recover(|context| async move {
                ...
                // recovery owner. The runtime snapshot then models a crash at this public cut.
                drop(bounded);
    ...
    fn test_init_sync_after_interrupted_bounded_publication_mmr() {
        init_sync_after_interrupted_bounded_publication_inner::<mmr::Family>();
    }

    #[test]
    fn test_init_sync_after_interrupted_bounded_publication_mmb() {
        init_sync_after_interrupted_bounded_publication_inner::<mmb::Family>();
    }
    ...
    /// Tear the tail page of node blob `blob` so its next open must repair it.
    ...
    async fn init_sync_reset_leaves_pruned_blobs_unopened_inner<F: Family>(
        context: deterministic::Context,
    ) {
        let hasher = Standard::<Sha256>::new(ForwardFold);
        let restart = Location::<F>::new(7);
    ...
    #[test_traced]
    fn test_init_sync_reset_leaves_pruned_blobs_unopened_mmr() {
        deterministic::Runner::default()
            .start(init_sync_reset_leaves_pruned_blobs_unopened_inner::<mmr::Family>);
    }

    #[test_traced]
    fn test_init_sync_reset_leaves_pruned_blobs_unopened_mmb() {
        deterministic::Runner::default()
            .start(init_sync_reset_leaves_pruned_blobs_unopened_inner::<mmb::Family>);
    }
    ...
    async fn init_sync_genesis_drops_stale_pins_inner<F: Family>(context: deterministic::Context) {
        ...
        // other history.
        let mut metadata =
            Metadata::<_, U64, Vec<u8>>::init(context.child("metadata"), metadata_cfg.clone())
                .await
                .unwrap();
        metadata.put(
        ...
        // The stale pins are gone and a reopen sees the rebuilt nodes.
        let metadata = Metadata::<_, U64, Vec<u8>>::init(context.child("check"), metadata_cfg)
            .await
            .unwrap();
        assert!(
            F::nodes_to_pin(boundary)
                .all(|pos| metadata.get(&U64::new(NODE_PREFIX, *pos)).is_none())
        );
        drop(metadata);
        let merkle =
            Merkle::<F, _, Digest, Sequential>::init(context.child("reopen"), &hasher, cfg)
    ...
    // An interrupted sync to a higher range leaves that range's pins beside an untouched journal.
    // A later sync to a lower nonzero range that reuses the journal must drop them, or appends
    // that reach those positions read the stale pins ahead of the journal nodes.
    async fn init_sync_lower_range_drops_future_pins_inner<F: Family>(
        ...
        // Interruption point of `init_sync(abandoned..)`: its metadata is durable and the journal
        // is untouched. The pins describe some other history.
        let mut metadata =
            Metadata::<_, U64, Vec<u8>>::init(context.child("metadata"), metadata_cfg.clone())
                .await
                .unwrap();
        ...
        let lower_pins: BTreeSet<_> = F::nodes_to_pin(lower).collect();
        let planted: Vec<_> = F::nodes_to_pin(abandoned)
            .filter(|pos| !lower_pins.contains(pos))
            .collect();
        assert!(!planted.is_empty());
        for (i, pos) in planted.iter().enumerate() {
            metadata.put(
                U64::new(NODE_PREFIX, **pos),
                test_digest(1_000 + i).to_vec(),
            );
        }
        _ = metadata.sync().await.unwrap();

        // Sync the lower range, reusing the journal, then grow past the abandoned boundary.
        let mut merkle = Merkle::<F, _, Digest, Sequential>::init_sync(
            context.child("sync"),
            SyncConfig {
                config: cfg.clone(),
        ...
        // The abandoned pins are gone and a reopen sees the journal nodes.
        let metadata = Metadata::<_, U64, Vec<u8>>::init(context.child("check"), metadata_cfg)
            .await
            .unwrap();
        assert!(
