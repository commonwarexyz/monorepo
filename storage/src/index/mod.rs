...
//!
//! Keys are translated into a compressed, fixed-size representation using a
//! `Translator`. Depending on the size of the representation, this can lead to
//! a non-negligible number of collisions (even if the original keys are
//! collision-free). To workaround this issue, `get` returns all values that map
//! to the same translated key. If the same key is inserted multiple times (and
//! old values are not `removed`), all values will be returned.
//!
//! # Warning
//!
//! If the `Translator` maps many keys to the same translated key, the
//! performance of `Index` will degrade substantially (each conflicting key may
...
/// # Warning
///
/// The output of `transform` is used as the key in a hash table. If the output
/// is not uniformly distributed, the performance of [Index] will degrade
/// substantially.
pub trait Translator: Clone + BuildHasher {
    /// The type of the internal representation of keys.
    ///
    /// Although `Translator` is a [BuildHasher], the `Key` type must still
    /// implement [Hash] for compatibility
...
mod tests {
    ...
    fn test_index_many_keys() {
        ...
        // Insert enough keys to generate some collisions, then confirm each value we
        // inserted remains retrievable.
    ...
    fn test_insert_and_prune_vacant() {
        ...
        // Inserting into a *vacant* key behaves just like `insert`: 1 key, 1 item,
        // nothing pruned.
