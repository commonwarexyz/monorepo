...
//! [crate::translator::Translator]. Depending on the size of the representation, this can lead to a
//! non-negligible number of collisions (even if the original keys are collision-free). To workaround
//! this issue, [Index::get] returns all values that map to the same translated key. If the same key
//! is inserted multiple times (and old values
//! are not `removed`), all values will be returned.
//!
//! # Warning
//!
//! If the [crate::translator::Translator] maps many keys to the same translated key, the performance of [Index] will
