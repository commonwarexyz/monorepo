...
//!
//! The Binary Merkle Tree is constructed level-by-level. The first level
//! consists of position-hashed leaf digests. On each additional level, pairs of
//! nodes are hashed from the previous level (if a level contains an odd
//! number of nodes, the last node is duplicated). The root of the tree is the
//! digest of the node in the top level.
//!
//! For example, given three leaves A, B, and C, the tree is constructed as
//! follows:
//!
//! ```text
...
//! ```
//!
//! A proof for a given leaf is generated by collecting the sibling at each
//! level (from the leaf up to the root). An external process can then use this
//! proof (with some trusted root) to verify that the leaf (at a fixed position)
//! is part of the tree.
//!
//! # Example
//!
...
impl<H: Hasher> Tree<H> {
    ...
    ///
    /// The proof contains the sibling digest at each level needed to
    ...
    pub fn proof(&self, position: u32) -> Result<Proof<H>, Error> {
        ...
        for level in &self.levels {
            ...
            let sibling = if sibling_index < level.len() {
                ...
                //
                // This doesn't affect the robustness of the proof (allow a non-existent
                // position to be proven or enable multiple proofs to be
                // generated from a single leaf).
                level[index]
            };
...
impl<H: Hasher> Proof<H> {
    /// Verifies that a given `leaf` at `position` is included in a Binary
    /// Merkle Tree with `root` using the provided `hasher`.
    ///
    /// The proof consists of sibling hashes stored from the leaf up to the
    /// root. At each level, if the current node is a left child (even
    /// index), the sibling is combined to the right; if it is a right child
    /// (odd index), the sibling is combined to the left.
    pub fn verify(
        &self,
        hasher: &mut H,
        leaf: &H::Digest,
    ...
    pub fn serialize(&self) -> Vec<u8> {
        // There should never be more than 255 siblings in a proof (would mean the
        // Binary Merkle Tree has more than 2^255 leaves).
        assert!(
...
mod tests {
    ...
    fn test_odd_tree_duplicate_index_proof() {
        ...
        // Attempting to verify using an out-of-range index (e.g. 3, which would
        // correspond to a duplicate leaf that doesn't actually exist) should
        // fail.
