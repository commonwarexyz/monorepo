...
//!
//! The tree is constructed level-by-level. Level 0 consists of leaf nodes (formed by hashing provided
//! digests with their position). At each higher level, pairs of nodes are hashed (if a level contains
//! an odd number of nodes, the last node is duplicated). The root of the tree is the digest in the top level.
//!
