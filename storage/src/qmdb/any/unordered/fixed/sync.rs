...
mod tests {
    ...
    fn test_init_sync_no_existing_data() {
        ...
        executor.start(|context| async move {
            ...
            let lower_bound = 10;
            let upper_bound = 26;
            let mut sync_journal = fixed::Journal::<_, Digest>::init_sync(
                context.clone(),
                cfg.clone(),
    ...
    fn test_init_sync_existing_data_overlap() {
        ...
        executor.start(|context| async move {
            ...
            let lower_bound = 8;
            let upper_bound = 31;
            let mut journal = fixed::Journal::<_, Digest>::init_sync(
                context.clone(),
                cfg.clone(),
                lower_bound..upper_bound,
            )
    ...
    fn test_init_sync_invalid_range() {
        ...
        executor.start(|context| async move {
            ...
            let lower_bound = 6;
            let upper_bound = 6;
            let _result = fixed::Journal::<Context, Digest>::init_sync(
                context.clone(),
                cfg.clone(),
                lower_bound..upper_bound,
            )
    ...
    fn test_init_at_size() {
        ...
        executor.start(|context| async move {
            ...
            {
                let mut journal =
                    fixed::Journal::<_, Digest>::init_at_size(context.clone(), cfg.clone(), 0)
                        .await
            ...
            {
                let mut journal =
                    fixed::Journal::<_, Digest>::init_at_size(context.clone(), cfg.clone(), 10)
                        .await
            ...
            {
                let mut journal =
                    fixed::Journal::<_, Digest>::init_at_size(context.clone(), cfg.clone(), 7)
                        .await
            ...
            {
                let mut journal =
                    fixed::Journal::<_, Digest>::init_at_size(context.clone(), cfg.clone(), 23)
                        .await
