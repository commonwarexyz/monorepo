...
pub(crate) mod test {
    ...
    mod from_sync_testable {
        use super::*;
        use crate::{
            mmr::{iterator::nodes_to_pin, journaled::Mmr, Position, StandardHasher},
            qmdb::any::sync::tests::FromSyncTestable,
        };
        use futures::future::join_all;

        type TestMmr = Mmr<deterministic::Context, StandardHasher<Sha256>>;
    ...
    #[allow(dead_code)]
    fn assert_read_futures_are_send(db: &mut AnyTest, key: Digest, loc: Location) {
    ...
    #[allow(dead_code)]
    fn assert_batch_futures_are_send(db: &AnyTest, key: Digest, value: Vec<u8>) {
        assert_gettable(db, &key);
        assert_log_store(db);
