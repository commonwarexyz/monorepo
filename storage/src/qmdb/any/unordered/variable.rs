...
pub(crate) mod test {
    ...
    fn test_owned_batch_root_matches() {
        ...
        executor.start(|context| async move {
            ...
            let borrow_root = {
                ...
                batch.merkleize(None).await.unwrap().root()
            };

            // Owned batch path.
            let snap_root = {
                let mut batch = snap.new_batch(&db);
                for op in &ops {
                    match op {
                        unordered::Operation::Update(unordered::Update(k, v)) => {
                            batch = batch.write(*k, Some(v.clone()));
            ...
            assert_eq!(borrow_root, snap_root);
    ...
    fn test_owned_batch_changeset_apply() {
        ...
        executor.start(|context| async move {
            ...
            let child_snap = {
                let batch = snap.new_batch(&db).write(key, Some(value.clone()));
                batch.merkleize(None).await.unwrap()
            };

            // Apply the batch's changeset.
            let changeset = child_snap.finalize();
    ...
    fn test_owned_batch_chain_commit_parent_first() {
        ...
        executor.start(|context| async move {
            ...
            // Commit parent first.
            let parent_changeset = parent_snap.finalize();
            ...
            let child_changeset = child_snap.finalize_from(current_db_size);
    ...
    fn test_owned_batch_multiple_forks() {
        ...
        executor.start(|context| async move {
            ...
            let fork_a = {
                let batch = base.new_batch(&db).write(key_a, Some(vec![10u8; 8]));
                batch.merkleize(None).await.unwrap()
            };
            ...
            let fork_b = {
                let batch = base.new_batch(&db).write(key_b, Some(vec![20u8; 8]));
                batch.merkleize(None).await.unwrap()
            };
    ...
    fn test_owned_batch_homogeneous_collection() {
        ...
        executor.start(|context| async move {
            ...
            let snap1 = {
                ...
                let batch = base.new_batch(&db).write(key, Some(vec![1u8; 8]));
                batch.merkleize(None).await.unwrap()
            };
            collection.insert(snap1.root(), snap1);

            // Depth 2 (retrieve snap1 from collection, build child).
            let snap1_root = *collection.keys().next().unwrap();
            let snap1_ref = collection.get(&snap1_root).unwrap();
            let snap2 = {
                let key = Digest::random(&mut commonware_utils::test_rng_seeded(501));
                let batch = snap1_ref.new_batch(&db).write(key, Some(vec![2u8; 8]));
                batch.merkleize(None).await.unwrap()
            };
            collection.insert(snap2.root(), snap2);
    ...
    fn test_owned_batch_chain_three_deep() {
        ...
        executor.start(|context| async move {
            ...
            let child_snap = {
                let batch = parent_snap.new_batch(&db).write(key_c, Some(val_c.clone()));
                batch.merkleize(None).await.unwrap()
            };

            // Commit grandparent.
            db.apply_batch(grandparent_snap.finalize()).await.unwrap();
            db.commit().await.unwrap();
            assert_eq!(db.get(&key_a).await.unwrap().unwrap(), val_a);

            // Commit parent via finalize_from.
            let current_db_size = *db.bounds().await.end;
            db.apply_batch(parent_snap.finalize_from(current_db_size))
                .await
                .unwrap();
            db.commit().await.unwrap();
            assert_eq!(db.get(&key_b).await.unwrap().unwrap(), val_b);
