    ...
    V::Value: Send + Sync,
{
        ...
        F: FnMut(bool, Option<Location>),
    {
        ...
        for (key, value) in updates {
            ...
            self.snapshot.insert(&key, self.op_count().await);
            self.log
                .append(Operation::Update(Update(key, value)))
                .await?;
            callback(true, None);
...
pub(super) mod test {
    ...
    async fn test_any_db_empty<D: TestableAnyDb<Digest>>(
        ...
        assert_eq!(db.op_count().await, 2); // another commit op added
        ...
        for _ in 1..100 {
            ...
            assert!(clean_db.op_count().await - clean_db.inactivity_floor_loc().await <= 3);
            db = clean_db.into_mutable();
            assert!(db.op_count().await - db.inactivity_floor_loc().await <= 3);
        }
        ...
        <D as MerkleizedStore>::Operation: Codec,
    {
        ...
        assert_eq!(db.op_count().await, Location::new_unchecked(1478));
        ...
        assert_eq!(db.op_count().await, Location::new_unchecked(1957));
        assert_eq!(
        ...
        for loc in *db.inactivity_floor_loc().await..*db.op_count().await {
    ...
    pub(crate) async fn test_any_db_basic<D: TestableAnyDb<Digest>>(
        ...
        assert_eq!(db.op_count().await, 6); // 4 updates, 1 deletion + initial commit.
        assert_eq!(db.inactivity_floor_loc().await, Location::new_unchecked(0));
        let (db, _) = db.commit(None).await.unwrap();
        let mut db = db.into_mutable();
        ...
        assert_eq!(db.op_count().await, 10); // floor of 7 + 2 active keys.
        ...
        assert!(db.get(&d1).await.unwrap().is_none());
        assert!(db.get(&d2).await.unwrap().is_none());
        assert_eq!(db.op_count().await, 12); // 2 new delete ops.
        assert_eq!(db.inactivity_floor_loc().await, Location::new_unchecked(7));

        let (db, _) = db.commit(None).await.unwrap();
        let mut db = db.into_mutable();
        assert_eq!(db.inactivity_floor_loc().await, Location::new_unchecked(12));
        assert_eq!(db.op_count().await, 13); // only commit should remain.

        // Multiple deletions of the same key should be a no-op.
        assert!(!db.delete(d1).await.unwrap());
        assert_eq!(db.op_count().await, 13);
        ...
        let db = db
            ...
            .await
            .unwrap();
        assert_eq!(db.op_count().await, 14);
        let root = db.root().await;
        drop(db);
        let db = next_db().await;
        assert_eq!(db.op_count().await, 14);
        ...
        assert_eq!(db.op_count().await, 23);
        let root = db.root().await;
        let db = next_db().await;
