...
pub(crate) mod test {
    ...
    /// A prune that must persist its authorizing commit starts the operation-log and Merkle
    /// durability passes together.
    #[test_traced]
    fn test_prune_overlaps_required_syncs() {
        let executor = deterministic::Runner::default();
        executor.start(|context| async move {
            let pending = PendingSyncs::default();
            let open = open_delayed_db(&context, "delayed", "prune_overlap", &pending);
            let mut db = drive_pending_syncs(&pending, open).await.unwrap();
            let key = Sha256::hash(&[&0u64.to_be_bytes()]);
            db = apply_write(db, key, Sha256::hash(&[&100u64.to_be_bytes()])).await;
            ...
            let db = {
                ...
                assert!(
                    prune.as_mut().now_or_never().is_none(),
                    "prune completed while its durability passes were parked"
                );
                assert_eq!(
                    pending.starts(),
                    starts_before + 2,
                    "operation-log and Merkle syncs must start together"
                );
