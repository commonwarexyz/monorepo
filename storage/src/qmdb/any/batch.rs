    ...
    Operation<F, U>: Send + Sync,
{
    ...
    pub(crate) ancestor_diffs: Vec<Arc<DiffVec<U::Key, F, U::Value>>>,
    ancestor_base_locs: AncestorBaseLocs<U::Key, F>,
    ancestors: Vec<chain::AncestorBounds<F, H::Digest>>,
...
///
/// `snapshot` and the bitmap must hold the state already applied through the first `db_size`
/// operations, the last of which is the commit they currently end at. On return they reflect
/// `batch.bounds.tip`.
///
    ...
    Operation<F, U>: Send + Sync,
{
    ...
    if batch.ancestor_diffs.is_empty() {
        ...
        move_commit_bit(&mut bitmap, db_size - 1, tip);
        drop(bitmap);
