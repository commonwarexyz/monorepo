...
///
/// `snapshot` and `bitmap` must hold the state committed through the first `db_size` operations,
/// the last of which is the commit they currently end at. On return they reflect `batch.bounds.tip`.
