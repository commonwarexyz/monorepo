...
///
/// Built from a [`Prepared`] batch, so every stage reads the database it was validated
/// against and the resolved state cannot go stale between compaction and finalization.
struct ResolvedBatch<F: Family, H, U, S: Strategy>
where
    U: update::Update,
    H: Hasher,
    Operation<F, U>: Codec,
...
impl<F: Family, H, U, S: Strategy> ResolvedBatch<F, H, U, S>
where
    U: update::Update,
    H: Hasher,
    Operation<F, U>: Codec,
{
    ...
    /// Finalize the resolved operations by appending one CommitFloor and computing the root.
    ///
    ...
    Operation<F, update::Unordered<K, V>>: Codec,
{
    ...
    pub(crate) async fn merkleize_with_floor_scan(
        ...
        let db = self.db;
        let resolved = self.resolve(staged_updates).await?;
        let (budget, metadata) = plan(resolved.stats());
        resolved
    ...
    /// Resolve the batch's mutations and staged updates into operations and a diff.
    async fn resolve(
        self,
        staged_updates: StagedUpdates<F, update::Unordered<K, V>>,
    ) -> Result<ResolvedBatch<F, H, update::Unordered<K, V>, S>, crate::qmdb::Error<F>> {
        let Self {
            db,
            mut mutations,
            merkleizer: m,
    ...
    Operation<F, update::Ordered<K, V>>: Codec,
{
    ...
    async fn resolve(
        self,
        staged_updates: StagedUpdates<F, update::Ordered<K, V>>,
    ) -> Result<ResolvedBatch<F, H, update::Ordered<K, V>, S>, crate::qmdb::Error<F>> {
        let Self {
            db,
            mut mutations,
            merkleizer: m,
