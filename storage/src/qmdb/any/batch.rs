...
///
/// Borrows `&Db` for reads during the build phase. Consuming
/// [`UnmerkleizedBatch::merkleize`] (or its ordered/unordered variants)
/// produces an owned [`MerkleizedBatch`] and releases the borrow.
pub struct UnmerkleizedBatch<'a, E, C, I, H, U>
where
...
impl<'a, E, C, I, H, U> Merkleizer<'a, E, C, I, H, U>
where
    E: Storage + Clock + Metrics,
    U: update::Update + Send + Sync,
    C: Contiguous<Item = Operation<U>>,
    ...
    Operation<U>: Codec,
{
    ...
    async fn finish<S: FloorScan>(
        ...
        Ok(MerkleizedBatch {
            ...
            last_commit_loc: commit_loc,
            total_size: *commit_loc + 1,
            active_keys: total_active_keys as usize,
            db_size: self.db_size,
        })
    }
}

impl<'a, E, C, I, H, U> UnmerkleizedBatch<'a, E, C, I, H, U>
where
    E: Storage + Clock + Metrics,
    U: update::Update + Send + Sync,
    C: Contiguous<Item = Operation<U>>,
...
impl<'a, E, K, V, C, I, H> UnmerkleizedBatch<'a, E, C, I, H, update::Unordered<K, V>>
where
    E: Storage + Clock + Metrics,
    K: Key,
    V: ValueEncoding,
...
impl<'a, E, K, V, C, I, H> UnmerkleizedBatch<'a, E, C, I, H, update::Ordered<K, V>>
where
    E: Storage + Clock + Metrics,
    K: Key,
    V: ValueEncoding,
    ...
    Operation<U>: Codec,
{
    ...
    pub fn new_batch(&self) -> UnmerkleizedBatch<'_, E, C, I, H, U> {
...
// ---------------------------------------------------------------------------
// Owned MerkleizedBatch (no lifetime, storable in collections)
// ---------------------------------------------------------------------------

/// Owned, immutable batch of speculative QMDB state.
///
/// No lifetime parameters, no growing type parameter `P`.
/// Instances can be stored in homogeneous collections (e.g. `HashMap<Digest, MerkleizedBatch>`)
    ...
    Operation<U>: Send + Sync,
{
    ...
    pub(crate) journal: authenticated::MerkleizedBatch<D, Operation<U>>,
    ...
    Operation<U>: Codec,
{
    ...
    ///
    /// Use this when an ancestor batch has already been committed and the
    /// DB size has advanced. The changeset only contains diffs for keys that
    /// this batch's own mutations modified, with `old_loc` values adjusted to
    /// reflect locations in the current committed DB (not the original base).
        ...
        U: 'static,
    {
        ...
        let committed_actions = {
            ...
            for seg in &self.journal.items_chain {
    ...
    Operation<U>: Codec,
{
    ...
    pub fn to_snapshot(&self) -> MerkleizedBatch<H::Digest, U> {
        let journal_size = *self.last_commit_loc + 1;
        MerkleizedBatch {
            journal: self.log.to_snapshot(),
...
mod trait_impls {
    ...
    impl<'a, E, K, V, C, I, H> UnmerkleizedBatchTrait
    ...
    impl<'a, E, K, V, C, I, H> UnmerkleizedBatchTrait
        for UnmerkleizedBatch<'a, E, C, I, H, update::Ordered<K, V>>
    where
        E: Storage + Clock + Metrics,
        K: Key,
        V: ValueEncoding + 'static,
        ...
        Operation<update::Ordered<K, V>>: Codec,
    {
        ...
        type Batch<'a>
            = UnmerkleizedBatch<'a, E, C, I, H, update::Ordered<K, V>>
        where
            Self: 'a;
