...
pub(crate) mod test {
    ...
    {
        let key0 = Sha256::new().hash_encoded(0u64);
        ...
        <D as Provable<mmr::Family>>::Operation: Codec + PartialEq + std::fmt::Debug,
    {
        ...
        // Add more operations to the database
        {
            let mut batch = db.new_batch();
            for i in OPS..(OPS + 5) {
                let k = Sha256::new().hash_encoded(i + 1000); // different keys
                let v = make_value(i * 1000);
                batch = batch.write(k, Some(v));
            }
            let merkleized = batch.merkleize(&db, None).await.unwrap();
        ...
        V: Clone + CodecShared + Eq + std::fmt::Debug,
    {
        ...
        let key_at = |j: u64, i: u64| Sha256::new().hash_encoded(j * 1000 + i);
...
mod bitmap_tests {
    ...
    fn floor_scan_falls_through_to_uncommitted_tail() {
        deterministic::Runner::default().start(|context| async move {
            ...
            for i in 0..16u64 {
                let k = Sha256::new().hash_encoded(1000 + i);
