...
mod tests {
    use super::*;
    use crate::journal::{self, contiguous::fixed as fixed_journal};
    ...
    fn test_init_journal_existing_data_overlap() {
        ...
        executor.start(|context| async move {
            ...
            let mut journal =
                fixed_journal::Journal::<Context, Digest>::init(context.clone(), cfg.clone())
                    .await
    ...
    /// Test `init_journal` when existing data exactly matches the sync target range.
    ...
    #[should_panic]
    #[test_traced]
    fn test_init_journal_invalid_range() {
        let executor = deterministic::Runner::default();
        executor.start(|context| async move {
            let cfg = fixed_journal::Config {
    ...
    /// Test `init_journal_at_size` creates a journal in a pruned state at various sizes.
    #[test_traced]
    fn test_init_journal_at_size() {
        let executor = deterministic::Runner::default();
        executor.start(|context| async move {
            let cfg = fixed_journal::Config {
