    ...
    JournalOf<H>: Contiguous,
{
    ...
    executor.start(|mut context| async move {
        ...
        assert_eq!(synced_db.op_count().await, target_op_count);
        assert_eq!(
        ...
        let final_op_count = synced_db.op_count().await;
        ...
        assert_eq!(reopened_db.op_count().await, final_op_count);
        assert_eq!(
    ...
    JournalOf<H>: Contiguous,
{
    ...
    executor.start(|mut context| async move {
        ...
        assert_eq!(
            synced_db.inactivity_floor_loc().await,
            target_db.read().await.inactivity_floor_loc().await
        );
        assert_eq!(synced_db.inactivity_floor_loc().await, lower_bound);
        assert_eq!(
            synced_db.op_count().await,
            target_db.read().await.op_count().await
        );
    ...
    JournalOf<H>: Contiguous,
{
    ...
    executor.start(|mut context| async move {
        ...
        let new_root = {
            ...
            let new_upper_bound = target_db.op_count().await;
            ...
            assert_eq!(synced_db.op_count().await, new_upper_bound);
    ...
    JournalOf<H>: Contiguous,
{
    ...
    executor.start(|mut context| async move {
        ...
        let new_root = {
            ...
            let new_upper_bound = db.op_count().await;
    ...
    JournalOf<H>: Contiguous,
{
    ...
    executor.start(|mut context| async move {
        ...
        let sync_upper_bound = db.op_count().await;
        let target_db_op_count = db.op_count().await;
        ...
        // Verify database state
        assert_eq!(sync_db.op_count().await, target_db_op_count);
        assert_eq!(
    ...
    JournalOf<H>: Contiguous,
{
    ...
    executor.start(|mut context| async move {
        ...
        sync_db
            ...
            .await
            .unwrap();
        let sync_db_original_size = sync_db.op_count().await;
        ...
        let target_db_op_count = target_db.op_count().await;
        ...
        let sync_upper_bound = target_db.op_count().await;
    ...
    JournalOf<H>: Contiguous,
{
    ...
    executor.start(|mut context| async move {
        ...
        let upper_bound = source_db.op_count().await;
        ...
        let target_op_count = source_db.op_count().await;
        ...
        // Verify database state
        assert_eq!(db.op_count().await, target_op_count);
    ...
    JournalOf<H>: Contiguous,
{
    ...
    executor.start(|mut context| async move {
        ...
        assert_eq!(source_db.op_count().await, Location::new_unchecked(1));
        ...
        // Verify database state
        assert_eq!(synced_db.op_count().await, Location::new_unchecked(1));
        assert_eq!(
            synced_db.inactivity_floor_loc().await,
        ...
        assert!(synced_db.op_count().await > Location::new_unchecked(1));
