...
pub(crate) mod test {
    ...
    /// Type alias for concrete [Db] type used in these unit tests.
    ...
    fn test_ordered_any_fixed_db_span_maintenance_under_collisions() {
        ...
        executor.start(|mut context| async move {
            ...
            let config = fixed_db_config::<OneCap>(&seed.to_string(), &context);
            let mut db = Db::<Context, Digest, i32, Sha256, OneCap>::init(
            ...
            let mut db = Db::<Context, Digest, i32, Sha256, TwoCap>::init(
    ...
    #[allow(dead_code)]
    fn assert_db_futures_are_send(db: &mut AnyTest, key: Digest, value: Digest, loc: Location) {
