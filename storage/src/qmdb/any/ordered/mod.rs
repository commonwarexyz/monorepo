...
pub use operation::{FixedOperation, OrderedUpdate, VariableOperation};
    ...
    Operation<K, V>: Codec,
{
    ...
    async fn get_update_op(
        log: &AuthenticatedLog<E, C, H, S>,
        loc: Location,
    ) -> Result<OrderedUpdate<K, V>, Error> {
    ...
    /// Finds and returns the location and OrderedUpdate for the lexicographically-last key produced by
    ...
    Operation<K, V>: Codec,
{
    ...
    pub(crate) async fn update_with_callback(
        ...
        let op = match res {
            UpdateLocResult::Exists(next_key) => Operation::Update(OrderedUpdate {
                key: key.clone(),
                value,
                next_key,
            }),
            UpdateLocResult::NotExists(prev_data) => {
                self.active_keys += 1;
                self.log
                    .append(Operation::Update(OrderedUpdate {
                        key: key.clone(),
                        value,
                ...
                // the previous key.
                Operation::Update(OrderedUpdate {
                    key: prev_data.key,
    ...
    pub(crate) async fn create_with_callback(
        ...
        match res {
            ...
            UpdateLocResult::NotExists(prev_data) => {
                self.active_keys += 1;
                let value_update_op = Operation::Update(OrderedUpdate {
                    key: key.clone(),
                    value,
                    next_key: prev_data.next_key,
                });
                let next_key_update_op = Operation::Update(OrderedUpdate {
                    key: prev_data.key,
                    value: prev_data.value,
                    next_key: key,
                });
