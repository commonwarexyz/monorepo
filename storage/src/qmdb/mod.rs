...
/// Scans the status bitmap for active locations that need floor-raising. For each of `steps`
/// floor-raising steps, finds the next active location and records it. Returns the new floor
/// location and the active locations found.
///
/// Bitmap bits for scanned locations are set to false.
pub(crate) fn scan_floor_locs<E: Storage + Clock + Metrics, D: Digest, const N: usize>(
    status: &mut UnmerkleizedBitMap<E, D, N>,
    mut inactivity_floor_loc: Location,
    steps: u32,
