use crate::merkle::{verification::ProofStore, Error, Family, Location, Position, Proof};
...
pub fn verify_proof_and_extract_digests<H, F, Op>(
    ...
    operations: &[Op],
    target_root: &H::Digest,
) -> Result<Vec<(Position<F>, H::Digest)>, Error<F>>
where
    H: Hasher,
    F: Family,
    Op: Encode,
...
pub fn create_proof_store<H, F, Op>(
    ...
    operations: &[Op],
    root: &H::Digest,
) -> Result<ProofStore<F, H::Digest>, Error<F>>
where
    H: Hasher,
    F: Family,
    Op: Encode,
