...
mod tests {
    ...
    fn test_operation_to_key() {
        ...
        let commit_op =
            FixedUnordered::<U64, U64>::CommitFloor(None, Location::new_unchecked(42), PhantomData);
        ...
        let commit_op =
            FixedOrdered::<U64, U64>::CommitFloor(None, Location::new_unchecked(42), PhantomData);
        assert_eq!(None, commit_op.key());
    }
    ...
    fn test_operation_to_value() {
        ...
        let commit_op = FixedOrdered::<U64, U64>::CommitFloor(
            Some(value.clone()),
            Location::new_unchecked(42),
            PhantomData,
        );
    ...
    fn test_operation_basic() {
        ...
        assert_eq!(None, from.value());
        assert!(
            matches!(from, FixedUnordered::CommitFloor(None, loc,_) if loc == Location::new_unchecked(42))
        );
        ...
        assert!(
            matches!(&from, FixedUnordered::CommitFloor(Some(v), loc,_) if v == &value && *loc == Location::new_unchecked(42))
        );
        assert_eq!(commit_op, from);
    ...
    fn test_operation_ordered_basic() {
        ...
        assert_eq!(None, from.value());
        assert!(
            matches!(from, FixedOrdered::CommitFloor(None, loc, _) if loc == Location::new_unchecked(42))
        );
        assert_eq!(commit_op, from);
        ...
        assert_eq!(Some(&value), from.value());
        assert!(
            matches!(&from, FixedOrdered::CommitFloor(Some(v), loc, _) if v == &value && *loc == Location::new_unchecked(42))
        );
        assert_eq!(commit_op, from);
