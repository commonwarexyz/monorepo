    ...
    > Immutable<E, K, V, H, T, M, D>
{
    ...
    pub async fn get(&self, key: &K) -> Result<Option<V>, Error> {
        let oldest = self.journal.bounds().await.start;
    ...
    async fn get_from_loc(&self, key: &K, loc: Location) -> Result<Option<V>, Error> {
        if loc < self.journal.bounds().await.start {
    ...
    Immutable<E, K, V, H, T, Unmerkleized, NonDurable>
{
    ...
    > {
        ...
        let range = loc..self.bounds().await.end;
    ...
    > crate::qmdb::store::LogStore for Immutable<E, K, V, H, T, M, D>
{
    ...
    fn is_empty(&self) -> bool {
        // An immutable store starts with an initial commit at location 0.
        // It is empty if no Set operations have been committed (last commit is still the initial).
...
pub(super) mod test {
    ...
    pub fn test_immutable_db_pruning() {
        ...
        executor.start(|context| async move {
            ...
            let oldest_retained_loc = db.journal.bounds().await.start;
