...
impl<E: Storage + Clock + Metrics, V: VariableValue, H: Hasher> Keyless<E, V, H> {
    ...
    /// - Returns [crate::mmr::Error::LocationOverflow] if `loc` > [crate::merkle::Family::MAX_LOCATION]
...
mod test {
    ...
    fn test_keyless_db_non_empty_db_recovery() {
        ...
        executor.start(|mut context| async move {
            ...
            async fn recover_from_failure(
                ...
                let db = open_db(context.with_label(label1)).await;
    ...
    fn test_stale_changeset_rejected() {
        ...
        executor.start(|context| async move {
            ...
            assert_eq!(db.get(Location::new(1)).await.unwrap(), Some(vec![10]));
            assert_eq!(db.get_metadata().await.unwrap(), None);
