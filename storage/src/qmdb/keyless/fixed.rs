...
mod tests {
    ...
    async fn capped_standard<F: Family>(context: deterministic::Context) {
        for cap in [0, 1, 2, 3, 4, 6, 7, 8, 12, 13, 14, 100] {
            let cfg = db_config(&format!("caps-{cap}"), &context, Sequential);
            let mut db = TestDb::<F>::init(context.child("create"), cfg.clone())
                .await
                .unwrap();
            ...
            drop(db);
            let opened =
                TestDb::<F>::init_at_most(context.child("cap"), cfg.clone(), Location::new(cap))
                    .await;
            if cap == 0 {
                assert!(matches!(opened, Err(Error::InvalidInitializationBound)));
                let db = TestDb::<F>::init(context.child("unchanged"), cfg)
                    .await
                    .unwrap();
            ...
            drop(db);
            let mut db = TestDb::<F>::init(context.child("restart"), cfg.clone())
                .await
                .unwrap();
            assert_eq!((db.bounds().end, db.root()), expected);
            let batch = db
            ...
            drop(db);
            let db = TestDb::<F>::init(context.child("appended"), cfg)
                .await
                .unwrap();
    ...
    #[test_traced]
    fn test_standard_init_at_most_mmr() {
        deterministic::Runner::default().start(capped_standard::<mmr::Family>);
    }
    #[test_traced]
    fn test_standard_init_at_most_mmb() {
        deterministic::Runner::default().start(capped_standard::<mmb::Family>);
    }

    async fn capped_compact<F: Family>(context: deterministic::Context) {
        for cap in [0, 1, 2, 3, 4, 6, 7, 8, 12, 13, 14, 100] {
            let cfg = CompactConfig {
                strategy: Sequential,
                witness: crate::journal::contiguous::variable::Config {
            ...
            let mut db = TestCompactDb::<F>::init(context.child("create"), cfg.clone())
                .await
                .unwrap();
            ...
            let tip = *commits.last().unwrap();
            drop(db);
            let opened = TestCompactDb::<F>::init_at_most(
                context.child("cap"),
                cfg.clone(),
                Location::new(cap),
            )
            ...
            if cap == 0 {
                assert!(matches!(opened, Err(Error::InvalidInitializationBound)));
                let db = TestCompactDb::<F>::init(context.child("unchanged"), cfg)
                    .await
                    .unwrap();
            ...
            drop(db);
            let mut db = TestCompactDb::<F>::init(context.child("restart"), cfg.clone())
                .await
                .unwrap();
            assert_eq!((db.size(), db.root()), expected);
            let batch = db
            ...
            drop(db);
            let db = TestCompactDb::<F>::init(context.child("appended"), cfg)
                .await
                .unwrap();
    ...
    #[test_traced]
    fn test_compact_init_at_most_mmr() {
        deterministic::Runner::default().start(capped_compact::<mmr::Family>);
    }
    #[test_traced]
    fn test_compact_init_at_most_mmb() {
        deterministic::Runner::default().start(capped_compact::<mmb::Family>);
    }
    ...
    keyless_tests! {
        ...
        test_keyless_fixed_rewind_recovery => run_rewind_recovery, capped;
        test_keyless_fixed_rewind_pruned_target_errors => run_rewind_pruned_target_errors, capped;
        ...
        test_keyless_fixed_rewind_restores_floor => run_rewind_restores_floor, capped_floor;
        test_keyless_fixed_floor_at_commit_loc_accepted => run_floor_at_commit_loc_accepted, db;
        test_keyless_fixed_rewind_after_reopen_with_floor => run_rewind_after_reopen_with_floor, capped_indexed;
