    ...
    Operation<F, V>: EncodeShared,
{
    ...
    ) -> impl core::future::Future<Output = Result<Option<Vec<Self::Digest>>, qmdb::Error<F>>> + Send
    {
        // Clone the merkle config outside the future so the non-Sync journal config inside
        // `Config` is not captured across await points.
