...
// PrunableStore for Merkleized states (both Durable and NonDurable)
impl<
        E: RStorage + Clock + Metrics,
        K: Array,
        V: FixedValue,
...
// MerkleizedDurableAny trait implementation
impl<
        E: RStorage + Clock + Metrics,
        K: Array,
        V: FixedValue,
    ...
    > UnmerkleizedDurableAny for Db<E, K, V, H, T, N, Unmerkleized, Durable>
{
    ...
    fn into_mutable(self) -> Self::Mutable {
        Db {
            any: self.any.into_mutable(),
            status: self.status, // Already in Dirty (Unmerkleized) state
            context: self.context,
            bitmap_metadata_partition: self.bitmap_metadata_partition,
            cached_root: None,
        }
...
// UnmerkleizedNonDurableAny trait implementation
impl<
        E: RStorage + Clock + Metrics,
        K: Array,
        V: FixedValue,
...
pub mod test {
    ...
    async fn apply_random_ops(
        ...
        commit_changes: bool,
        rng_seed: u64,
        mut db: MutableCurrentTest,
    ) -> Result<MutableCurrentTest, Error> {
    ...
    pub fn test_current_db_proving_repeated_updates() {
        ...
        executor.start(|context| async move {
            ...
            for i in 1u8..=255 {
                ...
                let clean_db = dirty_db.into_provable().await.unwrap();
                db = clean_db;
                let root = db.root();
        ...
        executor.start(|context| async move {
            ...
            for i in 0..NUM_OPERATIONS {
                ...
                if i % 50 == 49 {
                    ...
                    let clean_no_pruning = durable_no_pruning.into_provable().await.unwrap();
                    let (durable_pruning, _) = db_pruning.commit(None).await.unwrap();
                    let mut clean_pruning = durable_pruning.into_provable().await.unwrap();
                    clean_pruning
            ...
            let db_no_pruning = db_no_pruning.into_provable().await.unwrap();
            let (db_pruning, _) = db_pruning.commit(None).await.unwrap();
            let db_pruning = db_pruning.into_provable().await.unwrap();
