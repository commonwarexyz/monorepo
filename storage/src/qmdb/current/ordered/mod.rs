...
pub mod tests {
    ...
    /// encoding (`V`).
    type TestDb<M, C, V> = db::Db<
        M,
        deterministic::Context,
        C,
        Digest,
        V,
        Index<OneCap, merkle::Location<M>>,
        Sha256,
        32,
    >;
    ...
    pub fn test_build_small_close_reopen<M, C, F, Fut>(mut open_db: F)
    where
        M: merkle::Graftable,
        C: DbAny<M> + BatchableDb<Family = M> + BitmapPrunedBits,
        C::Key: TestKey,
        ...
        Fut: Future<Output = C>,
    {
        ...
        executor.start(|context| async move {
            ...
            let v1: <C as DbAny<M>>::Value = TestValue::from_seed(10);
            ...
            // Delete that one key.
            assert!(db.get(&k1).await.unwrap().is_some());
            let metadata: <C as DbAny<M>>::Value = TestValue::from_seed(1);
            let finalized = db
                .new_batch()
                .write(k1, None)
    ...
    pub(super) fn test_verify_proof_over_bits_in_uncommitted_chunk<M, C, V, F, Fut>(mut open_db: F)
    where
        M: merkle::Graftable,
        C: Mutable<Item = Operation<M, Digest, V>> + Persistable<Error = JournalError> + 'static,
        V: ValueEncoding<Value = Digest> + 'static,
        Operation<M, Digest, V>: Codec,
        TestDb<M, C, V>: DbAny<M, Key = Digest, Value = Digest, Digest = Digest>
            + BatchableDb<Family = M>
            + 'static,
        F: FnMut(Context, String) -> Fut + 'static,
        Fut: Future<Output = TestDb<M, C, V>>,
    {
        let executor = deterministic::Runner::default();
        executor.start(|context| async move {
            ...
            let root = db.root();
            assert!(TestDb::<M, C, V>::verify_key_value_proof(
                &mut hasher,
                k,
                v1,
                &proof,
            ...
            assert!(TestDb::<M, C, V>::verify_range_proof(
                &mut hasher,
    ...
    pub(super) fn test_range_proofs<M, C, V, F, Fut>(mut open_db: F)
    where
        M: merkle::Graftable,
        C: Mutable<Item = Operation<M, Digest, V>> + Persistable<Error = JournalError> + 'static,
        V: ValueEncoding<Value = Digest> + 'static,
        ...
        Fut: Future<Output = TestDb<M, C, V>>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            let mut db = apply_random_ops::<M, TestDb<M, C, V>>(200, true, context.next_u64(), db)
                .await
                .unwrap();
            let finalized = db
                .new_batch()
            ...
            for loc in *start_loc..*end_loc {
                let loc = merkle::Location::<M>::new(loc);
                let (proof, ops, chunks) = db
                    .range_proof(&mut hasher, loc, NZU64!(max_ops))
                    .await
                    .unwrap();
                assert!(
                    TestDb::<M, C, V>::verify_range_proof(
                        &mut hasher,
                        &proof,
                        loc,
                        &ops,
    ...
    pub(super) fn test_key_value_proof<M, C, V, F, Fut>(mut open_db: F)
    where
        M: merkle::Graftable,
        C: Mutable<Item = Operation<M, Digest, V>> + Persistable<Error = JournalError> + 'static,
        V: ValueEncoding<Value = Digest> + 'static,
        ...
        Fut: Future<Output = TestDb<M, C, V>>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            let mut hasher = Sha256::new();
            let db = open_db(context.with_label("db"), partition.clone()).await;
            let mut db = apply_random_ops::<M, TestDb<M, C, V>>(500, true, context.next_u64(), db)
                .await
                .unwrap();
            let finalized = db
                .new_batch()
    ...
    /// value's proof fails.
    pub(super) fn test_proving_repeated_updates<M, C, V, F, Fut>(mut open_db: F)
    where
        M: merkle::Graftable,
        C: Mutable<Item = Operation<M, Digest, V>> + Persistable<Error = JournalError> + 'static,
        V: ValueEncoding<Value = Digest> + 'static,
        ...
        Fut: Future<Output = TestDb<M, C, V>>,
    {
        ...
        executor.start(|context| async move {
            ...
            for i in 1u8..=255 {
                ...
                let proof = db.key_value_proof(&mut hasher, k).await.unwrap();
                assert!(
                    TestDb::<M, C, V>::verify_key_value_proof(&mut hasher, k, v, &proof, &root),
                    "proof of update {i} failed to verify"
                );
                // Ensure the proof does NOT verify if we use the previous value.
                assert!(
                    !TestDb::<M, C, V>::verify_key_value_proof(
                        &mut hasher,
                        k,
                        old_val,
                        &proof,
    ...
    pub(super) fn test_exclusion_proofs<M, C, V, F, Fut>(mut open_db: F)
    where
        M: merkle::Graftable + PartialEq,
        C: Mutable<Item = Operation<M, Digest, V>> + Persistable<Error = JournalError> + 'static,
        ...
        Fut: Future<Output = TestDb<M, C, V>>,
    {
        ...
        executor.start(|context| async move {
            ...
            let empty_proof = db
                ...
                .await
                .unwrap();
            assert!(TestDb::<M, C, V>::verify_exclusion_proof(
                &mut hasher,
                &key_exists_1,
                &empty_proof,
                &empty_root,
...
pub enum ExclusionProof<F: merkle::Family, K: Key, V: ValueEncoding, D: Digest, const N: usize> {
