...
use crate::{
    ...
    qmdb::{
        any::{
            ordered::{fixed::Any, FixedOperation as Operation, FixedUpdate},
...
pub enum ExclusionProof<K: Array, V: FixedValue, D: Digest, const N: usize> {
    ...
    /// is excluded.
    KeyValue(OperationProof<D, N>, FixedUpdate<K, V>),
    ...
    > Current<E, K, V, H, T, N, S>
{
    ...
    pub fn verify_key_value_proof(
        ...
        let op = Operation::Update(FixedUpdate {
            key,
            value,
    ...
    /// DB is empty.
    pub async fn get_span(&self, key: &K) -> Result<Option<(Location, FixedUpdate<K, V>)>, Error> {
