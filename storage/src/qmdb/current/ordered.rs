...
use crate::{
    ...
    qmdb::{
        any::{
            ordered::{fixed::Any, FixedOperation as Operation, Update},
            CleanAny, DirtyAny, FixedEncoding, FixedValue,
        },
...
pub enum ExclusionProof<K: Array, V: FixedValue, D: Digest, const N: usize> {
    ...
    /// is excluded.
    KeyValue(OperationProof<D, N>, Update<K, FixedEncoding<V>>),
    ...
    > Current<E, K, V, H, T, N, S>
{
    ...
    pub async fn get_span(
        &self,
        key: &K,
    ) -> Result<Option<(Location, Update<K, FixedEncoding<V>>)>, Error> {
