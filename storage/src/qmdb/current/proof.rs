...
impl<D: Digest> RangeProof<D> {
    ...
    pub async fn new<H: CHasher<Digest = D>, S: Storage<crate::mmr::Mmr, D>, const N: usize>(
        hasher: &mut H,
        status: &BitMap<N>,
        storage: &S,
        ...
        let proof = verification::range_proof(storage, &mut mmr_hasher, range).await?;
...
impl<D: Digest, const N: usize> OperationProof<D, N> {
    ...
    pub async fn new<H: CHasher<Digest = D>, S: Storage<crate::mmr::Mmr, D>>(
        hasher: &mut H,
        status: &BitMap<N>,
        storage: &S,
        loc: Location,
