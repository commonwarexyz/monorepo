...
/// Fold peak digests while regrouping sub-grafting-height peaks into grafted chunks.
///
/// `start_leaf` is the leftmost leaf covered by the first peak in `peaks`, and `initial_acc`
/// contains any peaks that were already folded before `start_leaf`.
...
>(
    ...
    let mut chunk_acc: Option<D> = None;
    let mut current_chunk: Option<u64> = None;

    // Graft and fold the pending chunk accumulator into acc.
    let flush =
        |acc: &mut Option<D>, chunk_acc: &mut Option<D>, current_chunk: &mut Option<u64>| {
            if let Some(ops_digest) = chunk_acc.take() {
                let idx = current_chunk
                    .take()
                    .expect("pending chunk must have an index");
                let grafted = match get_chunk(idx) {
                    Some(chunk) if !chunk.as_ref().iter().all(|&b| b == 0) => {
    ...
    for (peak_height, digest) in peaks {
        ...
        let fold_directly =
            peak_height >= grafting_height || get_chunk(peak_start / chunk_size).is_none();

        if fold_directly {
            // Peak at or above grafting height (already a grafted node), or ungraftable
            // (partial trailing chunk): flush any pending accumulator and fold directly.
            flush(&mut acc, &mut chunk_acc, &mut current_chunk);
            acc = Some(acc.map_or(digest, |a| hasher.fold(&a, &digest)));
        } else {
            let chunk_idx = peak_start / chunk_size;
            if current_chunk != Some(chunk_idx) {
                // New graftable chunk: flush the previous one and start fresh.
                flush(&mut acc, &mut chunk_acc, &mut current_chunk);
                current_chunk = Some(chunk_idx);
            }
            chunk_acc = Some(chunk_acc.map_or(digest, |a| hasher.fold(&a, &digest)));
        }
    }
...
///
/// For MMR this produces the same result as `hasher.root(leaves, peaks)` because every
...
///
/// `get_chunk` returns the bitmap chunk for a given chunk index, or `None` if the chunk
/// is not graftable (e.g. the partial trailing chunk, or a chunk outside the scope).
...
/// A virtual [StorageTrait] that presents a grafted tree and ops tree as a single combined
/// Merkle structure.
///
...
///
/// Both the ops structure and the grafted structure use the same [Family] `F`. The combined
/// storage presents as `StorageTrait<F>` so that callers generic over `F` can use it
/// transparently.
...
mod tests {
    ...
    fn build_test_grafted_tree_for_family<F: Graftable>(
        ...
        if !chunks.is_empty() {
            ...
            grafted_tree.apply(changeset).unwrap();
        }
