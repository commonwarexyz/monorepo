...
pub type Db<E, K, V, H, T, const N: usize, S = Merkleized<H>, D = Durable> =
    super::db::Db<E, Journal<E, Operation<K, V>>, K, FixedEncoding<V>, Index<T, Location>, H, N, S, D>;
...
pub mod test {
    ...
    use crate::qmdb::current::tests::unordered_fixed_config;
    ...
    async fn open_db(
        ...
        CleanCurrentTest::init(context, unordered_fixed_config(&partition_prefix))
            .await
            .unwrap()
    }
