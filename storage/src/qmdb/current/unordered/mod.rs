...
pub mod tests {
        ...
        Fut: Future<Output = C>,
    {
        ...
        executor.start(|context| async move {
            ...
            assert_eq!(db.op_count().await, Location::new_unchecked(1));
            ...
            assert!(db.get_metadata().await.unwrap().is_none());
            assert_eq!(db.op_count().await, Location::new_unchecked(4)); // 1 update, 1 commit, 1 move + 1 initial commit.
            let root1 = db.root().await;
            assert_ne!(root1, root0);
            drop(db);
            let db: C = open_db(context.with_label("third"), partition.clone()).await;
            assert_eq!(db.op_count().await, Location::new_unchecked(4)); // 1 update, 1 commit, 1 moves + 1 initial commit.
            assert!(db.get_metadata().await.unwrap().is_none());
            ...
            assert_eq!(db.op_count().await, Location::new_unchecked(6)); // 1 update, 2 commits, 1 move, 1 delete.
            ...
            assert_eq!(db.op_count().await, Location::new_unchecked(7));
            ...
            assert!(db.get_bit(*op_count - 1).await);
