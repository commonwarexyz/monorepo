//! A partitioned variant of [crate::qmdb::current::unordered::variable] that uses a partitioned index for the snapshot.
//!
//! See [crate::qmdb::any::unordered::partitioned::fixed] for details on partitioned indices and
//! when to use them.

pub use crate::qmdb::current::unordered::db::KeyValueProof;
use crate::{
    bitmap::CleanBitMap,
    ...
    qmdb::{
        any::{
            unordered::partitioned::variable::{Db as AnyDb, Operation},
...
/// A partitioned variant of [crate::qmdb::current::unordered::variable::Db].
///
...
pub type Db<E, K, V, H, T, const P: usize, const N: usize, S = Merkleized<H>, D = Durable> =
    crate::qmdb::current::unordered::db::Db<E, Journal<E, Operation<K, V>>, K, VariableEncoding<V>, Index<T, Location, P>, H, N, S, D>;
...
mod test {
    ...
    fn test_build_small_close_reopen() {
        crate::qmdb::current::unordered::tests::test_build_small_close_reopen::<CleanCurrentTestP1, _, _>(
            open_db,
        );
    }
    ...
    fn test_build_big() {
        ...
        tests::test_current_db_build_big::<CleanCurrentTestP1, _, _>(open_db, 1957, 838);
    }
