...
mod tests {
    ...
    use commonware_runtime::{deterministic, Blob, Buf, BufMut, Observer, Runner, Storage, Supervisor};
    ...
    fn test_put_get() {
        ...
        executor.start(|context| async move {
            ...
            let mut store = Ordinal::<_, FixedBytes<32>>::init(context.child("ordinal"), cfg.clone())
                .await
    ...
    fn test_get_nonexistent() {
        ...
        executor.start(|context| async move {
            ...
            let store = Ordinal::<_, FixedBytes<32>>::init(context.child("ordinal"), cfg.clone())
                .await
                .expect("Failed to initialize store");
    ...
    fn test_prune_after_restart() {
        ...
        executor.start(|context| async move {
            ...
            {
                let store =
                    Ordinal::<_, FixedBytes<32>>::init(context.child("third"), cfg.clone())
                        .await
                        .expect("Failed to initialize store");
    ...
    fn test_init_skip_unparseable_record() {
        ...
        executor.start(|context| async move {
            ...
            {
                let mut store =
                    Ordinal::<_, DummyValue>::init(context.child("first"), cfg.clone())
                        .await
                        .expect("Failed to initialize store");
            ...
            {
                let store =
                    Ordinal::<_, DummyValue>::init(context.child("second"), cfg.clone())
                        .await
                        .expect("Failed to initialize store");
