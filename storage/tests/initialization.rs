...
fn test_sync_rejects_missing_acknowledged_data() {
    deterministic::Runner::default().start(|context| async move {
        ...
        assert!(
            result.is_err(),
            "must reject missing data instead of resetting corrupt storage"
        );
    });
}
...
fn test_keyless_synced_range_reopens() {
    ...
    deterministic::Runner::default().start(|context| async move {
        ...
        let source = Db::init(context.child("source"), make_config("source"))
            .await
            .unwrap();
        ...
        let reopened = Db::init(context.child("reopened"), config).await;
        assert!(
            reopened.is_ok(),
...
fn test_oversized_overshooting_cap_keeps_lazy_committed_validation() {
    deterministic::Runner::default().start(|context| async move {
        ...
        drop(markers.sync().await.unwrap());
        let (blob, _) = context
...
#[test]
fn test_uncapped_fixed_repairs_hole_after_full_capacity() {
    deterministic::Runner::default().start(|context| async move {
        ...
        assert!(
            result.is_ok(),
            "uncapped recovery no longer trims a hole after capacity: {result:?}"
        );
    });
}
