    ...
    R: Receiver<PublicKey = C::PublicKey>,
{
    ...
    pub async fn init(
        ...
        let mut arbiter = Arbiter::new(None, dealers.clone(), players.clone(), CONCURRENCY);

        // If the node crashed in the middle of dealing, recover the dealer state from storage.
        let dealer_meta = if let Some(meta) = store
            .get(&FixedBytes::new(epoch.to_le_bytes().into()))
            .await
            .unwrap()
        {
            ...
            if let Some((commitment, shares, acks)) = meta.deal {
                let (mut dealer, _, _) = Dealer::new(context, None, players.clone());
            ...
            let (dealer, commitment, shares) = Dealer::new(context, None, players.clone());
            Some(DealerMetadata {
                dealer,
                commitment,
                shares,
        ...
        for (idx, contributor) in needs_broadcast {
            ...
            if let Some((signer_index, ref mut player)) = self.player {
                if idx == signer_index as usize {
                    ...
                    self.store
                        .upsert(FixedBytes::new(self.epoch.to_le_bytes()), |meta| {
    ...
    pub async fn finalize(self, round: u64) -> Option<(Vec<C::PublicKey>, RoundResult<V>)> {
        ...
        match self.player {
            Some((signer_index, player)) => {
                ...
                Some((self.players, RoundResult::Output(output)))
            }
            None => Some((self.players, RoundResult::Polynomial(result.public))),
        }
    }
