...
/// Returns `Some(epoch)` if the height is the final block in the epoch, `None` otherwise.
pub fn is_boundary(height: u64) -> Option<u64> {
    // Genesis block is not in any epoch.
    if height == 0 {
        return None;
    }
    // Check if the height is the final block in the epoch.
...
pub fn is_height_in_epoch(height: u64, epoch: u64) -> bool {
    // Genesis block is not in any epoch.
    if height == 0 {
        return false;
    }
...
/// Returns the height of the final block in the epoch.
pub fn get_boundary_height(epoch: u64) -> u64 {
