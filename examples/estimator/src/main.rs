...
use commonware_runtime::{
    deterministic, BufferPooler, Clock, Handle, Metrics, Network as RNetwork, Quota, Runner,
    Spawner,
};
...
async fn run_simulation_logic<C: Spawner + Clock + Metrics + RNetwork + RngCore + BufferPooler>(
    context: C,
    proposer_idx: usize,
    peers: usize,
    distribution: &Distribution,
    ...
    let identities = setup_network_identities(&mut oracle, &pool, distribution).await;
