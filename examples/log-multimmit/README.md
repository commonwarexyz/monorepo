...
opaque producer tips and authenticates sparse ordering checkpoints; reconstructing and delivering
a dense application stream is a separate concern.
...
- **Logs**: a bounded, scrolling tail of raw consensus traces below the status lanes.

Press the arrows to scroll the logs and `esc` to quit.
...
Like `commonware-log`, the example omits payload custody and dissemination. The consensus engine
handles commitments and protocol metadata only. A production attachment would store and transport
payload bytes outside Multimmit, report their validity and availability through `Automaton`, and
use `Relay` notifications to drive that transport. Here the automaton owns proposal policy and
immediately returns a deterministic commitment for each request. The relay is a no-op, and the
typed Multimmit reporter discards authenticated activity.

The example is intentionally body-free and delivery-free: it has no payload store, marshal layer,
separate availability interface, ordered application stream, or backfill path. It needs no
Marshal-like service because it demonstrates consensus only; an application that requires dense,
acknowledged delivery would add that service outside Multimmit.
...
storage IOPS and throughput, profiling, trace sampling, P2P port, dashboard, and
binary filename can also be overridden. The command creates node configs, `dashboard.json`, and the
deployer's `config.yaml`. It does not create cloud resources.
