...
//! # Network
//!
//! Every node runs one authenticated network carrying data availability, consensus artifacts,
//! certificates, artifact recovery, complete-block broadcast, and body backfill on separate
//! channels. Channels have independent quotas and share each peer's priority queues and TCP
//! connection. A message already being written must drain before the next message can start.
//!
...
//! node binds only its configured port.
