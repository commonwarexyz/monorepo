...
impl App {
    pub(super) fn update(&mut self, key: KeyEvent) -> Update {
        ...
        match key.code {
            ...
            KeyCode::Up => {
                self.logs.tail_offset = self.logs.tail_offset.saturating_add(1);
                Update::render()
            }
            KeyCode::Down => {
                self.logs.tail_offset = self.logs.tail_offset.saturating_sub(1);
                Update::render()
            }
            _ => Update::none(),
        }
    ...
    pub(super) fn render(&mut self, frame: &mut Frame<'_>, area: Rect, state: &UiSnapshot) {
        ...
        let [lanes, logs] = screen_panes(area, chain_count);
        render_lanes(frame, lanes, state);
        self.logs.render(frame, logs, state);
    }
}
...
impl Logs {
    fn render(&self, frame: &mut Frame<'_>, area: Rect, state: &UiSnapshot) {
        ...
        let max_scroll = state.logs.len().saturating_sub(usize::from(height));
        let tail_offset = usize::from(self.tail_offset).min(max_scroll);
        let scroll = max_scroll.saturating_sub(tail_offset) as u16;
        let lines = state
            .logs
            .iter()
            ...
            Paragraph::new(Text::from(lines))
                .block(
                    Block::default()
                        .borders(Borders::ALL)
                        .border_type(BorderType::Rounded)
                        .title(format!(" Logs · {} events ", state.logs.len()))
                        .title_bottom(Line::styled(
...
mod tests {
    use super::*;
    use crate::gui::{EngineInspection, EngineSnapshot, UiSnapshot};
    ...
    fn snapshot() -> UiSnapshot {
        UiSnapshot {
            ...
            logs: vec![String::from("LOG_SENTINEL")],
        }
    }
    ...
    #[test]
    fn keyboard_scrolls_logs_and_release_is_ignored() {
        let mut app = App::default();
        let update = app.update(KeyEvent::new(KeyCode::Up, KeyModifiers::NONE));
        assert!(update.render);
        assert_eq!(app.logs.tail_offset, 1);
        ...
        app.update(KeyEvent::new(KeyCode::Down, KeyModifiers::NONE));
        assert_eq!(app.logs.tail_offset, 0);
        assert!(matches!(
    ...
    #[test]
    fn one_screen_renders_lanes_and_logs() {
        ...
        assert!(screen.contains("LOG_SENTINEL"));
