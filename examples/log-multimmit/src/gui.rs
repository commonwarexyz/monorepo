...
use crate::application::Block as ApplicationBlock;
use commonware_actor::Feedback;
use commonware_consensus::{
    Reporter,
    multimmit::{Inspection, marshal::Update},
};
use commonware_cryptography::{Digest, sha256::Digest as Sha256Digest};
...
use std::{
    ...
    env::var,
    fs::File,
    io::{Result as IoResult, Write, sink, stdout},
    sync::{
        Arc,
...
/// Bounded observer for marshal's dense finalized-block stream.
#[derive(Clone)]
...
impl<E: Spawner + Metrics> Gui<E> {
    /// Creates the UI, its ordered-delivery reporter, and the tracing writer.
    pub fn new(
        context: E,
        status: EngineStatus,
        me: u32,
        producer_chain: Option<u32>,
    ) -> (Self, OrderedReporter) {
        let (ordering, redraws) = OrderedReporter::channel();
        let trace_file = var("LOG_MULTIMMIT_TRACE").map_or_else(
            |_| {
    ...
    pub async fn run(mut self) {
        ...
        loop {
            ...
            let update = match event {
                ...
                Event::Delivery | Event::Tick => components::Update {
                    effect: None,
                    render: true,
                },
            };
...
mod tests {
    use super::*;
    use crate::application::{Block, Body};
    use bytes::Bytes;
