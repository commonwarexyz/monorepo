...
let pendingInvite: Invite | undefined;
...
async function initialize(): Promise<void> {
  ...
  try {
    const parsedInvite = parseInvite(new URL(window.location.href));
    if (parsedInvite) {
      pendingInvite = parsedInvite;
    ...
    const role = pendingInvite ? "responder" : "initiator";
    const invite = pendingInvite ?? createInvite(publicKey);
    if (role === "responder") {
