...
describe("application URL", () => {
  test("prefers an explicitly configured root URL", () => {
  ...
  test("rejects a public URL whose path the server cannot serve", () => {
    expect(() => selectApplicationUrl(
      "https://chat.example.com/pair/",
      "http",
      3000,
      ["192.168.1.42"],
    )).toThrow("PUBLIC_URL must not contain a path");
  });
...
test("does not cache either stable application shell path", () => {
