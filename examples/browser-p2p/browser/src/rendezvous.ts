...
export class RendezvousClient {
  ...
  constructor(url: string, session: string, role: Role) {
    ...
    this.#socket.addEventListener("error", () => {
      this.#rejectReady(new Error("The rendezvous connection failed."));
    });
    this.#socket.addEventListener("close", () => {
      this.#closed = true;
      this.#rejectReady(new Error("The rendezvous connection closed."));
    });
  }
