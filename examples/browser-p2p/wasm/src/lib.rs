...
use commonware_p2p::{
    ...
    authenticated::lookup::{AttachmentConfig, AttachmentNetwork, ExactPeerAdmission},
};
use commonware_runtime::{IoBuf, Quota, web::Runtime};
use commonware_stream::encrypted;
use commonware_utils::{NZUsize, sys_rng};
use commonware_webrtc::{WebRtcConfig, WebRtcConnection};
...
impl BrowserChat {
    ...
    pub fn attach(&self, expected_peer_hex: String, outbound: bool) -> Promise {
        ...
        runtime.spawn_root(move |context| async move {
            ...
            let (mut network, _oracle, attachments) = AttachmentNetwork::<
                _,
                _,
                WebRtcConnection,
                ExactPeerAdmission<ed25519::PublicKey>,
            >::new(context, config);
