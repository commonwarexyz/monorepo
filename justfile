...
# Run the fast Kani proofs: decode and recovery-geometry totality plus the
# container-substitution smoke test (the slow table proofs live in kani-table)
kani *args='':
    cargo kani -p commonware-runtime -Z stubbing --harness header_read_total --harness header_from_floor_bounded --harness superblock_decode_total --harness recovery_block_alignment_total --harness recovery_chunk_span_bounded --harness vecmap_symbolic_inserts {{ args }}
