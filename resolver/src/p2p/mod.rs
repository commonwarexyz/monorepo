...
//! and returns `true` if valid, completing the fetch, or `false` to retry. Canceling a fetch with
...
mod tests {
    ...
    fn test_concurrent_fetch_requests() {
        ...
        executor.start(|context| async move {
            ...
            // Add links between the requester and the two producers.
            add_link(&mut oracle, LINK.clone(), &peers, 0, 1).await;
            add_link(&mut oracle, LINK.clone(), &peers, 0, 2).await;
    ...
    fn test_rate_limit_retry_after_reset() {
        ...
        executor.start(|context| async move {
            ...
            // Issue 3 fetch requests to a single peer with a strict rate limit.
            // The mailbox sender path enqueues without waiting for this rate limit.
            mailbox1.fetch(Key(1));
            ...
            // Verify the mailbox sender path did not wait for rate-limit resets.
            let elapsed = context.current().duration_since(start).unwrap();
            assert!(
                elapsed < Duration::from_secs(1),
                "expected mailbox enqueue to avoid rate-limit delay, but took {elapsed:?}"
            );
        });
    }
