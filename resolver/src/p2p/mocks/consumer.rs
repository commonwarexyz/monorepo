...
pub enum ConsumerEvent<K, V, F> {
    Delivered(K, V),
    Failed(K, F),
}

pub struct Consumer<K: Array, V, F> {
    sender: mpsc::Sender<ConsumerEvent<K, V, F>>,
}

impl<K: Array, V: Clone, F: Clone> Consumer<K, V, F> {
    pub fn new(sender: mpsc::Sender<ConsumerEvent<K, V, F>>) -> Self {
        Self { sender }
    }
}
    ...
    for Consumer<K, V, F>
{
    ...
    async fn deliver(&mut self, key: Self::Key, value: Self::Value) {
        let _ = self.sender.send(ConsumerEvent::Delivered(key, value)).await;
    }

    async fn failed(&mut self, key: Self::Key, failure: Self::Failure) {
        let _ = self.sender.send(ConsumerEvent::Failed(key, failure)).await;
    }
}
