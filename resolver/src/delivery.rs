...
use crate::{Consumer, Delivery, DeliveryOutcome};
            ...
            PooledCompletion {
                ...
                completion: Completion {
                    ...
                    outcome: receiver
                        .await
                        .map(Into::into)
                        .unwrap_or(DeliveryOutcome::Invalid),
                },
            }
        });
        let entry = self.entries.get_mut(&key).expect("delivery entry");
...
mod tests {
    ...
    fn test_deliver_completes_with_context_and_consumer_result() {
        ...
        runner.start(|_| async move {
            ...
            assert_eq!(completed.outcome, DeliveryOutcome::Complete);
    ...
    fn test_redeliver_reuses_accepted_response_for_new_subscribers() {
        ...
        runner.start(|_| async move {
            ...
            assert_eq!(redelivered.outcome, DeliveryOutcome::Complete);
    ...
    fn test_rejected_response_can_be_discarded_and_replaced() {
        ...
        runner.start(|_| async move {
            ...
            assert_eq!(rejected.outcome, DeliveryOutcome::Invalid);
            ...
            assert_eq!(accepted.outcome, DeliveryOutcome::Complete);
        });
    }
}
