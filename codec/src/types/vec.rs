...
mod tests {
    ...
    fn test_vec() {
        ...
        assert!(matches!(
            Vec::<u8>::decode_range(Copying([0x02, 0x01].as_slice()), ..),
            Err(Error::EndOfBuffer)
        ));
        assert!(matches!(
            Vec::<Byte>::decode_range(Copying([0x02, 0x01].as_slice()), ..),
            Err(Error::EndOfBuffer)
        ));
        ...
        assert!(matches!(
            Vec::<u8>::decode_range(Copying([0x02, 0x01, 0x02, 0x03].as_slice()), ..),
            Err(Error::ExtraData(1))
        ));
        assert!(matches!(
            Vec::<Byte>::decode_range(Copying([0x02, 0x01, 0x02, 0x03].as_slice()), ..),
            Err(Error::ExtraData(1))
        ));
    ...
    fn test_vec_read_vec_bounds_preallocation() {
        ...
        let mut buf = Copying([0u8; 1].as_slice());
