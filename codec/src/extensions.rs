...
//!
//! These traits provide convenience methods (like `read()`, `decode()`,
//! `read_range()`, and `decode_range()`) that simplify common use cases of the
//! core [Read] and [Decode] traits, particularly when default configurations
//! (`()`) or [RangeCfg] are involved.

use crate::{Decode, Error, RangeCfg, Read};
use bytes::Buf;

/// Extension trait providing ergonomic read method for types requiring no
/// configuration (i.e. `Cfg = ()`).
///
/// Import this trait to use the `.read(buf)` method as a shorthand for
/// `.read_cfg(buf, ())`.
...
/// Extension trait providing ergonomic decode method for types requiring no
...
// Automatically implement `DecodeExt` for types that implement `Decode` with no
// config.
impl<X: Default, T: Decode<Cfg = X>> DecodeExt<X> for T {}

/// Extension trait for reading types whose config is `(RangeCfg, X)` where `X`
/// is [Default].
///
/// Useful for reading collections like [`Vec<T>`] where `T` implements [Read]
/// with no specific configuration. Import this trait to use the `.read_range()`
/// method.
pub trait ReadRangeExt<X: Default>: Read<Cfg = (RangeCfg, X)> {
    /// Reads a value using only a range configuration.
    ///
    /// The inner configuration type `X` must be [Default] and `X::default()` is
    /// used for it.
...
// Automatically implement `ReadRangeExt` for types that implement `Read` with
// config `(RangeCfg, X)`, where `X` is `Default`.
impl<X: Default, U: Read<Cfg = (RangeCfg, X)>> ReadRangeExt<X> for U {}

/// Extension trait for reading types whose config is `(RangeCfg, X)` where `X`
/// is [Default], ensuring the buffer is consumed.
///
/// Useful for decoding collections like [`Vec<T>`] where `T` implements [Read]
/// with no specific configuration. Import this trait to use the
/// `.decode_range()` method.
...
// Automatically implement `DecodeRangeExt` for types that implement `Decode`
// with config `(RangeCfg, X)`, where `X` has a default implementation.
