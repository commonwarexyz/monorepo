...
#[derive(Error, Debug)]
pub enum Error {
    /// Indicates that the input buffer (`Buf`) did not contain enough bytes to
    /// read the next piece of data required by a [crate::Read]
    /// implementation. This suggests the input data is truncated or
    /// incomplete.
    ...
    /// in the input buffer.
    ///
    /// This usually means the input data contained more than just the expected
    /// encoded value. The contained `usize` is the number of bytes that
    /// remained unconsumed.
    ...
    /// - The decoded varint value exceeds the capacity of the target integer
    ///   type.
    ///
    ...
    /// A byte representing a boolean was expected to be `0` (false) or `1`
    /// (true), but a different value was encountered during decoding.
    ...
    /// A length prefix (e.g., for `Vec<T>`, `Bytes`, `HashMap<K, V>`) was
    /// decoded, but its value fell outside the permitted range.
    ///
    ...
    /// A semantic validation error occurred during decoding, indicating the
    /// data, while perhaps structurally valid, does not meet
    /// application-specific criteria.
    ///
    /// - The first `&'static str` provides context (e.g., the name of the type
    ///   being decoded).
    /// - The second `&'static str` provides a specific error message.
    ///
    /// Example: Trying to decode a `HashMap` where keys were not in ascending
    /// order.
    #[error("Validation Error: Context({0}), Message({1})")]
    Invalid(&'static str, &'static str),

    /// An error occurred in underlying code (e.g., external library call,
    /// complex validation) and has been wrapped into a codec [enum@Error].
    ///
    /// - The `&'static str` provides context about the operation being
    ///   performed.
