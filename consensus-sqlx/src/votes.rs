//! SQLx-backed implementation of the [Votes](commonware_consensus::simplex::store::Votes) trait.
    ...
    D: Digest + Unpin,
{
    ...
    async fn append(
        ...
        let query = format!("INSERT INTO votes (epoch, view, artifact) VALUES ($1, $2, $3)",);
        sqlx::query(&query)
    ...
    async fn prune(&mut self, epoch: Epoch, min: View) -> Result<(), Self::Error> {
        sqlx::query("DELETE FROM table WHERE epoch = $1 AND view < $2 ")
            .bind(epoch.get() as i64)
            .bind(min.get() as i64)
            .execute(&self.pool)
            .await?;
    ...
    async fn replay(&mut self, epoch: Epoch) -> Result<Self::ReplayStream<'_>, Self::Error> {
        ...
        let stream =
            sqlx::query("SELECT artifact FROM votes WHERE epoch = $1 ORDER BY view ASC, id ASC")
                .bind(epoch.get() as i64)
