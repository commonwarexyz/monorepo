...
use commonware_codec::{Copying, Decode, DecodeExt, Encode, EncodeSize, Read, ReadExt};
...
pub fn fuzz(data: &[u8]) {
    ...
    let _ = Item::<Sha256Digest>::decode(Copying(rest));
    let _ = Ack::<Scheme, Sha256Digest>::decode(Copying(rest));
    let _ = TipAck::<Scheme, Sha256Digest>::decode(Copying(rest));
    let _ = Certificate::<Scheme, Sha256Digest>::decode_cfg(Copying(rest), &());
    let _ = Activity::<Scheme, Sha256Digest>::decode_cfg(Copying(rest), &());
}
...
fn roundtrip_item(u: &mut Unstructured<'_>) {
    ...
    let decoded = Item::<Sha256Digest>::read(&mut Copying(&encoded)).unwrap();
...
fn roundtrip_ack(u: &mut Unstructured<'_>) {
    ...
    let decoded = Ack::<Scheme, Sha256Digest>::read(&mut Copying(&encoded)).unwrap();
...
fn roundtrip_tip_ack(u: &mut Unstructured<'_>) {
    ...
    let decoded = TipAck::<Scheme, Sha256Digest>::read(&mut Copying(&encoded)).unwrap();
    assert_eq!(encoded, decoded.encode());
}
...
fn roundtrip_certificate(u: &mut Unstructured<'_>) {
    ...
    let decoded =
        Certificate::<Scheme, Sha256Digest>::read_cfg(&mut Copying(&encoded), &()).unwrap();
...
fn roundtrip_activity(u: &mut Unstructured<'_>) {
    ...
    let decoded = Activity::<Scheme, Sha256Digest>::read_cfg(&mut Copying(&encoded), &()).unwrap();
