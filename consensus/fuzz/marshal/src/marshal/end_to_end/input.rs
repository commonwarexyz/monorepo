...
fn sample_fault_rounds(
    ...
    let min_fault_rounds =
        commonware_consensus_fuzz_core::MIN_NUMBER_OF_FAULTS.min(fault_rounds_bound);
    let max_fault_rounds = (fault_rounds_bound
        / commonware_consensus_fuzz_core::FAULT_INJECTION_RATIO)
        .max(min_fault_rounds);
