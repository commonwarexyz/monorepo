...
- **Scenario** — the reproduction of one source test's state-producing prefix,
  written in the fuzz crate in `consensus/fuzz/marshal/src/scenarios/`. Every requirement in this document applies to
...
The invariants in `consensus/fuzz/marshal/src/marshal/end_to_end/invariants.rs` belong
...
- **R6 — Mode coverage.** The feature MUST support an honest mode (`N4F0C4`,
  ...
  byzantine, nodes 1–3 honest). These configurations are defined in `consensus/fuzz/core/src/lib.rs`
...
### 5.1 Feature module — `consensus/fuzz/marshal/src/scenarios/`
...
- `consensus/fuzz/marshal/fuzz_targets/marshal_scenario_standard_deferred_cert_mock.rs`
- `consensus/fuzz/marshal/fuzz_targets/marshal_scenario_standard_inline_cert_mock.rs`
- The corresponding `[[bin]]` entries in `consensus/fuzz/marshal/Cargo.toml`.
...
- The marshal-layer invariants in
  `consensus/fuzz/marshal/src/marshal/end_to_end/invariants.rs` are ground truth.
- The twins marshal stack and the from-floor engine entry point in
  `consensus/fuzz/marshal/src/marshal/end_to_end/twins/stack.rs`, including the
...
- **I1 — Marshal-layer invariants.** Every invariant defined in
  `consensus/fuzz/marshal/src/marshal/end_to_end/invariants.rs` MUST hold at the
...
2. **Source fidelity is demonstrated.**
   ...
   `rg '^/// Source:' consensus/fuzz/marshal/src/scenarios/scenarios.rs` lists exactly
   the retained scenarios, and every named test exists in the referenced file.


3. **Both targets and the runner compile.**
   `cargo check -p commonware-consensus-fuzz-marshal --features mocks --tests` succeeds,
...
4. **Scenario suite passes for all compatible variants and modes.**
   `cargo nextest run -p commonware-consensus-fuzz-marshal --features mocks scenarios::`
...
5. **Lint is clean.**
   `cargo clippy -p commonware-consensus-fuzz-marshal --features mocks --tests` reports no
   warnings.

6. **Both fuzz targets build and run.**
   `cargo +nightly fuzz build --fuzz-dir consensus/fuzz/marshal
...
`consensus/fuzz/marshal/src/scenarios/scenarios.rs` is the reference translation and the
