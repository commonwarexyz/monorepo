...
use crate::marshal::end_to_end::{
    ...
    twins::{
        ...
        stack::{
            DEFAULT_MAX_PENDING_ACKS, DeferredMarshal, InlineMarshal, MarshalChoice, TwinsMarshal,
            genesis_block, register_engine_networks, setup_network, setup_validator,
            start_engine_with_floor,
        },
    },
};
...
use commonware_consensus_fuzz_core::{NetworkChannels, simplex::Simplex, utils::apply_partition};
    ...
    M::Wrapper: Clone + Send + 'static,
{
    ...
    executor.start(|mut context| async move {
        // === SETUP ===
        let (participants, schemes) = P::setup(&mut context, commonware_consensus_fuzz_core::NAMESPACE, NUM_VALIDATORS);
...
mod tests {
    ...
    use crate::scenarios::input::{
        BackfillFault, BlockFault, ConsensusMutation, FaultPlan, ScenarioKind,
    };
    use commonware_consensus::simplex::ForwardPolicy;
    use commonware_consensus_fuzz_core::{
        Configuration, N4F0C4, N4F1C3, SimplexCertificateMock, utils::Partition,
    };
