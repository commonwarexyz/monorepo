...
//! over a stable action catalog ([`action`]) under the backend-agnostic Q-core.
...
//! not catalog actions. They replace node `crate::BYZANTINE_IDX`
...
//! - **Crash-stop is permanent, not terminal.** A crash-stop action leaves node 0
//!   down for the rest of the episode (never resurrected within it), but the episode
//!   CONTINUES over the surviving quorum: the crashed node is dropped from the
//!   reactive finalization / liveness sets and from every later step's legal mask
//!   (lifecycle and isolate-node-0 masked; partition and packet faults stay
//!   selectable with node 0 excluded as a packet target), its retained reporter stays
//!   in the safety set, and a distinct "node 0 crashed" bit keys its Q-state. The
