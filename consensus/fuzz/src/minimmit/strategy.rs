...
pub trait MinimmitStrategy: Send + Sync {
    fn random_proposal(
        &self,
        rng: &mut impl Rng,
        last_vote_view: u64,
    ...
    fn random_parent_payload(&self, rng: &mut impl Rng) -> Sha256Digest;
...
pub struct MinimmitSmallScope {
    pub fault_rounds: u64,
    pub fault_rounds_bound: u64,
}

impl MinimmitStrategy for MinimmitSmallScope {
    fn random_proposal(
        &self,
        rng: &mut impl Rng,
        last_vote_view: u64,
        ...
        let parent_payload = self.random_parent_payload(rng);
        Proposal::new(
    ...
    fn mutate_proposal(
        ...
        match rng.gen::<u8>() % 6 {
            ...
            4 => proposal_with_payload(proposal, tweak_payload(rng, proposal.payload)),
            _ => {
                let mut pp = proposal.parent_payload;
                let idx = rng.gen_range(0..pp.0.len());
                pp.0[idx] ^= 1 << (rng.gen::<u8>() % 8);
                Proposal::new(
                    Round::new(Epoch::new(EPOCH), View::new(view)),
                    proposal.parent,
                    pp,
...
pub struct MinimmitAnyScope;

impl MinimmitStrategy for MinimmitAnyScope {
    fn random_proposal(
        &self,
        rng: &mut impl Rng,
        last_vote_view: u64,
    ...
    fn mutate_proposal(
        ...
        match rng.gen::<u8>() % 4 {
            ...
            _ => {
                ...
                let parent_payload = random_payload(rng);
                Proposal::new(
                    Round::new(Epoch::new(EPOCH), View::new(view)),
                    View::new(parent),
                    parent_payload,
...
pub struct MinimmitFutureScope {
    pub fault_rounds: u64,
    pub fault_rounds_bound: u64,
}

impl MinimmitStrategy for MinimmitFutureScope {
    fn random_proposal(
        &self,
        rng: &mut impl Rng,
        last_vote_view: u64,
