...
impl<V: Variant> Harness<V> {
    ...
    async fn start(&mut self, node: usize, interrupt: bool) -> bool {
        ...
        let mut gate = interrupt.then(|| {
            self.applications[node]
                .gate_verifications(1)
                .pop()
                .expect("one recovery custody gate")
        });
        let reports = self.reporters[node].len();
        let proposed = self.applications[node].log().lock().proposed;
        let mut starting = Box::pin(engine.start(
        ...
        let result = if let Some(gate) = &mut gate {
            select! {
                result = &mut starting => result,
                () = gate.wait_started() => {
                    select! {
                        _ = &mut starting => panic!("recovery crossed unresolved custody"),
                        () = self.context.sleep(RELOAD_DELAY) => {},
                    }
                    assert_eq!(self.reporters[node].len(), reports);
                    assert_eq!(self.applications[node].log().lock().proposed, proposed);
                    drop(starting);
        ...
        let mut running = result.unwrap_or_else(|error| {
            panic!("engine {node} generation {generation} starts: {error:?}")
        });
...
mod tests {
    ...
    fn seeded_repeated_custody_recovery() {
        ...
        .start(|context| async move {
            ...
            >::new(context, SEED)
            .await;
            harness.applications[0].pause_building();
            ...
            harness.applications[0].produce_continuously();
            harness.finish().await;
            for _ in 0..2 {
                harness.applications[0].pause_building();
                harness.observe().await;
