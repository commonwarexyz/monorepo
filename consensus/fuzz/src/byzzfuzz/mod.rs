...
//!
//! - **round attribution** (`rnd(m)`): per-sender atomic (`SenderViewCell`)
//!   tracking "the maximal round in which the sender has sent or received a
//!   message". Outgoing forwarders fold each transmitted view in;
//!   `RoundTrackingReceiver` wrappers on inbound vote / cert / resolver
//!   receivers fold each *received* view in. Resolver inbound participates
//!   via a wire-format hand-decoder (`resolver_view_extractor`) that
//!   recovers the view from `Request(U64)` keys and from the `Certificate`
//!   embedded in `Response` bytes -- so even a node that has never voted
//!   on a view but receives a fetch request for it correctly raises its
//!   round cell;
