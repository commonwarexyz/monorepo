        ...
        Clone + Send + Sync + 'static,
{
    ...
    let make_resolver_forwarder = || {
        ...
        let scenario = scenario.clone();
        move |origin: SplitOrigin, _recipients: &Recipients<_>, message: &IoBuf| {
            let view = commonware_consensus_fuzz_core::twins_resolver_view::<P, Sha256Digest>(
                message, &codec,
            )?;
            let (primary, secondary) =
    ...
    let make_resolver_router = || {
        ...
        let scenario = scenario.clone();
        move |(sender, message): &(_, IoBuf)| {
            let Some(view) = commonware_consensus_fuzz_core::twins_resolver_view::<P, Sha256Digest>(
                message, &codec,
            ) else {
                return SplitTarget::None;
            };
    ...
    let primary = {
        ...
        let engine_cfg = config::Config {
            ...
            floor: Floor::Genesis(application::genesis::<Sha256>(Epoch::new(
                commonware_consensus_fuzz_core::EPOCH,
            ))),
            ...
            page_cache: CacheRef::from_pooler(
                &primary_context,
                commonware_consensus_fuzz_core::PAGE_SIZE,
                commonware_consensus_fuzz_core::PAGE_CACHE_SIZE,
            ),
            strategy: Sequential,
...
fn live_honest<P: Simplex>(managed: &[HonestValidator<P>]) -> HashSet<usize> {
    managed
        ...
        .filter(|mv| {
            matches!(
                mv.lifecycle(),
                commonware_consensus_fuzz_core::ValidatorLifecycle::Running
            )
        })
        .map(|mv| mv.idx())
        .collect()
        ...
        Clone + Send + Sync + 'static,
{
    ...
    input.certify = commonware_consensus_fuzz_core::CertifyChoice::Always;
    ...
    executor.start(|mut context| async move {
        ...
        commonware_consensus_fuzz_core::print_fuzz_input::<P>(commonware_consensus_fuzz_core::Mode::ChaosTwins, &input);
