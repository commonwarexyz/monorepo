...
Open `consensus/fuzz/simplex/src/byzzfuzz/specs/INDEX.md` and use the task table to find the relevant document.
...
Links between specs are relative from `consensus/fuzz/simplex/src/byzzfuzz/specs/`. Source paths are repo-root paths in backticks.
...
```markdown
- `consensus/fuzz/simplex/src/byzzfuzz/runner.rs` - ByzzFuzz run orchestration.
```
