...
The fuzz harness dispatches into ByzzFuzz through `Mode::Byzzfuzz` and `byzzfuzz::run`; ByzzFuzz may use shared harness setup helpers but private ByzzFuzz behavior stays inside `consensus/fuzz/simplex/src/byzzfuzz`.

## Interfaces

| Interface | Package | Consumed By | Purpose |
| --------- | ------- | ----------- | ------- |
| `pub mod byzzfuzz` | `consensus/fuzz/simplex/src/lib.rs` | Fuzz crate users | Exposes the ByzzFuzz module. |
| `pub use runner::run` | `consensus/fuzz/simplex/src/byzzfuzz/mod.rs` | `fuzz::<P, Byzzfuzz, CodeCoverage>` dispatch | Runs one ByzzFuzz iteration. |
| `pub struct Byzzfuzz` | `consensus/fuzz/simplex/src/lib.rs` | Fuzz targets | Marker type implementing `FuzzMode`. |
| `Mode::Byzzfuzz` | `consensus/fuzz/core/src/lib.rs` | `fuzz` dispatcher | Selects ByzzFuzz mode. |
| `setup_network` | `consensus/fuzz/core/src/lib.rs` | `runner.rs` | Builds simulated network, participants, schemes, and channel registrations. |
| `spawn_honest_validator` | `consensus/fuzz/core/src/lib.rs` | `runner.rs` | Starts Simplex engine, application, and reporter. |
| `byzzfuzz::log::take` | `consensus/fuzz/simplex/src/byzzfuzz/log.rs` | `lib.rs` panic hook | Drains decision trace. |
...
- If `byzzfuzz::run` signature changes, update `Mode::Byzzfuzz` dispatch in `consensus/fuzz/simplex/src/lib.rs` and all ByzzFuzz fuzz targets.
