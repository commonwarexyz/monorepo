...
| --------- | ------- | ----------- | ------- |
| `intercept::channel::<P>()` | `consensus/fuzz/simplex/src/byzzfuzz/intercept.rs` | `runner::setup_engines` | Constructs the sync->async `(UnboundedSender, UnboundedReceiver)` pair. |
| `Intercept<P>` | `consensus/fuzz/simplex/src/byzzfuzz/intercept.rs` | forwarders push, injector consumes | One captured byzantine outgoing message paired with one matching `procFault`. |
| `InterceptChannel` | `consensus/fuzz/simplex/src/byzzfuzz/intercept.rs` | forwarder, injector | Discriminates `Vote` / `Cert` / `Resolver` for routing. |
| `ByzzFuzzInjector::start` | `consensus/fuzz/simplex/src/byzzfuzz/injector.rs` | `runner::setup_engines` | Spawns the async loop with the cloned byzantine vote sender and intercept receiver. |
