...
- `consensus/fuzz/simplex/src/byzzfuzz/runner.rs` - ByzzFuzz setup, phase control, GST transition, and invariant checks.
- `consensus/fuzz/simplex/src/byzzfuzz/mod.rs` - exports the entry point and the fixed byzantine index.
- `consensus/fuzz/simplex/src/lib.rs` - mode dispatch, panic hook, and shared network/validator setup helpers.
- `consensus/fuzz/simplex/fuzz_targets/simplex_cert_mock_byzzfuzz.rs` - fuzz target that selects `Mode::Byzzfuzz`.
...
| Post-GST window length | `BYZZFUZZ_LIVENESS_WINDOW` (`byzzfuzz/runner.rs`) | Upper bound on phase 2 duration. |
...
- Change phase timing by editing `FAULT_PHASE` in `lib.rs` and
  `BYZZFUZZ_LIVENESS_WINDOW` in `byzzfuzz/runner.rs`, then update
  [ADR-005](../decisions/005-post-gst-required-container-catch-up.md) if the
  liveness model changes.
