...
- `consensus/fuzz/simplex/src/byzzfuzz/log.rs` - bounded decision log.
- `consensus/fuzz/simplex/src/byzzfuzz/forwarder.rs` - logs partition drops and process intercepts.
- `consensus/fuzz/simplex/src/byzzfuzz/injector.rs` - logs omit, skip, and replacement decisions.
- `consensus/fuzz/simplex/src/byzzfuzz/runner.rs` - logs sampled schedules and GST transition.
- `consensus/fuzz/simplex/src/lib.rs` - installs the ByzzFuzz panic hook and drains the log on successful runs.
