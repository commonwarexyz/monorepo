...
use crate::{
    ...
    happens_before, invariants, sniff_sink,
};
use bytes::Bytes;
...
use commonware_consensus_fuzz_core::{
    CertifyChoice, EPOCH, FAULT_INJECTION_RATIO, FAULT_PHASE, N4F0C4, PublicKeyOf, block_relay,
    simplex::Simplex, spawn_filtered_honest_validator, utils::Partition,
};
...
async fn setup_engines<P: Simplex>(
    ...
    input: &mut commonware_consensus_fuzz_core::FuzzInput,
    gate: FaultGate,
        ...
        Clone + Send + Sync + 'static,
{
    ...
    executor.start(|mut context| async move {
        ...
        commonware_consensus_fuzz_core::print_fuzz_input::<P>(
            commonware_consensus_fuzz_core::Mode::Byzzfuzz,
            &input,
        );
