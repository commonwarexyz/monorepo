...
//!
//! The buffer is process-wide. Mallory fuzz targets are run by libfuzzer one
//! input at a time on a single thread, so the global state is appropriate. For
//! test-suite usage, runs share the buffer; the FIFO cap keeps memory bounded.
