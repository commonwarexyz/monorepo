...
use commonware_consensus_fuzz_core::{N4F0C4, utils::SetPartition};
...
/// faultable identity ([`commonware_consensus_fuzz_core::BYZANTINE_IDX`]):
/// - `CrashStop = 8`
...
pub(crate) enum Fault {
    ...
    NoFault,
    /// Isolate the single faultable identity ([`commonware_consensus_fuzz_core::BYZANTINE_IDX`]) from the
    ...
    PacketReorder,
    /// Crash-stop the single faultable identity ([`commonware_consensus_fuzz_core::BYZANTINE_IDX`]): abort
...
pub(crate) enum FaultPlan {
    ...
    None,
    /// Isolate [`commonware_consensus_fuzz_core::BYZANTINE_IDX`] from the other three validators.
    IsolateByzantine,
    ...
    /// Crash-stop [`commonware_consensus_fuzz_core::BYZANTINE_IDX`] (fixed target, no parameters). Enacted
    /// against the managed validator, not the network.
    CrashStop,
    /// Crash and durably restart [`commonware_consensus_fuzz_core::BYZANTINE_IDX`] (fixed target, no
    /// parameters). Enacted against the managed validator, not the network.
    CrashRestartDurable,
    /// Crash and restart [`commonware_consensus_fuzz_core::BYZANTINE_IDX`] on a FRESH (empty) storage
...
/// [`ManagedValidator`](commonware_consensus_fuzz_core::ManagedValidator) to target at all. The topology
