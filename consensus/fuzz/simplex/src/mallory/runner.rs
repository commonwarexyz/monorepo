...
use commonware_consensus_fuzz_core::{
    BYZANTINE_IDX, CertCfgOf, CertifyChoice, ManagedValidator, N4F0C4, PublicKeyOf,
    ValidatorLifecycle, build_validator, build_validator_with_reporter, simplex::Simplex,
};
...
///   safety history survives; it returns to [`Running`](commonware_consensus_fuzz_core::ValidatorLifecycle::Running).
...
///   [`Amnesiac`](commonware_consensus_fuzz_core::ValidatorLifecycle::Amnesiac), treated as Byzantine for the
///   rest of the episode.
///
        ...
        Clone + Send + Sync + 'static,
{
    ...
    let mut fresh =
        commonware_consensus_fuzz_core::utils::register(oracle, std::slice::from_ref(&validator))
            .await;
...
/// ([`commonware_consensus_fuzz_core::setup_network`], [`build_validator`], the [`SniffingReceiver`]
        ...
        Clone + Send + Sync + 'static,
{
    ...
    input.partition = commonware_consensus_fuzz_core::utils::Partition::Connected;
    ...
    executor.start(|mut context| async move {
        ...
        let (mut oracle, participants, schemes, mut registrations) =
            commonware_consensus_fuzz_core::setup_network::<P>(&mut context, &input).await;
        commonware_consensus_fuzz_core::print_fuzz_input::<P>(commonware_consensus_fuzz_core::Mode::MalloryContainer, &input);
        ...
        let topology =
            network::Topology::new(oracle.clone(), participants.clone(), commonware_consensus_fuzz_core::default_link());
