...
//!
//! In a byzantine episode `BYZANTINE_IDX` ([`commonware_consensus_fuzz_core::BYZANTINE_IDX`]) is an unmanaged
...
impl<P: Simplex> RoleMultiplexer<P> {
    ...
    pub(crate) async fn set_role(
        ...
        let mut fresh = commonware_consensus_fuzz_core::utils::register(
            &mut self.oracle,
            std::slice::from_ref(&self.node0),
        )
        .await;
        let channels = fresh
            .remove(&self.node0)
