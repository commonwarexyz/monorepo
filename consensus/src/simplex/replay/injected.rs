//! Null p2p primitives used by the [`Replayer`](super::Replayer).
//!
//! - [`Injector`] + [`InjectedReceiver`]: push pre-signed payloads onto a
//!   [`Receiver`] stream as if they came from a peer.
//! - [`NullSender`] + [`NullCheckedSender`]: silently drop outbound
//!   broadcasts. The engine's broadcast path is driven *into* the replayer
//!   via [`Event::Construct`](super::Event::Construct) for sender-local
//!   state, so outbound traffic is discarded.
//! - [`PendingReceiver`]: a receiver that never yields (for the resolver
//!   network, which replay doesn't exercise).
//! - [`NullBlocker`]: the engine's peer blocker — no-op.
...
impl Injector {
    /// Push `payload` onto the paired receiver, attributed to `sender`.
