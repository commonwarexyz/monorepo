...
impl<S: Scheme, D: Digest> Round<S, D> {
    ...
    /// Returns true if this round can support finalizing its same-term child.
    pub const fn allows_child_finalize(&self) -> bool {
        self.broadcast_finalize() || self.finalization.is_some()
    }
