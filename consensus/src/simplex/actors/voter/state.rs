...
pub struct State<E: Clock + CryptoRngCore + Metrics, S: Scheme<D>, L: ElectorConfig<S>, D: Digest> {
    ...
    /// Views for which we have voted to nullify. Used to enforce the term safety rule:
    /// do not finalize any block in a term where we have previously voted to nullify
    /// and have not yet seen a finalization certificate at or above that view in the same term.
    ...
    indirectly_notarized_views: BTreeSet<View>,
    ...
    State<E, S, L, D>
{
    const fn optimistic_validation_enabled(&self) -> bool {
        self.term_length.get() > 1 && self.optimistic_validation_depth > 0
    }
    ...
    fn optimistic_anchor(&self, view: View) -> View {
        self.views
            .range(..view)
            .rev()
    ...
    fn within_optimistic_depth(&self, view: View) -> bool {
        ...
        let anchor = self.optimistic_anchor(view);
        let optimistic_hops = view.get().saturating_sub(anchor.get()).saturating_sub(1);
        optimistic_hops <= self.optimistic_validation_depth
    }
    ...
    pub fn new(context: E, cfg: Config<S, L>) -> Self {
        ...
        Self {
            ...
            indirectly_notarized_views: BTreeSet::new(),
    ...
    /// Returns true if `view` can emit a timeout-style nullify immediately.
    pub fn should_timeout_nullify(&self, view: View) -> bool {
        self.views
            .get(&view)
            .is_some_and(|round| round.has_timeout_reason())
    }
    ...
    pub fn add_notarization(
        ...
        if result.0 {
            self.mark_indirectly_notarized(view);
        }
    ...
    ///
    /// The term safety rule applies: do not vote to finalize any block in a term
    /// if we voted to nullify a view in the same term and have not yet
    /// seen a finalization certificate at or above that view.
    ///
    ...
    pub fn replay(&mut self, artifact: &Artifact<S, D>) {
        ...
        if matches!(
            ...
            self.rebuild_indirectly_notarized();
        }
    }
    ...
    pub fn prune(&mut self) -> Vec<View> {
        ...
        // Safe to prune: min = last_finalized - activity_timeout, so every
        // pruned entry has view < last_finalized. The term safety check in
        // construct_finalize only blocks when nullify_view > last_finalized,
        ...
        self.indirectly_notarized_views = self.indirectly_notarized_views.split_off(&min);
    ...
    fn is_notarized(&self, view: View) -> bool {
        view == GENESIS_VIEW
            ...
            || (self.optimistic_validation_enabled()
                && self.indirectly_notarized_views.contains(&view))
    }
    ...
    fn mark_indirectly_notarized(&mut self, view: View) {
        ...
        while let Some(parent) = cursor {
            ...
            self.indirectly_notarized_views.insert(parent);
            cursor = Some(proposal.parent);
        }
    }

    /// Rebuilds the indirect notarization cache from currently tracked direct certificates.
    fn rebuild_indirectly_notarized(&mut self) {
        self.indirectly_notarized_views.clear();
...
mod tests {
    ...
    fn parent_payload_uses_term_skip_nullification_anchors() {
        ...
        runtime.start(|mut context| async move {
            ...
            for v in [View::new(4), View::new(5), View::new(6)] {
                let nullify_votes: Vec<_> = schemes
                    .iter()
                    .map(|scheme| {
    ...
    fn parent_payload_reports_missing_term_anchor() {
        ...
        runtime.start(|mut context| async move {
            ...
            for v in [View::new(4), View::new(5)] {
                let nullify_votes: Vec<_> = schemes
                    .iter()
                    .map(|scheme| {
                        Nullify::sign::<Sha256Digest>(scheme, Rnd::new(Epoch::new(1), v)).unwrap()
    ...
    #[test]
    fn optimistic_validation_depth_zero_disables_intra_term_lookahead() {
        let runtime = deterministic::Runner::default();
        runtime.start(|mut context| async move {
            let namespace = b"ns".to_vec();
            let Fixture { schemes, .. } = ed25519::fixture(&mut context, &namespace, 4);
    ...
    #[test]
    fn optimistic_validation_depth_bounds_chain_length() {
        let runtime = deterministic::Runner::default();
        runtime.start(|mut context| async move {
            let namespace = b"ns".to_vec();
            let Fixture { schemes, .. } = ed25519::fixture(&mut context, &namespace, 4);
    ...
    fn indirect_notarization_still_requires_certification_for_finalize() {
        ...
        runtime.start(|mut context| async move {
            ...
            assert!(added);
            assert!(equivocator.is_none());
            assert!(state.indirectly_notarized_views.contains(&View::new(1)));
    ...
    fn indirect_notarization_requires_verified_unequivocated_ancestors() {
        ...
        runtime.start(|mut context| async move {
            ...
            assert!(!state.indirectly_notarized_views.contains(&View::new(2)));
        });
    }
    ...
    fn replay_restores_indirect_notarization_state() {
        ...
        runtime.start(|mut context| async move {
            ...
            assert!(restarted.indirectly_notarized_views.contains(&View::new(1)));
            assert_eq!(
