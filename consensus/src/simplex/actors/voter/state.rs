    ...
    State<E, S, L, D>
{
    ...
    pub fn get_best_certificate(&self) -> Option<Certificate<S, D>> {
        ...
        // Check if there was a nullification in the previous view
        if let Some(nullification) = self.nullification(prev).cloned() {
        ...
        if self.term_length.get() > 1 && self.view.term_start(self.term_length) == self.view {
    ...
    pub fn replay(&mut self, artifact: &Artifact<S, D>) {
        if let Artifact::Nullify(nullify) = artifact {
            self.nullify_views.insert(nullify.view());
        }
    ...
    pub fn prune(&mut self) -> Vec<View> {
        ...
        // It is safe to drop nullification and nullify views here less than the last finalized view
        // since a view cannot be finalized in a term after a nullification in that term.
...
mod tests {
    ...
    fn parent_payload_uses_term_skip_nullification_anchors() {
        ...
        runtime.start(|mut context| async move {
            ...
            for v in [View::new(4), View::new(5), View::new(6), View::new(11)] {
                let nullify_votes: Vec<_> = schemes
                    .iter()
                    .map(|scheme| {
            ...
            let proposal = Proposal::new(
                Rnd::new(Epoch::new(1), View::new(14)),
                parent_view,
    ...
    fn parent_payload_reports_missing_term_anchor() {
        ...
        runtime.start(|mut context| async move {
            ...
            assert_eq!(
                state.parent_payload(&proposal),
                Err(ParentPayloadError::MissingNullification {
                    proposal_view: View::new(14),
                    parent_view,
                    missing_view: View::new(11),
                })
            );
        });
    }
    ...
    fn nullification_at_term_end_skips_correctly() {
        ...
        runtime.start(|mut context| async move {
            ...
            let cfg = Config {
                ...
                certification_timeout: Duration::from_secs(2),
                timeout_retry: Duration::from_secs(3),
                term_length: commonware_utils::NZU64!(3),
            };
            let mut state = State::new(context, cfg);
            state.set_genesis(test_genesis());
