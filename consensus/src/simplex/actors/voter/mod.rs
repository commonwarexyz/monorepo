...
mod tests {
    ...
    /// 1. A stuck validator (view 4) cannot be rescued by notarizations from future views
    /// 2. The only escape route is a finalization certificate (which requires Byzantine cooperation)
    ///
    /// Once the f+1 honest validators certify view 4 and advance to view 5,
    /// they can only vote to nullify view 5 (their current view) without equivocating.
