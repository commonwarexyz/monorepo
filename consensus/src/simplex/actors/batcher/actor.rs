    ...
    T: Strategy,
{
    ...
    fn process_view(
        ...
        loop {
            ...
            let ready = if round.ready_notarizes() {
            ...
            let Some((verified_votes, failed)) = ready else {
            ...
            let batch = verified_votes.len() + failed.len();
    ...
    pub async fn run(
        ...
        select_loop! {
            ...
            Some(message) = self.mailbox_receiver.recv() else break => match message {
                Message::Update {
                    ...
                    let has_forwardable_proposal = self.scheme.me().is_some_and(|me| {
                        work.get(&current.view)
                            .is_some_and(|round| round.has_forwardable_proposal(me))
                    });
            ...
            Ok((sender, message)) = certificate_receiver.recv() else break => {
                ...
                let enables_finalization = matches!(message, Certificate::Notarization(_));
                ...
                // A notarization certificate may unlock already-buffered finalize votes.
                if enables_finalization {
                    dirty_views.insert(view);
                } else {
                    continue;
                }
            ...
            Ok((sender, message)) = vote_receiver.recv() else break => {
                ...
                // Ignore votes at or below the finalized tip. This also means equivocation
                // at or below the tip is not detected or reported: fault evidence is only
                // collected for views still in progress.
                if view <= finalized {
                    continue;
                }
