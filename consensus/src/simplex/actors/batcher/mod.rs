...
mod tests {
    ...
    fn test_reporter<S: Scheme<Sha256Digest>>(
        context: &mut deterministic::Context,
        scheme: &S,
    ...
    /// Builds a batcher [Config] with the standard test defaults, overriding
    /// only the fields in `options`.
    fn test_config<S: Scheme<Sha256Digest>, B: Blocker, Re: crate::Reporter, Rl: Relay>(
        scheme: S,
        blocker: B,
        reporter: Re,
        relay: Rl,
    ...
    fn sign_vote<S: Scheme<Sha256Digest>>(
        ...
        match kind {
            Kind::Notarization => Vote::Notarize(Notarize::sign(scheme, proposal.clone()).unwrap()),
            Kind::Nullification => {
                Vote::Nullify(Nullify::sign::<Sha256Digest>(scheme, round).unwrap())
            }
            Kind::Finalization => Vote::Finalize(Finalize::sign(scheme, proposal.clone()).unwrap()),
        }
    }

    /// A local vote and pending network votes jointly certify each kind. Proposal
    /// certificates establish authority, and later votes cannot reopen certification.
    async fn certify_mixed_votes<S, F>(mut fixture: F)
    where
        S: Scheme<Sha256Digest, PublicKey = PublicKey>,
        F: FnMut(&mut TestRng, &[u8], u32) -> Fixture<S>,
    {
        for kind in [Kind::Notarization, Kind::Nullification, Kind::Finalization] {
            let mut rng = test_rng();
            let Fixture {
                participants,
                schemes,
                ..
            } = fixture(&mut rng, b"batcher_optimistic_recovery", 5);
            ...
            // Keep the local vote in the verified buffer and the remaining quorum
            // in the pending buffer, so construction must use both sources.
            ...
            if kind == Kind::Notarization {
                tracked.set_leader(Participant::from_usize(0));
            }
            assert!(matches!(
                tracked.accept_vote(votes[0].clone(), true),
                Outcome::Added { retained: true }
            ));
            for (i, vote) in votes.iter().enumerate().skip(1) {
                assert!(tracked.add_network(participants[i].clone(), vote.clone()));
            }

            // The certificate authenticates the subject without falling back to
            // individual vote verification. Only pending votes count as batch work.
            ...
            if kind != Kind::Nullification {
                ...
                assert!(!tracked.record_certificate(&certificate));
                assert_eq!(
                    tracked.try_forward_proposal(Participant::from_usize(1)),
                    Some(proposal.clone())
                );
            }

            // A late vote can enter the tracker but cannot reopen certification.
            let late = sign_vote(&schemes[quorum], kind, round_id, &proposal);
        ...
        F: FnMut(&mut TestRng, &[u8], u32) -> Fixture<S>,
    {
        for kind in [Kind::Notarization, Kind::Nullification, Kind::Finalization] {
            ...
            if kind == Kind::Notarization {
                tracked.set_leader(Participant::from_usize(0));
            } else if kind == Kind::Finalization {
                let notarization = build_notarization(&schemes, &proposal, quorum);
                tracked.record_certificate(&Certificate::Notarization(notarization));
            }

            // An extra vote lets fallback retain a full quorum immediately.
            // Otherwise, reserve the remaining votes for replacement attempts.
            let batch = quorum + usize::from(valid_quorum);
            for (i, vote) in votes.iter().take(batch).enumerate() {
                assert!(tracked.add_network(participants[i].clone(), vote.clone()));
            }

            // The invalid leading vote forces fallback even when enough later
            // votes are valid to construct the certificate in the same attempt.
    ...
    async fn test_optimistic_assembly_after_other_kind_fails() {
        ...
        let mut notarizes: Vec<_> = schemes
            ...
            .map(|scheme| Notarize::sign(scheme, proposal.clone()).unwrap())
            .collect();
        notarizes[0].attestation.signature = notarizes[quorum].attestation.signature.clone();

        // Make notarization and nullification ready together. The notarize batch
        // contains one invalid share, while every nullify vote is valid.
        let mut tracked = super::Round::new(
            round_id,
            Arc::new(schemes[0].clone()),
            NoopBlocker,
        ...
        tracked.set_leader(Participant::from_usize(0));
        for i in 0..quorum {
            assert!(tracked.add_network(
                participants[i].clone(),
                Vote::Notarize(notarizes[i].clone())
            ));
            let nullify = Nullify::sign::<Sha256Digest>(&schemes[i], round_id).unwrap();
            assert!(tracked.add_network(participants[i].clone(), Vote::Nullify(nullify)));
        }

        // Notarizes have priority, but their failed attempt cannot consume or
        // complete the independently buffered nullify quorum.
        let notarize = tracked
            .try_construct(&mut rng, &Sequential)
            .await
            .expect("notarize candidate quorum must be processed first");
        assert!(notarize.fallback);
        assert!(notarize.certificate.is_none());
        assert_eq!(notarize.invalid, vec![Participant::from_usize(0)]);
    ...
    async fn vrf_seed_only_corruption<V: Variant>() {
        ...
        // Reuse the same signer's seed signature from another round, preserving
        // the valid vote signature and all structural checks on the attestation.
        ...
        // Certificate verification must reject the seed and fallback must identify
        // its signer, even though every vote component authenticates the proposal.
        let Batch {
            invalid, fallback, ..
        } = tracked
            .try_construct(&mut rng, &Sequential)
    ...
    /// A quorum of locally constructed votes can establish proposal authority
    /// before a leader update arrives.
        ...
        F: FnMut(&mut TestRng, &[u8], u32) -> Fixture<S>,
    {
        ...
        // A single local vote is a quorum in this fixture. It is already trusted,
        // but the round cannot forward its proposal until authority is established.
        round.accept_vote(
        ...
        // Construction uses the verified quorum without waiting for the leader,
        // then records the certificate so the proposal becomes forwardable.
        let result = round
            .try_construct(&mut rng, &Sequential)
            .await
    ...
    async fn test_constructed_leader_notarize_is_not_reverified() {
        ...
        // One more unique signer completes the quorum. The trusted local vote
        // contributes to construction without entering the pending batch.
    ...
    /// quorum and a notarize quorum are already buffered; a single pass must
    /// verify every ready vote kind, yielding the proposal, a notarization,
    /// and a finalization together.
    #[test_traced]
    fn test_leader_proposal_verifies_all_ready_vote_kinds() {
        let n = 5;
        let namespace = b"batcher_test";
        ...
        executor.start(|mut context| async move {
            ...
            let mut participant_senders = Vec::with_capacity(n as usize);
            ...
            let leader = usize::from(leader);
            let notarize = Notarize::sign(&schemes[leader], proposal).unwrap();
            participant_senders[leader]
                .as_mut()
                .unwrap()
                .send(
                    Recipients::One(me.clone()),
            ...
            let mut received_proposal = false;
            let mut received_notarization = false;
            ...
            for _ in 0..3 {
                ...
                match message {
                    voter::Message::Proposal { .. } => received_proposal = true,
                    voter::Message::Verified {
                        certificate: Certificate::Notarization(_),
                        ..
                    } => received_notarization = true,
                    voter::Message::Verified {
                        certificate: Certificate::Finalization(_),
                        ..
                    } => received_finalization = true,
                    _ => panic!("unexpected voter message"),
                }
            }
    ...
    fn test_finalization_construction_filters_overridden_proposal() {
        ...
        executor.start(|_context| async move {
            ...
            round.record_certificate(&Certificate::Notarization(notarization_b));
            round.accept_vote(
                Vote::Finalize(Finalize::sign(&schemes[quorum - 1], proposal_b.clone()).unwrap()),
                true,
            );
            assert!(
    ...
    fn test_rejected_finalize_still_detects_conflicting_finalize() {
        ...
        executor.start(|mut context| async move {
            ...
            // The first finalize is reserved in the tracker even though the
            // verifier filters it (it references the displaced proposal).
            ...
            assert!(
                ...
                "a single filtered finalize is not conflicting evidence"
            );
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            // Initialize batcher
            let view = View::new(1);
            batcher_mailbox.update(Span::none(), view, Participant::new(0), View::zero(), None);
            ...
            assert!(
                matches!(output, voter::Message::Verified { certificate: Certificate::Notarization(n), .. } if n.view() == view)
            );
            ...
            let output = voter_receiver.recv().await.unwrap();
            assert!(
                matches!(output, voter::Message::Verified { certificate: Certificate::Nullification(n), .. } if n.view() == view)
            );
            ...
            let output = voter_receiver.recv().await.unwrap();
            assert!(
                matches!(output, voter::Message::Verified { certificate: Certificate::Finalization(f), .. } if f.view() == view)
            );
        });
    }
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            batcher_mailbox
                .update(Span::none(), target_view, Participant::new(0), View::zero(), None);
            ...
            let output = voter_receiver.recv().await.unwrap();
            assert!(
                matches!(output, voter::Message::Verified { certificate: Certificate::Nullification(n), .. } if n.view() == target_view)
            );

            // Simulate voter-driven view advance after nullification to V+1.
            batcher_mailbox
                .update(Span::none(), target_view.next(), Participant::new(1), View::zero(), None);
            ...
            let output = voter_receiver.recv().await.unwrap();
            assert!(
                matches!(output, voter::Message::Verified { certificate: Certificate::Notarization(n), .. } if n.view() == target_view)
            );
        });
    }
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            // The certificate-established proposal is forwarded even though
            // no leader is known for the round.
            let output = voter_receiver.recv().await.unwrap();
            assert!(
                matches!(&output, voter::Message::Proposal { proposal, .. } if proposal == &future_proposal)
            );
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            // Vote processing and certificate recovery are attributed to the
            // view span adopted from the update.
            traces
                .get_by_level(Level::TRACE)
                .expect_event(|event| {
                    event.metadata.content == "processed votes"
                        && event
    ...
    /// The actor must attribute an invalid threshold share after optimistic
    /// recovery fails, count the fallback, and still forward the certificate
    /// once a valid replacement completes the quorum.
    #[test_traced]
    fn test_actor_optimistic_recovery_fallback() {
        let executor = deterministic::Runner::timed(Duration::from_secs(10));
        executor.start(|mut context| async move {
            let n = 5;
            let quorum = quorum(n) as usize;
            ...
            let Fixture {
                ...
                &mut context,
                b"batcher_actor_optimistic_fallback",
                n,
            );

            // Observe peer blocking, voter deliveries, and attempt metrics through
            // the actor's normal network and mailbox inputs.
            let oracle =
                start_test_network_with_peers(context.child("network"), participants.clone()).await;
            let reporter = test_reporter(&mut context, &schemes[0]);
            let blocked = Arc::new(Mutex::new(Vec::new()));
            let blocker = RecordingBlocker(blocked.clone());
            let me = participants[0].clone();
            ...
            let (voter_sender, mut voter_receiver) =
                mailbox::new::<
                    voter::Message<bls12381_threshold_vrf::Scheme<PublicKey, MinPk>, Sha256Digest>,
                >(context.child("mailbox"), NZUsize!(32));
            let voter_mailbox = voter::Mailbox::new(voter_sender);
            let (_vote_sender, vote_receiver) = oracle
                .control(me.clone())
                .register(0, TEST_QUOTA)
            ...
            // Corrupt only the leader's seed component while preserving the
            // valid vote signature and in-range signer index.
            let mut leader_vote = Notarize::sign(&schemes[1], proposal.clone()).unwrap();
            let other = Notarize::sign(
                &schemes[1],
                Proposal::new(
            ...
            participant_senders[1].as_mut().unwrap().send(
                Recipients::One(me.clone()),
                Vote::Notarize(leader_vote).encode(),
                true,
            );
            for i in 2..quorum {
                let vote = Notarize::sign(&schemes[i], proposal.clone()).unwrap();
                participant_senders[i].as_mut().unwrap().send(
                    Recipients::One(me.clone()),
                    Vote::Notarize(vote).encode(),
                    true,
                );
            }
            batcher_mailbox.constructed(Vote::Notarize(
                Notarize::sign(&schemes[0], proposal.clone()).unwrap(),
            ));
            context.sleep(Duration::from_millis(50)).await;

            // The attempt must report its invalid signer and metrics while the
            // view remains unfinished, without waiting for finalization.
            ...
            // reached the voter yet (the proposal itself is expected).
            loop {
                select! {
                    message = voter_receiver.recv() => match message {
                        Some(voter::Message::Proposal { .. }) => {}
                        Some(voter::Message::Verified { .. }) => {
                            panic!("invalid candidate quorum produced a certificate")
                        }
                        Some(_) => {}
                        None => panic!("voter receiver closed"),
                    },
            ...
            // A fresh signer completes the retained quorum in a second attempt,
            // which must deliver the certificate without another fallback.
            ...
            loop {
                let message = select! {
                    ...
                    _ = context.sleep(Duration::from_millis(100)) => {
                        panic!("timed out waiting for recovered certificate")
                    },
                };
                if let voter::Message::Verified {
                    certificate: Certificate::Notarization(notarization),
                    ..
                } = message
                {
            ...
            // Late traffic for the finalized view is still reported. Waiting for
            // that report ensures it has been processed before checking the metrics.
            ...
            // Repeated finalization and pruning must not record completed work again.
            // Each reported local vote confirms that the preceding update was processed.
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            loop {
                ...
                if matches!(
                    output,
                    voter::Message::Verified { certificate: Certificate::Notarization(n), .. } if n.view() == view
                ) {
                    saw_notarization = true;
                    break;
                }
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            loop {
                ...
                match output {
                    voter::Message::Proposal { proposal: p, .. } => {
            ...
            batcher_mailbox.update(Span::none(), view3, leader3, View::zero(), Some(proposal_a.clone()));
            context.sleep(Duration::from_millis(50)).await;
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            let output = voter_receiver.recv().await.unwrap();
            assert!(matches!(&output, voter::Message::Proposal { proposal: p, .. } if p.view() == view));
    ...
    /// 3. Local leader inactivity is suppressed.
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            // Test 1: Before skip_timeout elapses, leaders should stay active.
            let leader = Participant::new(1);
            ...
            // Seed quorum activity from peers only. If local-leader suppression
            // were removed, the next update would return Inactivity.
            for (i, mut sender) in peer_senders {
                let vote =
                    Nullify::sign::<Sha256Digest>(&schemes[i], Round::new(epoch, View::new(99)))
                        .unwrap();
                sender
                    .send(
                        Recipients::One(me.clone()),
                        Vote::<S, Sha256Digest>::Nullify(vote).encode(),
                        true,
                    );
            }
            context.sleep(Duration::from_millis(50)).await;

            // Test 4: local leader inactivity should not trigger a fast-timeout hint.
            let self_leader = Participant::new(0);
            let view = View::new(101);
            batcher_mailbox.update(Span::none(), view, self_leader, View::zero(), None);
            expect_no_timeout(&mut context, &mut voter_receiver).await;
        });
    }
    ...
    /// Test that a stale leader is reported inactive once the rest of the network is active.
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            let self_vote = Nullify::sign::<Sha256Digest>(&schemes[0], Round::new(epoch, view))
                .expect("self nullify");
            batcher_mailbox.constructed(Vote::<S, Sha256Digest>::Nullify(self_vote));

            for (i, mut sender) in participant_senders {
    ...
    #[test_traced]
    fn test_leader_inactivity_reported_after_quorum_activity() {
        leader_inactivity_reported_after_quorum_activity(
            bls12381_threshold_vrf::fixture::<MinPk, _>,
        );
        leader_inactivity_reported_after_quorum_activity(
            bls12381_threshold_vrf::fixture::<MinSig, _>,
        ...
        leader_inactivity_reported_after_quorum_activity(bls12381_multisig::fixture::<MinPk, _>);
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            // Seed quorum activity without the leader. If the leader nullify below
            // is not recorded as activity, the next update will return Inactivity.
            let self_vote = Nullify::sign::<Sha256Digest>(
                &schemes[0],
                Round::new(epoch, View::new(skip_timeout - 1)),
            )
            .unwrap();
            batcher_mailbox.constructed(Vote::<S, Sha256Digest>::Nullify(self_vote));
            for (i, mut sender) in participant_senders {
                let vote = Nullify::sign::<Sha256Digest>(
                    &schemes[i],
                    Round::new(epoch, View::new(skip_timeout)),
                )
                .unwrap();
                sender
                    .send(
            ...
            // Send a nullify vote from the leader for a prior view. This records
            // activity without triggering the current-view leader-nullify fast path.
            let round = Round::new(epoch, View::new(skip_timeout - 1));
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            batcher_mailbox.update(Span::none(), active_view, leader, View::zero(), None);
            expect_no_timeout(&mut context, &mut voter_receiver).await;

            // Seed quorum activity without the leader. If the leader certificate below
            // is not recorded as activity, the next update will return Inactivity.
            let self_vote = Nullify::sign::<Sha256Digest>(
                &schemes[0],
                Round::new(epoch, active_view.previous().unwrap()),
            )
            .unwrap();
            batcher_mailbox.constructed(Vote::<S, Sha256Digest>::Nullify(self_vote));
            for (i, mut sender) in participant_senders {
                let vote =
                    Nullify::sign::<Sha256Digest>(&schemes[i], Round::new(epoch, active_view))
                        .unwrap();
                sender
                    .send(
                        Recipients::One(me.clone()),
            ...
            assert!(matches!(
                ...
                voter::Message::Verified { certificate: Certificate::Finalization(f), .. } if f.view() == active_view
            ));
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            // Enter view 1 first.
            batcher_mailbox.update(Span::none(), View::new(1), Participant::new(1), View::zero(), None);
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            batcher_mailbox.update(Span::none(), view1, leader, View::zero(), None);
            ...
            loop {
                let output = voter_receiver.recv().await.unwrap();
                match output {
                    voter::Message::Proposal { .. } => continue,
                    voter::Message::Verified { certificate: Certificate::Notarization(n), .. } => {
                        assert_eq!(
                            n.view(),
                            view1,
            ...
            let view3 = View::new(3);
            batcher_mailbox.update(Span::none(), view3, leader, view2, None);
        ...
        F: FnMut(&mut deterministic::Context, &[u8], u32) -> Fixture<S>,
    {
        ...
        executor.start(|mut context| async move {
            ...
            // Initialize batcher actor (participant 0) with a restored
            // finalized floor, simulating a restart before the voter has
            // replayed its journal and published the first update.
            let me = participants[0].clone();
            let floor = View::new(10);
            let batcher_cfg = test_config(
                schemes[0].clone(),
                oracle.control(me.clone()),
                reporter.clone(),
            ...
            // Start the batcher without sending an initial update
            batcher.start(voter_mailbox, vote_receiver, certificate_receiver);

            // Send a stale view-1 vote before the first update
            let stale_view = View::new(1);
            ...
            // Publish the restored view and send a live vote to confirm the
            ...
            while reporter
                ...
                .lock()
                .get(&current)
                .and_then(|payloads| payloads.get(&live_proposal.payload))
