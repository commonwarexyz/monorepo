...
impl<C: Scheme, D: Array> Prover<C, D> {
    ...
    fn deserialize_proposal(
        ...
        let public_key = C::PublicKey::read(&mut proof).ok()?;
        let signature = C::Signature::read(&mut proof).ok()?;
    ...
    fn deserialize_conflicting_proposal(
        ...
        let signature_1 = C::Signature::read(&mut proof).ok()?;
        let parent_2 = proof.get_u64();
        let payload_2 = D::read(&mut proof).ok()?;
        let signature_2 = C::Signature::read(&mut proof).ok()?;
    ...
    pub fn deserialize_nullify_finalize(
        ...
        let parent = proof.get_u64();
        let payload = D::read(&mut proof).ok()?;
        let signature_finalize = C::Signature::read(&mut proof).ok()?;
        let signature_null = C::Signature::read(&mut proof).ok()?;
