...
//! # Usage
//!
//! The standard variant uses the core [`super::core::Actor`] and
//! [`super::core::Mailbox`] with [`Standard`] as the variant type parameter.
...
mod tests {
    ...
    use crate::{
        ...
        simplex::{
            ...
            marshal::{
                ...
                config::{Config, Start},
                core::{
                    Actor, CommitmentFallback, DigestFallback, Mailbox, cache,
    ...
    fn test_standard_restart_repairs_trailing_missing_finalized_block() {
        ...
        runner.start(|mut context| async move {
            ...
            let recovering = StandardHarness::setup_validator_with(
                ...
                ConstantProvider::new(schemes[0].clone()),
                NZUsize!(1),
                crate::simplex::marshal::mocks::application::Application::manual_ack(),
            )
            .await;
    ...
    impl crate::simplex::marshal::core::Buffer<Standard<B>> for RecordingBuffer {
        type PublicKey = PublicKey;
        ...
        fn retire(&self, _update: crate::simplex::marshal::core::Retirement<D>) {}
    ...
    where
        ...
        Buf: crate::simplex::marshal::core::Buffer<Standard<B>, PublicKey = PublicKey> + Clone,
    {
        let config = Config {
            provider,
    ...
    impl<T: crate::simplex::marshal::store::Blocks> crate::simplex::marshal::store::Blocks
        for PacedStore<T>
    {
        type Block = T::Block;
        type Error = T::Error;
    ...
    impl<T: crate::simplex::marshal::store::Certificates>
        crate::simplex::marshal::store::Certificates for PacedStore<T>
    {
    ...
    fn test_standard_cache_writes_do_not_clone_block_payloads() {
        type CountedBlock = crate::simplex::marshal::mocks::block::Block<D, CloneCounter>;
    ...
    /// Regression test for the [`crate::simplex::marshal::Update::Block`] pruning
    /// contract.
    ///
