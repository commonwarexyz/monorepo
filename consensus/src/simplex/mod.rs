...
mod tests {
    ...
    fn test_children_shutdown_on_engine_abort() {
        ...
        executor.start(|context| async move {
            ...
            // Create engine
            let relay = Arc::new(mocks::relay::Relay::new());
            let view_validators = BTreeMap::from_iter(vec![(0, vec![validator.clone()])]);
            let supervisor_config = mocks::supervisor::Config {
                namespace: namespace.clone(),
                participants: view_validators,
            };
            let supervisor =
                mocks::supervisor::Supervisor::<PublicKey, Sha256Digest>::new(supervisor_config);
            let application_cfg = mocks::application::Config {
            ...
            let (voter, resolver) = registrations
                .remove(&validator)
                .expect("validator should be registered");
            let handle = engine.start(voter, resolver);
