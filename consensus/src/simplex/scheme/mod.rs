...
//! execution in that same round. Each round's seed partial signs only the round and is carried by
//! every vote type, so the `f` Byzantine participants of a `3f+1` committee can recover the seed
//! once any `f+1` honest participants have voted (even for conflicting proposals), before the
//! round resolves and without any certificate forming. A malicious leader can use this early
