...
struct Harness {
    state: State<Scheme, Digest, crate::elector::RoundRobinElector<Scheme>>,
    schemes: Vec<Scheme>,
    verifier: Scheme,
    actions: ActionQueue,
...
impl Harness {
    fn new(n: u32) -> Self {
        ...
        let elector = crate::elector::RoundRobin::<commonware_cryptography::Sha256>::default()
...
mod engine_tests {
    ...
    async fn link_validators(
        ...
        for (i1, v1) in validators.iter().enumerate() {
            for (i2, v2) in validators.iter().enumerate() {
                ...
                match action {
                    LinkAction::Update(_) | LinkAction::Unlink => {
                ...
                match action {
                    LinkAction::Link(ref link) | LinkAction::Update(ref link) => {
                        oracle
    ...
    /// Helper to calculate minimmit quorum (M-quorum = 2f+1 for n >= 5f+1)
    #[allow(dead_code)]
    fn quorum(n: u32) -> u32 {
        M5f1::quorum(n)
    }
