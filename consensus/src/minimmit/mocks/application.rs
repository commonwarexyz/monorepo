//! Mock application used by `minimmit` tests to produce and verify payloads,
//! simulating proposal/verification latency and broadcasting via a mock relay.

use super::relay::Relay;
use crate::{minimmit::types::Context, types::Epoch, Automaton as Au, Relay as Re};
use bytes::Bytes;
...
/// Mock application actor for testing minimmit consensus.
...
impl<E: Clock + RngCore + Spawner, H: Hasher, P: PublicKey> Application<E, H, P> {
    ...
    pub fn new(context: E, cfg: Config<H, P>) -> (Self, Mailbox<H::Digest, P>) {
        ...
        // Generate samplers
        let propose_latency = Normal::new(cfg.propose_latency.0, cfg.propose_latency.1)
            .expect("invalid propose latency");
        let verify_latency = Normal::new(cfg.verify_latency.0, cfg.verify_latency.1)
            .expect("invalid verify latency");
    ...
    async fn verify(
        ...
        let (parsed_round, parent, _) =
            <(crate::types::Round, H::Digest, u64)>::decode(&mut contents)
