...
impl<S: Scheme, D: Digest> ConflictingNotarize<S, D> {
    ...
    ///
    /// Returns `None` if the notarizes are not from the same signer/view, or if
    /// the proposals are identical (only different proposals prove equivocation).
    ...
    ///
    /// Returns `false` if the proposals are identical (not true equivocation).
    pub fn verify<R>(&self, rng: &mut R, scheme: &S, strategy: &impl Strategy) -> bool
    where
        R: CryptoRngCore,
        S: scheme::Scheme<D>,
    {
        self.first.proposal != self.second.proposal
            && self.first.verify(rng, scheme, strategy)
            && self.second.verify(rng, scheme, strategy)
    }
}
...
mod tests {
    ...
    /// Regression: `ConflictingNotarize::new` must reject two identical votes for the
    /// same proposal. Accepting them would let an attacker frame an honest validator
    /// for equivocation using a single legitimate vote.
    #[test]
    fn conflicting_notarize_rejects_identical_proposals() {
        let schemes = setup_fixture();
        ...
        let vote = Notarize::sign(&schemes[0], proposal).unwrap();

        let evidence = ConflictingNotarize::new(vote.clone(), vote);
        assert!(
            evidence.is_none(),
            "ConflictingNotarize::new must reject two votes for the same proposal"
        );
    }

    /// Regression: `ConflictingNotarize::read_cfg` must reject deserialized evidence
    /// where both votes are for the same proposal. Without this check, an attacker
    /// can craft a wire message that frames an honest validator.
    #[test]
    fn conflicting_notarize_read_rejects_identical_proposals() {
        ...
        // Manually construct the wire bytes: two identical notarize votes
        let evidence = ConflictingNotarize {
            first: vote.clone(),
            second: vote,
        };
        let encoded = evidence.encode();

        let result = ConflictingNotarize::<Scheme, Sha256>::decode(encoded);
        assert!(
            result.is_err(),
            "ConflictingNotarize::read_cfg must reject identical proposals on the wire"
        );
    }

    /// Regression: `ConflictingNotarize::verify` must reject evidence where both
    /// votes are for the same proposal, even if signatures are valid.
    #[test]
    fn conflicting_notarize_verify_rejects_identical_proposals() {
        let schemes = setup_fixture();
        let mut rng = test_rng();
        let round = Round::new(Epoch::new(1), View::new(1));
        let proposal = Proposal::new(
        ...
        // Manually construct evidence with identical proposals
        let evidence = ConflictingNotarize {
            first: vote.clone(),
            second: vote,
        };
        ...
        assert!(
            !evidence.verify(&mut rng, &schemes[0], &Sequential),
            "verify must reject evidence with identical proposals"
        );
    }
