//! Resolver actor implementation for minimmit consensus.
//!
//! The resolver fetches missing certificates from peers when the voter
//! needs them to advance views. Unlike simplex, minimmit has no certification
//! phase, so the resolver only tracks notarizations and nullifications.
    ...
    > Actor<E, S, B, D, T>
{
    ...
    /// Starts the resolver actor event loop.
    pub fn start(
        mut self,
    ...
    /// Core event loop that handles certificate fetching and distribution.
    async fn run(
        mut self,
            ...
            mailbox = self.mailbox_receiver.next() => {
                ...
                match message {
                    MailboxMessage::Updated(certificate) => {
                        // Certificate from voter - update state
    ...
    /// Validates an incoming message, returning the parsed certificate if valid.
