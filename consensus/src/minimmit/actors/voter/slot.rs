//! Proposal slot tracking for minimmit consensus.
//!
//! Tracks proposal state, build/verify flags, and conflicts within a round.

use crate::minimmit::types::Proposal;
    ...
    D: Digest + Clone + PartialEq,
{
    ...
    pub fn built(&mut self, proposal: Proposal<D>) {
        if let Some(existing) = &self.proposal {
            ...
            // we would ignore this case but it is required to support Twins testing.
            debug!(
                ?existing,
                ?proposal,
