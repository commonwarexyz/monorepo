    ...
    async fn execute_action<V: Sender, C: Sender>(
        ...
        match action {
            ...
            Action::Finalized(finalization) => {
                ...
                info!(%view, "finalized");
                // Persist finalization before any side effects so crash recovery restores
                // `last_finalized` and rejects stale/conflicting artifacts for this view.
                self.append_journal(view, Artifact::Finalization(finalization.clone()))
                    .await;
                self.sync_journal(view).await;
                // Only the leader sees an unbiased latency sample, so record it now.
                if let Some(elapsed) = self.leader_elapsed(view) {
...
#[cfg(test)]
mod tests {
    use super::{batcher, resolver, ActionContext, Actor, Config, Resolved};
    use crate::{
        elector::{Config as _, RoundRobin},
        minimmit::{
            actors::voter::egress::Egress,
            mocks::{application, reporter},
            scheme::ed25519,
            state::{Action, State},
            types::{Artifact, Certificate, Finalization, MNotarization, Notarize, Proposal},
        },
        mocks::relay,
    ...
    #[test]
    fn regression_finalization_is_journaled() {
        let executor = deterministic::Runner::default();
        executor.start(|context: deterministic::Context| async move {
            let mut rng = test_rng();
            let Fixture { schemes, .. } =
                ed25519::fixture(&mut rng, b"minimmit-finalization-journal", 6);
            let scheme = schemes[0].clone();
            let participants = scheme.participants().clone();
            let me = participants
                .get(Participant::new(0).into())
            ...
            let partition = "voter_finalization_journal".to_string();
            let cfg = Config {
                scheme: scheme.clone(),
                elector: elector.clone(),
                blocker: NoopBlocker,
            ...
            let (mut actor, _) = Actor::new(context.with_label("voter"), cfg);
            let journal = Journal::<_, Artifact<ed25519::Scheme, Sha256Digest>>::init(
                context.with_label("journal"),
                JConfig {
                    partition,
                    compression: None,
            ...
            .await
            .expect("journal init");
            actor.journal = Some(journal);

            let mut state = State::new(
                Epoch::new(1),
                scheme.clone(),
                elector.build(scheme.participants()),
                Sha256Digest::from([0u8; 32]),
            );
            ...
            let notarizes: Vec<_> = schemes
                ...
                .map(|scheme| Notarize::sign(scheme, proposal.clone()).expect("notarize"))
                .collect();
            let finalization = Finalization::from_notarizes(&scheme, notarizes.iter(), &Sequential)
                .expect("finalization quorum");

            let (batcher_sender, _batcher_receiver) = mpsc::channel(8);
            let mut batcher = batcher::Mailbox::new(batcher_sender);
            let (resolver_sender, _resolver_receiver) = mpsc::channel(8);
            let mut resolver = resolver::Mailbox::new(resolver_sender);
            let vote_sender = TestSender::<Ed25519PublicKey>::default();
            let certificate_sender = TestSender::<Ed25519PublicKey>::default();
            let mut egress = Egress::new(
                vote_sender,
                certificate_sender,
                actor.outbound_messages.clone(),
            );
            let mut timeout_deadline = context.current() + Duration::from_secs(1);
            ...
            actor
                .execute_action(Action::Finalized(finalization.clone()), &mut action_context)
                .await;

            let journal = actor.journal.as_mut().expect("journal must be initialized");
            journal.sync_all().await.expect("sync");
            let stream = journal
            ...
            while let Some(entry) = stream.next().await {
                let (_, _, _, artifact) = entry.expect("artifact entry");
                if let Artifact::Finalization(found) = artifact {
                    if found.proposal == finalization.proposal {
            ...
            assert!(
                saw_finalization,
                "finalization action must persist a finalization artifact for crash recovery"
            );
        });
    }
}
