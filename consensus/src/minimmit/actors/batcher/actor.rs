    ...
    T: Strategy,
{
    ...
    async fn handle_certificate(
        ...
        work: &mut BTreeMap<View, Round<S, B, D, R>>,
        sender: S::PublicKey,
        message: Certificate<S, D>,
        finalized: View,
        current: View,
    ) {
        ...
        // Do not allocate round state before verification.
        let already_have = work.get(&view).is_some_and(|round| match &message {
...
mod tests {
    ...
    #[test]
    fn regression_invalid_far_future_certificate_does_not_allocate_round() {
        ...
        executor.start(|context: deterministic::Context| async move {
            ...
            let (mut actor, _mailbox) = Actor::new(context.with_label("batcher"), cfg);

            let current = View::new(10);
            let finalized = View::new(9);
            let mut work = BTreeMap::new();
            work.entry(current).or_insert_with(|| actor.new_round());
            let (voter_tx, _voter_rx) = mpsc::channel(8);
            let mut voter = voter::Mailbox::new(voter_tx);
            let sender = actor
                .participants
                .key(Participant::new(0))
                .expect("participant key exists")
                .clone();

            let forged_view = View::new(1_000_000);
            let signed_round = Rnd::new(Epoch::new(1), forged_view);
            let nullifies: Vec<_> = schemes
                .iter()
                .take(3)
                .map(|scheme| Nullify::sign::<Sha256Digest>(scheme, signed_round).unwrap())
                .collect();
            let mut nullification = crate::minimmit::types::Nullification::from_nullifies(
                &schemes[0],
                nullifies.iter(),
                &Sequential,
            )
            .expect("build certificate");
            // Corrupt after signing so decoding succeeds but verification fails.
            nullification.round = Rnd::new(Epoch::new(1), View::new(1_000_001));
            let message = Certificate::Nullification(nullification);

            let before = work.len();
            actor
                .handle_certificate(&mut voter, &mut work, sender, message, finalized, current)
                .await;
            let after = work.len();

            assert_eq!(
                before, after,
                "invalid far-future certificates must not allocate per-view round state"
            );
        });
    }
}
