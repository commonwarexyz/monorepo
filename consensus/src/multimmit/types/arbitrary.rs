...
use commonware_cryptography::{
    Digest, Digestible, Hasher, Sha256,
    bls12381::{
...
where
    ...
    B: Codec + Digestible<Digest = H::Digest> + Arbitrary<'a>,
{
    fn arbitrary(u: &mut Unstructured<'a>) -> ArbitraryResult<Self> {
        let epoch = u.arbitrary()?;
        let chain = ChainId::new(u.int_in_range(0..=(PARTICIPANTS - 1) as u32)?);
...
mod conformance {
    ...
    use commonware_codec::{
        Decode, Encode, EncodeSize, Error as CodecError, Read, ReadExt, Write,
    ...
    struct ConformanceBody(u64);

    impl<'a> Arbitrary<'a> for ConformanceBody {
        fn arbitrary(u: &mut Unstructured<'a>) -> ArbitraryResult<Self> {
            Ok(Self(u.arbitrary()?))
        }
    }

    impl Write for ConformanceBody {
        fn write(&self, buf: &mut impl BufMut) {
            self.0.write(buf);
        }
    }

    impl Read for ConformanceBody {
        type Cfg = ();
    ...
    impl EncodeSize for ConformanceBody {
        fn encode_size(&self) -> usize {
            self.0.encode_size()
        }
    }

    impl Digestible for ConformanceBody {
    ...
    fn generated_values_round_trip() {
        ...
        assert_round_trip::<TransactionBlock<Sha256, ConformanceBody>>((), 128);
    ...
    commonware_conformance::conformance_tests! {
        ...
        CodecConformance<TransactionBlock<Sha256, ConformanceBody>> => 128,
