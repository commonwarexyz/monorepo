...
impl<D: Digest> SelectedCommitments<D> {
    /// Each path must contain at least two references whose adjacent parent edges have been
    /// authenticated in `epoch`.
    ...
    pub(crate) fn paths(&self) -> &[Arc<[BlockRef<D>]>] {
        &self.paths
    }

    pub(crate) fn owned_bytes(&self) -> Option<usize> {
        let allocation = 2 * core::mem::size_of::<usize>();
        let mut bytes = allocation.checked_add(core::mem::size_of_val(self.paths.as_ref()))?;
