...
use super::{BlockRef, CertificateId, ChainId, MisplacedChain, ensure_canonical_chains};
use crate::{
    Epochable,
...
impl<D: Digest> Read for EpochGenesis<D> {
    ...
    fn read_cfg(buf: &mut impl Buf, cfg: &Self::Cfg) -> Result<Self, CodecError> {
        ...
        Self::new(epoch, leader, vqc, lqc, tips)
            .map_err(|error| CodecError::Wrapped("EpochGenesis", error.into()))
    }
}
...
fn validate_tips<D: Digest>(tips: &[BlockRef<D>]) -> Result<(), GenesisError> {
    ...
    ensure_canonical_chains(tips.iter().map(BlockRef::chain)).map_err(
        |MisplacedChain { index, actual }| GenesisError::UnexpectedChain {
            index,
            expected: ChainId::new(index as u32),
            actual,
        },
    )?;
    if let Some(tip) = tips.iter().find(|tip| !tip.height().is_zero()) {
...
mod tests {
    use super::*;
    use crate::multimmit::types::PathLimits;
    ...
    fn codec_round_trips_with_exact_chain_bound() {
        ...
        let codec = CodecConfig::new(3, 3, PathLimits::new(4, 0).unwrap()).unwrap();
        assert_eq!(
            EpochGenesis::decode_cfg(genesis.encode(), &codec).unwrap(),
            genesis
        );

        let wrong_bound = CodecConfig::new(2, 2, PathLimits::new(4, 0).unwrap()).unwrap();
