...
/// header binds the chain coordinates, canonical parent identity, and an opaque commitment to that
...
impl<D: Digest> TransactionBlockHeader<D> {
    ...
    /// Returns the canonical application block digest.
    pub const fn commitment(&self) -> D {
        self.commitment
    }
