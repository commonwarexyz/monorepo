...
impl<D: Digest> VoteBody<D> {
    ...
    pub fn for_leader<H: Hasher<Digest = D>, V: Variant>(
        ...
        Self::for_leader_digest(leader, leader.digest::<H>(), positions, extensions, limits)
    }

    pub(crate) fn for_leader_digest<V: Variant>(
        leader: &LeaderBlock<V, D>,
        leader_digest: D,
        positions: Vec<Position>,
        extensions: Vec<Extension<D>>,
        ...
        let body = Self::new(leader.round(), leader_digest, positions, extensions, limits)?;
