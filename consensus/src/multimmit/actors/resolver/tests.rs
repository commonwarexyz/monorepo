...
use commonware_runtime::{Clock as _, IoBufs, Runner as _, Supervisor as _, deterministic::Runner};
...
fn resolver_responses_are_prioritized() {
    Runner::timed(Duration::from_secs(10)).start(|context| async move {
        ...
        let (actor, _endpoints): (Actor<_, Sha256, _, MinPk, _, _>, _) = Actor::new(
            ...
            Config {
                peers: vec![local.clone(), peer.clone()],
                me: Some(local.clone()),
        ...
        let (voter, _voter_receiver) =
            mailbox::new(context.child("voter"), NonZeroUsize::new(8).unwrap());
        let task = actor.start(voter, (local_sender, local_receiver));
