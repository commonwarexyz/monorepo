...
#[derive(Clone)]
struct RejectOnlyBlocker<B>(B);

impl<B: Blocker> Blocker for RejectOnlyBlocker<B> {
    ...
    D: Digest,
{
    ...
    let completion = strategy
        .manual()
        .spawn(1, move |_| worker.in_scope(operation))
        .await;
    (span, completion)
}
    ...
    async fn run(
        ...
        let (engine, mut resolver) = p2p::Engine::new(
            ...
            p2p::Config {
                ...
                blocker: RejectOnlyBlocker(
                    self.blocker.take().expect("resolver blocker is installed"),
                ),
