...
type VerifyResult<V, D> = (
    Span,
    Result<VerificationCompletion<V, D>, VerificationTaskPanicked>,
);
                ...
                received = self.receiver.recv() => {
                    ...
                    let operation = async move {
                        ...
                        strategy.spawn(bytes.len(), move |_| {
                            let prepared = Self::prepare(plane, bytes, &config, received_at);
                            (peer, prepared)
                        }).await
                    };
    ...
    C: Strategy,
{
    ...
    ///
    /// The completion carries the caller's span so the voter can preserve trace causality.
    ...
    async fn run(
        ...
        let mut jobs: VerifyResults<V, H::Digest> = Pool::default();
        ...
        select_loop! {
            ...
            Some(message) = self.mailbox.recv() else break => {
                match message {
                    ...
                    Message::Block { peers } => {
                        for peer in peers {
                            self.block(peer, "authenticated equivocation");
                        }
                    }
    ...
    fn apply_ready_ingress(
        ...
        for _ in 0..capacity {
            ...
            *next = plane.next();
            ...
            if let Ok((lane, group)) = prepared {
                self.buffer(lanes, lane, peer, group);
            }
        }
        Ok(())
    }
    ...
    /// Counts and blocks one peer for an objective protocol fault.
    ...
    fn deliver(
        &mut self,
        completions: &mailbox::Sender<Completed<V, H::Digest>>,
        completion: VerifyResult<V, H::Digest>,
    ) -> bool {
...
mod tests {
    ...
    fn ready_ingress_rotates_after_invalid_results_and_stops_at_budget() {
        deterministic::Runner::default().start(|context| async move {
            ...
            assert!(actor.blocker.blocked().is_empty());
