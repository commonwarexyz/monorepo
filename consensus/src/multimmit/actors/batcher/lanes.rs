...
impl<P: PublicKey, V: Variant, D: Digest> Lanes<P, V, D> {
    ...
    pub(super) fn flush(&mut self, budget: usize) -> Vec<Selected<P, V, D>> {
        ...
        let budget = match plane {
            Plane::Consensus | Plane::Certificate => budget.min(VIEW_COHORT_ITEMS),
            Plane::Data => budget,
        };
...
mod tests {
    ...
    fn flush_selects_a_single_plane_per_cohort() {
        ...
        let novote = Artifact::NoVote(committee.novote(2, 1));
        let nullification = Artifact::Nullification(committee.nullification(1));
        let block = Artifact::TransactionBlock(committee.signed_block(0, Sha256::hash(&[b"pp"])));
        ...
        assert_eq!(lanes.flush(16), vec![novote]);
        assert_eq!(lanes.flush(16), vec![nullification]);
        assert_eq!(lanes.flush(16), vec![block]);
        assert!(lanes.flush(16).is_empty());
    }
    ...
    fn view_plane_cohorts_are_capped() {
        ...
        for signer in 0..6usize {
            let novote = Artifact::NoVote(committee.novote(signer, 1));
            lanes
                .push_group(
                    LaneId::Consensus,
                    peer(signer as u8),
        ...
        // next rotation.
        assert_eq!(lanes.flush(16).len(), VIEW_COHORT_ITEMS);
