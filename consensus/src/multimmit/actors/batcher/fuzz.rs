    ...
    for chunk in input.as_chunks::<3>().0 {
        ...
        match op % 3 {
            0 => {
                ...
                if lanes.push_group(lane, peer, Group::one(artifact, SystemTime::UNIX_EPOCH)).is_ok() {
                    buffered += 1;
                }
            }
            1 => {
                ...
                if lanes
                    .push_group(LaneId::Consensus, peer, Group::pair(artifacts, SystemTime::UNIX_EPOCH))
                    .is_ok()
                {
                    buffered += 2;
                }
