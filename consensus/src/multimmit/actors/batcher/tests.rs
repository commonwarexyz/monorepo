...
#[test_traced]
fn a_relayed_da_vote_reaches_the_pool_without_blocking_its_sender() {
    let executor = DeterministicRunner::default();
    executor.start(|context| async move {
        ...
        // Peer two forwards a share participant three signed.
        ...
        // The signer's own copy is an exact duplicate.
        ...
        let artifacts = harness.observed(2).await;
        assert!(artifacts.iter().all(
            |artifact| matches!(artifact, Artifact::DaVote(vote) if vote.signer().get() == 3)
        ));
        assert!(harness.blocker.blocked().is_empty());
    });
}
