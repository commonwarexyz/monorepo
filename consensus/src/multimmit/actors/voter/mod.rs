...
//!
//! shared tasks: build[reserved 1], validate[global cap, one per chain], crypto
//! storage task: append -> one covering sync -> exact prefix acknowledgements
//! actor: resolver/serving
//! ```
//!
//! A local application build has a dedicated permit outside remote-validation capacity. Pending
//! validations stay in per-producer queues under one rotating cursor; there is no actor per chain.
//! Signing and critical aggregation reserve worker and completion capacity that bulk crypto cannot
//! consume. Every exhausted core quantum explicitly reschedules through the Commonware runtime.
//!
...
pub struct VoterLimits {
    ...
    /// Interval between periodic metrics and producer-stall checks.
