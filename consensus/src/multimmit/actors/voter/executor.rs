    ...
    S3: Sender<PublicKey = P>,
{
    ...
    /// Starts a validation immediately or holds it behind its producer chain's active job.
    ...
    fn spawn_validation(
        ...
        let commitment = block.header().commitment();
    ...
    fn cancel_validations(&mut self, chain: ChainId, through: Height) {
        let Some(id) = self.core_mut().cancel_validations(chain, through) else {
            return;
        };
