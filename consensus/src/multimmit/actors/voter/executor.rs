    ...
    S3: Sender<PublicKey = P>,
{
    ...
    fn execute_verification(
        ...
        match capability {
            ...
            VerificationCapability::Block(participants) => {
                self.block_participants(&participants)?;
            }
        }
        Ok(())
    }
    ...
    fn block_participants(&mut self, participants: &[Participant]) -> Result<(), Fatal> {
    ...
    fn schedule_verification(
        &mut self,
        pending: PendingVerification<V, H::Digest>,
    ) -> Result<(), Fatal> {
    ...
    fn try_schedule_verification(
        &mut self,
        pending: PendingVerification<V, H::Digest>,
    ) -> Result<Option<PendingVerification<V, H::Digest>>, Fatal> {
