...
use crate::{
    ...
    multimmit::{
        ...
        machine::{
            ...
            CoreError, CoreState, CoreTransition, CoreTurn, CoreWork, Cursor,
            CustodyCancellation, CustodyCompletion, CustodyJob, DaRecoveryCompletion,
            DurabilityCapability, DurableEffect, EffectId, IdentifiedArtifact, InputTicket, JobId,
            LeaderCapability, LqcAggregateCompletion, NullificationRecoveryCompletion,
            Observation, ObservationStatus, PersistDirective, ProducerCapability, ProducerProgress,
        ...
        types::{
            Activity, BlockRef, ChainId, Context, SignedTransactionBlock,
    ...
    S3: Sender<PublicKey = P>,
{
    ...
    fn publish_due(&mut self) -> Result<(), Fatal> {
        ...
        } in due
        {
            ...
            let relay_ready = !relay_due || self.relay_ready(&transmissions);
    ...
    /// Relays every canonical transaction-block header digest required by one publication attempt.
    fn relay_ready(&mut self, transmissions: &[Transmission<P, H::Digest>]) -> bool {
        for transmission in transmissions {
            let Some(header_digest) = transmission.relay else {
                continue;
            };
...
mod tests {
    ...
    fn generation_reset_releases_actor_runtime_correlations() {
        ...
        let mut active_custody =
            BTreeMap::from([(BuildId::fabricate(0), Some(custody_cancel))]);
        let mut active_validations = BTreeMap::from([(
            ValidationId::fabricate(0),
