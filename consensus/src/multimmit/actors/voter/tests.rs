...
use crate::{
    ...
    multimmit::{
        ...
        types::{
            Activity, ChainId, Context, DaCertificate, DaVote, LeaderBlock, SignedTransactionBlock,
    ...
    executor.start(|context| async move {
        for (seed, result, instance) in [
            ...
            let mut node = Node::start_with_attachments(
                ...
                instance,
                Attachments {
                    application: MockApplication::with_verification_result(result),
                    ..Attachments::default()
                },
            )
            .await;
    ...
    executor.start(|context| async move {
        ...
        loop {
            let (_, bytes) = select! {
                ...
                () = context.sleep_until(deadline) => {
                    panic!("an invalid share stalled the honest quorum")
                },
            };
            let envelope = Envelope::<DataMessage<MinPk, Sha256Digest>>::decode_cfg(
                bytes,
...
fn relayed_double_vote_blocks_its_signer_not_the_relays() {
    let executor = DeterministicRunner::default();
    executor.start(|context| async move {
        let node = Node::start(&context, 82, Role::Observer, "objective_equivocation").await;
        let (mut first_relay, _) = node.peer(1, 1).await;
        let (mut second_relay, _) = node.peer(2, 1).await;
        let first_leader = node.committee.leader_block(1);
        ...
        first_relay.send(
            ...
            node.envelope(ConsensusMessage::Vote(
                node.committee.vote(signer, &first_leader),
            ))
            .encode(),
            false,
        );
        second_relay.send(
            Recipients::One(node.me.clone()),
            node.envelope(ConsensusMessage::Vote(
                node.committee.vote(signer, &alternate),
            ))
            .encode(),
            false,
        );
...
fn same_chain_validations_overlap_without_voting_past_pending_parent() {
    DeterministicRunner::timed(Duration::from_secs(10)).start(|context| async move {
        let (application, mut validations) = MockApplication::with_gated_verify_heights(
            ChainId::new(1),
        ...
        let application_log = application.log();
        let limits = VoterLimits {
            inflight_application: NonZeroUsize::new(2).unwrap(),
            ..voter_limits()
        };
        ...
        let parent = node
            .committee
            .signed_block(1, Sha256::hash(&[b"pipelined parent payload"]));
        ...
        while voted.len() < 2 {
            let (_, bytes) = select! {
                result = data_rx.recv() => result.expect("network stays up"),
                () = context.sleep_until(deadline) => {
                    panic!("the contiguous valid prefix did not receive DA votes");
                },
            };
            let envelope = Envelope::<DataMessage<MinPk, Sha256Digest>>::decode_cfg(
                bytes,
...
fn da_certificate_cancels_superseded_local_custody() {
    DeterministicRunner::timed(Duration::from_secs(10)).start(|context| async move {
        let (application, mut custody) = MockApplication::with_gated_verify_chain(ChainId::new(0));
        let committee = Committee::<MinPk>::new(109, 6, Limits::new(1, 1).unwrap());
        let role = Role::Validator(Participant::new(0));
        let voter_profile = profile(&committee, role);
        let me = committee.identities[0].clone();
        let oracle = start_network(&context, committee.identities.clone(), 1024 * 1024).await;
        ...
        let replacement = select! {
            header = next_block(&node, &mut data_rx) => header,
            () = context.sleep(Duration::from_secs(1)) => {
                panic!("superseded custody prevented replacement production");
            },
        };
...
fn da_certificate_cancels_all_obsolete_pipelined_validations() {
    DeterministicRunner::timed(Duration::from_secs(10)).start(|context| async move {
        let (application, mut validations) =
            MockApplication::with_gated_verify_chain_count(ChainId::new(1), 3);
        application.pause_building();
        let application_log = application.log();
        let limits = VoterLimits {
            inflight_application: NonZeroUsize::new(2).unwrap(),
        ...
        let first = node
            .committee
            .signed_block(1, Sha256::hash(&[b"obsolete parent payload"]));
        ...
        loop {
            let (_, bytes) = select! {
                result = data_rx.recv() => result.expect("network stays up"),
                () = context.sleep_until(deadline) => {
                    panic!("the certified frontier did not release the surviving validation");
                },
            };
            let envelope = Envelope::<DataMessage<MinPk, Sha256Digest>>::decode_cfg(
                bytes,
    ...
    executor.start(|context| async move {
        ...
        loop {
            let (_, bytes) = select! {
                result = consensus_rx.recv() => result.expect("network stays up"),
                () = context.sleep_until(deadline) => {
                    panic!("closed relay withheld the node's consensus transmissions");
                },
            };
            let envelope = Envelope::<ConsensusMessage<MinPk, Sha256Digest>>::decode_cfg(
                bytes,
...
async fn inactivity_timeout_scenario(
    ...
    assert_eq!(
        metric_sample(&context.encode(), "inactivity_voter_view_timeouts_total"),
        0.0,
    ...
    let metrics = context.encode();
    let armed = metric_sample(&metrics, "inactivity_voter_view_timer_armed");
    ...
    if expected_early {
        ...
        while !(novote && nullify) {
            let (_, bytes) = select! {
                result = peers[0].1.recv() => result.unwrap(),
                () = context.sleep_until(deadline) => panic!("early timeout did not publish both shares"),
            };
            let envelope = Envelope::<ConsensusMessage<MinPk, Sha256Digest>>::decode_cfg(
                bytes,
                &node.envelope_cfg(node.committee.codec()),
    ...
    DeterministicRunner::timed(Duration::from_secs(10)).start(|context| async move {
        ...
        revived.send(
            ...
            node.envelope(DataMessage::Block(
                node.committee
                    .signed_block(2, Sha256::hash(&[b"revived leader"])),
            ))
            .encode(),
            false,
        );
        ...
        assert_eq!(
            metric_sample(&context.encode(), "inactivity_voter_view_timer_armed"),
            1.0
        );
    });
}
...
fn inactivity_timeout_recovery_starts_without_peer_history() {
    ...
    DeterministicRunner::from(checkpoint).start(|context| async move {
        ...
        let metrics = context.encode();
        assert_eq!(
            metric_sample(&metrics, "recovered_inactivity_voter_view_timeouts_total"),
            0.0
        );
        assert_eq!(
            metric_sample(&metrics, "recovered_inactivity_voter_view_timer_armed"),
            1.0
        );
    });
}
