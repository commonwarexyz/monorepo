...
use commonware_codec::{Codec, CodecShared, Encode as _, EncodeSize as _};
...
pub struct Config<T: Translator, V: Variant, B: Codec + Digestible> {
    ...
    /// Maximum encoded bytes accepted in one producer block.
    ///
    /// This bounds catalog admission and pending storage geometry independently of the aggregate
    /// resolver response bound.
    pub max_block_bytes: NonZeroUsize,
    /// Maximum encoded bytes accepted in one resolved artifact or aggregate peer response.
    ///
    /// This must accommodate one maximum block plus its response framing. Larger values allow
    /// finalized backfill to fetch several consecutive blocks in one bounded response.
    ...
    /// Maximum blocks dispatched to the application whose `Exact` acknowledgement is unresolved.
    ///
    /// Contiguous application-ready prefixes coalesce in constant space while delivery cursor
    /// durability runs, so cursor storage latency does not occupy this window.
    ...
    /// duplicate payload bytes. Delivery reserves compact committed descriptors before optional
    /// bodies so body eviction does not require another catalog lookup. Size this for the expected
    /// finalized-delivery lag and short scheduling stalls on the normal path.
    ...
    /// [`Self::resolver_max_value_bytes`], capped by [`Self::backfill_concurrency`]. The resulting
    /// response capacity also bounds staged finalized-custody lookahead. A single maximum-sized
    /// response is always permitted so a smaller budget cannot prevent progress.
    ...
    /// up to the same number of references independently of cache eviction. Hints establish
    /// ancestry; complete bodies still require validated custody.
...
impl<T: Translator, V: Variant, B: Codec + Digestible> Config<T, V, B> {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        if self
            ...
            .is_none_or(|minimum| minimum > self.resolver_max_value_bytes.get())
        {
    ...
    pub(super) fn effective_backfill_concurrency(&self) -> NonZeroUsize {
        let by_bytes = (self.max_backfill_bytes.get() / self.resolver_max_value_bytes.get()).max(1);
    ...
    B::Cfg: Clone,
{
    ...
    ) -> impl Future<Output = Result<Spawned<E, H, V, B>, String>> + Send
    where
        ...
        H: Hasher<Digest = B::Digest>,
    {
        ...
        .await
        .map_err(|error| error.to_string())?;
        let mut metadata: Metadata<E, Unit, CatalogState<H::Digest>> = Metadata::init(
        ...
        let mut initialized_delivery = None;
        if metadata
            .get(&Unit)
        ...
        let catalog_state = metadata
            .get(&Unit)
            .expect("catalog checkpoint was initialized");
        let stored_checkpoint = catalog_state.checkpoint().clone();
        let recovery_checkpoint = catalog_state
            .install_checkpoint()
            .unwrap_or_else(|| catalog_state.checkpoint())
            .clone();
        let delivery_store = match initialized_delivery {
            Some(store) => store,
            None => DeliveryStore::init(
                context.child("delivery"),
                name("delivery"),
                catalog_state.install_checkpoint().is_some()
                    || stored_checkpoint.committed().is_none(),
                recovery_checkpoint.generation(),
                recovery_checkpoint.committed(),
            )
            .await
            .map_err(|error| error.to_string())?,
        };
        let delivery_generation = delivery_store.generation();
        let durable_acknowledged = delivery_store.acknowledged();
        ...
        let promoter = if self.finalized_blocks == ArchiveMode::Immutable {
            ...
            .await
            .map_err(|error| error.to_string())?;
            let mut promotion = Metadata::init(
...
mod tests {
    ...
    fn default_construction_is_bounded_and_exact_acknowledgement() {
        deterministic::Runner::default().start(|context| async move {
            ...
            assert_eq!(config.max_block_bytes.get(), 1024 * 1024);
            assert_eq!(config.resolver_max_value_bytes.get(), 4 * 1024 * 1024);
            ...
            assert_eq!(config.max_materialized_block_bytes.get(), 512 * 1024 * 1024);
    ...
    fn validation_rejects_cross_context_configuration() {
        deterministic::Runner::default().start(|context| async move {
            ...
            let mut invalid = config(&context, Epoch::new(7), 2);
            invalid.max_block_bytes = NZUsize!(1024);
            invalid.resolver_max_value_bytes = NZUsize!(1024);
            assert_eq!(
                invalid.validate(),
                Err(ConfigError::ResolverResponseCapacity)
            );
            invalid.resolver_max_value_bytes = NZUsize!(1025);
    ...
    async fn open_geometry_namespace(
        ...
        let (_, handle, _, promoter, _) = config
    ...
    fn oversized_commit_cleanup_is_rejected_before_finalized_writes() {
        deterministic::Runner::default().start(|context| async move {
            ...
            let (client, handle, _, promoter, _) = bounded
            ...
            let (client, handle, _, promoter, _) = reopened
    ...
    fn trusted_start_floor_seeds_only_a_fresh_namespace() {
        deterministic::Runner::default().start(|context| async move {
            ...
            let (catalog, handle, _, promoter, _) = config
    ...
    fn reopen_rejects_changed_finalized_archive_modes() {
        deterministic::Runner::default().start(|context| async move {
            ...
            let (_, handle, _, promoter, _) = original
