...
use crate::{
    ...
    multimmit::{
        config::Limits,
        machine::{Artifact, FinalityFact, FinalityId},
        marshal::{
            ...
            actors::catalog::CatalogClient, open, storage::state::StoredRef,
        },
        mocks::{
            Committee,
    ...
    runner(132).start(|context| async move {
        ...
        let votes = (0..harness.committee.codec().view_quorum())
            .map(|signer| {
                ...
                .unwrap();
                sign_vote(&harness.committee.signers[signer], vote).unwrap()
            })
            .collect::<Vec<_>>();
        batch.proof = Arc::new(
            harness
        ...
        let mailbox = harness.mailbox(0);
        batch.submit(&mailbox).await;
        let initial_fact = FinalityFact::for_test(
        ...
        let future = FinalityFact::for_test(
        ...
        let fact = FinalityFact::for_test(
...
fn delivery_materializes_only_the_cold_prefix_before_a_retained_hot_output() {
    runner(130).start(|context| async move {
        ...
        let metrics = harness.context.encode();
        for (metric, expected) in [
            ("custody_local_outputs_total", 2.0),
            ("custody_fetched_outputs_total", 0.0),
            ("delivery_stored_outputs_total", 1.0),
            ("delivery_hot_outputs_total", 1.0),
        ] {
            assert_eq!(metric_total(&metrics, metric), expected, "{metrics}");
        }
        harness.shutdown().await;
