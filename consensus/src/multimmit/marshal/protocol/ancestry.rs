...
impl<D: Digest> Ancestry<D> {
    ...
    pub(in crate::multimmit::marshal) const fn bounds(&self) -> (BlockRef<D>, BlockRef<D>) {
        (self.expected, self.cursor)
    }
        ...
        H: Hasher<Digest = D>,
    {
        ...
        self.accept_parent(parent)
    }

    /// Advances through an authenticated parent edge.
    pub(in crate::multimmit::marshal) fn accept_parent(
        &mut self,
        parent: BlockRef<D>,
    ) -> Result<(), Error> {
        let request = self.next().ok_or(Error::Complete)?;
