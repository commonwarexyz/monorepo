...
type Key<D> = (BlockRef<D>, BlockRef<D>);

/// A fixed reference budget covers complete path allocations and bounds the lookup index.
/// Each indexed block belongs to its newest retained path; FIFO eviction preserves newer owners.
pub(in crate::multimmit::marshal) struct Cache<D: Digest> {
    epoch: Epoch,
    capacity: usize,
    retained: usize,
    chains: usize,
    paths: BTreeMap<Key<D>, Arc<[BlockRef<D>]>>,
    by_block: BTreeMap<BlockRef<D>, Key<D>>,
    order: VecDeque<Key<D>>,
}
...
impl<D: Digest> Cache<D> {
    ...
    pub(in crate::multimmit::marshal) fn insert(&mut self, commitments: &SelectedCommitments<D>) {
        ...
        for path in commitments.paths() {
            ...
            let key = (path[0], path[path.len() - 1]);
            let chain = key.0.chain().get() as usize;
            if chain >= self.chains || self.paths.contains_key(&key) {
                continue;
            }
    ...
    pub(in crate::multimmit::marshal) fn parent(&self, tip: BlockRef<D>) -> Option<BlockRef<D>> {
    ...
    pub(in crate::multimmit::marshal) fn resolve(
        &self,
        low: BlockRef<D>,
        high: BlockRef<D>,
    ) -> Option<Forward<D>> {
...
/// An exact branch pinned for one output plan, independent of cache eviction.
pub(in crate::multimmit::marshal) struct Forward<D: Digest> {
    ranges: Vec<Range<D>>,
}

impl<D: Digest> Forward<D> {
    pub(in crate::multimmit::marshal) fn get(&self, height: Height) -> Option<BlockRef<D>> {
...
mod tests {
    ...
    fn exact_endpoints_join_overlapping_ranges_without_copying() {
        ...
        let mut cache = Cache::new(Epoch::new(1), 2, 32);
    ...
    fn thousands_of_adjacent_ranges_share_exact_lookup_and_survive_overlapping_eviction() {
        ...
        let mut cache = Cache::new(Epoch::new(1), 2, 8192);
    ...
    fn evicting_overlapping_paths_preserves_only_the_retained_owner() {
        ...
        let mut cache = Cache::new(Epoch::new(1), 2, 10);
    ...
    fn full_allocation_budget_eviction_and_epoch_are_independent_of_active_plan() {
        ...
        let mut cache = Cache::new(Epoch::new(1), 2, 5);
