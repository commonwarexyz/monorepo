...
use crate::{
    ...
    multimmit::{
        ...
        marshal::{
            ...
            storage::{
                delivery::Store as DeliveryStore,
                scratch::{BlockScratch, HistoryScratch},
            },
            types::Update,
            wire,
        },
    ...
    P: PublicKey,
{
    ...
    delivery_store: DeliveryStore<E>,
    ...
    P: PublicKey,
{
    ...
    let resolver_max_value_bytes = config.resolver_max_value_bytes;
    ...
    let (catalog, catalog_handle, promoter, promoter_handle, delivery_store) = config
    ...
    B: Codec + Digestible<Digest = H::Digest>,
{
    ...
    subscription_completions: AbortablePool<'static, SubscriptionCompletion<H, B>>,
    subscription_callers: Pool<'static, BlockRef<H::Digest>>,
    max_pending: usize,
    ...
    B: Codec + Digestible<Digest = H::Digest>,
{
    ...
    fn dispatch(&mut self, command: Command<H, V, B>) {
        match command.request {
            ...
            Request::Hint(activity) => match activity {
                ...
                crate::multimmit::types::Activity::CommitmentsAccepted { commitments } => {
                ...
                crate::multimmit::types::Activity::LeaderFinalized { fact }
                | crate::multimmit::types::Activity::LeaderFinalityUpdated { fact } => {
    ...
    async fn run(mut self) -> Result<(), mailbox::Error> {
        loop {
            ...
            self.metrics.update(self.jobs.len(), subscriptions, callers);
