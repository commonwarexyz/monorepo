...
use crate::multimmit::{
    marshal::wire::{Key, MAX_SEGMENT_ITEMS, max_block_segment_items},
    types::BlockRef,
};
use bytes::Bytes;
    ...
    B: Codec + Digestible<Digest = H::Digest>,
{
    let value = match key {
        ...
        Key::ProducerBlocks { head, max_items } => {
            ...
            for header in headers {
                ...
                if expected.height().get() == 1 {
                    break;
                }
                expected = BlockRef::new(
                    expected.chain(),
                    crate::types::Height::new(expected.height().get() - 1),
                    header.parent(),
                );
            }
            let values = bodies
                .blocks(references.clone())
                .await
                .map_err(storage_error)?;
            ...
            for (reference, block) in references.into_iter().zip(values) {
                ...
                let count = blocks.len() + 1;
                if next_block_bytes
                    .checked_add(count.encode_size())
                    .is_none_or(|size| size > max_value_bytes)
                {
                    break;
                }
    ...
    B: Codec + Digestible<Digest = H::Digest>,
{
    ...
    let actor = Actor {
        ...
        max_blocks_per_value: max_block_segment_items(max_block_bytes.get(), max_value_bytes.get()),
