...
async fn next_catalog_event<D, A, M, S, C, Q>(
    durability: D,
    acknowledgement: A,
    materialization: M,
    seal: S,
...
) -> CatalogEvent<D::Output, A::Output, M::Output, S::Output, C::Output, Q::Output>
where
    D: Future,
    A: Future,
    M: Future,
    ...
    B::Cfg: Clone,
{
    ...
    async fn run_loop(&mut self) -> Result<(), Error> {
        ...
        loop {
            ...
            {
                match read {
                    Some(read) => self.process_read(read).await?,
            ...
            let result = async {
                match event {
                    ...
                    CatalogEvent::Read(Some(read)) => self.process_read(read).await?,
    ...
    async fn process_read(&mut self, read: TracedCommand<H, V, B>) -> Result<(), Error> {
    ...
    async fn process_lookup(&mut self, command: Command<H, V, B>) -> Result<(), Error> {
        match command {
            ...
            Command::HeaderSegments(requests, max_bytes, reply) => {
                self.start_header_segments(requests, max_bytes, reply).await
            }
        ...
        loop {
            ...
            select! {
                ...
                read = read => {
                    ...
                    self.process_read(read).await.map_err(Error::storage)?;
                },
            }
        }
    }
    ...
    async fn start_history_segment(
        &mut self,
        ...
        while records.len() < max_items {
            ...
            let record = match self.stores.history(commitment).await {
                Ok(Some(record)) => record,
                Ok(None) => break,
                Err(error) => return respond(reply, Err(error)),
            };
            let next_item_bytes = item_bytes.saturating_add(record.encode_size());
        ...
        respond(reply, Ok(records))
    }

    async fn start_header_segments(
        &mut self,
        ...
        for (mut reference, max_items) in requests {
            ...
            while headers.len() < max_items {
                ...
                let header = match self.stores.block_header(reference).await {
                    Ok(Some(header)) => header,
                    Ok(None) => break,
                    Err(error) => return respond(reply, Err(error)),
                };
                let next_item_bytes = item_bytes.saturating_add(header.encode_size());
        ...
        respond(reply, Ok(segments))
    }

    async fn start_output_refs(
        &mut self,
        start: OutputIndex,
        max_items: usize,
        max_bytes: NonZeroUsize,
        ...
        for index in start.get()..=committed.get() {
            if outputs.len() == max_items {
                break;
            }
            if reply.is_closed() {
                return Ok(());
            }
            let output = match self.stores.stored_ref(index).await {
                Ok(output) => output,
                Err(error) => return respond(reply, Err(error)),
            };
            if !outputs.is_empty()
    ...
    async fn process(&mut self, command: Command<H, V, B>) -> Result<(), Error> {
        match command {
            ...
            | Command::BodyCandidateByDigest(..)
            | Command::HeaderSegments(..)
            | Command::OutputRefs(..)) => self.process_lookup(command).await,
...
pub(in crate::multimmit::marshal) async fn spawn<R, T, E, H, V, B>(
    ...
    header_request_capacity: NonZeroUsize,
...
mod tests {
    ...
    fn committed_output_descriptors_preserve_order_and_bounds(#[case] archive: ArchiveMode) {
        deterministic::Runner::timed(std::time::Duration::from_secs(10)).start(
            |context| async move {
                let committee = Committee::<MinPk>::new_with_namespace_and_producers(
                    62,
    ...
    fn archive_durable_checkpoint_volatile_commit_replays_exactly(#[case] all_archives: bool) {
        ...
        deterministic::Runner::default().start(|context| async move {
            ...
            let record = Arc::new(
                TipRecord::at_tips(current.history(), current.ordered().to_vec()).unwrap(),
            );
            let history = record.commitment::<Sha256>();
