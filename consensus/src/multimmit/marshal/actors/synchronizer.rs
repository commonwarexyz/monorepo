...
//!
//! The synchronizer builds a dense output stream from authenticated producer commitments. Known
//! ranges feed forward output plans directly; unknown ancestry is walked concurrently. It batches
//! local catalog lookups and retains bodyless plans until bounded checkpoint-last publication.
//! Two publications may be pipelined. Resolver requests are issued only for gaps established by
    ...
    multimmit::{
        ...
        marshal::{
            ...
            protocol::{
                ...
                commitments::{self, Forward},
                order::{self, FinalSweep, HistoryState, Horizontal, OrderedSlots, Reconciliation},
            },
            storage::checkpoint::{ArchiveLayout, Checkpoint},
            types::OutputIndex,
            wire::{MAX_SEGMENT_ITEMS, max_block_segment_items},
        },
        types::{
            BlockRef, CertificateId, Lqc, SelectedCommitments, TipRecord, TransactionBlock,
...
type CustodyFetch<H, B> = (
    MissingRun<<H as Hasher>::Digest>,
    Result<Vec<FetchedBlock<H, B>>, Arc<str>>,
);
const COMMIT_WINDOW: usize = 2;
pub(in crate::multimmit::marshal) const CUSTODY_LOOKUP_CONCURRENCY: usize = 2;

/// A recovered body retained until its durable custody row reaches delivery preparation.
pub(in crate::multimmit::marshal) struct FetchedBlock<H, B>
where
    H: Hasher,
    B: Codec + Digestible<Digest = H::Digest>,
{
    pub custody: catalog::CustodyRef<H::Digest>,
...
fn retain_stronger<D: Digest>(slot: &mut Option<FinalityFact<D>>, fact: FinalityFact<D>) {
    let replace = slot.as_ref().is_none_or(|current| {
        fact.round() == current.round()
            && fact.leader() == current.leader()
            && fact.parent() == current.parent()
    ...
    B: Codec + Digestible<Digest = H::Digest>,
{
    ...
    fn block(
        ...
        reason: FetchReason,
        reference: BlockRef<H::Digest>,
    ) -> impl Future<Output = Result<FetchedBlock<H, B>, Self::Error>> + Send;

    fn blocks(
        &mut self,
        reason: FetchReason,
        references: Vec<BlockRef<H::Digest>>,
    ) -> impl Future<Output = Result<Vec<FetchedBlock<H, B>>, Self::Error>> + Send {
        async move {
            let mut custody = Vec::with_capacity(references.len());
...
impl<V: Variant, D: Digest> Policy for Command<V, D> {
    ...
    fn handle(overflow: &mut Self::Overflow, command: Self) {
        ...
        match command {
            ...
            Self::Header(_, _) | Self::Commitments(_) | Self::Finality(_, _) => {}
...
impl<V: Variant, D: Digest> Client<V, D> {
    ...
    pub(in crate::multimmit::marshal) fn trigger(
        ...
        match self.commands.enqueue(Command::Synchronize(
            Span::current(),
            FinalityBatch::new(id, proof, self.max_proofs),
        )) {
    ...
    pub(in crate::multimmit::marshal) fn header(
        ...
        match self
            .commands
            .enqueue(Command::Header(Span::current(), header))
        {
            Feedback::Closed => Err(Error::Closed),
            Feedback::Ok | Feedback::Backoff => Ok(()),
        }
    ...
    pub(in crate::multimmit::marshal) fn commitments(
        &self,
        commitments: SelectedCommitments<D>,
    ) -> Result<(), Error> {
        match self.commands.enqueue(Command::Commitments(commitments)) {
            Feedback::Closed => Err(Error::Closed),
            Feedback::Ok | Feedback::Backoff => Ok(()),
        }
    }
    ...
    pub(in crate::multimmit::marshal) fn finality(
        &self,
        fact: FinalityFact<D>,
    ) -> Result<(), Error> {
        match self
            .commands
            .enqueue(Command::Finality(Span::current(), fact))
        {
            Feedback::Closed => Err(Error::Closed),
            Feedback::Ok | Feedback::Backoff => Ok(()),
        }
    ...
    B: Codec + Digestible<Digest = H::Digest>,
{
    ...
    reference: BlockRef<H::Digest>,
    custody: Option<catalog::CustodyRef<H::Digest>>,
    block: Option<Arc<TransactionBlock<H, B>>>,
}

struct MissingOutput<D: Digest> {
    sequence: usize,
    reference: BlockRef<D>,
}

/// One page-local consecutive producer run, retained oldest-first.
struct MissingRun<D: Digest> {
    outputs: Vec<MissingOutput<D>>,
}
    ...
    B: Codec + Digestible<Digest = H::Digest>,
{
    ...
    outputs: Vec<PlannedOutput<H, B>>,
    output_bytes: u64,
}

impl<H, V, B> PublicationBatch<H, V, B>
where
    H: Hasher,
    V: Variant,
    B: Codec + Digestible<Digest = H::Digest>,
    ...
    B: Codec + Digestible<Digest = H::Digest>,
{
    ...
    history_stack: S,
    block_stack: K,
    headers: Cache<BlockRef<H::Digest>, TransactionBlockHeader<H::Digest>>,
    commitments: commitments::Cache<H::Digest>,
    forward: Vec<Option<Forward<H::Digest>>>,
    verifier: Q,
    codec: CodecConfig,
    ...
    B: Codec + Digestible<Digest = H::Digest>,
{
    ...
    async fn recover(
        ...
        let max_fetch_blocks =
            max_block_segment_items(max_block_bytes.get(), resolver_max_value_bytes.get());
        ...
        let custody_batch_outputs = max_commit_outputs
            .min((max_commit_block_bytes.get() / max_block_bytes.get()).max(1))
        ...
        let mut this = Self {
            ...
            commitments: commitments::Cache::new(
                checkpoint.epoch(),
                codec.chains(),
                header_cache_capacity.get(),
            ),
            forward: (0..codec.chains()).map(|_| None).collect(),
            verifier,
            codec,
    ...
    async fn run(
        ...
        loop {
            ...
            match command {
                Command::Commitments(paths) => self.commitments.insert(&paths),
                ...
                Command::Synchronize(span, mut batch) => {
                        ...
                        .try_recv()
                    {
                        match command {
                            ...
                            Command::Finality(_, fact) => self.retain_finality(fact),
                    ...
                    let result = self
                        ...
                        .instrument(process)
                        .await;
                    self.finish_finality_sync();
                    result?;
                    self.synchronize_pending_finality().await?;
                }
                Command::Finality(span, fact) => {
                    let process = info_span!(
                        parent: &span,
                    ...
                    self.retain_finality(fact);
                    self.synchronize_pending_finality()
                        .instrument(process)
                        .await?;
                }
                Command::InstallFloor(span, checkpoint, reply) => {
                    self.prepare_finality(checkpoint.anchor.view());
                    let process = info_span!(
                        parent: &span,
    ...
    fn retain_finality(&mut self, fact: FinalityFact<H::Digest>) {
        let valid = matches!(fact.id(), FinalityId::Direct(_))
            && fact.round().epoch() == self.epoch
            && !fact.round().view().is_zero()
            && fact.votes() >= self.codec.view_quorum()
            && fact.votes() <= self.codec.participants()
            && fact.blocks().len() == self.codec.chains()
            && fact.positions().len() == self.codec.chains()
            && fact.settled().len() == self.codec.chains()
            && fact.blocks().iter().enumerate().all(|(chain, block)| {
                u32::try_from(chain).map(crate::multimmit::types::ChainId::new) == Ok(block.chain())
            })
            && fact
                .positions()
                .iter()
                .all(|position| position.get() as usize <= self.codec.pipeline_depth());
        if !valid || fact.round().view() < self.floor_view {
            return;
        }

        let usable_view = self.synchronizing_view.unwrap_or(self.floor_view);
        match fact.round().view().cmp(&usable_view) {
            std::cmp::Ordering::Less => {}
            std::cmp::Ordering::Equal => retain_stronger(&mut self.pending_finality, fact),
            std::cmp::Ordering::Greater => retain_later(&mut self.future_finality, fact),
        }
    }

    fn prepare_finality(&mut self, view: View) {
        self.synchronizing_view = Some(view);
        self.reclassify_finality();
    }

    fn finish_finality_sync(&mut self) {
        self.synchronizing_view = None;
        self.reclassify_finality();
    }

    fn reclassify_finality(&mut self) {
        let pending = self.pending_finality.take();
        let future = self.future_finality.take();
    ...
    async fn synchronize_pending_finality(&mut self) -> Result<(), Error> {
        loop {
            let Some(fact) = self.pending_finality.take() else {
                return Ok(());
            };
            match fact.round().view().cmp(&self.floor_view) {
                std::cmp::Ordering::Less => continue,
                std::cmp::Ordering::Greater => {
                    self.pending_finality = Some(fact);
                    return Ok(());
                }
                std::cmp::Ordering::Equal => {}
            }
    ...
    async fn synchronize_proofs(
        ...
        let mut batch = PublicationBatch::<H, V, B>::new(self.max_commit_outputs, proofs.len());
        let mut next = self
            .history_stack
            .read_reverse()
            .await
        ...
        for (id, proof) in proofs {
            ...
            self.process_final_sweep(&mut preview_sweep, &mut batch)
                .await?;
    ...
    async fn process_final_sweep(
        &mut self,
        sweep: &mut FinalSweep<H::Digest>,
        batch: &mut PublicationBatch<H, V, B>,
    ) -> Result<(), Error> {
        let target = sweep.target().to_vec();
        let emitted = self.state.emitted().to_vec();
        let common = self.stage(sweep, &target, &emitted).await?;
        self.state.validate_reconciliation(&target, &common)?;
        self.drive_staged(sweep, &emitted, batch).await
    }
        ...
        I: OrderedSlots<H::Digest>,
    {
        ...
        self.block_stack
            ...
            .await
            .map_err(|error| Error::Stack(message(error)))?;
        let mut stage_max = stream.maxima();
    ...
    async fn walk_producers(
        ...
        self.forward.iter_mut().for_each(|path| *path = None);
        for (chain, walk) in walks.iter_mut().enumerate() {
            let (low, high) = walk.ancestry.bounds();
        ...
        loop {
            ...
            for _ in 0..ready.len() {
                ...
                if let Some(parent) = self.commitments.parent(request) {
                    walks[chain].ancestry.accept_parent(parent)?;
                    self.stage_producer_reference(&mut walks[chain], request)
                        .await?;
    ...
    async fn accept_producer_header(
        ...
        self.stage_producer_reference(walk, request).await
    }

    async fn stage_producer_reference(
        &mut self,
        walk: &mut ProducerWalk<H::Digest>,
        request: BlockRef<H::Digest>,
    ) -> Result<(), Error> {
        if let Some(boundary) = walk
            .boundaries
            .pop_if(|boundary| boundary.height() == request.height())
            && boundary != request
        {
            return Err(Error::Invalid(
                "producer ancestry conflicts with a history boundary",
        ...
        I: Iterator<Item = order::Slot<H::Digest>>,
    {
        ...
        loop {
            ...
            while staged.len() < self.custody_window_outputs && !stream_finished {
                let Some((slot, reference)) = self.next_staged(stream, initially_emitted).await?
                else {
            ...
            while lookups.len() < CUSTODY_LOOKUP_CONCURRENCY
                && (unscheduled >= self.custody_batch_outputs
                    || (stream_finished && unscheduled > 0))
            {
            ...
            while fetches.len() < self.backfill_concurrency {
                let Some(run) = missing.pop_front() else {
                    break;
                };
                let mut fetcher = self.fetcher.clone();
                let references = run.references_newest_first();
                ...
                .is_some_and(|output| output.custody.is_some())
            {
                ...
                self.emit_output(slot, custody, output.block, batch).await?;
                ready -= 1;
                emitted = true;
            }
            if emitted {
            ...
            commonware_macros::select! {
                ...
                fetch = fetches.next_completed() => {
                    let (run, result): CustodyFetch<H, B> = fetch;
                    let custody = result.map_err(Error::Fetch)?;
                    let fetched = self.apply_custody_fetch(
                        run,
                        custody,
                        &mut staged,
                        &mut missing,
        ...
        I: Iterator<Item = order::Slot<H::Digest>>,
    {
        for slot in stream {
            ...
            let reference = if let Some(path) = &self.forward[chain] {
                path.get(slot.height())
            } else {
                self.block_stack
                    .read_oldest(chain)
                    .await
    ...
    fn staged_mut<'a>(
        &self,
        staged: &'a mut VecDeque<StagedOutput<H, B>>,
        sequence: usize,
    ) -> Result<&'a mut StagedOutput<H, B>, Error> {
        let first = staged
            .front()
            .ok_or(Error::Invalid("custody window is empty"))?
            .sequence;
    ...
    fn apply_custody_lookup(
        ...
        staged: &mut VecDeque<StagedOutput<H, B>>,
        missing: &mut VecDeque<MissingRun<H::Digest>>,
    ) -> Result<usize, Error> {
        ...
        for (offset, custody) in custody.into_iter().enumerate() {
            ...
            if let Some(custody) = custody {
                ...
                let extends = runs.get(&chain).is_some_and(|run| {
                    run.outputs.len() < self.max_fetch_blocks
                ...
                runs.entry(chain)
                    .or_insert_with(|| MissingRun {
                        outputs: Vec::with_capacity(self.max_fetch_blocks.min(expected)),
                    })
                    .outputs
                    .push(MissingOutput {
                        sequence,
        ...
        missing.extend(runs.into_values());
        Ok(local)
    }

    fn apply_custody_fetch(
        &self,
        run: MissingRun<H::Digest>,
        fetched_blocks: Vec<FetchedBlock<H, B>>,
        staged: &mut VecDeque<StagedOutput<H, B>>,
        missing: &mut VecDeque<MissingRun<H::Digest>>,
    ) -> Result<usize, Error> {
        if fetched_blocks.is_empty() || fetched_blocks.len() > run.outputs.len() {
            return Err(Error::Invalid(
                "resolved output range cardinality is invalid",
            ));
        }
        for (output, fetched) in run.outputs.iter().rev().zip(&fetched_blocks) {
            if self.validate_fetched(fetched)? != output.reference {
                return Err(Error::Invalid(
                    "resolved output block does not match its request",
                ));
            }
        }
        ...
        for (output, fetched) in run.outputs.iter().rev().zip(fetched_blocks) {
            let staged = self.staged_mut(staged, output.sequence)?;
            staged.custody = Some(fetched.custody);
            staged.block = Some(fetched.block);
        }
        let remaining = run.outputs.len() - fetched;
        if remaining > 0 {
            missing.push_back(MissingRun {
    ...
    fn validate_fetched(&self, fetched: &FetchedBlock<H, B>) -> Result<BlockRef<H::Digest>, Error> {
        let reference = self.validate_custody(&fetched.custody)?;
        if fetched.block.reference() != reference
            || fetched.block.header() != fetched.custody.meta().header()
            || u64::try_from(fetched.block.encode_size()).ok()
                != Some(fetched.custody.meta().encoded_len())
        {
            return Err(Error::Invalid(
                "resolved output body does not match its durable custody",
            ));
        }
        Ok(reference)
    }
    ...
    async fn commit(&mut self, batch: PublicationBatch<H, V, B>) -> Result<(), Error> {
        if batch.is_empty() {
            return Ok(());
        }
        ...
        let outputs = outputs
            ...
            .map(|output| {
                ...
                if let Some(block) = block {
                    handoff.push(delivery::DurableOutput {
                        index,
                        block,
                        encoded_len: custody.meta().encoded_len(),
                    });
                }
                catalog::OutputRow::new(index, custody)
            })
            .collect();
    ...
    async fn commit_pending(&mut self, batch: &mut PublicationBatch<H, V, B>) -> Result<(), Error> {
        if batch.is_empty() {
            return Ok(());
        }
...
mod tests {
    ...
    use crate::{
        ...
        multimmit::{
            ...
            types::{
                Anchor, ChainId, ChainProposal, Extension, LeaderBlock, Position, TransactionBlock,
                TransactionBlockHeader, VoteBody, genesis_history,
            },
        },
    ...
    impl Fetcher<Sha256, MinPk, TestBody> for MockFetcher {
        ...
        ) -> Result<FetchedBlock<Sha256, TestBody>, Self::Error> {
        ...
        ) -> Result<Vec<FetchedBlock<Sha256, TestBody>>, Self::Error> {
    ...
    impl CatalogPort<Sha256, MinPk, TestBody> for MockCatalog {
        ...
        async fn start_commit(
            ...
            handoff: Vec<delivery::DurableOutput<Sha256, TestBody>>,
        ) -> Result<Self::CommitToken, Self::Error> {
    ...
    async fn actor(
        ...
        actor_with_backfill(checkpoint, blocks, codec, max, 32).await
    }
    ...
    fn selected_commitments_bypass_only_known_ancestry(#[case] start: Option<usize>) {
        deterministic::Runner::timed(Duration::from_secs(10)).start(|_| async move {
            let committee = committee(63, 1, Limits::new(1, 1).unwrap());
            ...
            let expected = blocks[0]
                ...
                .collect::<Vec<_>>();
            let mut actor =
                actor_with_backfill(current, blocks.clone(), committee.codec(), 8, 2).await;
    ...
    fn interleaved_chains_form_independent_range_fetches() {
        block_on(async {
            let committee = committee(37, 3, Limits::new(1, 1).unwrap());
            let epoch = committee.config.epoch();
            let bases = vec![base(0, 0), base(1, 0), base(2, 0)];
            let blocks = bases
                .iter()
    ...
    fn custody_window_matches_backfill_response_capacity() {
        block_on(async {
            let committee = committee(41, 1, Limits::new(1, 1).unwrap());
            let epoch = committee.config.epoch();
            let base = base(0, 0);
            let actor = actor(
                checkpoint(
            ...
            assert_eq!(actor.custody_batch_outputs, 16);
            assert_eq!(actor.backfill_concurrency, 32);
            assert_eq!(actor.max_fetch_blocks, 15);
            assert_eq!(
                actor.custody_window_outputs,
                actor.backfill_concurrency * actor.max_fetch_blocks
            );
        });
    }
            ...
            for _ in 0..HINTS {
                assert_eq!(
                    commands.enqueue(Command::Synchronize(
                        Span::none(),
                        FinalityBatch::new(id, Arc::clone(&proof), HINTS,)
                    )),
                    Feedback::Ok
                );
            }
    ...
    fn run_batches_distinct_queued_synchronization_hints() {
        deterministic::Runner::default().start(|context| async move {
            ...
            assert_eq!(
                commands.enqueue(Command::Synchronize(
                    Span::none(),
                    FinalityBatch::new(second_id, second, 2)
                )),
                Feedback::Ok
            );
            drop(commands);
    ...
    fn run_coalesces_finality_before_fetching_history() {
        deterministic::Runner::default().start(|context| async move {
            ...
            assert_eq!(
                commands.enqueue(Command::Synchronize(
                    Span::none(),
                    FinalityBatch::new(lower.id::<Sha256>(), lower, 2,)
                )),
                Feedback::Ok
            );
            assert_eq!(
                commands.enqueue(Command::Synchronize(
                    Span::none(),
                    FinalityBatch::new(higher_id, higher, 2)
                )),
                Feedback::Ok
            );
            drop(commands);
        ...
        deterministic::Runner::default().start(|context| async move {
            ...
            let driver = async move {
                assert_eq!(
                    commands.enqueue(Command::Synchronize(
                        Span::none(),
                        FinalityBatch::new(first.id::<Sha256>(), first, 2,)
                    )),
                    Feedback::Ok
                );
                ...
                assert_eq!(
                    commands.enqueue(Command::Synchronize(
                        Span::none(),
                        FinalityBatch::new(second.id::<Sha256>(), second, 2,)
                    )),
                    Feedback::Ok
                );
