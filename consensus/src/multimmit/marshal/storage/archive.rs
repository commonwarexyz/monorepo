    ...
    V: CodecShared,
{
    ...
    /// Starts durability for buffered writes. Immutable archives finish syncing before returning.
    pub(in crate::multimmit::marshal) async fn start_sync(
        ...
        match self.0 {
            ...
            Backend::Immutable(archive) => {
                let archive = archive.sync().await?;
                Ok((Self(Backend::Immutable(archive)), Handle::ready(Ok(()))))
            }
        }
    }
