...
const CATALOG_STATE_VERSION: u8 = 3;
...
/// The committed output index is optional because index zero is valid and cannot double as the
...
mod tests {
    ...
    fn ordinary_transitions_preserve_and_coalesce_cleanup() {
        ...
        let with_newer_selection = without_selection
            .publish_commit(current, Some(12), Some(View::new(12)))
    ...
    fn decode_rejects_wrong_version_epoch_and_chain_count() {
        ...
        assert!(matches!(
            TestCheckpoint::decode_cfg(wrong_version, &config()),
