//! Durable machine snapshots, stored outside the safety journal.
//!
//! The store holds one acknowledged machine [`Snapshot`], atomically swapped in a
//! [`Metadata`] store: writes land in the older of two blobs and only replace the newer once
//! synced, so a crash mid-write always recovers the previous complete snapshot. Keeping
//! the snapshot out of the journal decouples its size, which grows with the committee, from
//! the journal's per-record bounds.

use crate::{
    multimmit::machine::{Snapshot, SnapshotCodecConfig},
    types::Epoch,
};
use commonware_cryptography::{Digest, bls12381::primitives::variant::Variant};
use commonware_storage::{
    Context as StorageContext,
    metadata::{Config as MetadataConfig, Error as MetadataError, Metadata},
};
use commonware_utils::sequence::U64;

/// The single key under which the newest snapshot lives.
const SNAPSHOT_KEY: u64 = 0;

/// Atomic store for the newest acknowledged machine snapshot.
pub struct SnapshotStore<E: StorageContext, V: Variant, D: Digest> {
    inner: Metadata<E, U64, Snapshot<V, D>>,
}

/// A snapshot store failure. Fatal to the current instance.
#[derive(Debug, thiserror::Error)]
pub enum SnapshotError {
    /// The underlying metadata store failed.
    #[error("checkpoint store failed: {0}")]
    Storage(#[from] MetadataError),
    /// The stored snapshot belongs to another epoch.
    #[error("checkpoint belongs to epoch {actual}, expected {expected}")]
    EpochMismatch { expected: Epoch, actual: Epoch },
}

impl<E, V, D> SnapshotStore<E, V, D>
where
    E: StorageContext,
    V: Variant,
    D: Digest,
{
    /// Opens the store and returns the newest durable snapshot, if any.
    pub async fn open(
        context: E,
        partition: String,
        codec: SnapshotCodecConfig,
        epoch: Epoch,
    ) -> Result<(Self, Option<Snapshot<V, D>>), SnapshotError> {
        let inner = Metadata::init(
            context,
            MetadataConfig {
                partition,
                codec_config: codec,
            },
        )
        .await?;
        let snapshot: Option<Snapshot<V, D>> = inner.get(&U64::new(SNAPSHOT_KEY)).cloned();
        if let Some(snapshot) = snapshot.as_ref()
            && snapshot.epoch() != epoch
        {
            return Err(SnapshotError::EpochMismatch {
                expected: epoch,
                actual: snapshot.epoch(),
            });
        }
        Ok((Self { inner }, snapshot))
    }

    /// Durably replaces the newest snapshot.
    ///
    /// Blocks until the swap is synced; callers off the hot path may run it on a task and
    /// reclaim the store from the handle.
    pub async fn store(self, snapshot: Snapshot<V, D>) -> Result<Self, SnapshotError> {
        let inner = self
            .inner
            .put_sync(U64::new(SNAPSHOT_KEY), snapshot)
            .await?;
        Ok(Self { inner })
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::multimmit::{
        config::{Profile, Role, Tuning},
        machine::{CoreState, Cursor},
        mocks::Committee,
    };
    use commonware_cryptography::{
        Sha256, bls12381::primitives::variant::MinPk, sha256::Digest as Sha256Digest,
    };
    use commonware_runtime::{
        Blob as _, Runner as _, Spawner as _, Storage as _, Supervisor as _, WriteOptions,
        deterministic,
        mocks::{
            DeferredSync, DelayedSyncContext, PendingSyncs, WriteFaultContext, WriteFaults,
            next_pending_sync,
        },
    };

    const PARTITION: &str = "multimmit-checkpoints";

    type TestSnapshot = Snapshot<MinPk, Sha256Digest>;

    /// Returns the cursor of the snapshot `store` holds, or zero when it is empty.
    fn covered<E: StorageContext>(store: &SnapshotStore<E, MinPk, Sha256Digest>) -> Cursor {
        store
            .inner
            .get(&U64::new(SNAPSHOT_KEY))
            .map_or(Cursor::zero(), Snapshot::cursor)
    }

    struct Fixture {
        epoch: Epoch,
        codec: SnapshotCodecConfig,
        first: TestSnapshot,
        replacement: TestSnapshot,
    }

    fn fixture() -> Fixture {
        let epoch = Epoch::new(7);
        let committee = Committee::<MinPk>::builder(7, 1).build();
        let profile: Profile<Sha256Digest> =
            Profile::new::<MinPk>(committee.config, Role::Observer, Tuning::default()).unwrap();
        let codec = SnapshotCodecConfig::from_profile(&profile);
        let snapshot = CoreState::<Sha256, MinPk>::fresh(profile)
            .unwrap()
            .machine()
            .live_snapshot_for_test();
        let first = snapshot.clone().at_cursor_for_test(Cursor::new(7));
        let replacement = snapshot.at_cursor_for_test(Cursor::new(11));

        Fixture {
            epoch,
            codec,
            first,
            replacement,
        }
    }

    #[test]
    fn opens_empty() {
        let fixture = fixture();
        deterministic::Runner::default().start(|context| async move {
            let (store, snapshot) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                context.child("open"),
                PARTITION.into(),
                fixture.codec,
                fixture.epoch,
            )
            .await
            .unwrap();

            assert_eq!(snapshot, None);
            assert_eq!(covered(&store), Cursor::zero());
        });
    }

    #[test]
    fn stores_and_reopens_exact_snapshot() {
        let fixture = fixture();
        deterministic::Runner::default().start(|context| async move {
            let (store, snapshot) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                context.child("open"),
                PARTITION.into(),
                fixture.codec,
                fixture.epoch,
            )
            .await
            .unwrap();
            assert_eq!(snapshot, None);

            let store = store.store(fixture.first.clone()).await.unwrap();
            assert_eq!(covered(&store), fixture.first.cursor());
            drop(store);

            let (store, snapshot) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                context.child("reopen"),
                PARTITION.into(),
                fixture.codec,
                fixture.epoch,
            )
            .await
            .unwrap();
            assert_eq!(snapshot, Some(fixture.first.clone()));
            assert_eq!(covered(&store), fixture.first.cursor());
        });
    }

    #[test]
    fn replaces_snapshot() {
        let fixture = fixture();
        deterministic::Runner::default().start(|context| async move {
            let (store, _) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                context.child("open"),
                PARTITION.into(),
                fixture.codec,
                fixture.epoch,
            )
            .await
            .unwrap();
            let store = store.store(fixture.first).await.unwrap();
            let store = store.store(fixture.replacement.clone()).await.unwrap();
            drop(store);

            let (store, snapshot) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                context.child("reopen"),
                PARTITION.into(),
                fixture.codec,
                fixture.epoch,
            )
            .await
            .unwrap();
            assert_eq!(snapshot, Some(fixture.replacement.clone()));
            assert_eq!(covered(&store), fixture.replacement.cursor());
        });
    }

    #[test]
    fn rejects_snapshot_from_another_epoch() {
        let fixture = fixture();
        deterministic::Runner::default().start(|context| async move {
            let (store, _) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                context.child("open"),
                PARTITION.into(),
                fixture.codec,
                fixture.epoch,
            )
            .await
            .unwrap();
            let store = store.store(fixture.first).await.unwrap();
            drop(store);

            let result = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                context.child("wrong_epoch"),
                PARTITION.into(),
                fixture.codec,
                Epoch::new(fixture.epoch.get() + 1),
            )
            .await;
            assert!(matches!(
                result,
                Err(SnapshotError::EpochMismatch { expected, actual })
                    if expected == Epoch::new(fixture.epoch.get() + 1) && actual == fixture.epoch
            ));
        });
    }

    #[test]
    fn checksum_mismatch_recovers_previous_snapshot() {
        let fixture = fixture();
        deterministic::Runner::default().start(|context| async move {
            let (store, _) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                context.child("open"),
                PARTITION.into(),
                fixture.codec,
                fixture.epoch,
            )
            .await
            .unwrap();
            let store = store.store(fixture.first.clone()).await.unwrap();
            let store = store.store(fixture.replacement).await.unwrap();
            drop(store);

            let (current, _) = context.open(PARTITION, b"left").await.unwrap();
            current
                .write_at(0, b"malformed".to_vec(), WriteOptions::SYNC)
                .await
                .unwrap();
            drop(current);

            let (store, snapshot) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                context.child("reopen"),
                PARTITION.into(),
                fixture.codec,
                fixture.epoch,
            )
            .await
            .unwrap();
            assert_eq!(snapshot, Some(fixture.first.clone()));
            assert_eq!(covered(&store), fixture.first.cursor());
        });
    }

    #[test]
    fn failed_replacement_preserves_previous_snapshot() {
        let fixture = fixture();
        deterministic::Runner::default().start(|context| async move {
            let faults = WriteFaults::default();
            let storage = WriteFaultContext {
                inner: context.child("open"),
                faults: faults.clone(),
            };
            let (store, _) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                storage,
                PARTITION.into(),
                fixture.codec,
                fixture.epoch,
            )
            .await
            .unwrap();
            let store = store.store(fixture.first.clone()).await.unwrap();

            faults.arm();
            let result = store.store(fixture.replacement).await;
            assert!(matches!(result, Err(SnapshotError::Storage(_))));
            faults.disarm();

            let storage = WriteFaultContext {
                inner: context.child("reopen"),
                faults,
            };
            let (store, snapshot) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                storage,
                PARTITION.into(),
                fixture.codec,
                fixture.epoch,
            )
            .await
            .unwrap();
            assert_eq!(snapshot, Some(fixture.first.clone()));
            assert_eq!(covered(&store), fixture.first.cursor());
        });
    }

    #[test]
    fn crash_during_replacement_recovers_old_then_completed_replacement_recovers_new() {
        let fixture = fixture();
        let epoch = fixture.epoch;
        let codec = fixture.codec;
        let first = fixture.first;
        let replacement = fixture.replacement;
        let crash_first = first.clone();
        let crash_replacement = replacement.clone();
        let (_, checkpoint) =
            deterministic::Runner::default().start_and_recover(|context| async move {
                let pending = PendingSyncs::default();
                let storage = DelayedSyncContext {
                    inner: context.child("open"),
                    pending: pending.clone(),
                };
                let (store, _) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                    storage,
                    PARTITION.into(),
                    codec,
                    epoch,
                )
                .await
                .unwrap();
                let store = store.store(crash_first).await.unwrap();

                pending.arm();
                let DeferredSync { release, blocked } = next_pending_sync(&pending);
                let _swap = context
                    .child("swap")
                    .spawn(move |_| async move { store.store(crash_replacement).await });
                blocked.await.unwrap();
                drop(release);
            });

        let recovered_first = first;
        let completed_replacement = replacement.clone();
        let (_, checkpoint) =
            deterministic::Runner::from(checkpoint).start_and_recover(|context| async move {
                let (store, snapshot) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                    context.child("recover_old"),
                    PARTITION.into(),
                    codec,
                    epoch,
                )
                .await
                .unwrap();
                assert_eq!(snapshot, Some(recovered_first));

                let store = store.store(completed_replacement.clone()).await.unwrap();
                assert_eq!(covered(&store), completed_replacement.cursor());
            });

        deterministic::Runner::from(checkpoint).start(|context| async move {
            let (store, snapshot) = SnapshotStore::<_, MinPk, Sha256Digest>::open(
                context.child("recover_new"),
                PARTITION.into(),
                codec,
                epoch,
            )
            .await
            .unwrap();
            assert_eq!(snapshot, Some(replacement.clone()));
            assert_eq!(covered(&store), replacement.cursor());
        });
    }
}
