...
use commonware_utils::{
    N5f1, Participant, TryFromIterator, non_empty,
    ordered::{BiMap, Set},
};
...
impl<P: PublicKey, V: Variant> Scheme<P, V> {
    ...
    fn aggregate(
        ...
        (signature.element() != &V::Signature::zero())
            .then_some(signature)
            .ok_or(Error::Signature)
    }

    fn verify_aggregate<R: CryptoRng>(
        &self,
        rng: &mut R,
        transcript: &[TranscriptEntry<'_>],
        signature: &aggregate::Signature<V>,
        ...
        let Some(claim) = self.aggregate_claim(transcript, signature) else {
            return false;
        };
        let mut verifier = batch::Verifier::<V>::new(1);
        verifier.queue(
            claim.signature,
            claim
                .terms
        ...
        verifier.verify(rng, strategy).is_empty()
    }
...
mod tests {
    ...
    fn roster_rejects_rogue_keys<V: Variant>() {
        ...
        assert!(verifier.verify(&mut rng, &Sequential).is_empty());
