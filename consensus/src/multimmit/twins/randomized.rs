...
use crate::{
    ...
    multimmit::{
        fuzz::ByteSchedule,
        types::{
            Activity, Anchor, BlockRef, CertificateId, DigestedLeader, FinalityFact, FinalityId,
            LeaderBlock, Lqc, TransactionBlockHeader, VoteBody, Vqc,
        },
    },
    types::{Attributable as _, Height},
};
...
use {
    crate::{
        ...
        multimmit::{
            mocks::Committee,
            types::{SignedTransactionBlock, ViewMessage},
        },
        types::Round,
    },
    commonware_codec::Encode as _,
};
...
/// Drops, holds, duplicates, and delays frames by a byte script until a deadline, then delivers
/// fairly.
struct RandomizedDelivery {
    script: [u8; 16],
    deadline: SystemTime,
...
impl<E> PumpStrategy<E> for RandomizedDelivery
where
    E: Debug + std::error::Error + Send + Sync + 'static,
{
    type Tag = ();
    type Observer = WireEvidence;
    ...
    fn taken(evidence: &Self::Observer, frame: &Frame<E>, (): Self::Tag) {
        if let Ok(message) = frame {
            evidence.record_accountability(&message.1);
        }
    }
}
...
impl AccountabilityOracle {
    ...
    fn record_vqc(&mut self, observer: usize, certificate: &Vqc<MinPk, Sha256Digest>) {
        ...
        for signer in certificate.tally().signers().iter() {
            ...
            let vote = certificate
                .tally()
                .vote(digested, signer, self.codec)
    ...
    fn record_lqc(&mut self, observer: usize, certificate: &Lqc<MinPk, Sha256Digest>) {
        if self
            .verifier
        ...
        let leader = DigestedLeader::new::<Sha256>(certificate.leader());
        for signer in certificate.tally().signers().iter() {
            if !self.targets.contains(&signer) {
                continue;
            }
...
impl Agreement {
    ...
    fn record_vote_body(&mut self, node: usize, body: &VoteBody<Sha256Digest>) {
        ...
        if !body.valid_for(DigestedLeader::new::<Sha256>(&leader)) {
            return;
        }
            ...
            for (index, ancestor) in known.iter().copied().enumerate() {
                for descendant in known[index + 1..].iter().copied() {
                    if let Some(compatible) = self.descends_from(descendant, ancestor) {
                        assert!(
                            compatible,
                            "honest nodes finalized incompatible producer histories on chain \
                             {chain_id} at heights {} and {} (latest reporter: {node})",
                            ancestor.height(),
                            descendant.height(),
                        );
                    }
    ...
    fn record_vqc(
        ...
        let digested = DigestedLeader::new::<Sha256>(certificate.leader());
        let leader = digested.digest();
        self.record_leader_block(node, certificate.leader());
        for signer in certificate.tally().signers().iter() {
            let vote = certificate
                .tally()
                .vote(digested, signer, codec)
    ...
    fn record_wire(
        ...
        let Some(bounds) = codec.encoded_bounds::<MinPk, Sha256Digest>() else {
            return;
        };
        ...
        for (index, (ancestor_view, ancestor)) in finalized.iter().copied().enumerate() {
            for (_, descendant) in finalized[index + 1..].iter().copied() {
                    ...
                    self.leader_descends_from(descendant, ancestor, ancestor_view)
                {
                    assert!(
                        compatible,
                        "honest nodes finalized incompatible leader histories \
                         (latest reporter: {node})"
                    );
                }
            }
        }
        ...
        for block in blocks {
            ...
            if let Some(previous) = chain.insert(block.height().get(), *block) {
                assert_eq!(
                    previous,
                    *block,
                    "honest nodes finalized different producer blocks at chain {} height {} \
                     (latest reporter: {node})",
                    block.chain(),
                    block.height()
                );
...
pub(crate) fn exercise(input: &[u8]) -> BTreeSet<(usize, usize)> {
    let seed = u64::from_le_bytes(ByteSchedule::padded(input).take());
    let mut schedule = ByteSchedule::cycling(&input[..input.len().min(MAX_SCHEDULE_BYTES)]);
    ...
    let leaders = LeaderSchedule::from_order(
        leader_order
            .iter()
            .copied()
    ...
    executor.start(move |context| async move {
        ...
        for &participant in &honest {
            ...
            for plane in 0..4 {
                ...
                let receiver = PumpedReceiver::spawn(
                    context
                        .child("random_delivery")
                        .with_attribute("participant", participant)
                        .with_attribute("plane", plane),
                    receiver,
                    RandomizedDelivery {
                        script: delivery_scripts[participant][plane as usize],
                        deadline,
            ...
            cluster
                .launch_over(
                    LaunchSpec::new(participant).reporter(safety.reporter(participant)),
                    Planes {
                        data,
                        consensus,
                        certificates,
        ...
        for from in 0..participants as usize {
            for to in 0..participants as usize {
                if from != to {
                    cluster.set_directed_success_rate(from, to, 1.0).await;
                }
            }
        }
        ...
        for snapshot in snapshots.iter() {
            assert!(
                ...
                "honest observer {} blocked participant {} before receiving authenticated \
                 conflicting claims from that signer",
                snapshot.observer,
                snapshot.target,
            );
        ...
        for (&participant, slots) in byzantine.iter().zip(&twins) {
            ...
            let selective = slots.count_selective_broadcasts(participant, &scenario, &identities);
        ...
        assert!(
            complete,
            "seed={seed}, byzantine={byzantine:?}, coverage={reached:?}: \
             fair suffix must finalize honest producers"
        );
...
fn accountability_oracle_requires_delivered_verified_evidence() {
    let fixture = Committee::<MinPk>::builder(7, PARTICIPANTS).build();
    let mut oracle = oracle(&fixture);
    let first = fixture.signed_block(ChainId::new(0), Sha256::hash(&[b"first"]));
    let second = fixture.signed_block(ChainId::new(0), Sha256::hash(&[b"second"]));
...
fn accountability_oracle_covers_wire_equivocation_families() {
    let fixture = Committee::<MinPk>::builder(11, PARTICIPANTS).build();
    let mut oracle = oracle(&fixture);

    let first = fixture.signed_block(ChainId::new(0), Sha256::hash(&[b"parent"]));
    ...
    .unwrap();
    let adjacent = fixture.signers[0].sign_transaction_block(adjacent).unwrap();
    ...
    let plain_leader = fixture.leader_block(View::new(2));
    let parent = fixture.vqc(View::new(1));
    let parented_leader = fixture.leader_block_with_parent(View::new(2), &parent);
    deliver_consensus(
        &mut oracle,
        &fixture,
        1,
    ...
    let first_vote = fixture.vote(Participant::new(3), &plain_leader);
    let second_vote = fixture.vote(Participant::new(3), &parented_leader);
    ...
    let vote = fixture.vote(Participant::new(4), &plain_leader);
    let novote = fixture.novote(Participant::new(4), View::new(2));
    ...
    let first_header = fixture.transaction_header(ChainId::new(0), Sha256::hash(&[b"da first"]));
    let second_header = fixture.transaction_header(ChainId::new(0), Sha256::hash(&[b"da second"]));
    deliver_data(
        &mut oracle,
        &fixture,
        4,
        DataMessage::DaVote(fixture.da_vote(Participant::new(5), first_header)),
    );
    deliver_data(
        &mut oracle,
        &fixture,
        4,
        DataMessage::DaVote(fixture.da_vote(Participant::new(5), second_header)),
    );
...
fn accountability_oracle_expands_authenticated_certificate_transcripts() {
    let fixture = Committee::<MinPk>::builder(13, PARTICIPANTS).build();
    let mut oracle = oracle(&fixture);
    let leader = fixture.leader_block(View::new(1));
    let messages =
        std::iter::once(ViewMessage::NoVote(
            fixture.novote(Participant::new(0), View::new(1)),
        ))
        .chain((1..fixture.codec().view_quorum()).map(|signer| {
            ViewMessage::Vote(fixture.vote(Participant::from_usize(signer), &leader))
        }))
        .collect::<Vec<_>>();
    let vqc = fixture
        .verifier
    ...
    deliver_certificate(
        ...
        &fixture,
        0,
        CertificateMessage::Lqc(fixture.lqc(View::new(1))),
    );
    assert!(
...
fn randomized_twins_smoke() {
    exercise(&0u64.to_le_bytes());
    exercise(&[10, 10, 1]);
}

#[test]
fn randomized_twins_correlates_blocks_with_observer_evidence() {
    // These placement-shuffle draws swap only slots 0 and 3, making participant 3 the twin.
    // The zero-filled remainder gives observer 0 lossless delivery of both producer claims.
    let mut input = [0; MAX_SCHEDULE_BYTES];
    input[2..7].copy_from_slice(&[5, 4, 0, 2, 1]);
    let outcome = exercise(&input);
    assert!(outcome.contains(&(0, 3)), "objective blocks: {outcome:?}");
}
