...
async fn launch_scenario_twins(
    ...
    let (primary, secondary) = launch_halves(cluster, byzantine, primary, secondary, labels).await;
    TwinSlots {
        primary,
        secondary,
        traffic,
...
fn vote_counts_for(
    ...
    if position == 1 {
        return proposal.payloads().first() == Some(&target.commitment());
    }
    if position != 0 {
        return false;
    }
    vote.body().extensions()[chain].payloads().first() == Some(&target.commitment())
}
...
fn proposal_extends(
    ...
    target.parent() == anchor.digest()
        ...
        && chain_proposal.payloads().first() == Some(&target.commitment())
}
...
fn well_disseminated_block_survives_byzantine_leader_censorship() {
    ...
    executor.start(|context| async move {
        ...
        launch_traced_honest(&mut cluster, &honest, &trace).await;
