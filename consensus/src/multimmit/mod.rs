...
//! application when a transaction header should be disseminated outside the protocol wire, using
//! the complete canonical header digest rather than the opaque application-body commitment.
//!
//! [`crate::Automaton::propose`] selects a complete local payload and returns its commitment without
//! waiting for durability. Multimmit may prepare descendants from that commitment, then calls
//! [`crate::Automaton::verify`] for each exact local header. Returning `true` is the custody fence:
//! the attached system promises that the payload is valid, locally available, and reconstructible
//! after a crash. A temporary lack of bytes or durable custody keeps verification pending, and a
//! local `false` contradicts the earlier proposal. Multimmit reserves no transaction-header signing
//! subject before the matching custody fence. Remote transaction blocks use the same verification
//! contract, where `false` means permanently invalid, before becoming eligible for a DA share.
