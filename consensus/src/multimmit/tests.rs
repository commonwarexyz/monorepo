//! Deterministic multi-node Multimmit integration tests.
//!
...
use super::{
    mocks::{
        MockApplication,
        cluster::{Cluster, ClusterOptions, Plane},
    },
    test_utils::benchmarks::{
        ENGINE_BLOCKS_PER_CHAIN, ENGINE_NODES, ENGINE_VIEW_ADVANCE, run_engine_profile,
    },
};
use crate::{
    Epochable as _, Roundable as _, Viewable as _,
    multimmit::{
        config::{Profile, Role, Tuning},
        machine::{Generation, Inspection},
        testing::metric_total,
        types::{ChainId, Context, TransactionBlockHeader, ViewProof},
...
fn producer_subset_progresses_with_validator_only_nodes() {
    ...
    executor.start(|context| async move {
        ...
        cluster
            .wait_finalized_all(2, Duration::from_secs(240))
            .await;
        ...
        cluster
            .wait_finalized_all(3, Duration::from_secs(240))
            .await;
...
fn assert_profile_resource_bounds(
    ...
    assert_eq!(inspection.chain_progress().len(), profile.codec().chains());
    if let Some(producer) = inspection.producer() {
        assert_eq!(
            producer.pipeline_depth(),
            u64::try_from(profile.codec().pipeline_depth())
...
fn checkpoint_compaction_soak_survives_a_bounded_restart() {
    ...
    executor.start(|context| async move {
        ...
        for target in 1..=BUILDS_PER_CHAIN {
            ...
            cluster.wait_produced(&all, target, Duration::from_secs(120)).await;
        }
        ...
        assert_eq!(recovered.generation(), Generation::new(before.generation().get() + 1));
        cluster.observe_finality(&all).await;
        ...
        // crash, it retains the first section the durable checkpoint does not cover, and a
        // checkpoint published right after recovery prunes everything below the live section,
        // whose blob only appears on its first append. The soak above already observed two
        // completed rolls, so nothing beyond that uncovered section and the live suffix may
        // survive the restart.
        let sections = cluster.journal_sections(5).await;
        assert!(
            sections.len() <= 2,
            "restart retained more than the uncovered prefix and the live suffix: {sections:?}"
        );

        cluster.produce_once();
        cluster
            .wait_finalized_all(BUILDS_PER_CHAIN + 1, Duration::from_secs(360))
            .await;
        cluster.observe_finality(&all).await;
    });
}
            ...
            for _ in 0..4000 {
                ...
                if lowest >= target {
                    for node in 0..NODES {
                        ...
                        // The post-checkpoint section is created lazily by its first append, so
                        // a fully covered journal may have no physical suffix.
                        let sections = cluster.journal_sections(node).await;
                        assert!(
                            sections.len() <= 2,
...
fn n7_f1_cluster_survives_crash_and_recovery() {
    ...
    executor.start(|context| async move {
        ...
        cluster
            .wait_finalized_all(1, Duration::from_secs(120))
            .await;
        ...
        cluster
            .wait_finalized(&rest, &[0, 1, 2, 3, 4, 5], 2, Duration::from_secs(120))
            .await;
        ...
        cluster
            .wait_finalized_all(3, Duration::from_secs(240))
            .await;
        cluster.observe_finality(&cluster.all_nodes()).await;
    });
}
...
fn run_hailstorm(seed: u64) -> String {
    ...
    executor.start(|context| async move {
        ...
        cluster
            .wait_finalized_all(finalized_height, Duration::from_secs(240))
            .await;
        ...
        for crashed in CRASHES {
            ...
            cluster
                .wait_view(&survivors, survivor_view, Duration::from_secs(24))
                .await;
            ...
            cluster
                .wait_view(
                    &all,
                    View::new(survivor_view.get() + 2),
                    Duration::from_secs(36),
                )
                .await;
            finalized_height += 1;
            cluster
                .wait_finalized_all(finalized_height, Duration::from_secs(360))
                .await;
            cluster.observe_finality(&all).await;
        }
...
fn staggered_startup_recovers_after_initial_asynchrony() {
    ...
    executor.start(|context| async move {
        ...
        for height in 1..=3 {
            cluster
                .wait_finalized_all(height, Duration::from_secs(360))
                .await;
            cluster.observe_finality(&all).await;
        }
    });
...
fn n11_f2_finalizes_under_partition_and_heal() {
    ...
    executor.start(|context| async move {
        ...
        cluster.wait_finalized_all(1, Duration::from_secs(60)).await;
        ...
        cluster.heal().await;
        cluster.wait_finalized_all(2, Duration::from_secs(60)).await;
        cluster.observe_finality(&cluster.all_nodes()).await;
    });
}
...
fn stalled_producer_does_not_block_other_chains() {
    ...
    executor.start(|context| async move {
        ...
        let starting_view = cluster
            ...
            .view();
        cluster
            .wait_finalized(&all, &[0, 1, 2, 3, 4], 3, Duration::from_secs(240))
            .await;
        cluster
            .wait_view(
                &all,
                View::new(starting_view.get() + 12),
...
fn lossy_jittered_network_converges_after_healing() {
    ...
    executor.start(|context| async move {
        ...
        cluster.produce_once();
        cluster
            .observe_finality_progress(&all, Duration::from_secs(5))
            .await;
        cluster.stop_producing();
        cluster.heal().await;
...
async fn wait_served(
    ...
    view: View,
    rounds: usize,
    accepts: impl Fn(&ViewProof<MinPk, Sha256Digest>) -> bool,
) -> bool {
    for _ in 0..rounds {
        if cluster
            .serve(node, view)
...
fn duplicated_reordered_certificates_converge_after_healing() {
    ...
    executor.start(|context| async move {
        ...
        let target_view = cluster
            ...
            .view()
            .get();
        let current = fixture.vqc(View::new(target_view));
        let earlier = fixture.vqc(View::new(target_view + 1));
        let later = fixture.vqc(View::new(target_view + 2));
        ...
        assert!(
            wait_served(
                ...
                400,
                |proof| matches!(proof, ViewProof::Vqc(proof) if proof.as_ref() == &later),
            )
            .await,
        ...
        assert!(
            wait_served(
                ...
                400,
                |proof| matches!(proof, ViewProof::Vqc(proof) if proof.as_ref() == &earlier),
            )
            .await,
        ...
        assert!(
            wait_served(
                ...
                400,
                |proof| match proof {
                    ViewProof::Vqc(proof) => proof.as_ref() == &earlier,
                    ViewProof::Lqc(proof) => proof.view() >= requested,
                    _ => false,
                }
            )
            .await,
        ...
        assert!(
            wait_served(
                ...
                400,
                |proof| match proof {
                    ViewProof::Vqc(proof) => proof.as_ref() == &current,
                    ViewProof::Lqc(proof) => proof.view() >= View::new(target_view),
                    _ => false,
                },
            )
            .await,
        ...
        cluster
            .observe_finality_progress(&all, Duration::from_secs(2))
            .await;

        cluster.heal().await;
        cluster.produce_once();
        cluster
            .wait_finalized_all(2, Duration::from_secs(360))
            .await;
        cluster.observe_finality(&all).await;
    });
}
...
fn validator_recovers_durable_state_within_retention_window() {
    ...
    executor.start(|context| async move {
        ...
        cluster.produce_once();
        cluster
            .wait_produced(&[5], 1, Duration::from_secs(120))
            .await;
        cluster
            .wait_finalized_all(1, Duration::from_secs(240))
            .await;
        ...
        cluster.crash(5).await;
        cluster
            .wait_view(&peers, recovery_view, Duration::from_secs(12))
            .await;
        for peer in peers {
            let view = cluster
                .inspect(peer)
        ...
        cluster.observe_finality(&all).await;
        cluster
            .wait_view(&all, recovery_view, Duration::from_secs(12))
            .await;

        cluster.produce_once();
        cluster
            .wait_produced(&[5], produced_before + 1, Duration::from_secs(240))
            .await;
        cluster
            .wait_finalized_all(finalized_height_before + 1, Duration::from_secs(360))
            .await;
...
fn all_validators_recover_durable_timeout_batch_before_certificate() {
    ...
    executor.start(|context| async move {
        ...
        for &node in &online {
            ...
            assert_eq!(
                recovered.generation(),
                Generation::new(generations[&node].get() + 1)
            );
        }
        ...
        for &node in &online {
            ...
            match retained
                .expect("engine retains evidence for the recovered view")
                .as_ref()
            {
                ...
                ViewProof::Vqc(proof) => assert_eq!(proof.view(), stalled),
                ViewProof::Lqc(proof) => assert!(proof.view() >= stalled),
            }
        }
    });
}
...
fn validator_beyond_retention_resumes_from_covering_lqc_before_new_epoch_bootstrap() {
    ...
    executor.start(|context| async move {
        ...
        old.start_all().await;
        old.produce_once();
        old.wait_produced(&[OFFLINE], 1, Duration::from_secs(120))
            .await;
        old.wait_finalized_all(1, Duration::from_secs(240)).await;
        old.stop_producing();
        ...
        for peer in peers {
            assert!(
                wait_served(&context, &old, peer, view, 200, |proof| match proof {
                    ViewProof::Nullification(proof) => proof.view() == stopped_view,
                    ViewProof::Vqc(proof) => proof.view() == stopped_view,
                    ViewProof::Lqc(proof) => proof.view() >= stopped_view,
                })
                .await,
        ...
        old.wait_view(&peers, beyond_retention, Duration::from_secs(24))
            .await;
        for peer in peers {
        ...
        for peer in peers {
            assert!(
                wait_served(&context, &old, peer, view, 400, |proof| {
                    matches!(proof, ViewProof::Lqc(proof) if proof.view() >= stopped_view)
                })
                .await,
                "peer {peer} does not serve an L-QC covering the retired view"
            );
            old.block(Plane::Certificates, peer, OFFLINE);
        }
        ...
        old.observe_finality_progress(&[OFFLINE], Duration::from_secs(2))
            .await;
        let resumed = old
            .inspect(OFFLINE)
            .await
        ...
        old.produce_once();
        old.wait_produced(&[OFFLINE], produced_before + 1, Duration::from_secs(240))
            .await;
        old.wait_finalized_all(finalized_height_before + 1, Duration::from_secs(360))
            .await;
        assert!(application.lock().built > built_before);
        old.observe_finality(&old.all_nodes()).await;
        ...
        new.produce_once();
        new.wait_finalized_all(1, Duration::from_secs(240)).await;
...
fn resolver_recovers_exact_view_proof_after_rearmed_restart() {
    ...
    executor.start(|context| async move {
        ...
        let mut cluster = Cluster::<MinPk>::new(
            ...
            ClusterOptions {
                extras: 1,
                view_retention: ViewDelta::new(128),
        ...
        cluster.block(Plane::Certificates, SERVER, TARGET);
        ...
        let fixture = cluster.fixture();
        let epoch = fixture.config.epoch();
        let certificate = fixture.vqc(crashed_view);
        let (mut sender, _receiver) = cluster.tap(EXTRA, 2).await;
        let _ = sender.send(
        ...
        assert!(
            wait_served(&context, &cluster, SERVER, key, 200, |proof| {
                matches!(proof, ViewProof::Vqc(proof) if proof.as_ref() == &certificate)
            })
            .await,
...
fn authenticated_flood_services_engine_control_and_every_da_chain() {
    ...
    executor.start(|context| async move {
        ...
        for chain_index in 0..5u32 {
            let chain = ChainId::new(chain_index);
            let producer = fixture.config.producer(chain).unwrap();
            let mut parent = fixture.config.genesis().tips()[chain_index as usize].digest();
            for height in 1..=fixture.config.codec_config().pipeline_depth() as u64 {
                let height = Height::new(height);
                let context = Context::new(epoch, chain, height, parent).unwrap();
                let payload = format!("mock payload {}", height.get());
                let body = MockApplication::block_digest(context, payload.as_bytes());
                let header =
                    TransactionBlockHeader::new(epoch, chain, height, parent, body).unwrap();
                parent = header.digest::<Sha256>();
                let block = fixture.signers[producer.get() as usize]
                    .sign_transaction_block(header)
                    .unwrap();
                flood.push(Envelope::new(epoch, DataMessage::Block(block)).encode());
            }
        }
        ...
        let timeout_before = metric_total(&context.encode(), "voter_view_timeouts_total");
        cluster.produce();
        ...
        for _ in 0..400 {
            ...
            let timer_serviced =
                metric_total(&context.encode(), "voter_view_timeouts_total") > timeout_before;
        ...
        assert!(
            cluster.blocked_peers().await.is_empty(),
            "canonical authenticated traffic must not trigger peer blocking"
        );
    ...
    executor.start(|context| async move {
        ...
        let mut cluster = Cluster::<MinPk>::new(
            ...
            ClusterOptions {
                ...
                production: Duration::from_millis(250),
                view_retention: ViewDelta::new(64),
...
fn three_sequential_committees_rotate() {
    ...
    executor.start(|context| async move {
        ...
        for epoch_index in 0..3u64 {
            ...
            cluster
                .wait_finalized_all(1, Duration::from_secs(360))
                .await;
...
fn node_with_hung_verifications_keeps_following_finality() {
    ...
    executor.start(|context| async move {
        ...
        cluster
            .wait_view(&all, target, Duration::from_secs(96))
            .await;
        cluster
            .wait_finalized(
                &[0, 1, 2, 3, 4],
...
fn late_first_start_catches_up_beyond_retention() {
    ...
    executor.start(|context| async move {
        ...
        let chains = cluster.all_chains();
        cluster
            .wait_finalized(&running, &chains, 1, Duration::from_secs(60))
            .await;
        let held = cluster
            .inspect(0)
            .await
...
fn early_crash_long_gap_restart_catches_up() {
    ...
    executor.start(|context| async move {
        ...
        let held = cluster
            ...
            .view();
        cluster
            .wait_view(&rest, View::new(held.get() + 128), Duration::from_secs(96))
            .await;
...
// network observation through DA-vote signing. Each limit sits a few histogram buckets above the
// slowest profile's reproducible baseline, which leaves deterministic scheduling headroom while
// still failing on a regression.
...
#[test]
fn engine_profile_latency_stays_within_gates() {
    for storage_delay in [false, true] {
        let report = run_engine_profile(storage_delay).report;
        assert_eq!(report.blocks, ENGINE_NODES as u64 * ENGINE_BLOCKS_PER_CHAIN);
        assert!(report.views >= ENGINE_VIEW_ADVANCE);
        assert!(report.workload_views >= ENGINE_VIEW_ADVANCE);
        assert!(report.build_samples >= 100);
        assert!(report.da_vote_samples >= 50);
        assert!(report.build_p95 <= BUILD_P95_LIMIT, "{report:?}");
        assert!(report.build_p99 <= BUILD_P99_LIMIT, "{report:?}");
        assert!(report.da_vote_p95 <= DA_VOTE_P95_LIMIT, "{report:?}");
        assert!(report.da_vote_p99 <= DA_VOTE_P99_LIMIT, "{report:?}");
    }
}
