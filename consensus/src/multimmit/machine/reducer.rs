...
pub enum StepStatus<D: Digest> {
    ...
    BlockBuilt,
    /// A matched local block-custody completion was accepted.
    BlockCustodied,
...
impl<H: Hasher, V: Variant> Machine<H, V> {
    ...
    pub(super) fn step(
        ...
        let mut step = match input {
            ...
            Input::BlockCustodied(completion) => {
                self.ensure_live()?;
                self.complete_block_custody(completion)
            }
    ...
    pub(crate) fn step_observations(
        ...
        let report_admissions = self.lifecycle == Lifecycle::Live;
        self.ensure_live()?;
        let mut step = self.observe_artifacts(artifacts)?;
        if report_admissions {
        ...
        while processed < budget && pass.phase != VerificationPassPhase::Complete {
            match pass.phase {
                ...
                VerificationPassPhase::Apply => {
                    ...
                    let da_equivocator = pass.completion.take_da_equivocator(pass.position);
                    if let Some(verdict_valid) = self.apply_verification_verdict(
                        verdict,
                        validated_vqc,
                        validated_lqc,
                        da_equivocator,
            ...
            match pass.phase {
                ...
                SigningBatchPassPhase::Prepare => {
                    ...
                    if let SignRequest::DaVote(da_request) = request {
                        ...
                        // A batch may carry several chains and, per chain, one run of strictly
                        // consecutive heights: each DA vote counts as sent for the next one's
                        // eligibility within the same authorization.
                        let run_ordered = pass
                            .da_runs
                            .get(&chain)
                            .is_none_or(|last| header.height() == last.next());
                        pass.da_runs.insert(chain, header.height());
                        pass.da_seen += 1;
                        pass.all_da_votes &= in_committee && run_ordered;
    ...
    fn drain_activities(&mut self) -> Vec<Activity<V, H::Digest>> {
        let finalized = self.finality.drain_finality_updates();
        if self.newly_ready.is_empty() && finalized.is_empty() {
            return Vec::new();
        }
        ...
        for (_, artifact_id, artifact) in accepted {
            if let Some(commitments) = self.views.selected_commitments(&artifact) {
    ...
    fn discard_activities(&mut self) {
        ...
        self.finality.clear_finality_updates();
    }
    ...
    fn complete_block_build(
        ...
        let status = match outcome {
            ...
            BuildOutcome::Empty | BuildOutcome::Prepared => StepStatus::BlockBuilt,
        };
        self.wake_components();
        Ok(Step::new(status, Vec::new()))
    }

    fn complete_block_custody(
        &mut self,
        ...
        self.wake_components();
        Ok(Step::new(StepStatus::BlockCustodied, Vec::new()))
    }

    fn complete_custody_cancellation(
        &mut self,
        cancellation: CustodyCancellation,
    ) -> Result<Step<V, H::Digest>, StepError> {
        if !self
            .chain
            .complete_custody_cancellation(cancellation, self.durable.generation)?
        {
            return Ok(Step::new(StepStatus::StaleCompletion, Vec::new()));
        }
        self.wake_components();
    ...
    fn next_durable_da_certificate(
        ...
        self.chain
            .next_certificate_above(&self.durable.certified_tips, preferred)
    ...
    fn take_chain_capabilities(&mut self) -> Result<Capabilities<V, H::Digest>, StepError> {
        ...
        for chain_effect in pending {
            match chain_effect {
                ...
                ChainEffect::Custody(job) => {
                    capabilities.push(Capability::Producer(ProducerCapability::Custody(job)));
                }
                ChainEffect::CancelCustody(cancellation) => {
                    capabilities.push(Capability::Producer(ProducerCapability::CancelCustody(
                        cancellation,
                    )));
                }
            ...
            let known = match artifact.as_ref() {
                ...
                Artifact::DaVote(vote) => self
                    ...
                    .filter_map(|candidate| {
                        let entry = self.artifacts.get(candidate)?;
    ...
    fn apply_verification_verdict(
        ...
        if verdict.valid() {
            ...
            let mut proven = self.accountability.observe::<H, V>(
                &artifact,
                prepared.as_ref(),
                self.profile.resources().max_cached_artifacts(),
            );
            if let Some(participant) = da_equivocator
                && self.accountability.observe_da_equivocator(participant)
            {
                proven.push(participant);
            }
            self.pending_blocks.extend(proven);
    ...
    fn finish_verification_prefix(
        ...
        let capabilities = if self.pending_blocks.is_empty() {
            Vec::new()
        } else {
            vec![Capability::Verification(VerificationCapability::Block(
                take(&mut self.pending_blocks),
            ))]
        };
        Ok(Step::new(
        ...
        while let Some(id) = ready.pop_first() {
            ...
            if self.lifecycle == Lifecycle::Live {
                ...
                if !self.artifacts[&id].view_observed {
                    self.views
                        .observe::<H>(id, observation, &artifact, ready_vqc, &self.profile)
                        .map_err(|_| StepError::ViewInvariant)?;
                    self.artifacts
                        .get_mut(&id)
    ...
    fn restore_ready_artifacts(&mut self) -> Result<Vec<Activity<V, H::Digest>>, StepError> {
        let mut ready = self
            .artifacts
            .iter()
            .filter_map(|(id, entry)| {
    ...
    fn forget_retired_artifacts(&mut self) -> Result<(), StepError> {
        ...
        self.accountability
            .retire(accountability_floor, &self.durable.certified_tips);
    ...
    fn apply_event(&mut self, event: &DomainEvent<V, H::Digest>) -> Result<(), ReplayError> {
        ...
        match event.change() {
            ...
            Change::DaCertificateAdvanced {
                ...
                let successor = publication
                    .as_ref()
                    .map(|_| DurableEffect::Broadcast(Arc::clone(artifact)));
                ...
                if block.height() <= floor.height()
                    ...
                    || successor
                        .as_ref()
                        .is_some_and(|effect| !effect.authorized(&self.profile))
                ...
                if let Some(id) = publication {
                    let successor = successor.expect("publication constructs a successor");
