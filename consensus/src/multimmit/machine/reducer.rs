...
use super::{
    ...
    DaRecoveryJob, DaVoteRequest, Dependency, DomainEvent,
...
impl<H: Hasher, V: Variant> Machine<H, V> {
    ...
    /// Reports new direct finality and every artifact promoted to ready since the last drain.
    ///
    ...
    fn drain_activities(&mut self) -> Vec<Activity<V, H::Digest>> {
        let finalized = self.finality.drain_newly_finalized();
        ...
        activities.extend(
            finalized
                .into_iter()
                .map(|fact| Activity::LeaderFinalized { fact }),
        );
    ...
    fn discard_activities(&mut self) {
        ...
        self.finality.clear_newly_finalized();
    }
