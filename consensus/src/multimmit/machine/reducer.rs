...
impl<H: Hasher, V: Variant> Machine<H, V> {
    ...
    fn drain_activities(&mut self) -> Vec<Activity<V, H::Digest>> {
        ...
        activities.extend(finalized.into_iter().map(|update| match update {
            FinalityUpdate::Finalized(fact) => Activity::LeaderFinalized { fact },
            FinalityUpdate::Advanced(fact) => Activity::LeaderFinalityUpdated { fact },
        }));
