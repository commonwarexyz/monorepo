...
pub(super) struct Machine<H: Hasher, V: Variant> {
    ...
    pub(crate) accountability: AccountabilityState<H::Digest>,
    pub(crate) pending_blocks: Vec<Participant>,
...
impl<H: Hasher, V: Variant> Machine<H, V> {
    ...
    pub fn new(profile: Profile<H, V>) -> Self {
        ...
        Self {
            ...
            jobs: BTreeMap::new(),
            future: BTreeSet::new(),
            accountability: AccountabilityState::new(),
    ...
    pub fn restore(
        ...
        machine
            .chain
            .reconcile_da_choices::<H>(da_headers)
            .map_err(|_| ReplayError::Transition)?;
    ...
    fn project_chain_progress(&self, finality: &[FinalityFact<H::Digest>]) -> Vec<ChainProgress> {
        ...
        self.chain
            ...
            .into_iter()
            .enumerate()
            .map(|(index, (known, certified, da_voted))| {
