use super::{
    accountability::AccountabilityState, algebra::VqcExtraction, tests::test_utils::cohort, *,
};
use crate::{
    Viewable,
    multimmit::{
        ...
        types::{
            Activity, Anchor, BlockRef, CertificateId, ChainId, ChainProposal, DaVote,
            EpochGenesis, Extension, Height, LeaderBlock, Position, TransactionBlockHeader,
...
fn blocked<V: Variant>(step: &Step<V, Digest>) -> Vec<Participant> {
    step.capabilities()
        ...
        .filter_map(|capability| match capability {
            Capability::Verification(VerificationCapability::Block(participants)) => {
                Some(participants.iter().copied())
            }
            _ => None,
        })
        .flatten()
...
fn execute_objective_equivocation_trace<V: Variant>() {
    ...
    let alternate_leader = LeaderBlock::new(
        leader.round(),
        leader.parent(),
        digest(b"alternate leader history", 1),
    ...
    let genesis = fixture.protocol.genesis().tips()[chain.get() as usize];
    let first_header = TransactionBlockHeader::new(
        fixture.protocol.epoch(),
        chain,
        Height::new(1),
        genesis.digest(),
        digest(b"producer body", 1),
    )
    .unwrap();
    let fork_header = TransactionBlockHeader::new(
        fixture.protocol.epoch(),
        chain,
        Height::new(1),
        genesis.digest(),
        digest(b"producer fork", 1),
    )
    .unwrap();
    assert_objective_fault(
        &fixture,
    ...
    let signed_header = |height, marker| {
        let header = TransactionBlockHeader::new(
            ...
            chain,
            Height::new(height),
            digest(b"saturated producer parent", height),
            digest(b"saturated producer body", marker),
        )
        .unwrap();
        Artifact::TransactionBlock(
            fixture.signers[producer.get() as usize]
    ...
    let mut saturated = AccountabilityState::new();
    for artifact in [
        signed_header(10, 10),
        signed_header(20, 20),
        signed_header(30, 30),
        ...
        assert!(
            saturated
                .observe::<Sha256, V>(&artifact, None, 2)
                .is_empty()
        );
    }
    assert_eq!(
        saturated.observe::<Sha256, V>(&signed_header(30, 31), None, 2),
        vec![producer],
    );
    ...
    let maximum_header = TransactionBlockHeader::new(
        ...
        chain,
        Height::new(u64::MAX),
        digest(b"maximum parent", 0),
        digest(b"maximum body", 0),
    )
    .unwrap();
    ...
    assert_objective_fault(
        ...
        Artifact::TransactionBlock(
            fixture.signers[producer.get() as usize]
                .sign_transaction_block(maximum_header)
                .unwrap(),
        ),
        Artifact::TransactionBlock(
            fixture.signers[producer.get() as usize]
                .sign_transaction_block(maximum_fork)
                .unwrap(),
        ),
        producer,
    );
    let contradictory_child = TransactionBlockHeader::new(
        ...
        chain,
        Height::new(2),
        digest(b"wrong signed parent", 1),
        digest(b"producer body", 2),
    )
    .unwrap();
    assert_objective_fault(
        &fixture,
    ...
    let first_da_vote = fixture.signers[da_voter.get() as usize]
        .sign_da_vote(first_header.clone())
        .unwrap();
    let second_da_vote = fixture.signers[da_voter.get() as usize]
        .sign_da_vote(
    ...
    assert_objective_fault(
        &fixture,
        Artifact::DaVote(first_da_vote.clone()),
        Artifact::DaVote(second_da_vote.clone()),
        da_voter,
    );
    assert_objective_fault_in_one_batch(
        &fixture,
        Artifact::DaVote(first_da_vote),
        Artifact::DaVote(second_da_vote),
        da_voter,
    );
    let valid_da_vote = fixture.signers[da_voter.get() as usize]
        .sign_da_vote(first_header)
        .unwrap();
    let forged_da_vote = DaVote::new(
        TransactionBlockHeader::new(
            fixture.protocol.epoch(),
            chain,
            Height::new(1),
            genesis.digest(),
            digest(b"unsigned DA fork", 1),
        )
        .unwrap(),
        valid_da_vote.share().clone(),
    );
    assert_compatible(
        &fixture,
        Artifact::DaVote(valid_da_vote),
        Artifact::DaVote(forged_da_vote),
    );
