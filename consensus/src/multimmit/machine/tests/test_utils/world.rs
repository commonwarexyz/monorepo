...
impl<'a> World<'a> {
    ...
    fn apply(&mut self, action: Action) {
        ...
        if matches!(action, Action::Block { .. }) {
            let Capability::Verification(VerificationCapability::Block(participants)) =
                self.take(replica, CompletionOrder::Oldest, is_block)
            else {
                unreachable!()
            };
            assert!(
        ...
        let step = match action.clone() {
            ...
            Action::Custody { order, .. } => {
                let Capability::Producer(ProducerCapability::Custody(job)) =
                    self.take(replica, order, is_custody)
                else {
                    unreachable!()
                };
                self.replicas[replica]
    ...
    fn crash_and_restore(&mut self, replica: usize) -> Step<MinPk, Digest> {
        ...
        state.recovery_expected = Some(expected);
    ...
    fn schedulable_action(
        ...
        if local && self.has(replica, is_build) {
...
const fn is_block(effect: &Capability<MinPk, Digest>) -> bool {
    matches!(
        effect,
        Capability::Verification(VerificationCapability::Block(_))
    )
}
...
const fn is_custody(effect: &Capability<MinPk, Digest>) -> bool {
    matches!(effect, Capability::Producer(ProducerCapability::Custody(_)))
}
...
fn run_scenario_with_poll_budget(
    ...
    world.settle(0, CompletionOrder::Oldest, CompletionOrder::Oldest, false);
    assert!(world.has(0, is_build));
    ...
    world.settle(0, CompletionOrder::Oldest, CompletionOrder::Oldest, true);
...
fn exercise_prefix_crash(fixture: &Fixture, plan: &ReplayPlan, selector: [u8; 3]) {
    let boundaries = plan
        .actions
        .iter()
        .enumerate()
    ...
    let crash = if selector[2] & 4 != 0 && world.has(replica, is_persist) {
        Action::CrashAfterAppend { replica }
    } else {
        Action::CrashAndRestore { replica }
    };
    world.apply(crash);
    world.settle(replica, order, order, true);
    ...
    assert!(world.replicas[replica].runner.inspect().view() >= View::new(3));
    assert!(world.replicas[replica].runner.inspect().finality().len() >= 2);
    world.apply(Action::CrashAndRestore { replica });
    world.settle(replica, order, order, true);
    world.assert_locally_drained(replica);
...
pub(super) fn exercise(input: &[u8]) {
    ...
    exercise_prefix_crash(&replay_fixture, &plan, [byte(8), byte(9), byte(10)]);
}

#[cfg(test)]
fn prefix_crash_plan() -> ReplayPlan {
    let (plan, _) = run_scenario(
        6,
        [true, false, true],
        [false, true, false],
        [true, false, true],
...
fn crash_prefix_can_interrupt_recovery() {
    ...
    assert!(world.replicas[replica].recovery_expected.is_some());
    world.apply(Action::CrashAndRestore { replica });
    world.settle(
        replica,
        CompletionOrder::Oldest,
...
fn crash_prefix_oracle_rejects_unacknowledged_signature() {
    ...
    let artifact = *state
        .exposed_signatures
        .values()
        .find(|artifact| !state.externally_injected.contains(*artifact))
        .expect("the scenario exposes a locally signed artifact");
    let origin = state.signing_events.get_mut(&artifact).unwrap();
    origin.covering_cursor = Cursor::new(state.acknowledged_cursor.get() + 1);
...
mod successor_matrix {
    ...
    fn prepare_publication_case(
        ...
        let profile = Profile::new(protocol, role, tuning).unwrap();
