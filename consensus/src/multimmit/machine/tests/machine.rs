...
fn validate_block(
    ...
    let validation = verified
        ...
        .expect("authenticated block must be validated");
...
#[test]
fn successful_build_wakes_the_next_parent() {
    let mut machine = active_machine(Role::Validator(Participant::new(0)));
    let first = machine.step(Input::ProducerWake).unwrap();
    let first = settle(&mut machine, first);
    let build = build_job(&first);
    let completed = machine
        .step(Input::BlockBuilt(BuildCompletion::new(
            build.id(),
            build.generation(),
            build.parent(),
            Some(digest(b"block one")),
        )))
        .unwrap();
    let completed = settle(&mut machine, completed);
    // The signing request releases with the step that stages the producer choice.
    let sign = completed
        ...
        .expect("the staged producer choice must release signing");
    let SignRequest::TransactionBlock(header) = sign_request(sign) else {
        panic!("producer build must request a transaction-block signature");
    };
    assert_eq!(header.height(), Height::new(1));
    assert_eq!(header.parent(), build.parent().digest());
    let durable = persist(&mut machine, &persist_job(&completed));
    let next_build = build_job(&durable);
    assert_eq!(next_build.parent().height(), Height::new(1));
}
    ...
    assert!(!completed.capabilities().iter().any(|effect| {
        matches!(
            ...
            Some(DurableEffect::Sign(SignRequest::TransactionBlock(header)))
                if header.commitment() == old_commitment
        )
    }));
}
...
fn producer_window_stops_before_the_third_uncertified_block() {
    ...
    // Pending work stays pending, so the producer keeps building as soon as the previous block is
    // signed. Follow the build jobs the machine emits rather than driving one per report.
    let wake = machine.step(Input::ProducerWake).unwrap();
    let mut step = settle(&mut machine, wake);
    for height in 1..=2u64 {
        let build = build_job(&step);
        let completed = machine
            .step(Input::BlockBuilt(BuildCompletion::new(
                build.id(),
                build.generation(),
        ...
        step = persist(&mut machine, &persist_job(&completed));
    }
...
fn recovered_payloads_are_the_exact_local_producer_and_da_union() {
    ...
    let local_da = validate_block(&mut machine, local.clone(), 0);
    ...
    assert_eq!(
        ...
        vec![
            (Context::from(&local), local.commitment()),
            (Context::from(&remote), remote.commitment()),
        ],
    ...
    assert_eq!(
        restored.recovered_payloads(),
        vec![(Context::from(&local), local.commitment())],
...
fn durable_da_vote_enables_the_next_height_in_the_same_drain() {
    ...
    let held = validate_block(&mut machine, second.clone(), 1);
    assert!(held.capabilities().is_empty());
    let first_choice = validate_block(&mut machine, first, 1);
...
fn certification_lag_does_not_silence_later_da_votes() {
    ...
    let held = validate_block(&mut machine, third.clone(), 1);
    assert!(held.capabilities().is_empty());
    let held = validate_block(&mut machine, second.clone(), 1);
    assert!(held.capabilities().is_empty());
    let unlocked = validate_block(&mut machine, first.clone(), 1);
...
fn application_digest_collision_retains_distinct_header_ancestry() {
    ...
    let second = make(digest(b"conflicting parent"));
    let first_artifact =
        Artifact::TransactionBlock(SignedTransactionBlock::new(first.clone(), attestation(1)));
    let second_artifact =
        Artifact::TransactionBlock(SignedTransactionBlock::new(second.clone(), attestation(1)));
    ...
    assert_eq!(machine.chain.retained_ancestry(), 2);
}
...
fn da_voting_stops_at_the_uncertified_pipeline_boundary() {
    ...
    for header in headers.iter().rev() {
        let step = validate_block(&mut machine, header.clone(), 1);
...
fn recovery_requires_the_held_path_before_extending_a_da_vote() {
    ...
    let recovery = restored.step(Input::RecoveryComplete).unwrap();
    persist(&mut restored, &persist_job(&recovery));
    let held_second = validate_block(&mut restored, second.clone(), 1);
    assert!(held_second.capabilities().is_empty());

    let restored_path = validate_block(&mut restored, first, 1);
    assert!(matches!(
...
fn proposal_fact_uses_the_highest_certificate_and_voted_suffix() {
    ...
    assert_eq!(proposal.payloads(), &[second.commitment()]);
}
...
fn record_protocol_acceptances(
    ...
    accepted.extend(activities.iter().map(|activity| {
        let Activity::ProtocolAccepted { artifact_id, .. } = activity;
        *artifact_id
    }));
}
...
fn vote_projection_uses_voted_prefix_and_extension() {
    ...
    assert_eq!(body.extensions()[0].payloads(), &[second.commitment()]);
}
...
proptest! {
    ...
    fn held_chain_is_da_voted_in_height_order(
        ...
        let mut choices = Vec::new();
        for index in order {
            let step = validate_block(&mut machine, headers[index].clone(), 1);
            drain_da_choices(&mut machine, step, &mut choices);
        }
        prop_assert_eq!(
