...
static HASH_TEST_LOCK: Mutex<()> = Mutex::new(());
...
fn complete_custody(machine: &mut TestMachine, job: &CustodyJob<Digest>) -> Step<MinPk, Digest> {
    let completed = machine
...
fn durable_broadcast_replays_with_stable_id() {
    ...
    assert_eq!(
        machine.live_snapshot_for_test().outbox().get(&id),
...
fn replayed_durable_prefix_advances_the_acknowledged_cursor() {
    ...
    let mut replayed = Machine::restore(profile, before).unwrap();
    for event in job.events() {
        replayed.replay(event.clone()).unwrap();
    }
    assert_eq!(replayed.acked, replayed.durable.cursor);
}
...
fn restore_rejects_mismatched_publication_obligation_rows() {
    ...
    assert!(Machine::restore(profile.clone(), wrong_family.live_snapshot_for_test()).is_err());
    ...
    assert!(Machine::restore(profile, wrong_object.live_snapshot_for_test()).is_err());
}
...
fn remote_da_certificate_retires_vote_without_rebroadcast_and_replays() {
    let profile = profile_for_producers(
...
fn higher_da_certificate_replacement_replays_atomically() {
    ...
    let da_choice_job = persist_job(&da_choice);
    persist(&mut machine, &da_choice_job);
    let checkpoint = machine.live_snapshot_for_test();
...
fn replay_requires_the_views_first_vqc_forwarding_before_a_current_lqc_floor() {
    ...
    Machine::restore(machine.profile().clone(), restored.live_snapshot_for_test()).unwrap();
}
...
fn prepared_builds_pipeline_and_authorize_only_the_custodied_prefix() {
    ...
    assert!(
        first_prepared.capabilities().iter().all(|capability| {
    ...
    assert!(
        second_prepared.capabilities().iter().all(|capability| {
            !matches!(durable_effect(capability), Some(DurableEffect::Sign(_)))
        })
    );
    assert!(
    ...
    assert!(
        out_of_order.capabilities().iter().all(|capability| {
            !matches!(durable_effect(capability), Some(DurableEffect::Sign(_)))
        })
    );
...
fn prepared_suffix_is_discarded_across_restart_without_authority() {
    ...
    assert!(
        prepared.capabilities().iter().all(|capability| {
            !matches!(durable_effect(capability), Some(DurableEffect::Sign(_)))
        })
    );
    ...
    assert_eq!(stale.status(), &StepStatus::StaleCompletion);
    assert!(
        stale.capabilities().iter().all(|capability| {
            !matches!(durable_effect(capability), Some(DurableEffect::Sign(_)))
        })
    );
}
...
fn producer_builds_after_an_external_own_chain_certificate() {
    ...
    let obligations_before = machine.live_snapshot_for_test().obligations().clone();
...
fn recovered_payloads_are_the_exact_local_producer_and_da_union() {
    ...
    let verified = complete_with_step(&mut machine, &observed, true);
    persist(&mut machine, &persist_job(&verified));
    let restored = Machine::restore(recovery_profile, machine.live_snapshot_for_test()).unwrap();
    assert_eq!(
        restored.recovered_payloads(),
    ...
    let observer = Machine::restore(
        ...
        Machine::new(observer_profile).live_snapshot_for_test(),
    )
    .unwrap();
...
fn producer_subset_snapshots_restore_for_every_validator_role() {
    ...
    for participant in [Participant::new(3), Participant::new(4)] {
        ...
        let machine = Machine::new(profile.clone());
        Machine::restore(profile, machine.live_snapshot_for_test()).expect("K<n snapshot restores");
    }
}
...
fn verified_vqc_reuses_validation_derivations() {
    ...
    let validated = validate_vqc::<CountingHasher, MinPk, Digest>(
        certificate,
...
fn frontier_chain(machine: &TestMachine, count: u64) -> Vec<TransactionBlockHeader<Digest>> {
    let mut parent = machine.profile().protocol().genesis().tips()[1];
    (1..=count)
        .map(|height| {
            let header = TransactionBlockHeader::new(
...
fn position_finality_covers_proposed_blocks_without_local_bodies() {
    ...
    complete(&mut machine, &observed, true);
...
fn headers_admitted_after_the_seal_are_counted() {
    let view = View::new(1);
    let leader = LeaderSchedule::round_robin(6).leader(view);
    let mut machine = Machine::new(profile_for(Role::Validator(leader), 6, 3));
    let start = machine.step(Input::Start).unwrap();
    persist(&mut machine, &persist_job(&start));
...
#[test]
fn newly_validated_da_choices_precede_the_ordinary_vote_snapshot() {
    let participants = 20;
    let chains = 10;
    let probe = profile_for(Role::Observer, participants, chains);
    ...
    let (mut machine, _) = start_profile(profile_for(Role::Validator(voter), participants, chains));

    let genesis = machine.profile().protocol().genesis().tips();
    let headers = (0..chains)
        .map(|chain| {
            TransactionBlockHeader::new(
                machine.profile().protocol().epoch(),
                ChainId::new(chain),
                ...
                digest(format!("vote frontier {chain}").as_bytes()),
            )
            .unwrap()
        })
        .collect::<Vec<_>>();
    ...
    let Change::OutboxQueued { effect, .. } = choice_job.events()[0].change() else {
        panic!("the validated DA frontier must be durably selected");
    };
    let DurableEffect::SignBatch(requests) = effect.as_ref() else {
        panic!("all ready DA choices must share one signing reservation");
    };
    assert_eq!(requests.len(), headers.len());
    assert!(requests.iter().zip(&headers).all(|(request, header)| {
        matches!(request, SignRequest::DaVote(request) if request.header() == header)
    }));
    let Change::OutboxQueued { effect, .. } = choice_job.events()[1].change() else {
        panic!("the ordinary vote must follow the DA frontier");
    };
    let DurableEffect::Sign(SignRequest::Vote(request)) = effect.as_ref() else {
        panic!("the next durable choice must be the ordinary vote");
    };
    for (chain, header) in headers.iter().enumerate() {
        assert_eq!(
            request.body().extensions()[chain].payloads(),
            &[header.body_digest()]
        );
    }
}
...
#[test]
fn ordinary_vote_does_not_wait_for_newly_enabled_da_choices() {
    let participants = 6;
    let probe = profile_for(Role::Observer, participants, 3);
    let leader_participant = probe.protocol().leader(View::new(1));
    let voter = Participant::new((leader_participant.get() + 1) % participants as u32);
    let (mut machine, _) = start_profile(profile_for(Role::Validator(voter), participants, 3));
    ...
    assert!(matches!(
        ...
        Change::OutboxQueued { effect, .. }
            if matches!(effect.as_ref(), DurableEffect::Sign(SignRequest::Vote(request))
                if request.body().extensions()[0].payloads() == [first.body_digest()])
    ));
}

#[test]
fn ordinary_vote_freezes_one_ready_da_frontier() {
    let participants = 6;
    let chains = 3;
    let probe = profile_for(Role::Observer, participants, chains);
    let leader_participant = probe.protocol().leader(View::new(1));
    ...
    let headers = (0..chains)
        .map(|chain| {
            TransactionBlockHeader::new(
                ...
                Height::new(1),
                genesis[chain as usize].digest(),
                digest(format!("finite vote frontier {chain}").as_bytes()),
            )
            .unwrap()
        })
        .collect::<Vec<_>>();
    ...
    assert_eq!(choice_job.events().len(), 2);
    assert!(matches!(
        choice_job.events()[0].change(),
        Change::OutboxQueued { effect, .. }
            if matches!(effect.as_ref(), DurableEffect::SignBatch(requests)
                if requests.len() == 2
                    && requests.iter().zip(&headers[..2]).all(|(request, header)| {
                        matches!(request, SignRequest::DaVote(request)
                            if request.header() == header)
                    }))
    ));
    let Change::OutboxQueued { effect, .. } = choice_job.events()[1].change() else {
        panic!("the ordinary vote must share the ready-frontier barrier");
    };
    let DurableEffect::Sign(SignRequest::Vote(request)) = effect.as_ref() else {
        panic!("the ready frontier must be followed by the ordinary vote");
    };
    assert_eq!(
        request.body().extensions()[0].payloads(),
        &[headers[0].body_digest()]
    );
    assert_eq!(
        request.body().extensions()[1].payloads(),
    ...
    assert!(late.capabilities().iter().all(|capability| {
        !matches!(capability,
            Capability::Durability(DurabilityCapability::Persist(job))
                if job.events().iter().any(|event| matches!(event.change(),
                    Change::OutboxQueued { effect, .. }
                        if matches!(effect.as_ref(), DurableEffect::Sign(SignRequest::Vote(_)))))
        )
    }));
    persist(&mut machine, &choice_job);
}

#[test]
fn oversized_ready_da_frontier_reaches_the_ordinary_vote() {
    let chains = contracts::CORE_BUDGET as usize + 1;
    let participants = chains + 2;
    let view = View::new(1);
    let leader_participant = LeaderSchedule::round_robin(participants).leader(view);
    let voter = (chains..participants)
        .map(Participant::from_usize)
        .find(|participant| *participant != leader_participant)
        .unwrap();
    let resource_bound = NonZeroUsize::new(4_096).unwrap();
    let resources = ResourceLimits::new(
    ...
    let profile: Profile<Sha256, MinPk> = Profile::with_limits(
        config_for_producers(
            Epoch::new(7),
            participants,
            (0..chains).map(Participant::from_usize).collect(),
            1,
        ),
        Role::Validator(voter),
        Tuning {
            max_artifact_bytes: NonZeroUsize::new(4 * 1024 * 1024).unwrap(),
            ..Tuning::default()
        },
    ...
    for _ in 0..8 {
        ...
        for event in job.events() {
            ...
            match effect.as_ref() {
                DurableEffect::Sign(SignRequest::DaVote(request)) => {
                    selected.insert(request.header().chain());
                }
                DurableEffect::SignBatch(requests) => {
                    batch_sizes.push(requests.len());
                    assert!(requests.len() <= contracts::CORE_BUDGET as usize);
                    selected.extend(requests.iter().map(|request| {
                        let SignRequest::DaVote(request) = request else {
                            panic!("the frontier batch must contain only DA choices");
                        };
                        request.header().chain()
                    }));
                }
                DurableEffect::Sign(SignRequest::Vote(request)) => {
                    vote = Some(request.clone());
                }
                _ => {}
            }
        }
    ...
    assert_eq!(
        selected.len(),
        chains,
        "batch sizes {batch_sizes:?}, pending {}, safe advances {}",
        machine
            .pending_vote_da
            .as_ref()
            .map_or(0, |pending| pending.blocks.len()),
        machine
            .durable
            .da_safety_heights
            .iter()
    ...
    let vote = vote.expect("a finite oversized frontier must not starve the ordinary vote");
    assert!(
        vote.body()
            .extensions()
            .iter()
            .zip(&headers)
            .all(|(extension, header)| extension.payloads() == [header.body_digest()])
    );
}
...
fn vote_body_pass_ignores_later_da_choices() {
    let participants = 6;
    let (mut machine, _) = start_profile(profile_for(
        Role::Validator(Participant::new(1)),
        participants,
        3,
    ));
...
fn completed_ordinary_vote_survives_da_observation_until_reserved() {
    ...
    let leader_participant = probe.protocol().leader(View::new(1));
    let voter = Participant::new((leader_participant.get() + 1) % participants as u32);
    let (mut machine, _) = start_profile(profile_for(Role::Validator(voter), participants, 2));
    let proposed = leader(&machine, 1);
    let proposal = observe(
        &mut machine,
        Artifact::LeaderBlock(SignedLeaderBlock::new(
    ...
    let first = loop {
        let drive = machine
            ...
            .drive_regular_sign_request::<Sha256>(
                ...
                View::new(1),
                &machine.chain,
                contracts::CORE_BUDGET as usize,
            )
            .unwrap();
        if let Some(request) = drive.request {
            break request;
    ...
    machine
        .views
        .observe_sign_request(&SignRequest::DaVote(DaVoteRequest::new(block)))
        .unwrap();
    let resumed = machine
        .views
        .drive_regular_sign_request::<Sha256>(
    ...
    assert_eq!(resumed.request, Some(first));
}
...
fn last_voter_signature_still_publishes_its_vote() {
    ...
    let probe = profile_for(Role::Observer, 6, 2);
    let leader_participant = probe.protocol().leader(View::new(1));
    let voter = Participant::new((leader_participant.get() + 1) % 6);
    let (mut machine, _) = start_profile(profile_for(Role::Validator(voter), 6, 2));
    ...
    let published = complete_with_step(&mut machine, &verification, true);
    ...
    let signed = machine
        .step(Input::EffectCompleted(EffectCompletion::Signed {
            id: sign.id(),
            generation: sign.generation(),
            artifact: Arc::new(Artifact::Vote(own)),
        }))
        .unwrap();
    let mut step = settle(&mut machine, signed);

    // Drive persistence; the vote's broadcast publication must release before retirement.
    let mut vote_published = false;
    for _ in 0..64 {
        vote_published |= step.capabilities().iter().any(|effect| {
            matches!(
                effect,
                Capability::Durability(DurabilityCapability::Released(job))
                    if matches!(job.request(), DurableEffect::Broadcast(artifact)
...
fn pending_proposal_parent_counts_against_recovery_capacity() {
    ...
    assert!(matches!(
        Machine::restore(constrained, machine.live_snapshot_for_test()),
        Err(ReplayError::Transition)
    ));
}
...
fn view_retention_retires_history_while_finality_stalls() {
    ...
    let restored = Machine::restore(profile, machine.live_snapshot_for_test()).unwrap();
