...
use commonware_codec::{
    Buf, Copying, Decode, Encode, EncodeSize, Error, RangeCfg, Read, ReadExt, Write,
};
...
impl<V: Variant, D: Digest> Read for DurableEffect<V, D> {
    ...
    fn read_cfg(buf: &mut impl Buf, config: &Self::Cfg) -> Result<Self, Error> {
        match u8::read(buf)? {
            ...
            1 => {
                let count = usize::read_cfg(buf, &RangeCfg::from(2..=config.max_artifacts))?;
...
impl<V: Variant, D: Digest> Read for Snapshot<V, D> {
    ...
    fn read_cfg(buf: &mut impl Buf, config: &Self::Cfg) -> Result<Self, Error> {
        ...
        let signing_reservations = SigningReservations::decode_cfg(
            Copying(envelope.signing_reservations.bytes()),
        ...
        let obligations = if envelope.obligations.bytes().is_empty() {
            ...
            let obligations = ObligationsSnapshot::decode_cfg(
                Copying(envelope.obligations.bytes()),
...
mod tests {
    ...
    use crate::multimmit::{
        config::Limits,
        types::{Attestation, BlockRef, ChainId, Height},
    };
    ...
    fn rejects_removed_durability_tags() {
        for tag in [1, 2, 10, 13, 14] {
            assert!(matches!(
                Change::<MinSig, Sha256Digest>::decode_cfg(Copying(&[tag]), &config(4)),
    ...
    fn rejects_removed_publication_tags_without_renumbering_send_batch() {
        assert!(matches!(
            PublicationKind::decode(Copying(&[5])),
            Err(Error::InvalidEnum(5))
        ));
        assert!(matches!(
            DurableEffect::<MinSig, Sha256Digest>::decode_cfg(Copying(&[6]), &config(4)),
    ...
    fn rejects_removed_component_tags_without_renumbering_publication_outbox() {
        ...
        for tag in 4..=6 {
            ...
            assert!(matches!(
                Event::decode_cfg(encoded, &config(4)),
                Err(Error::InvalidEnum(actual)) if actual == tag
            ));
        }
