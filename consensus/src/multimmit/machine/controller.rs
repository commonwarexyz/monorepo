...
pub(crate) struct CoreState<H: Hasher, V: Variant> {
    ...
    /// Active application requests grouped by producer chain.
    ///
...
impl<H: Hasher, V: Variant> CoreState<H, V> {
    ...
    pub(crate) fn producer_custodied(
        &mut self,
        completion: CustodyCompletion<H::Digest>,
    ) -> Result<InputTicket, CoreError> {
        self.enqueue(Input::BlockCustodied(completion), 1)
    }

    pub(crate) fn producer_custody_cancelled(
        &mut self,
