...
impl<D: Digest> BuildCompletion<D> {
    ...
    ///
    /// A present commitment must content-address a complete block body that the attached application
    /// has made durably retrievable before returning this completion. The machine deliberately retains
    /// only that protocol commitment; storage and wire representation of the application body remain
    /// application-defined.
    pub const fn new(
        id: BuildId,
        generation: u64,
...
pub(crate) enum BuildOutcome<V: Variant, D: Digest> {
    Stale,
    Superseded,
    Empty,
...
pub(crate) struct ChainState<V: Variant, D: Digest> {
    ...
    pending_build: Option<PendingBuild<D>>,
...
impl<V: Variant, D: Digest> ChainState<V, D> {
    pub(crate) fn new<H: Hasher<Digest = D>>(profile: &Profile<H, V>) -> Self {
        ...
        let validation_limits = ValidationLimits {
            ...
            items_per_chain: resources.max_verification_batch(),
            bytes: resources
                .max_cached_artifacts()
                .saturating_mul(resources.max_artifact_bytes()),
            bytes_per_chain: resources
    ...
    pub(crate) fn observe<H: Hasher<Digest = D>>(
        ...
        match artifact {
            Artifact::TransactionBlock(block) => {
                ...
                self.schedule_ready_validation(generation)?;
            }
    ...
    pub(crate) fn complete_build<H: Hasher<Digest = D>>(
    ...
    ) -> Result<BuildOutcome<V, D>, ChainError> {
        ...
        if completion.generation != pending.job.generation
            || completion.id != pending.job.id
            || completion.parent != pending.job.parent
            || pending.reserved
        {
            return Ok(BuildOutcome::Stale);
        }
        if self.produced != Some(pending.job.parent) {
        ...
        let request = self
            .pending_build_sign_request(profile)?
            .expect("a completed build has a signing request");
        Ok(BuildOutcome::Sign(request))
    }

    /// Returns the signing request held by a completed build until durability is available.
    pub(crate) fn pending_build_sign_request<H: Hasher<Digest = D>>(
        &self,
        profile: &Profile<H, V>,
    ) -> Result<Option<SignRequest<V, D>>, ChainError> {
        let Some(pending) = self
            .pending_build
            .as_ref()
            .filter(|pending| !pending.reserved)
        else {
            return Ok(None);
        };
        let Some(commitment) = pending.commitment else {
            return Ok(None);
        };
        let parent = pending.job.parent;
        let height = parent
            .height()
            .get()
            .checked_add(1)
        ...
        Ok(Some(SignRequest::TransactionBlock(header)))
    }

    pub(crate) const fn mark_build_reserved(&mut self) {
        self.pending_build
            .as_mut()
            .expect("a completed build must remain pending until signing is reserved")
            .reserved = true;
    }
    ...
    pub(crate) fn build_reservations(&self) -> usize {
        usize::from(
            self.pending_build
                .as_ref()
                .is_some_and(|pending| !pending.reserved),
        )
    }
    ...
    /// Reserves item and encoded-byte capacity before retaining and emitting one validation job.
    ///
    ...
    fn schedule_ready_validation(&mut self, generation: u64) -> Result<bool, ChainError> {
        ...
        for offset in 0..self.blocks.len() {
            ...
            let candidate = self.blocks[chain_index]
                .iter()
                .flat_map(|(height, records)| {
                    records
                        .iter()
                        .enumerate()
                        .filter(|(_, record)| record.state == ValidationState::Ready)
                        .map(move |(index, record)| {
                            (record.observation, *height, index, record.artifact)
                        })
                })
                .min_by_key(|(observation, _, _, _)| *observation);
            let Some((_, height, record_index, artifact)) = candidate else {
                continue;
            };
    ...
    pub(crate) fn recovered_payloads(&self) -> Vec<(Context<D>, D)> {
        ...
        headers.sort_by(|left, right| {
            left.chain()
                ...
                .then_with(|| left.commitment().as_ref().cmp(right.commitment().as_ref()))
        });
        headers.dedup();
        headers
            .iter()
            .map(|header| (Context::from(header), header.commitment()))
            .collect()
    }
    ...
    fn advance_produced(&mut self, tip: BlockRef<D>) {
        ...
        if self
            .pending_build
            .as_ref()
            .is_some_and(|pending| pending.commitment.is_some() || pending.reserved)
        {
            self.pending_build = None;
        }
    ...
    pub(crate) fn resume_proposal_pass<H: Hasher<Digest = D>>(
        ...
        if pass.attempted < pass.limit {
            ...
            pass.payloads.push(header.commitment());
        ...
        match pass.phase {
            ...
            VoteBodyPhase::Extension => {
                if pass.extension_index < pass.extension_bound {
                    ...
                    pass.extension_payloads.push(header.commitment());
                    *parent = header.block_ref::<H>();
                    return Ok(VoteBodyProgress::Pending);
                }
    ...
    pub(crate) fn vote_body<H: Hasher<Digest = D>>(
        ...
        for (index, proposal) in leader.proposals().iter().enumerate() {
            ...
            for _ in 0..config.extension_bound() {
                ...
                payloads.push(header.commitment());
                parent = header.block_ref::<H>();
            }
    ...
    pub(crate) fn drive<H: Hasher<Digest = D>>(
        ...
        let Some(parent) = self.produced else {
            return Ok(());
        };
