...
impl<V: Variant, D: Digest> VerifyJob<V, D> {
        ...
        H: Hasher<Digest = D>,
    {
        ...
        let mut da_equivocators = Vec::new();
        let verdicts = valid
            .into_iter()
            .zip(&self.items)
            .enumerate()
                    ...
                    Artifact::DaVote(vote) => {
                        let prior = item
                            ...
                            .chain(
                                self.items[..index]
                                    .iter()
                                    .map(VerificationItem::shared_artifact),
                            )
                        ...
                        if scheme.verify_da_vote(vote)
                            && prior
                                .into_iter()
                                .any(|candidate| scheme.verify_da_vote(candidate))
                        {
                            da_equivocators.push((index, vote.signer()));
                        }
