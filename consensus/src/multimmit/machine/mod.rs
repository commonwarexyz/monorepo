...
//!
//! `Automaton::propose` and `Automaton::verify` receive application context and opaque payload
//! digests only. A successful result is the application validity and crash-recoverable custody
//! fence required before producer or DA authority is reserved. `Automaton` is not a fetch interface
//! and never receives a V-QC, nullification, L-QC, leader block, or other consensus proof. Missing
//! view proofs use the resolver port; producer-body retrieval remains application-owned.
//!
...
//!
//! future external marshal: proofs + producer history -> dense application order
//! ```
//!
