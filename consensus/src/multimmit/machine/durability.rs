    ...
    pub(crate) fn authorized<H: Hasher<Digest = D>>(&self, profile: &Profile<H, V>) -> bool {
        ...
        match self {
            ...
            Self::SignBatch(requests) => {
                ...
                // Data-availability votes may share a signing barrier: several producer chains,
                // and per chain one run of strictly consecutive heights, where each vote counts
                // as sent for the next one's eligibility within the same authorization.
                let mut runs = std::collections::BTreeMap::new();
                let da_votes = requests.iter().all(|request| {
                    matches!(request, SignRequest::DaVote(request)
                        if {
                            let header = request.header();
                            (header.chain().get() as usize)
                                < protocol.codec_config().chains()
                                && runs
