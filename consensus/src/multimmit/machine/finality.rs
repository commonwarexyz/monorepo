...
pub(crate) struct FinalityState<V: Variant, D: Digest> {
    ...
    newly_finalized: Vec<FinalityFact<D>>,
...
impl<V: Variant, D: Digest> FinalityState<V, D> {
    ...
    fn insert_vote<H: Hasher<Digest = D>>(
        ...
        let (tips_changed, newly_finalized) = {
            let pool = self
                .direct_pool_mut(key)
            ...
            let newly_finalized = if first_finality {
            ...
            (tips_changed, newly_finalized)
        };
        if let Some(fact) = newly_finalized {
            self.newly_finalized.push(fact);
        }
        let _ = tips_changed;
        self.stage_lqc_if_witnessed(key)?;
        Ok(true)
    ...
    pub(crate) fn drain_newly_finalized(&mut self) -> Vec<FinalityFact<D>> {
        std::mem::take(&mut self.newly_finalized)
    }

    pub(crate) fn clear_newly_finalized(&mut self) {
        self.newly_finalized.clear();
    }
