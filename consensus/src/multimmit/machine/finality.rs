...
use crate::{
    ...
    multimmit::{
        ...
        types::{
            BlockRef, CertificateId, ChainId, LeaderBlock, Lqc, Position, SelectedCommitments,
            Vote, VoteBody, Vqc,
        },
    },
...
pub(crate) struct FinalityState<V: Variant, D: Digest> {
    ...
    finality_updates: Vec<FinalityUpdate<D>>,
...
impl<V: Variant, D: Digest> LqcAggregateCompletion<V, D> {
    ...
    pub const fn certificate(&self) -> &Lqc<V, D> {
        &self.certificate
    }
}
...
impl<V: Variant, D: Digest> FinalityState<V, D> {
            ...
            ClaimVerdict::Valid => {
                ...
                let certificate_key = match claim.artifact.as_ref() {
                    Artifact::Vqc(certificate) => Some((
                        ...
                        derived_leader.unwrap_or_else(|| certificate.leader().digest::<H>()),
                    )),
    ...
    fn observe_vqc<H: Hasher<Digest = D>>(
        ...
        for vote in votes {
            if claim.retains((vote.body.round(), vote.body.leader())) {
    ...
    fn insert_vote<H: Hasher<Digest = D>>(
        ...
        let finality_update = {
            ...
            let first_finality = pool.final_tips.is_none();
            let tips = pool
                .extractor
                .final_tips()
                .map_err(|_| FinalityError::Algebra)?;
            ...
            let finality_update = if tips_changed {
                pool.finality_fact(key.1, config)?.map(|fact| {
                    let commitments = pool.extractor.selected(
                        pool.leader.round().epoch(),
                        pool.final_tips
                            .as_ref()
                            .expect("final tips were assigned above"),
                    );
                    if first_finality {
        ...
        if let Some(fact) = finality_update {
            self.finality_updates.push(fact);
        }
        self.stage_lqc_if_witnessed(key)?;
        Ok(true)
    }

    pub(crate) fn drain_finality_updates(&mut self) -> Vec<FinalityUpdate<D>> {
        std::mem::take(&mut self.finality_updates)
    }

    pub(crate) fn clear_finality_updates(&mut self) {
        self.finality_updates.clear();
    }
