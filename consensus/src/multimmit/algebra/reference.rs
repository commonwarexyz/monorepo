...
use super::tips::deeper;
use crate::{
    multimmit::types::{
        Anchor, BlockRef, ChainId, CodecConfig, LeaderBlock, Position, TransactionBlockHeader,
        VoteBody,
    },
    ...
    pub(super) fn deepest(
        ...
        candidates.into_iter().fold(base, |deepest, candidate| {
            let support = votes
                ...
                .count();
            if support >= threshold && self.extends(base, candidate) && deeper(candidate, deepest) {
                candidate
            } else {
                deepest
            }
