//! Ancestry, safe-tip, final-tip, and pool algebra tests.
...
use crate::{
    multimmit::types::{
        Anchor, BlockRef, CertificateId, ChainId, ChainProposal, CodecConfig, DaCertificate,
...
pub(super) fn config(participants: usize) -> CodecConfig {
    CodecConfig::new(participants, participants, PathLimits::new(2, 2).unwrap()).unwrap()
}
...
fn proposal_and_vote_paths_reconstruct_exact_headers() {
    ...
    let paths =
        VotePaths::new::<Sha256, MinSig>(DigestedLeader::new::<Sha256>(&leader), &proposals, &body)
            .unwrap();
    let mut ancestry = PathIndex::default();
    proposals.index(&mut ancestry).unwrap();
...
fn safe_tips_apply_position_rank_and_branch_aware_carry() {
    ...
    let left = VotePaths::new::<Sha256, MinSig>(
        DigestedLeader::with_digest(&leader, leader_digest),
        &proposals,
        &votes[0],
    )
    .unwrap();
    let right = VotePaths::new::<Sha256, MinSig>(
        DigestedLeader::with_digest(&leader, leader_digest),
        &proposals,
        &votes[3],
    )
...
fn final_tips_use_pool_support_and_generalized_settlement() {
    ...
    let proposals = ProposalPaths::new::<Sha256, MinSig>(&leader).unwrap();
    let path = VotePaths::new::<Sha256, MinSig>(
        DigestedLeader::new::<Sha256>(&leader),
        &proposals,
        &votes[0],
    )
...
#[test]
fn extraction_errors_name_the_rejected_input() {
    let config = config(6);
    let leader = leader(6);
    let body = vote(&leader, 2, &[], config);
    let outsider = Participant::new(config.participants() as u32);

    // A leader proposing for six chains does not fit a seven-chain epoch.
    assert!(matches!(
        PoolExtractor::new::<Sha256, MinSig>(&leader, self::config(7)),
        Err(Error::Leader)
    ));
    assert_eq!(
        FinalTips::from_pool::<Sha256, MinSig, _>(
            &leader,
            Vec::<(Participant, &VoteBody<Digest>)>::new(),
            self::config(7)
        ),
        Err(Error::Leader)
    );
    ...
    let mut pool = PoolExtractor::new::<Sha256, MinSig>(&leader, config).unwrap();
    assert_eq!(
        pool.insert::<Sha256, MinSig>(&leader, outsider, &body),
        Err(Error::Signer)
    );
    assert_eq!(
        FinalTips::from_pool::<Sha256, MinSig, _>(&leader, [(outsider, &body)], config),
        Err(Error::Signer)
    );

    let mut blocks = leader
        .proposals()
        .iter()
        .map(|proposal| proposal.anchor().block_ref::<Sha256>())
        .collect::<Vec<_>>();
    assert!(Tips::new(blocks.clone()).is_ok());
    blocks.swap(0, 1);
    assert_eq!(Tips::new(blocks.clone()), Err(Error::ChainOrder));
    let positions = vec![Position::new(0); blocks.len()];
    let settled = vec![false; blocks.len()];
    assert_eq!(
        FinalTips::new(blocks, positions, settled),
        Err(Error::ChainOrder)
    );
}
...
fn cached_final_tip_extraction_performs_no_ancestry_walks() {
    for depth in [2, 16, 64] {
        let config = CodecConfig::new(6, 6, PathLimits::new(2, depth).unwrap()).unwrap();
        let leader = leader(6);
        let body = vote_with_digests(
            &leader,
            2,
...
/// Builds the votes of one cross-certificate scenario. Each field is one independent axis of the
/// vote matrix that every vote in the scenario shares.
struct CrossVotes<'a> {
    leader: &'a LeaderBlock<MinSig, Digest>,
    pattern: CrossPattern,
    config: CodecConfig,
    /// Signers that extend the forked chain on branch `a` rather than branch `b`.
    branch: &'a BTreeSet<Participant>,
    /// Signers that vote the higher position on the rank chain.
    high_rank: &'a BTreeSet<Participant>,
}

impl CrossVotes<'_> {
    /// Returns `signer`'s vote, given its slot in the finality pool (if any) and whether it
    /// signs the alternate, lower rank-chain position.
    fn vote(
        &self,
        signer: Participant,
        ...
        let quorum = self.config.view_quorum();
        ...
        if pool_slot.is_some_and(|slot| slot < 3 * faults + 1) {
            positions[self.pattern.settled_chain] = Position::new(2);
        }

        positions[self.pattern.final_extension_chain] = Position::new(2);
        if pool_slot.is_some_and(|slot| slot < quorum) {
            extensions[self.pattern.final_extension_chain] =
                Extension::new(vec![shared(self.pattern.final_extension_chain, "final")], 2)
                    .unwrap();
        }

        if let Some(chain) = self.pattern.fork_chain {
            positions[chain] = Position::new(2);
            extensions[chain] = Extension::new(
                vec![shared(
                    chain,
                    if self.branch.contains(&signer) {
                        "a"
                    } else {
                        "b"
                    },
        ...
        positions[self.pattern.rank_chain] = Position::new(if alternate {
            0
        } else if self.high_rank.contains(&signer) {
            2
        } else {
            1
        });
        ...
        VoteBody::for_leader(
            DigestedLeader::new::<Sha256>(self.leader),
            positions,
            extensions,
            self.config,
        )
...
#[test]
fn cross_certificate_safe_tips_match_reference_across_designated_tallies() {
    let mut rng = TestRng::new(0x5afe_e17e);
    for participants in [6, 7, 11, 12] {
        let config = config(participants);
        let quorum = config.view_quorum();
        for designated_tally in config.designation_quorum()..=quorum {
            ...
            let (branch, high_rank) = (BTreeSet::new(), BTreeSet::new());
            let votes = CrossVotes {
                leader: &fixture.leader,
                pattern: cross_pattern(participants, fixture.certificate_chain, false),
                config,
                branch: &branch,
                high_rank: &high_rank,
            };
            let tallies = messages.map(|messages| {
                messages
                    .designated_votes
                    .iter()
                    .map(|signer| (*signer, votes.vote(*signer, None, false)))
                    .collect::<Vec<_>>()
            });
                ...
                for designated_tally in designation..=quorum {
                    ...
                    let canonical = (0..participants)
                        .map(|index| {
                            ...
                            (
                                signer,
                                votes.vote(signer, pool_slots.get(&signer).copied(), false),
                            )
                        })
                        .collect::<Vec<_>>();
                        ...
                        signers
                            ...
                            .map(|signer| {
                                let vote = if faulty.contains(signer) {
                                    votes.vote(*signer, pool_slots.get(signer).copied(), true)
                                } else {
