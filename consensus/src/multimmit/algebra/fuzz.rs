...
use crate::{
    multimmit::{
        ...
        types::{
            ...
            Extension, LeaderBlock, PathLimits, Position, VoteBody,
        },
    },
...
pub(crate) fn exercise(input: &[u8]) {
    let mut schedule = ByteSchedule::cycling(input);
    let mut next = || schedule.index();
    ...
    let config = CodecConfig::new(
        participants,
        chains,
        PathLimits::new(depth as u32, 6).unwrap(),
    )
    .unwrap();
