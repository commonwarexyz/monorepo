...
This document defines Multimmit's in-process ownership, application-custody, durability, recovery,
and resource contracts. The [Multimmit v4 specification](https://arxiv.org/html/2607.21021v4)
defines protocol semantics. [`PROPERTIES.md`](PROPERTIES.md) maps its formal results and optimization
choices to the implementation and executable evidence.
...
`Engine` is the only public signing entry point. Its configuration takes an `Automaton` and a `Relay`
directly; Multimmit adds no application supertrait, binding codec, lease protocol, or bootstrap API.
The traits remain concerned only with opaque digests.
...
artifact identification, batched cryptographic verification, and network blocking. Decode and
identification run as bounded strategy jobs whose completions retain the authenticated peer. Core
can name a signer proven to have equivocated, but the voter must map that participant through the
committee before Batcher invokes the blocker. `Resolver` owns transport, retry, and peer choice for
view-proof fetches.
...
reservations, publication obligations, retention, and snapshots. One validator-plane task per
producer chain owns a volatile block store, application-validation jobs, and a read-copy of Core's
durable DA choices. The local producer's plane receives already-custodied headers through the same
boundary. A plane may offer a contiguous eligible run to Core, but only Core may reserve a DA vote or
mutate a certified frontier.
...
The Multimmit marshal is the executable owner of the implementation's Section 6-permitted two-pass
`Ord`/`Emit` variant. Consensus emits authenticated L-QCs, accepted producer headers, and available
history openings through the same best-effort `Activity` stream. Marshal validates each exact
identity, persists useful hints, and resolves missing openings or blocks by authenticated digest.
Dense delivery and application acknowledgement never enter consensus storage.
...
- **Validator planes:** one per producer chain, holding volatile authenticated block copies,
  application-validation jobs, derived eligibility, and a read-copy of the chain's durable DA
  choices. Planes are rebuilt from Core state after restart and cannot create authority.
...
journal owner, validator plane, or reporter.
...
identity. Producer and leader work remain visibly separate at the Core API:
...
- `Synchronizer` walks recursive history commitments and producer ancestry through disk-backed
  ...
  dense rows. Producer ancestry is
...
### Property implementation crosswalk

[`PROPERTIES.md`](PROPERTIES.md) defines the property contracts and maps the v4 results. This table
locates their implementation mechanisms and stable invariant checks.

| Properties | State-machine sections | Invariants |
|---|---|---|
| P-ADM-1, P-OBS-1, P-OWNER-1, P-ASYNC-1 | Ownership; one-way control flow; admission and verification; async boundaries | C-INV-01, C-INV-08, C-INV-09 |
| P-SIGN-1, P-ACCT-1, P-EXP-1, P-REP-1, P-PUB-1, P-SNAP-1 | Startup and recovery; objective equivocation; proposal; timeout; persistence and publication; checkpointing | C-INV-01 through C-INV-04, C-INV-08, C-INV-12 |
| P-AUTO-1, P-CUST-1, P-PIPE-1, P-DA-1, P-LIVE-1 | Producer construction; application verification and DA voting | C-INV-02, C-INV-05 through C-INV-07, C-INV-09 |
| P-COMMIT-1, P-VIEW-1, P-VQC-1, P-RES-1 | Proposal; direct vote; timeout; V-QC and view exit | C-INV-02, C-INV-05, C-INV-07, C-INV-08 |
| P-TIPS-1, P-LEAD-1, P-CONS-1 | V-QC and view exit; leader finality; marshal lifecycle | C-INV-05, C-INV-07, C-INV-08; M-INV-01 through M-INV-04 |
| P-BND-1, P-RET-1, P-BOUND-1 | Retention; async boundaries; steady-state resource proof | C-INV-08 through C-INV-11 |
| P-MATTACH-1, P-MSTORE-1, P-MRES-1, P-MSYNC-1, P-MDEL-1, P-MBOUND-1 | Marshal service lifecycle; leader finality; steady-state resource proof | M-INV-01 through M-INV-07 |
...
participant to the committee identity, and Batcher asks the network blocker to quarantine that
...
1. Core assigns the observation identity before batched signature verification. After a valid
   verdict, it admits the producer header, records ancestry and forks, then routes an `ObserveBlock`
   command to that producer chain's validator plane. The command carries the process generation and
   whether local custody is already established.
...
3. Using Core's certified anchor and read-copy of durable DA choices, the plane derives the next
   contiguous eligible run and returns `DaVoteReady`. The plane cannot reserve a vote, change the
   anchor, or make a validation result durable. Its bounded mailbox and jobs isolate backpressure to
   that producer chain.
...
5. In increasing height order, `reserve_da_vote` may choose at most one header per `(chain, height)`
   and may not cross a gap, an unavailable predecessor, or depth `d` above the certified anchor.
6. The exact choice is staged durably before threshold-share signing. The share is exposed only after
   its covering prefix is acknowledged and is sent only to the producer that owns the chain.
7. `admit_da_share` retains the first valid share per participant and exact header. At `n - 2f`
   distinct shares the producer may request recovery of the canonical subset.
8. `da_recovered` accepts only the exact live job/transcript. The certificate is durably admitted and
   published, advances the certified anchor monotonically, retires covered block/share publications
   and obsolete validation work, sends the new anchor and choices to the plane, and then re-evaluates
   the next contiguous suffix.

These rules implement the specification's one-vote-per-height, contiguous-path, and depth-`d` DA
conditions without moving signing authority into the validator plane.
...
4. A current-view V-QC may authorize the specification's second voting route without repeating
   first-route validity checks. Its `2f + 1` designated votes prove that correct voters validated the
   proposal.
...
3. Only the first newly observed or assembled V-QC for a view is independently forwarded. A stronger
   same-view certificate remains durable candidate parent state. Under the v4 optimized anchor
   choice, a later proposal may select it but must disseminate that exact certificate because the
   receiver cannot rely on the earlier, different V-QC. An exact certificate already forwarded need
   not be sent twice.
...
7. Catch-up liveness uses the specification's total fault bound. Crashed or unavailable participants
   ...
   L-QC. Above `f`, exact exits still retire at the bounded retention floor and no additional witness
   or recovery branch attempts to restore progress outside the protocol model.
...
4. Per chain, the finalized proposal position is the `(3f + 1)`-th greatest position. Extension carry
   ...
   largest retained-voter count for any exact immediate child of the current finalized tip, or zero
   if no child is observed. This is more permissive when beyond-tip votes split across incompatible
...
The complete mapping from v4's lemmas, theorems, corollaries, and optimizations to implementation
properties is in [Specification v4 crosswalk](PROPERTIES.md#specification-v4-crosswalk). Consensus
owns leader-chain compatibility, safe and finalized tips, and authenticated history commitments.
Marshal owns history materialization, the selected deterministic sweep, dense output, and durable
delivery. Tests exercise these mechanisms and their boundary; they do not replace the protocol
proofs.
...
| application verify | generation, plane and validation or build request, exact header, payload digest | local custody or a plane's eligible-run input; Core alone reserves authority |
...
retries. Correct completions may arrive in any order. The exact live slot decides whether a late
result remains useful; an async worker never owns view order, producer-chain order, or a separate
semantic capacity limit.
...
- **C-INV-01:** Observation and event cursors are monotone; every pending completion references one
  live exact reservation of the same generation.
...
- **C-INV-03:** `acknowledged <= staged <= next`; staged ranges are contiguous; no authority exposure
  ...
  completion capacity.
- **C-INV-04:** Every publication obligation has one stable effect identity and exact durable owner;
...
- **M-INV-01:** Every archived object matches its exact key: L-QC ID, recursive history commitment,
  or full canonical producer-header `BlockRef`. A body commitment is never a producer-block identity.
- **M-INV-02:** The selected finalized LQC, history index, ordered frontier, emitted frontier,
...
- **M-INV-04:** Evidence, openings, blocks, and output rows are durable before checkpoint exposure. An
  ordinary checkpoint carries its exact remaining temporary-cleanup obligation, which startup
  completes before serving reads. Delivery advances only through a contiguous FIFO prefix whose
  `Exact` acknowledgements all resolve, and Catalog syncs that prefix's high-water.
- **M-INV-05:** A runtime floor first persists one bounded intent containing the exact verified LQC,
...
- **M-INV-06:** Resolver fetches recheck Catalog before peer work. Deliveries are bounded,
  generation-scoped, request-key exact, decoded under protocol limits, and independently verified
  before Catalog admission. Buffered broadcast is only an eager complete-block cache.
- **M-INV-07:** Every actor mailbox, router job, resolver request/subscriber set, block waiter, dense
...
Core tests call production transitions and inspect one normalized authoritative projection. The
validator-plane harness runs the same volatile per-chain state used by runtime tasks and offers
generation-scoped eligible runs back to Core. An attached-runtime parity test sends the same
observations through the runtime-event path and compares the authoritative projection.

Deterministic voter and Engine tests use real mailboxes, stores, application attachment, timers,
resolver, egress, and network actors. Focused recovery tests derive the exact retained producer/DA
payload union and exclude certificate-only roots and observers.
...
Engine and marshal and checks cursor recovery without duplicate delivery. Another full-service case
reports an L-QC without its local opening and verifies exact history and block backfill before
delivery. Public wire keys and durable types have conformance fixtures.

Bounded fuzz campaigns extend these fixed scenarios. Randomized Twins varies faulty identity
placement, network schedules, and a fair suffix while checking exact cross-node finality.
Crash-prefix campaigns compare replay with durable outcomes in the symbolic machine and interrupt
custody recovery in real engines. Structured certificate generation checks valid baselines and
targeted Byzantine mutations. Differential algebra compares incremental safe/final tip extraction
with materialized support counts. Differential marshal schedules compare incremental sweep,
reconciliation, and replay with explicit slot enumeration. Raw wire fuzzing covers bounded decode
and canonical re-encoding. The precise coverage and limits are listed in
[Fuzz campaign coverage](PROPERTIES.md#fuzz-campaign-coverage).
...
[`PROPERTIES.md`](PROPERTIES.md) maps each safety, liveness, and resource property to its authority
owner, legal mutators, durable witness, executable evidence, and invariant predicate.
