...
the in-scope consensus properties and their focused, deterministic, static, and assumption evidence.
...
and application acknowledgements are outside consensus. Consensus owns opaque application digests,
signed producer headers, DA evidence, protocol-constant leader blocks, tip commitments, and finalized
leader proofs. Reporter notifications are lossy hints and confer no delivery authority.

A producer header carries two different digests with different owners. Its `commitment` is the opaque
application-body digest passed to `Automaton` and `Relay`. Its protocol identity is the hash of the
complete canonical header `(epoch, chain, height, parent, commitment)`. Parent links, `BlockRef`s, DA
anchors, safe/finalized tips, and tip-history commitments always use that header identity. Keeping
these domains separate makes a producer fork unambiguous even when the application legitimately
returns the same body digest in different producer contexts.

A future Multimmit marshal is the executable owner of the paper's dense `Ord`/`Emit` transition. It
must independently archive or resolve authenticated finalized leader proofs and tip-history openings,
retrieve producer bodies, apply the settled/unsettled sweep and duplicate removal, and durably advance
its own delivery cursor. Consensus publication plus a Reporter hint is not an acknowledged handoff and
cannot substitute for that archive. Building the marshal is out of scope here; preserving the exact
proof/tip-commitment boundary it needs is in scope.
...
leader blocks -> V-QCs -> finalized leader chain + tip commitments
        |
        | sparse authenticated agreement facts
        v
future marshal                               (outside consensus)
        | resolve bodies + Ord/Emit + durable delivery cursor
        v
...
- **Local production:** the local certified anchor, greatest durably chosen header, at most one live
  proposal request, pacing state, and the exact pending signing/publication state.
...
The `log-multimmit` example is the compiling end-to-end reference. `Inspection` and `serve(view)` are
diagnostic/proof-serving APIs; neither is an application delivery stream.
...
1. A wake is eligible only when the local role owns a producer chain, work/pacing policy allows it,
   no build is live, and `next_height - certified_height <= d`.
2. The application receives only protocol context and the parent digest. Its `propose` success means
   both parent and returned candidate are durably reconstructible in application-owned storage.
...
4. Success stages one durable producer-header signing reservation. The signature may be computed in
   parallel with journal I/O, but neither signature nor header may leave private process memory until
   the exact covering durable prefix is acknowledged.
5. After acknowledgement, signing completion self-admits the header and installs stable body-relay
   and header-publication obligations. Relay acceptance may order the first attempts, but volatile
   transport feedback never retires either obligation.
6. Only a later durable semantic successor, such as a covering DA certificate, retires an obligation.

### Application verification and DA voting

1. A correctly signed remote header may be verified speculatively and out of height order. The
   application receives only context and the opaque digest. `verify(true)` means the parent and
   candidate are durably reconstructible and the candidate is permanently valid. Temporary absence
   keeps the request pending; `false` is permanent invalidity.
...
7. Consensus publishes sparse finalized-leader/protocol activity. It does not materialize, store, or
   acknowledge the paper's dense `Emit` stream.
...
- Theorem 3 (block inclusion, hence censorship resistance) is conditional: every correct processor
  ...
  when the current leader omits it; the external marshal realizes the theorem's dense ordering.
- Corollary 3 (chain-local finality) fixes one chain's contribution through its finalized tip and
  ...
  monotone finalized tip and authenticated tip history. The future marshal owns the dense `Ord` and
  `Emit` realization, so consensus tests cannot claim to execute the corollary's full ordering path.
...
satisfies Theorem 3's hypotheses. Neither finite test is a proof of the paper result.
...
| application propose/verify | generation, request, producer context, payload digest | whether a producer header/DA choice may be reserved |
...
- Verification, application, signing, recovery, and aggregation jobs each consume a class-specific
  permit plus reserved completion capacity before becoming mandatory. Per-chain rotation prevents one
  producer from monopolizing remote validation.
...
- Stable publication retries retain one bounded durable row per live obligation, not one row per
  ...
  outbox scan or starvation at a saturated clock. Reporter backpressure retains no semantic state.
- Dependency availability is retired by provider and consumer reachability for every artifact kind,
  including leader dependencies; unique retired leaders cannot accumulate forever.
...
Therefore memory and disk usage plateau across arbitrarily many views, including runs where producer
or leader finality stalls, provided configured external network/application/storage capabilities
continue making the progress required by the protocol. A stalled async dependency consumes its
pre-reserved bounded slot; it cannot create new unbounded work.
