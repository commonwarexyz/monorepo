...
fn voter_limits<H: Hasher, V: Variant>(
    ...
    VoterLimits {
        inflight_application: NonZeroUsize::new(profile.validation_capacity())
...
impl From<ReplayError> for OpenError {
    ...
    V: Variant,
{
    ...
    let (checkpoints, checkpoint) = CheckpointStore::<_, V, H::Digest>::open(
...
mod tests {
    ...
    fn voter_limits_are_nonzero_and_overflow_safe() {
        ...
        for view_timeout in [Duration::from_nanos(1), Duration::MAX] {
            ...
            assert_eq!(limits.inflight_application.get(), 12);
    ...
    #[test]
    fn voter_limits_cover_each_producer_pipeline() {
        let producers = (0..11).map(Participant::new).collect();
        let committee = Committee::<MinPk>::new_with_namespace_and_producers(
            79,
            b"_COMMONWARE_CONSENSUS_MULTIMMIT_ALL_CHAIN_CAPACITY_TEST",
            11,
            producers,
            Limits::new(2, 1).unwrap(),
        );
        let deployed_profile = profile(&committee, Role::Observer);

        assert_eq!(
            voter_limits(&deployed_profile, CHECKPOINT_INTERVAL)
    ...
    #[test]
    fn voter_limits_do_not_exceed_retained_artifact_capacity() {
        let producers = (0..100).map(Participant::new).collect();
        let committee = Committee::<MinPk>::new_with_namespace_and_producers(
            89,
            b"_COMMONWARE_CONSENSUS_MULTIMMIT_APPLICATION_ARTIFACT_CAP_TEST",
            100,
            producers,
            Limits::new(800, 1).unwrap(),
        );
        let profile: Profile<Sha256, MinPk> = Profile::new(
            committee.config,
            Role::Observer,
        ...
        let uncapped = profile
            ...
            .codec_config()
            .chains()
            .checked_mul(profile.validation_parallelism())
            .unwrap();

        assert!(uncapped > profile.resources().max_cached_artifacts());
        assert_eq!(
            voter_limits(&profile, CHECKPOINT_INTERVAL)
                .inflight_application
                .get(),
