...
use crate::{
    ...
    multimmit::types::{Activity as MultimmitActivity, Context},
};
...
pub struct MockApplication {
    ...
    verify_gate: Option<Arc<ApplicationGateState>>,
...
impl MockApplication {
    ...
    pub fn with_gated_verification_result(result: Option<bool>) -> (Self, MockBuildGate) {
        let mut application = Self::new();
        let (gate_state, gate) = ApplicationGateState::one_shot();
        application.verify_gate = Some(gate_state);
...
impl Automaton for MockApplication {
    ...
    fn verify(
        ...
        let log = Arc::clone(&self.log);
        let gate = self.verify_gate.take();
...
/// Configurable payload relay that records every commitment and returned response.
#[cfg(test)]
...
impl<D: Digest, P: PublicKey> RecordingRelay<D, P> {
    ...
    /// Returns the commitments and responses observed so far, in call order.
