...
impl<V: Variant> Cluster<V> {
    ...
    /// Launches one engine with its best-effort activity stream attached to `reporter`.
    #[cfg(test)]
    pub async fn launch_with_reporter<S>(&mut self, index: usize, mut reporter: S)
    where
        S: Reporter<Activity = Activity<V, Sha256Digest>> + Send + 'static,
    {
        ...
        let mut planes = planes.into_iter();
        let data = planes.next().unwrap();
        let consensus = planes.next().unwrap();
        let certificates = planes.next().unwrap();
        let resolver = planes.next().unwrap();
        let consensus = (
            BlockingSender {
                inner: consensus.0,
                me: me.clone(),
        ...
        Rr: commonware_p2p::Receiver<PublicKey = ed25519::PublicKey>,
    {
        self.launch_with_callback(
            ...
            certificates,
            resolver,
            ClusterReporter::default(),
        )
        .await;
    }

    #[allow(clippy::too_many_arguments)]
    pub(crate) async fn launch_with_transports_and_reporter<S, Sd, Sg, Rc, Sc, Sr, Rr>(
        &mut self,
        slot: usize,
        signer: usize,
        label: Option<&'static str>,
    ...
    #[allow(clippy::too_many_arguments)]
    async fn launch_with_callback<Sd, Sg, Rc, Sc, Sr, Rr>(
        &mut self,
        slot: usize,
        signer: usize,
        label: Option<&'static str>,
