...
//!
//! Nothing here is protocol-specific. A caller supplies its own participants,
//! its own view-to-round mapping through [`TermLength`], and its own leader
//! policy: [`RoundScenario::leader`] is the scenario's *chosen* leader, which a
//! protocol with a fixed schedule may instead use to select which view runs the
//! round. Campaigns cover small representative schedules rather than
//! production-sized validator sets; participant sets are capped at 64 so
//! recipient sets and residual cell boundaries fit in plain `u64` masks.
//!
