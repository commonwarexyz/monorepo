//! Protocol-neutral Twins scenario generation.
//!
...
//! twins. This harness uses a stronger recipient-set model: the primary and
//! secondary halves are independent recipient masks, so a participant may be
//! visible to both halves, one half, or neither half in a scripted round. After
//! the scripted prefix, traffic returns to all-to-all synchrony.
//!
//! The generator does not depend on any protocol. A caller supplies its own
//! participants, maps views to scripted rounds through [`TermLength`], and makes
//! each scripted round's view elect [`RoundScenario::leader`]: simplex wraps its
//! elector in [`crate::simplex::mocks::twins::Elector`], and a protocol with a
//! fixed leader schedule can place each round's leader at that view's slot.
//! Campaigns cover small representative schedules rather than production-sized
//! validator sets; participant sets are capped at 64 so recipient sets and
//! residual cell boundaries fit in plain `u64` masks.
//!
...
//! are ever emitted. When the scenario space exceeds the configured budget,
//! sampled campaigns choose canonical scenarios uniformly without replacement.
//! For each selected scenario, [`cases`] generates only the unique
//! compromised-node assignments; two assignments differing only in which
//! members of a symmetry cell are chosen collapse to one representative.

use crate::types::{TermLength, View};
...
impl RoundScenario {
    /// Creates one scripted round from explicit recipient masks.
    ///
    /// Bit `i` of each mask selects participant `i`; the masks may overlap, in which case that
    /// participant exchanges messages with both twin halves.
    pub const fn new(leader: usize, primary_mask: u64, secondary_mask: u64) -> Self {
        Self {
            leader,
            primary_mask,
            secondary_mask,
...
impl Scenario {
    /// Creates a scenario from explicit scripted rounds.
    pub const fn new(rounds: Vec<RoundScenario>) -> Self {
        Self { rounds }
    }
...
/// Returns the zero-based scripted round index for `view`.
pub fn term_index(view: View, term_length: TermLength) -> usize {
...
mod tests {
    use super::*;
    use commonware_utils::{NZU32, TestRng, test_rng};
    use std::{collections::HashSet, panic::catch_unwind};

    fn round(leader: usize, primary_mask: u64, secondary_mask: u64) -> RoundScenario {
        RoundScenario {
            leader,
            primary_mask,
            secondary_mask,
    ...
    fn expected_single_round_transitions(n: usize) -> HashSet<(RoundScenario, Cells)> {
        ...
        for outside in 0..n {
            ...
            out.insert((round(leader, mask, mask), cells));
        }
        ...
        for outside in 0..n {
            ...
            for both in 0..=remaining_after_outside {
                ...
                for secondary in 0..=remaining {
                    ...
                    out.insert((
                        round(leader, primary_mask | leader_bit, secondary_mask),
                        cells.clone(),
                    ));
                    out.insert((
                        round(leader, primary_mask, secondary_mask | leader_bit),
                        cells.clone(),
                    ));
                    out.insert((
                        round(
    ...
    fn scripted_partitions_preserve_participant_order_and_overlap() {
        let scenario = Scenario {
            rounds: vec![round(3, 0b1011, 0b0110)],
        };
    ...
    fn route_supports_shared_non_leaders_in_split_rounds() {
        let scenario = Scenario {
            rounds: vec![round(4, 0b11010, 0b00110)],
        };
        let participants: Vec<u32> = (0..5).collect();
        assert_eq!(
    ...
    fn leader_visible_to_both_halves_preserves_partitions_and_route() {
        let scenario = Scenario {
            rounds: vec![round(3, 0b1010, 0b1100)],
        };
        let participants: Vec<u32> = (0..4).collect();
        let (primary, secondary) =
            scenario.partitions(View::new(1), TermLength::ONE, &participants);
    ...
    fn route_selects_correct_half() {
        let scenario = Scenario {
            rounds: vec![round(0, 0b0011, 0b1100)],
        };
        let participants: Vec<u32> = (0..4).collect();
        assert_eq!(
            scenario.route(View::new(1), TermLength::ONE, &0, &participants),
    ...
    fn route_returns_none_for_participants_outside_selected_partitions() {
        let scenario = Scenario {
            rounds: vec![round(0, 0b0001, 0b0010)],
        };
        let participants: Vec<u32> = (0..4).collect();
        assert_eq!(
            scenario.route(View::new(1), TermLength::ONE, &2, &participants),
