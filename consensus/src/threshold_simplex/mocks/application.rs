...
impl<E: Clock + RngCore, H: Hasher, P: Array> Application<E, H, P> {
    ...
    async fn skipped(&mut self, proof: Proof, payload: View) {
        if !self.skipped_views.insert(payload) {
            self.panic("view already skipped");
        }
        let _ = self
            .tracker
            .send((self.participant.clone(), Progress::Skipped(proof, payload)))
            .await;
    }
            ...
            select! {
                message = self.mailbox.next() => {
                    ...
                    match message {
                        ...
                        Message::Skipped { proof, payload } => {
                            self.skipped(proof, payload).await;
                        }
                    }
                },
