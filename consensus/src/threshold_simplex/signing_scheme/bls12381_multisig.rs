...
fn message_bytes<D: Digest>(namespace: &[u8], context: VoteContext<'_, D>) -> (Vec<u8>, Vec<u8>) {
    match context {
...
impl<V: Variant + Send + Sync> signing_scheme::Scheme for Scheme<V> {
    ...
    fn sign_vote<D: Digest>(&self, namespace: &[u8], context: VoteContext<'_, D>) -> Vote<Self> {
        ...
        let (domain, message) = message_bytes(namespace, context);
        let signature = sign_message::<V>(private_key, Some(domain.as_ref()), message.as_ref());
        ...
        I: IntoIterator<Item = Vote<Self>>,
    {
        let (domain, message) = message_bytes(namespace, context);
        let message_hash =
            hash_message_namespace::<V>(V::MESSAGE, domain.as_ref(), message.as_ref());
        ...
        I: Iterator<Item = (VoteContext<'a, D>, &'a Self::Certificate)>,
    {
        ...
        for (context, certificate) in certificates {
            ...
            let (domain, message) = message_bytes(namespace, context);
            let hm = hash_message_namespace::<V>(V::MESSAGE, domain.as_ref(), message.as_ref());
...
mod tests {
    ...
    fn scheme_clone_and_into_verifier<V: Variant>() {
        ...
        let pure_verifier = Scheme::<V>::verifier(participants);
