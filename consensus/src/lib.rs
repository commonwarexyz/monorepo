...
stability_scope!(BETA, cfg(not(target_arch = "wasm32")) {
    ...
    /// bucket.
    ///
    /// Every consensus latency histogram shares this set so a deployment pays one bucket
    /// layout, and so attached applications can report on the same scale.
    pub const LATENCY: [f64; 58] = [
