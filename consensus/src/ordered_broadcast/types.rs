...
/// Used when signing and verifying chunks to prevent signature reuse across
/// different message types.
pub const CHUNK_SUFFIX: &[u8] = b"_CHUNK";

/// Suffix used to identify an acknowledgment (ack) namespace for domain
/// separation. Used when signing and verifying acks to prevent signature reuse
/// across different message types.
pub const ACK_SUFFIX: &[u8] = b"_ACK";

/// Returns a suffixed namespace for signing a chunk.
///
/// This provides domain separation for signatures, preventing cross-protocol
/// attacks by ensuring signatures for chunks cannot be reused for other message
/// types.
#[inline]
...
///
/// This provides domain separation for signatures, preventing cross-protocol
/// attacks by ensuring signatures for acks cannot be reused for other message
/// types.
#[inline]
...
///
/// This is not a single "View" in the sense of a consensus protocol, but rather
/// a continuous sequence of views in which the set of sequencers and validators
/// is constant. When the set of participants changes, the epoch increments.
pub type Epoch = u64;

/// Used as the [crate::Automaton::Context] type.
///
/// Carries the necessary context for the automaton to verify a payload,
/// including the sequencer's public key and its sequencer-specific height.
...
/// Chunk is a message generated by a sequencer that is broadcasted to all
/// validators.
///
/// A chunk represents a unit of data in the ordered broadcast system. Each
/// sequencer maintains its own chain of chunks with monotonically increasing
/// heights. Validators acknowledge chunks with partial signatures, which are
/// then aggregated into threshold signatures to prove reliable broadcast.
...
/// Parent is a message that contains information about the parent (previous
/// height) of a Chunk.
///
/// The sequencer and height are not provided as they are implied by the
/// sequencer and height of the current chunk. The parent includes a threshold
/// signature which proves that a quorum of validators have seen and
/// acknowledged the parent chunk, making it an essential part of the chain
/// linking mechanism.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
...
impl<V: Variant, D: Digest> Parent<V, D> {
    ...
    /// the threshold signature that proves the predecessor was reliably
    /// broadcast.
...
/// Node is a message from a sequencer that contains a Chunk and a proof that
/// the parent was correctly broadcasted.
///
/// It represents a newly-proposed tip of the chain for the given sequencer. The
/// node includes:
/// 1. The chunk itself (sequencer, height, payload)
/// 2. The sequencer's signature over the chunk
/// 3. For non-genesis nodes (height > 0), proof that the previous chunk was
///    acknowledged by a quorum of validators
///
/// Nodes form a linked chain from each sequencer, ensuring that new chunks can
/// only be added after their predecessors have been properly acknowledged by
/// the validator set.
#[derive(Clone, Debug)]
pub struct Node<C: PublicKey, V: Variant, D: Digest> {
    ...
    ///
    /// This part is not signed over, but it is used to verify that the previous
    /// chunk in the chain was correctly broadcast. It contains the
    /// threshold signature that proves a quorum of validators acknowledged
    /// the parent.
    ///
...
impl<C: PublicKey, V: Variant, D: Digest> Node<C, V, D> {
    ...
    /// For non-genesis nodes (height > 0), a parent with threshold signature
    /// must be provided.
...
///
/// These partial signatures from validators can be aggregated to form a
/// threshold signature once enough validators (a quorum) have acknowledged the
/// chunk. This threshold signature serves as proof that the chunk was reliably
/// broadcast.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct Ack<P: PublicKey, V: Variant, D: Digest> {
    ...
    /// This is a cryptographic signature that can be combined with other
    /// partial signatures to form a threshold signature once a quorum is
    /// reached.
...
impl<P: PublicKey, V: Variant, D: Digest> Ack<P, V, D> {
    ...
    ///
    /// This constructs the message that is signed by validators when
    /// acknowledging a chunk. It contains both the chunk and the epoch to
    /// ensure domain separation and prevent signature reuse across epochs.
    ...
    /// Verify the Ack.
    ///
    /// This ensures that the partial signature is valid for the given chunk and
    /// epoch, using the provided polynomial (which contains the BLS public
    /// polynomial).
    ///
    ...
    /// Generate a new Ack.
    ///
    /// This is used by validators to create and sign new acknowledgments for
    /// chunks. It creates a partial signature over the chunk and epoch
...
///
/// The Reporter is notified of these activities so it can track the state of
/// the system and provide the appropriate information to other components.
#[allow(clippy::large_enum_variant)]
#[derive(Clone, Debug, PartialEq)]
pub enum Activity<C: PublicKey, V: Variant, D: Digest> {
    /// A new tip for a sequencer
    ///
    /// This activity is only emitted when the application has verified some
    /// peer proposal.
    Tip(Proposal<C, D>),
    /// A threshold signature for a chunk, indicating it has been acknowledged
    /// by a quorum
    Lock(Lock<C, V, D>),
}
...
/// Proposal is a message that is generated by a sequencer when proposing a new
/// chunk.
///
/// This represents a new chunk that has been created by a sequencer and is
/// being broadcast to validators for acknowledgment. It contains the chunk
/// itself and the sequencer's signature over that chunk.
#[derive(Clone, Debug)]
...
/// Lock is a message that can be generated once `2f + 1` acks are received for
/// a Chunk.
///
/// A Lock represents proof that a quorum of validators (at least 2f+1, where f
/// is the maximum number of faulty validators) have acknowledged a chunk. This
/// proof is in the form of a threshold signature that can be verified by anyone
/// with the public key of the validator set.
///
...
impl<P: PublicKey, V: Variant, D: Digest> Lock<P, V, D> {
    ...
    /// Verify the Lock.
    ///
    /// This ensures that the threshold signature is valid for the given chunk
    /// and epoch, using the provided public key of the validator set.
    ///
    /// Returns true if the signature is valid, false otherwise.
...
mod tests {
    ...
    fn test_node_genesis_with_parent_panics() {
        // Try to create a genesis node (height 0) with a parent - should panic on
        // decode
        let public_key = sample_scheme(0).public_key();
        let chunk = Chunk::new(public_key.clone(), 0, sample_digest(1));
        let chunk_namespace = chunk_namespace(NAMESPACE);
    ...
    fn node_verify_invalid_parent_signature<V: Variant>() {
        ...
        // Verification should fail because the parent signature doesn't verify with the
        // correct public key
        assert!(matches!(
    ...
    fn test_proposal_verify_wrong_sequencer() {
        ...
        // Verification should fail because the signature doesn't match the sequencer's
        // public key
        assert!(!proposal.verify(NAMESPACE));
    }
    ...
    fn node_genesis_with_parent_fails<V: Variant>() {
        ...
        // Create a parent with a random BLS signature (content doesn't matter for this
        // test)
        let n = 4;
        let t = quorum(n as u32);
