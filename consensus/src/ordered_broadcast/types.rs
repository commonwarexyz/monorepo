...
impl<C: Verifier, D: Digest> Node<C, D> {
    ...
    pub fn sign<S: Scheme<PublicKey = C::PublicKey, Signature = C::Signature>>(
        ...
        let signature = scheme.sign(Some(chunk_namespace), &chunk.encode());
...
impl<P: Array, D: Digest> Ack<P, D> {
    ...
    /// Generate a new Ack.
    pub fn sign(share: &Share, chunk: Chunk<P, D>, epoch: Epoch, ack_namespace: &[u8]) -> Self {
        // Construct signing payload
        let message = Self::payload(&chunk, &epoch);

        // Sign message
        let signature = ops::partial_sign_message(share, Some(ack_namespace), &message);
...
/// Verify a threshold signature over a chunk.
fn verify_threshold<P: Array, D: Digest>(
    public_key: &Public,
    chunk: &Chunk<P, D>,
    epoch: &Epoch,
    signature: &Signature,
