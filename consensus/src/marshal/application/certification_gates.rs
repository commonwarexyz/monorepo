    ...
    D: Eq + Hash + Copy,
{
    ...
    /// panics at its source; a dropped receiver or handle means the marshal actor is gone at
    /// shutdown, so the gate is left unresolved and `certify` falls back to its recovery fetch.
    /// Returns whether the block reached durability.
