    ...
    T: Strategy,
{
    ...
    async fn handle_external_proposal<Sr: Sender<PublicKey = P>>(
        ...
        if scheme.participants().index(&leader).is_none() {
    ...
    /// Drops all subscriptions associated with a commitment, including
    /// digest-scoped block subscriptions.
        ...
        fixture.start(
            |config, context, _oracle, mut peers, coding_config| async move {
                ...
                assert!(
                    ...
                    "digest subscription should remain pending after failed reconstruction"
                );
                ...
                let reconstructed_by_digest = digest_sub
                    .await
                    .expect("digest subscription should resolve after valid reconstruction");
                assert_eq!(reconstructed_by_digest.commitment(), real_commitment1);
            },
        );
    }
