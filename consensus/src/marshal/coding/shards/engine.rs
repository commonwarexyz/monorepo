...
mod tests {
    ...
    fn test_cross_epoch_buffered_shard_not_blocked() {
        ...
        executor.start(|context| async move {
            ...
            let (engine, mut mailbox) = ShardEngine::new(context.with_label("receiver"), config);
    ...
    fn test_failed_reconstruction_digest_mismatch_then_recovery() {
        ...
        fixture.start(
            |config, context, _oracle, mut peers, coding_config| async move {
                ...
                let mut block_sub = peers[receiver_idx]
                    .mailbox
                    .subscribe(fake_commitment)
                    .await;
                ...
                let mut strong_shard =
                    coded_block2.shard::<H>(receiver_shard_idx).expect("missing shard");
                strong_shard.commitment = fake_commitment;
                peers[0]
                    .sender
                    .send(Recipients::One(receiver_pk.clone()), strong_shard.encode(), true)
                    .await
                    .expect("send failed");
