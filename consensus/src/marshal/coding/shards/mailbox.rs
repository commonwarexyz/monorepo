...
use crate::{
    marshal::coding::{
        ...
        types::{CodedBlock, CodingCommitment, DigestOrCommitment},
    },
...
where
    ...
    P: PublicKey,
{
    /// Broadcast a [CodedBlock]'s shards to a set of peers.
    Broadcast {
    ...
    P: PublicKey,
{
    ...
    pub fn new(sender: mpsc::Sender<Message<B, S, C, P>>) -> Self {
        Self { sender }
    }

    /// Broadcast an erasure coded block's shards to a set of peers.
    pub async fn broadcast(&mut self, block: CodedBlock<B, C>, peers: Vec<P>) {
