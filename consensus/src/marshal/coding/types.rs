...
impl<B: Block, C: Scheme<Commitment = B::Digest>> CodedBlock<B, C> {
    ...
    fn encode(
        ...
        C::encode(&config, buf.as_slice(), concurrency).expect("must encode block successfully")
    }

    /// Create a new [CodedBlock] from a [Block] and a configuration.
    pub fn new(inner: B, config: CodingConfig, concurrency: usize) -> Self {
        let (commitment, shards) = Self::encode(&inner, config, concurrency);
        Self {
            inner,
            config,
            commitment,
    ...
    pub fn shards(&mut self, concurrency: usize) -> &[C::Shard] {
        match self.shards {
            Some(ref shards) => shards,
            None => {
                let (commitment, shards) = Self::encode(&self.inner, self.config, concurrency);
...
impl<B: Block, C: Scheme> Read for CodedBlock<B, C> {
    type Cfg = (usize, <B as Read>::Cfg);

    fn read_cfg(
        buf: &mut impl bytes::Buf,
        (concurrency, block_cfg): &Self::Cfg,
    ) -> Result<Self, commonware_codec::Error> {
        ...
        let (commitment, shards) =
            C::encode(&config, buf.as_slice(), *concurrency).map_err(|_| {
...
impl<B: Block, C: Scheme<Commitment = B::Digest>> StoredCodedBlock<B, C> {
...
impl<B: Block, C: Scheme<Commitment = B::Digest>> Committable for StoredCodedBlock<B, C> {
    type Commitment = CodingCommitment;
...
impl<B: Block, C: Scheme<Commitment = B::Digest>> Read for StoredCodedBlock<B, C> {
...
impl<B: Block, C: Scheme<Commitment = B::Digest>> Block for StoredCodedBlock<B, C> {
    fn height(&self) -> u64 {
        self.inner.height()
    }
...
mod test {
    ...
    fn test_distribution_shard_codec_roundtrip() {
        ...
        let (_, shards) = RS::encode(&CONFIG, MOCK_BLOCK_DATA, CONCURRENCY).unwrap();
    ...
    fn test_coded_block_codec_roundtrip() {
        ...
        let coded_block = CodedBlock::<Block, RS>::new(block, CONFIG, CONCURRENCY);

        let encoded = coded_block.encode();
        let decoded =
            CodedBlock::<Block, RS>::decode_cfg(encoded.freeze(), &(CONCURRENCY, ())).unwrap();
    ...
    fn test_stored_coded_block_codec_roundtrip() {
        ...
        let decoded = StoredCodedBlock::<Block, RS>::decode_cfg(encoded.freeze(), &()).unwrap();
