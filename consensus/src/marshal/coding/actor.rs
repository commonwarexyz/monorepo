...
/// [Block]s as [Shard]s.
///
/// When enough [Shard]s are present in the mailbox, the [Actor] may facilitate
/// reconstruction of the original [Block] and notify any subscribers waiting for it.
pub struct Actor<E, V, S, H, B, P>
where
    ...
    P: PublicKey,
{
    ...
    buffer: buffered::Mailbox<P, Shard<S, H>>,
...
impl<E, V, S, H, B, P> Actor<E, V, S, H, B, P>
where
    E: Rng + Spawner + Metrics + Clock + GClock,
    V: Variant,
    S: Scheme,
    ...
    P: PublicKey,
{
    ...
    async fn run(mut self) {
        let mut shard_validity_waiters =
            AbortablePool::<((CodingCommitment, usize), Shard<S, H>)>::default();
            ...
            select! {
                ...
                message = self.mailbox.next() => {
                    let Some(message) = message else {
                        info!("Shard mailbox closed, shutting down");
                        return;
                    };
                    match message {
                        ...
                        Message::SubscribeBlock {
                            ...
                            self.subscribe_block(commitment, response).await;
                        }
    ...
    async fn broadcast_shards(&mut self, mut block: CodedBlock<B, S>, participants: Vec<P>) {
        assert_eq!(
            participants.len(),
            block.shards().len(),
    ...
    async fn broadcast_shard(&mut self, shard: Shard<S, H>) {
    ...
    pub async fn subscribe_shard_validity(
        ...
        pool: &mut AbortablePool<((CodingCommitment, usize), Shard<S, H>)>,
    ) {
    ...
    pub async fn subscribe_block(
        ...
        // Attempt reconstruction in case it hasn't been attempted yet.
        let _ = self.try_reconstruct(commitment).await;
    }
}
...
mod test {
    ...
    // type SMailbox = ShardMailbox<deterministic::Context, ReedSolomon<Sha256>, Sha256, B, PublicKey>;
    //
