    ...
    A: Acknowledgement,
{
        ...
        if let Some((height, commitment, round)) = tip {
            application
                .report(Update::Tip(
                    round,
                    height,
                    commitment.block_digest::<B::Digest>(),
                ))
                .await;
            self.tip = height;
                ...
                match message {
                    Message::GetInfo { identifier, response } => {
                        let info = match identifier {
                            ...
                            BlockID::Latest => self.get_latest().await.map(|(h, c, _)| (h, c.block_digest::<B::Digest>())),
                        };
                        ...
                        match identifier {
                            ...
                            BlockID::Latest => {
                                let block = match self.get_latest().await {
                                    Some((_, commitment, _)) => self.find_block(&mut buffer, DigestOrCommitment::Commitment(commitment)).await,
                                    None => None,
                                };
    ...
    /// returned.
    async fn get_latest(&mut self) -> Option<(Height, CodingCommitment, Round)> {
    ...
    async fn find_block<K: PublicKey>(
        ...
        let digest = match id {
            ...
            DigestOrCommitment::Digest(digest) => digest,
        };
