...
mod tests {
    ...
    fn test_subscription_by_commitment_rejects_digest_alias() {
        ...
        runner.start(|mut context| async move {
            ...
            let block_rx = marshal.subscribe_by_commitment(Some(round), commitment_a).await;
            context.sleep(Duration::from_millis(100)).await;
            marshal.proposed(round, coded_block_b).await;
            select! {
                result = block_rx => {
                    let block = result.expect("subscription should stay open for mismatched commitment");
                    panic!(
            ...
            select! {
                result = block_rx => {
                    let block = result.expect("subscription should resolve for matching commitment");
                    assert_eq!(
                        block.commitment(),
                        commitment_a,
