...
mod tests {
        ...
        marshal::{
            ...
            mocks::{
                harness::{
                    ...
                    BLOCKS_PER_EPOCH, LINK, NAMESPACE, NUM_VALIDATORS, QUORUM, S,
                    UNRELIABLE_LINK, V,
                },
    ...
    #[test_traced("WARN")]
    fn test_verify_rejects_invalid_coding_config_one_zero() {
        // A commitment with coding config (minimum_shards=1, extra_shards=0) is invalid
        // for the validator set in this harness and should be rejected at verify().
        let runner = deterministic::Runner::timed(Duration::from_secs(30));
        runner.start(|mut context| async move {
            let mut oracle = setup_network(context.clone(), None);
            let Fixture {
            ...
            let invalid_payload = Commitment::from((
                Sha256::hash(b"invalid-block"),
                Sha256::hash(b"invalid-root"),
                context_hash(&verify_context),
            ...
            let verify_rx = marshaled.verify(verify_context, invalid_payload).await;
            select! {
                result = verify_rx => {
                    assert!(
                        !result.unwrap(),
                        "verify should reject invalid coding config (1 minimum, 0 extra)"
                    );
                },
                _ = context.sleep(Duration::from_secs(5)) => {
                    panic!("verify should reject invalid coding config promptly");
                },
            }
        })
    }
    ...
    fn test_backfill_block_mismatched_commitment() {
        ...
        runner.start(|mut context| async move {
            ...
            let coding_config_b = commonware_coding::Config {
                ...
                extra_shards: coding_config_a.extra_shards - 1,
            };
