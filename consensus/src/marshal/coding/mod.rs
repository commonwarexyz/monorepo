...
mod tests {
    ...
    fn test_prune_finalized_archives() {
        ...
        runner.start(|mut context| async move {
            ...
            // Initial setup
            let (mut mailbox, mut shards, application) = init_marshal(context.clone()).await;
            ...
            drop(mailbox);
            let (mut mailbox, _shards, _application) = init_marshal(context).await;
    ...
    fn test_subscribe_blocks_from_different_sources() {
        ...
        runner.start(|mut context| async move {
            ...
            let block1 = CodedBlock::new(
                B::new::<Sha256>(parent, Height::new(1), 1),
                coding_config,
                &Sequential,
            );
            let block2 = CodedBlock::new(
                B::new::<Sha256>(block1.digest(), Height::new(2), 2),
                coding_config,
                &Sequential,
            );
            let block3 = CodedBlock::new(
                B::new::<Sha256>(block2.digest(), Height::new(3), 3),
                coding_config,
                &Sequential,
            );
    ...
    fn test_get_info_latest_progression_multiple_finalizations() {
        ...
        runner.start(|mut context| async move {
            ...
            let block1 = CodedBlock::new(
                B::new::<Sha256>(parent0, Height::new(1), 1),
                coding_config,
                &Sequential,
            );
            ...
            let block2 = CodedBlock::new(
                B::new::<Sha256>(d1, Height::new(2), 2),
                coding_config,
                &Sequential,
            );
            ...
            let block3 = CodedBlock::new(
                B::new::<Sha256>(d2, Height::new(3), 3),
                coding_config,
                &Sequential,
            );
    ...
    fn test_get_block_by_commitment_from_sources_and_missing() {
        ...
        runner.start(|mut context| async move {
            ...
            let fin_block = CodedBlock::new(
                B::new::<Sha256>(not_digest, Height::new(2), 2),
                coding_config,
                &Sequential,
            );
    ...
    fn test_hint_finalized_triggers_fetch() {
        ...
        runner.start(|mut context| async move {
            ...
            for i in 1..=5u64 {
                let block = CodedBlock::new(
                    B::new::<Sha256>(parent, Height::new(i), i),
                    coding_config_for_participants(NUM_VALIDATORS as u16),
                    &Sequential,
                );
                let commitment = block.commitment();
