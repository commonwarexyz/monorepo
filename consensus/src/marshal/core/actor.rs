    ...
    A: Acknowledgement,
{
        ...
        >,
    {
        ...
        match message {
            ...
            Message::Notarization { notarization, .. } => {
                ...
                let commitment = notarization.claim.payload;
            ...
            Message::Finalization { finalization, .. } => {
                ...
                let commitment = finalization.claim.payload;
                let digest = V::commitment_to_inner(commitment);
    ...
    async fn handle_deliver<Buf: Buffer<V>>(
        ...
        match key {
            ...
            Key::Notarized { round } => {
                ...
                if V::commitment(&block) != notarization.claim.payload {
                    response.send_lossy(false);
                    return false;
                }
        ...
        for (index, item) in delivers.drain(..).enumerate() {
            ...
            match item {
                PendingVerification::Finalized {
                    ...
                    response.send_lossy(true);
                    let block = V::from_application_block(block, finalization.claim.payload);
                    let round = finalization.round();
                    let height = block.height();
                    let digest = block.digest();
    ...
    async fn get_info_by_height(
        ...
        if let Some(finalization) = self.get_finalization_by_height(height).await {
            return Some((height, V::commitment_to_inner(finalization.claim.payload)));
        }
    ...
    async fn get_latest(&mut self) -> Option<(Height, <V::Block as Digestible>::Digest, Round)> {
        ...
        Some((
            height,
            V::commitment_to_inner(finalization.claim.payload),
            finalization.round(),
        ))
    }
