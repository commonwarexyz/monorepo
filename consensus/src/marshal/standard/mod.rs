...
mod tests {
    ...
    fn test_standard_restart_surfaces_block_without_finalization() {
        ...
        runner.start(|mut context| async move {
            ...
            assert_eq!(
                recovering.mailbox.get_info(Identifier::Latest).await.unwrap_or_default(),
                Some((Height::new(1), block_one.digest())),
                "latest tip should be derived from the highest stored finalization"
            );
            assert_eq!(
                recovering.mailbox.get_block(Height::new(2)).await.unwrap_or_default(),
                Some(block_two.clone()),
                "block without a finalization row should still be queryable by height"
            );
            assert_eq!(
                recovering.mailbox.get_info(Height::new(2)).await.unwrap_or_default(),
    ...
    fn test_standard_verify_height_lie_parent_fetch_is_round_bound() {
        for kind in wrapper_kinds() {
            ...
            runner.start(|mut context| async move {
                ...
                assert!(marshal.verified(child_round, child).await.is_ok());
    ...
    fn test_standard_verify_parent_fetch_invalid_first_retries() {
        for kind in wrapper_kinds() {
            ...
            runner.start(|mut context| async move {
                ...
                assert!(honest_mailbox.verified(parent_round, parent.clone()).await.is_ok());
                StandardHarness::report_notarization(&mut honest_mailbox, parent_notarization)
                    .await;
                assert!(honest_mailbox.get_block(&parent_digest).await.unwrap_or_default().is_some());
                ...
                let child_digest = child.digest();
                assert!(victim_mailbox.verified(child_round, child).await.is_ok());
    ...
    fn test_standard_actor_starts_without_buffer() {
        ...
        runner.start(|mut context| async move {
            ...
            let digest = block.digest();
            assert!(mailbox.verified(round, block).await.is_ok());
            mailbox.forward(
                round,
                digest,
    ...
    fn test_standard_start_floor_fetches_async_and_serves_requests() {
        ...
        runner.start(|mut context| async move {
            ...
            assert!(mailbox.verified(served_round, served.clone()).await.is_ok());
            let (response, response_rx) = oneshot::channel();
            ...
            assert!(mailbox.verified(next_round, next.clone()).await.is_ok());
            ...
            assert!(mailbox.verified(floor_round, floor_block).await.is_ok());
    ...
    fn test_standard_start_floor_applies_local_anchor_without_fetch() {
        ...
        runner.start(|mut context| async move {
            ...
            assert_eq!(
                mailbox.get_block(Height::new(5)).await.unwrap_or_default().unwrap().digest(),
                floor_block.digest()
            );
    ...
    fn test_standard_floor_jump_ignores_stale_application_ack() {
        ...
        runner.start(|mut context| async move {
            ...
            assert!(mailbox.verified(block1_round, block1.clone()).await.is_ok());
            ...
            assert!(mailbox.verified(floor_round, floor_block.clone()).await.is_ok());
            assert_eq!(
                mailbox
            ...
            let _ = mailbox.get_block(Height::new(5)).await.unwrap_or_default();
            assert_eq!(
    ...
    fn test_standard_local_floor_jump_ignores_stale_application_ack() {
        ...
        runner.start(|mut context| async move {
            ...
            assert!(mailbox.verified(block1_round, block1).await.is_ok());
            StandardHarness::report_finalization(&mut mailbox, block1_finalization).await;
            wait_until(
                &context,
                Duration::from_secs(5),
    ...
    fn test_standard_set_floor_repairs_gap_after_anchor_arrives() {
        ...
        runner.start(|mut context| async move {
            ...
            assert!(mailbox.verified(later_round, later.clone()).await.is_ok());
            ...
            assert!(mailbox.get_finalization(Height::new(7)).await.unwrap_or_default().is_some());
            assert!(
    ...
    fn test_standard_newer_pending_floor_supersedes_older_anchor() {
        ...
        runner.start(|mut context| async move {
            ...
            assert!(mailbox.verified(old_next_round, old_next.clone()).await.is_ok());
            ...
            assert!(mailbox.get_finalization(Height::new(6)).await.unwrap_or_default().is_some());

            let new_next_round = Round::new(Epoch::zero(), View::new(8));
            let new_next = make_raw_block(new_floor_block.digest(), Height::new(8), 800);
            assert!(mailbox.verified(new_next_round, new_next.clone()).await.is_ok());
            ...
            assert!(mailbox.get_finalization(Height::new(8)).await.unwrap_or_default().is_some());
    ...
    fn test_standard_pending_floor_drops_in_flight_ack_before_anchor() {
        ...
        runner.start(|mut context| async move {
            ...
            assert!(mailbox.verified(block2_round, block2).await.is_ok());
            StandardHarness::report_finalization(&mut mailbox, block2_finalization).await;
            assert!(mailbox.get_finalization(Height::new(2)).await.unwrap_or_default().is_some());
            context.sleep(Duration::from_millis(100)).await;
    ...
    fn test_standard_local_floor_below_in_flight_keeps_pending_ack() {
        ...
        runner.start(|mut context| async move {
            ...
            mailbox.set_floor(floor_finalization);
            assert!(mailbox.get_block(Height::zero()).await.unwrap_or_default().is_some());
    ...
    fn test_standard_same_height_floor_anchor_keeps_pending_ack() {
        ...
        runner.start(|mut context| async move {
            ...
            assert!(mailbox.get_block(Height::new(1)).await.unwrap_or_default().is_some());
    ...
    fn test_standard_stale_floor_anchor_advances_round_floor() {
        ...
        runner.start(|mut context| async move {
            ...
            assert!(mailbox.verified(block_round, block.clone()).await.is_ok());
    ...
    fn test_standard_floor_anchor_uses_parent_digest_as_commitment() {
        ...
        runner.start(|mut context| async move {
            ...
            assert!(
                mailbox.get_block(Height::new(2)).await.unwrap_or_default().is_some(),
                "standard floor anchor should be archived using its parent digest"
            );
            assert!(
                mailbox.get_finalization(Height::new(2)).await.unwrap_or_default().is_some(),
    ...
    fn test_standard_restart_rejects_mismatched_genesis_anchor() {
        ...
        runner.start(|mut context| async move {
            ...
            .await;
            assert_eq!(
                mailbox.get_block(Height::zero()).await.unwrap_or_default().unwrap().digest(),
    ...
    fn test_standard_processed_round_restored_after_restart() {
        ...
        runner.start(|mut context| async move {
            ...
            .await;
            assert_eq!(
                mailbox.get_info(Identifier::Latest).await.unwrap_or_default(),
    ...
    fn test_standard_hint_finalized_below_floor_is_noop() {
        ...
        runner.start(|mut context| async move {
            ...
            mailbox.set_floor(finalization);
            assert!(mailbox.verified(floor_round, floor_anchor).await.is_ok());
            context.sleep(Duration::from_millis(50)).await;
    ...
    fn test_standard_hint_finalized_skips_when_already_finalized() {
        ...
        runner.start(|mut context| async move {
            ...
            while mailbox.get_finalization(Height::new(1)).await.unwrap_or_default().is_none() {
                context.sleep(Duration::from_millis(10)).await;
            }
    ...
    fn test_standard_prune_above_floor_is_rejected() {
        ...
        runner.start(|mut context| async move {
            ...
            assert!(mailbox.get_block(Height::new(1)).await.unwrap_or_default().is_some());
            assert!(mailbox.get_finalization(Height::new(1)).await.unwrap_or_default().is_some());
        });
    }
}
