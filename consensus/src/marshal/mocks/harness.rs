...
/// Test that floor updates cancel unresolved block subscriptions.
pub fn subscribe_canceled_on_floor_update<H: TestHarness>() {
    let runner = deterministic::Runner::timed(Duration::from_secs(30));
    runner.start(|mut context| async move {
        // 1) Build a validator and open a subscription for a missing commitment.
        let mut oracle = setup_network(context.clone(), None);
        let Fixture {
            participants,
            schemes,
        ...
        let commitment = H::commitment(&missing);

        let subscription_rx = mailbox
            .subscribe_by_commitment(Some(Round::new(Epoch::zero(), View::new(1))), commitment)
            .await;

        // 2) Move floor forward to invalidate old-history waiters.
        mailbox.set_floor(Height::new(10)).await;
        // Barrier: mailbox messages are FIFO.
        let _ = mailbox.get_finalization(Height::new(10)).await;
        context.sleep(Duration::from_millis(10)).await;

        let canceled = subscription_rx
            .now_or_never()
            .expect("subscription should be canceled after floor update");
        // 3) Receiver should resolve via cancellation, not hang indefinitely.
        assert!(
            canceled.is_err(),
            "subscription should resolve with cancellation after floor update"
        );
    })
}

/// Test that pruning cancels unresolved block subscriptions.
pub fn subscribe_canceled_on_prune<H: TestHarness>() {
    let runner = deterministic::Runner::timed(Duration::from_secs(30));
    runner.start(|mut context| async move {
        // 1) Build a validator and set floor so prune is accepted.
        let mut oracle = setup_network(context.clone(), None);
        let Fixture {
            participants,
            schemes,
        ...
        // 2) Prune finalized history and require stale waiters to be canceled.
        mailbox.prune(Height::new(10)).await;
        // Barrier: mailbox messages are FIFO.
        let _ = mailbox.get_finalization(Height::new(10)).await;
        context.sleep(Duration::from_millis(10)).await;
        ...
        let canceled = subscription_rx
            .now_or_never()
            .expect("subscription should be canceled after prune");
        // 3) Receiver should resolve via cancellation, not hang indefinitely.
        assert!(
            canceled.is_err(),
            "subscription should resolve with cancellation after prune"
        );
    })
}
