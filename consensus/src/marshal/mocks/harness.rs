...
pub fn genesis_emitted_once<H: TestHarness>() {
    ...
    runner.start(|mut context| async move {
        ...
        // Give marshal a turn to persist the height-zero acknowledgement before restart.
        context.sleep(Duration::from_millis(10)).await;
...
pub fn init_processed_height<H: TestHarness>() {
    ...
    runner.start(|mut context| async move {
        ...
        // The durable height starts at zero, and genesis is delivered to the
        // application before subsequent finalized blocks.
        assert_eq!(initial_height, Height::zero());
        while !app.blocks().contains_key(&Height::zero()) {
            context.sleep(Duration::from_millis(10)).await;
        }
