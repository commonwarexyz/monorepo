...
use core::{alloc::GlobalAlloc, cell::Cell, num::NonZeroU32};
...
// SAFETY: Every method forwards the caller's exact layout contract to `System` and only adds
// side-effect-free counting, so the global-allocator requirements hold exactly as they do for
// `System` itself.
unsafe impl GlobalAlloc for CountingAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        // SAFETY: This forwards the allocator contract and its unchanged layout to System.
    ...
    unsafe fn realloc(&self, ptr: *mut u8, layout: Layout, new_size: usize) -> *mut u8 {
        // SAFETY: This forwards the allocator contract, pointer, and layout to System.
...
/// Runs one synchronous benchmark operation and returns its heap-allocation count.
///
/// Only allocations on the calling thread are counted. The Multimmit corpus using this helper is
/// deliberately synchronous so unrelated Criterion or worker activity cannot contaminate it.
