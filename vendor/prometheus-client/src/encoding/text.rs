...
/// Encode a single metric descriptor and metric into the provided [`Write`]r
/// using the OpenMetrics text format.
///
/// This is a lower-level helper for callers that already manage their own
/// metric storage and only need `prometheus-client`'s text encoding logic for
/// one metric at a time.
pub fn encode_metric<W, M>(
    writer: &mut W,
    name: &str,
    help: &str,
    labels: &[(Cow<'static, str>, Cow<'static, str>)],
    unit: Option<&Unit>,
    metric: &M,
...
{
    encode_descriptor(writer, name, help, unit, metric.metric_type())?;
    encode_metric_samples(writer, name, labels, unit, metric)
}

/// Encode a metric descriptor into the provided [`Write`]r using the
...
where
    W: Write,
{
    let mut encoder = DescriptorEncoder::new(writer);
    let _ = encoder.encode_descriptor(name, help, unit, metric_type)?;
    Ok(())
}

/// Encode a metric's samples into the provided [`Write`]r using the
/// OpenMetrics text format without emitting the descriptor.
pub fn encode_metric_samples<W, M>(
    writer: &mut W,
    name: &str,
    labels: &[(Cow<'static, str>, Cow<'static, str>)],
    unit: Option<&Unit>,
